...
def _stat_db_file_identity(path: Path) -> Optional[Tuple[int, int]]:
    ...
    Mirrors the curie_state helper of the same name; kept local so this
    module has no import-time dependency on curie_state (which imports
...
def acquire(db_path: Optional[Path] = None) -> "SessionDB":
    ...
    shared generation was opened — e.g. by ``curie sessions recover`` or
...
# ── Backwards-compatible aliases (curie_state re-exports) ──
# Kept so call sites and tests can import either from curie_state
