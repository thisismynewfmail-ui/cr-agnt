"""ACP session manager — maps ACP sessions to Curie AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.curie/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Curie itself is running in WSL.

    Windows ACP clients can launch ``curie acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from curie_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Curie from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["curie-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Curie agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Curie AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Curie runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.curie/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``CURIE_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_curie_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=curie_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _restore(self, session_id: str) -> Optional[SessionState]:
        ...
        # for the rest of the session (see curie_state.get_messages_as_conversation).
        try:
    ...
    def _make_agent(
        ...
        *,
        ...
        # the automatic late-refresh (see CurieACPAgent._schedule_mcp_late_refresh).
        try:
        ...
        # editor/session cwd instead of the Curie daemon's process cwd.
