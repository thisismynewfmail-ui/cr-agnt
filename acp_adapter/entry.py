"""CLI entry point for the curie-agent ACP adapter.

Loads environment variables from ``~/.curie/.env``, configures logging
...
try:
    ...
    # shadowing Curie's own modules — ``curie acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from CURIE_HOME (default ``~/.curie``)."""
    from curie_cli.env_loader import load_curie_dotenv

    curie_home = get_curie_home()
    loaded = load_curie_dotenv(curie_home=curie_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", curie_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="curie-acp",
        description="Run Curie Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Curie version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Curie provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.curie/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import CurieACPAgent  # noqa: F401

    print("Curie ACP check OK")


def _run_setup() -> None:
    from curie_cli.main import main as curie_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "curie", "model"]
        curie_main()
    finally:
        sys.argv = old_argv
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting curie-agent ACP adapter")
    ...
    import acp
    from .server import CurieACPAgent
    ...
    if os.environ.get("CURIE_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
