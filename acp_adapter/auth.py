"""ACP auth helpers — detect and advertise Curie authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "curie-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Curie runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Curie can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Curie.
    ...
    not have Curie provider credentials configured yet, so Curie always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Curie using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Curie provider",
            description=(
                "Open Curie' interactive model/provider setup in a terminal. "
                "Use this when Curie has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
