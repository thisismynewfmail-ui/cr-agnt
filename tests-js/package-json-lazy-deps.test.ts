 ...
 *
 * The root ``package.json`` is installed by ``curie update`` on every user,
 ...
 *   workspace graph entirely. ``curie update`` and ``curie doctor --fix``
 ...
 *   ``curie tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``curie update`` for
 ...
 * fails — read the lazy-install guidance in the ``curie-agent-dev`` skill
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      'Camofox post_setup handler in curie_cli/tools_config.py so it ' +
      'only installs when the user explicitly selects Camofox via ' +
      '`curie tools` → Browser Automation → Camofox.'
  )
})
