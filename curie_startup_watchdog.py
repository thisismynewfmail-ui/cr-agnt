...
Config surface is deliberately env-only (``CURIE_STARTUP_WATCHDOG=0`` to
disable, ``CURIE_STARTUP_WATCHDOG_TIMEOUT_S`` to tune): the watchdog must be
...
ENV_STARTUP_WATCHDOG = "CURIE_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "CURIE_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module-level singleton: the arm sites (curie_cli.main / curie_cli.gateway
# / gateway.run.main / cli.py --gateway / scripts/curie-gateway) and the
...
def _process_curie_home() -> Path:
    """CURIE_HOME for process-level diagnostic files.

    Stdlib-only replica of ``curie_constants``' platform default — this
    module must not import application code (see module docstring). Hosted
    images always set ``CURIE_HOME`` explicitly.
    """
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<CURIE_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_curie_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``CURIE_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``CURIE_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
