...
``<curie home>/plugins/<name>/``. That tree is the plugin *install* dir —
``curie plugins remove`` deletes it and ``curie plugins update`` git-pulls
...
``<curie home>/plugin-data/<name>/``, owned by the user, untouched by
...
through ``agent.secret_scope`` / ``.env`` like everywhere else in Curie.
...
# Mirrors the plugin-name shape `curie plugins install` accepts. Anything
...
def plugin_data_dir(name: str) -> Path:
    ...
    ``<curie home>/plugin-data/<name>/`` — survives plugin update and
    removal, and follows the active profile because it resolves through
    :func:`curie_constants.get_curie_home` on every call. Don't cache the
    result across profile switches.
    """
    from curie_constants import get_curie_home

    root = get_curie_home() / "plugin-data" / _validate_name(name)
