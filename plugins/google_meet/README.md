...
Let the curie agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where curie runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `curie meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `curie meet` CLI |
...
| `cli.py` | `curie meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$CURIE_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `curie meet node {run,list,approve,remove,status,ping}` |
...
curie meet install                                      # pip + Chromium
curie meet setup                                        # preflight
curie meet auth                                         # optional
curie meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
curie meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.curie/.env
curie meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
curie meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
curie meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.curie/.env
curie meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, curie will **not** switch your system audio input automatically — the
...
curie plugins enable google_meet
curie meet node run --display-name my-mac --host 0.0.0.0 --port 18789
