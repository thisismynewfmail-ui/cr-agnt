"""`curie meet node ...` subcommand tree.

Wired into the existing ``curie meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``curie meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="curie-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    ...
    if cmd == "run":
        ...
        print(f"             curie meet node approve <name> ws://<host>:{args.port} {token}")
        try:
