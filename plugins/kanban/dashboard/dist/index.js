/**
 * Curie Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.curie/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__CURIE_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__CURIE_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "curie-kanban-dot-triage",
    todo: "curie-kanban-dot-todo",
    ready: "curie-kanban-dot-ready",
    running: "curie-kanban-dot-running",
    blocked: "curie-kanban-dot-blocked",
    review: "curie-kanban-dot-review",
    done: "curie-kanban-dot-done",
    archived: "curie-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-curie-task";
  ...
  const DOCS_URL = "https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/features/kanban.md";
  const DOCS_TUTORIAL_URL = "https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/features/kanban-tutorial.md";
  ...
  const LS_BOARD_KEY = "curie.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="curie-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])curie-kanban-md-code\1/i.test(attrs)) {
      return ' class="curie-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "curie-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "curie-kanban-md",
  ...
  // standard `drop` event and our `curie-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("curie-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("curie-kanban-column--drop");
          if (target) target.classList.add("curie-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("curie-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("curie-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("curie-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__CURIE_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "curie-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "curie-kanban-attention",
        "curie-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "curie-kanban-attention-bar" },
        h("span", { className: "curie-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "curie-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "curie-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "curie-kanban-attention-row",
                  "curie-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "curie-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "curie-kanban-attention-row-id" }, task.id),
                h("span", { className: "curie-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "curie-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "curie-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "curie-kanban-diag-action-btn",
      action.suggested ? "curie-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " curie-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".curie-kanban-drawer-comment-row input, .curie-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "curie-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("curie-kanban-diag", sevClass) },
      h("div", { className: "curie-kanban-diag-header" },
        h("span", { className: "curie-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "curie-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "curie-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "curie-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "curie-kanban-diag-data-row" },
                  h("span", { className: "curie-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "curie-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "curie-kanban-diag-data-row" },
                h("span", { className: "curie-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "curie-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "curie-kanban-diag-reassign-row" },
            h("span", { className: "curie-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "curie-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "curie-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "curie-kanban-diag-msg",
              msg.ok ? "curie-kanban-diag-msg--ok" : "curie-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "curie-kanban-section" },
      h("div", { className: "curie-kanban-section-head-row" },
        h("span", { className: "curie-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "curie-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "curie-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "curie-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "curie-kanban-docs-link",
      title: "Open Curie Kanban docs in a new tab",
      "aria-label": "Curie Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `curie kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "curie-kanban-msg-ok" : "curie-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "curie-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "curie-kanban-boardswitcher" },
      h("div", { className: "curie-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "curie-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "curie-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "curie-kanban-dialog-title" },
        ...
        h("div", { className: "curie-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Curie profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "curie-kanban-bulk" },
      h("span", { className: "curie-kanban-bulk-count" },
      ...
      h("div", { className: "curie-kanban-bulk-priority",
      ...
      h("div", { className: "curie-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Curie profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "curie-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("curie-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("curie-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "curie-kanban-trash",
        dragOver ? "curie-kanban-trash--drop" : "",
        props.draggingTaskId ? "curie-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "curie-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "curie-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".curie-kanban-card")) return true;
      if (target.closest && target.closest(".curie-kanban-column-add")) return true;
      if (target.closest && target.closest(".curie-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("curie-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("curie-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".curie-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "curie-kanban-columns",
        isScrollable ? "curie-kanban-columns--scrollable" : "",
        isPanning ? "curie-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("curie-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("curie-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "curie-kanban-column",
        dragOver ? "curie-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "curie-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "curie-kanban-col-check",
        ...
        h("span", { className: cn("curie-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "curie-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "curie-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "curie-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "curie-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "curie-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "curie-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "curie-kanban-lane" },
                  h("div", { className: "curie-kanban-lane-head" },
                    h("span", { className: "curie-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "curie-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "curie-kanban-card--stale-red";
    if (age >= tier.amber) return "curie-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".curie-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "curie-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "curie-kanban-card",
        props.selected ? "curie-kanban-card--selected" : "",
        props.failed ? "curie-kanban-card--failed" : "",
        props.draggingSource ? "curie-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "curie-kanban-card-content" },
          h("div", { className: "curie-kanban-card-row" },
            h("label", {
              className: "curie-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "curie-kanban-card-check",
            ...
            h("span", { className: "curie-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or curie kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "curie-kanban-warning-badge",
                    "curie-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "curie-kanban-needs-assignee",
          ...
          h("div", { className: "curie-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "curie-kanban-card-row curie-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "curie-kanban-assignee",
                            title: `Assigned to Curie profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "curie-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "curie-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "curie-kanban-ago",
  ...
  // centered modal reusing the curie-kanban-dialog chrome so the form is
  ...
  function InlineCreate(props) {
    ...
    return h("div", {
      ...
      h("form", {
        className: "curie-kanban-dialog curie-kanban-create-dialog",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Curie profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Curie profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "curie-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "curie-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "curie-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "curie-kanban-drawer-close",
        ...
        data ? h("div", { className: "curie-kanban-drawer-comment-foot" },
          h("div", {
            className: "curie-kanban-comment-hint text-xs text-muted-foreground",
          ...
          h("div", { className: "curie-kanban-drawer-comment-row" },
            h(Input, {
              value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "curie-kanban-section" },
      h("div", { className: "curie-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "curie-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "curie-kanban-drawer-body" },
      h("div", { className: "curie-kanban-drawer-title" },
        h("span", { className: cn("curie-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "curie-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "curie-kanban-drawer-meta" },
      ...
      (function () {
        ...
        if (finalResult) {
          ...
          return h("div", { className: "curie-kanban-section" },
            h("div", { className: "curie-kanban-section-head" }, label),
            h(MarkdownBlock, { source: finalResult, enabled: props.renderMarkdown }),
          );
        }
        if (isDone && isParent) {
          return h("div", { className: "curie-kanban-section" },
            h("div", { className: "curie-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "curie-kanban-done-no-result curie-kanban-done-parent-note" },
        ...
        if (isDone) {
          return h("div", { className: "curie-kanban-section" },
            h("div", { className: "curie-kanban-section-head" }, tx(i18n, "result", "Result")),
            h("div", { className: "curie-kanban-done-no-result" },
      ...
      childResults.length > 0 ? h("div", { className: "curie-kanban-section" },
        h("div", { className: "curie-kanban-section-head" },
        ...
        childResults.map(function (child) {
          ...
          return h("div", { key: child.id, className: "curie-kanban-comment" },
            h("div", { className: "curie-kanban-comment-head" },
              h("span", { className: "curie-kanban-comment-author" },
                `${child.id} · ${child.title || tx(i18n, "untitled", "(untitled)")}`),
              h(Badge, { variant: "outline" }, child.status),
              h("button", {
                type: "button",
                className: "curie-kanban-diag-action-btn",
      ...
      h("div", { className: "curie-kanban-section" },
        h("div", { className: "curie-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "curie-kanban-comment" },
                h("div", { className: "curie-kanban-comment-head" },
                  h("span", { className: "curie-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "curie-kanban-comment-ago" },
      ...
      h("div", { className: "curie-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "curie-kanban-event",
              isDiag ? "curie-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "curie-kanban-event-header" },
                  h("span", { className: "curie-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "curie-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "curie-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "curie-kanban-event-header-plain" },
                  h("span", { className: "curie-kanban-event-kind" }, e.kind),
                  h("span", { className: "curie-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "curie-kanban-event-phantom-row" },
                  h("span", { className: "curie-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "curie-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "curie-kanban-event-payload" },
    ...
    return h("div", { className: "curie-kanban-section" },
      h("div", { className: "curie-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "curie-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `curie-kanban-run--${r.outcome || r.status || "ended"}`
          : "curie-kanban-run--active";
        return h("div", { key: r.id, className: cn("curie-kanban-run", outcomeClass) },
          h("div", { className: "curie-kanban-run-head" },
            h("span", { className: "curie-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "curie-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "curie-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "curie-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "curie-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "curie-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "curie-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "curie-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "curie-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "curie-kanban-pre curie-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "curie-kanban-meta-row" },
      h("span", { className: "curie-kanban-meta-label" }, props.label),
      h("span", { className: "curie-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "curie-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "curie-kanban-meta-row" },
        h("span", { className: "curie-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "curie-kanban-meta-value curie-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "curie-kanban-meta-row" },
        h("span", { className: "curie-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "curie-kanban-meta-value curie-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function ModelEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "curie-kanban-meta-row" },
        h("span", { className: "curie-kanban-meta-label" }, tx(t, "model", "Model")),
        h("span", {
          className: cn(
            "curie-kanban-meta-value curie-kanban-editable",
    ...
    return h("div", { className: "curie-kanban-meta-row" },
      h("span", { className: "curie-kanban-meta-label" }, tx(t, "model", "Model")),
      loading
        ? h("span", { className: "curie-kanban-meta-value text-muted-foreground" },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "curie-kanban-section" },
      h("div", { className: "curie-kanban-section-head-row" },
        h("span", { className: "curie-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "curie-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "curie-kanban-section" },
      h("div", { className: "curie-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "curie-kanban-deps-row" },
        h("span", { className: "curie-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "curie-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "curie-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "curie-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "curie-kanban-dep-chip-x",
      ...
      h("div", { className: "curie-kanban-deps-row" },
        h("span", { className: "curie-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "curie-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "curie-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "curie-kanban-section" },
      ...
      h("div", { className: "curie-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "curie-kanban-home-sub curie-kanban-home-sub--on"
              : "curie-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__CURIE_PLUGINS__ && typeof window.__CURIE_PLUGINS__.register === "function") {
    window.__CURIE_PLUGINS__.register("kanban", KanbanPage);
  }
})();
