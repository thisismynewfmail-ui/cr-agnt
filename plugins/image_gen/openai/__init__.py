...
three virtual model IDs so the ``curie tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `curie tools` → Image "
                    "Generation → OpenAI to configure, or `curie setup` "
                    "to add the key."
                ),
