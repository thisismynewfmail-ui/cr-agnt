"""Meta Model API (Muse Spark) provider plugin for Curie Agent.
...
Bundled from https://github.com/albertodepaola/curie-meta-provider. Curie'
...
* **Zero core edits.** Everything rides on ``ProviderProfile`` hooks. No changes
  to curie' ``model_metadata.py`` / ``models.py`` / ``run_agent.py`` are needed:
...
def _resolve_effort(reasoning_config: dict | None) -> str:
    """Map Curie' reasoning_config to a Meta-safe ``reasoning_effort`` value.
...
meta_ai = MetaAIProfile(
    ...
    # in curie_cli/providers.py and the retention hint in
