"""SimpleX Chat platform adapter (Curie plugin).
...
This adapter ships as a Curie platform plugin under
``plugins/platforms/simplex/``. The Curie plugin loader scans the
...
Optional environment variables:
    ...
    CURIE_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``curie setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Curie — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("CURIE_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``curie setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.curie/.env`` via
    ``curie_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "curie_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.curie/.env"
        )
        return
