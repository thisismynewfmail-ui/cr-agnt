...
under an immutable ``/opt/curie`` (read-only for the curie user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$CURIE_HOME/photon/sidecar`` (the durable data volume,
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_curie_home() / "photon" / "sidecar"
    try:
