...
// `message.content.text` before Curie can see it. We rewrite the two inbound
...
const MARKER = "Curie patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Curie to patch; keep the legacy v8 path below for older pinned installs.
    if (
