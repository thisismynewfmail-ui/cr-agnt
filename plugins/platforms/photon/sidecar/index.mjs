// Curie Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Curie): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Curie -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Curie-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `curie photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Curie can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "curie-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Curie'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Curie allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Curie, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-curie-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Curie supplied them so a known-good
