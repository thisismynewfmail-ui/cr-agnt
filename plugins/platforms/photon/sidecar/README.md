...
Small Node helper that bridges Curie Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Curie is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Curie-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Curie can receive inbound messages
...
The Curie plugin's `curie photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Curie is Python, so replies go through this sidecar
