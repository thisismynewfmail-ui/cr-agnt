"""
``curie photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Curie
...
# Writable sidecar runtime dir (mirrors to CURIE_HOME on immutable
...
# wiring imports this module for `curie --help`).
...
# when called later (gateway start, curie status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `curie photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Curie Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``curie photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Curie Agent" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.curie/.env,
    ...
    try:
        from curie_cli.config import write_platform_config_field
    ...
    print("  Start the gateway:  curie gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `curie photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`curie photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.curie/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `curie photon telemetry on` / `curie photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.curie/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  curie gateway restart")
    return 0
...
#
# `curie gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``curie photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `curie gateway setup` wizard."""
