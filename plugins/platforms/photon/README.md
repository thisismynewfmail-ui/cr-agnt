...
This plugin connects Curie Agent to iMessage (and other Spectrum
...
TypeScript-only, Curie runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Curie channel,
...
Run `curie photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.curie/.env` (the same place every other
...
Management metadata lives in `~/.curie/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `curie photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Curie
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Curie coalesces
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage read
  after forwarding it to Curie, so the sender sees `Read` without waiting for
  a model/tool turn. Inbound receipts for Curie-sent messages are consumed as
  presence telemetry and never create an agent turn. Set
  `PHOTON_READ_RECEIPTS=false` to keep messages at `Delivered`.
- **Native polls are supported.** Curie posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `curie send`) cannot spawn the sidecar;
  they authenticate to the gateway's live sidecar via the runtime record at
  `<curie-home>/runtime/photon-sidecar.json` (written after the sidecar's
...
6. Verify end-to-end: `curie photon status`, a DM and a group roundtrip,
