# A2A — Agent-to-Agent protocol for Curie
...
peer (another Curie, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
curie gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Curie serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.curie/a2a_audit.jsonl`.
- Conversations persist to `~/.curie/a2a_conversations/` — they survive context
