"""ntfy platform adapter (Curie plugin).
...
``plugins/platforms/ntfy/``. The Curie plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "curie-in"              # subscribe topic (incoming)
          publish_topic: "curie-out"     # optional — defaults to topic
...
_ECHO_TAG = "curie-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``curie cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Curie dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `curie gateway status` without
