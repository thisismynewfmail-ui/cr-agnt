...
# identify CurieAgent traffic — matching other Curie outbound surfaces
# that already set ``CurieAgent/<version>`` for platform-partner attribution.
try:
    from curie_cli import __version__ as _CURIE_VERSION
except Exception:
    _CURIE_VERSION = "unknown"
_CURIE_SLACK_USER_AGENT_PREFIX = f"CurieAgent/{_CURIE_VERSION}"
...
def _slack_unfurl_kwargs(extra: Optional[Dict[str, Any]]) -> Dict[str, bool]:
    ...
    ``_slack_unfurl_hints``: ``curie config set`` and Railway persist YAML
...
def _rewrite_known_bang_command(text: str) -> str:
    ...
    try:
        from curie_cli.commands import is_gateway_known_command
...
def _normalize_slack_text_for_dedupe(text: str, bot_uid: str = "") -> str:
    ...
    # After the label, so that ``<@U…|curie>`` is stripped like ``<@U…>``.
    if bot_uid:
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `curie slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `curie gateway setup` "
                "or in the active profile's ~/.curie/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `curie gateway setup` "
                "or add it to your active profile's ~/.curie/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `curie gateway setup` "
                "or in the active profile's ~/.curie/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `curie gateway setup` "
                "or add it to your active profile's ~/.curie/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        tokens_file = get_curie_home() / "slack_tokens.json"
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_CURIE_SLACK_USER_AGENT_PREFIX,
            )
            self._app = AsyncApp(
                token=primary_token,
            ...
            # Curie has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Curie can "
                    "be removed from the Slack app manifest via "
                    "`curie slack manifest`)",
                    (event or {}).get(
                        "type",
            ...
            # /model work directly without /curie prefix). A single regex
            ...
            # manifest (see `curie slack manifest`). In Socket Mode, Slack
            ...
            from curie_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/curie$")

            @self._app.command(_slash_pattern)
            async def handle_curie_command(ack, command):
            ...
            for _action_id in (
                "curie_approve_once",
                "curie_approve_session",
                "curie_approve_always",
                "curie_deny",
            ):
            ...
            for _action_id in (
                "curie_confirm_once",
                "curie_confirm_always",
                "curie_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("curie_feedback")(self._handle_feedback_action)
            ...
            self._app.action(
                _re.compile(r"^curie_clarify_choice_\d+$")
            )(self._handle_clarify_action)
            self._app.action("curie_clarify_other")(self._handle_clarify_action)
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'curie slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Curie handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Curie session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Curie owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_reaction(self, event: dict, removed: bool = False) -> None:
        ...
        if target_channel:
            ...
            synthetic["_curie_reaction_source_channel"] = channel_id
            if target_thread:
                synthetic["thread_ts"] = target_thread
            else:
                synthetic.pop("thread_ts", None)
                synthetic["_curie_no_thread_response"] = True
    ...
    async def _handle_slack_message_impl(
        ...
        if is_dm:
            ...
            if not thread_ts and self._dm_top_level_threads_as_sessions():
                thread_ts = ts
        elif event.get("_curie_no_thread_response"):
        ...
        force_process = bool(event.get("_curie_force_process"))
        ...
        # the shape produced by peer Curie agents in Socket Mode on some
        ...
        try:
            ...
            if not smart_denied and allow_session:
                actions.append({
                    ...
                    "action_id": "curie_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "curie_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "curie_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "curie_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "curie_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def send_clarify(
        ...
        (unique ``curie_clarify_choice_<idx>`` action_id, ``value`` packs
        ``clarify_id|idx``) plus a final "✏️ Other…" button
        (``curie_clarify_other``).  A choice click resolves the clarify
        ...
        try:
            ...
            for idx, choice in enumerate(choices):
                ...
                elements.append({
                    ...
                    "action_id": f"curie_clarify_choice_{idx}",
                    "value": f"{clarify_id}|{idx}",
                })
            elements.append({
                "type": "button",
                "text": {"type": "plain_text", "text": "✏️ Other…", "emoji": True},
                "action_id": "curie_clarify_other",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "curie_confirm_once": "once",
            "curie_confirm_always": "always",
            "curie_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "curie_approve_once": "once",
            "curie_approve_session": "session",
            "curie_approve_always": "always",
            "curie_deny": "deny",
        }
    ...
    async def _handle_clarify_action(self, ack, body, action) -> None:
        ...
        if action_id == "curie_clarify_other" or token == "other":
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/curie <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/curie
        ...
        if slash_name in {"curie", ""}:
            # Legacy /curie <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            legacy_text = raw_text.strip()
            from curie_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/curie   ``).
        ...
        # "Running /cmd…" ack shown by handle_curie_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/curie <question>" must produce public replies so
...
# ``curie_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        _tokens_file = get_curie_home() / "slack_tokens.json"
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``curie_cli/setup.py::_setup_slack``.
    """
    from pathlib import Path
    from curie_cli.config import get_env_value, remove_env_value, save_env_value
    from curie_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under CURIE_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from curie_cli.slack_cli import _build_full_manifest
            from curie_constants import get_curie_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Curie",
                bot_description="Your Curie agent on Slack",
            )
            target = Path(get_curie_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `curie slack manifest --write` anytime to refresh after "
                "Curie adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `curie update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/messaging/slack.md")
    print()
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``curie_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    """Plugin entry point — called by the Curie plugin system."""
    ctx.register_platform(
        ...
        install_hint="Run `curie setup` to install Slack support.",
        # Interactive setup wizard — replaces curie_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in curie_cli/gateway.py.
