"""
LINE Messaging API platform adapter for Curie Agent.
...
to Curie Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``CURIE_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            curie_home = Path(get_curie_home()).resolve()
        except Exception:
            curie_home = Path.home().joinpath(".curie").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``curie status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``curie status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``curie setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.curie/.env`` via ``curie_cli.config``.
    """
    print()
    ...
    try:
        from curie_cli.config import get_env_value as _get_env, save_env_value as _set_env
    except ImportError:
        print("curie_cli.config not available; set LINE_* vars manually in ~/.curie/.env")
        return
