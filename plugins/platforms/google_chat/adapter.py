...
# ``model_tools`` import time, so ``curie status``, ``curie chat``, etc.
...
# ``curie_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Curie is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a curie MessageType.
...
def card_spec_to_cards_v2(card_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"cardId": str(card_spec.get("card_id") or "curie-card"), "card": card}
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${CURIE_HOME}/google_chat_thread_counts.json so
        ...
        try:
            from curie_constants import get_curie_home as _get_curie_home
            _curie_home = _get_curie_home()
        except (ModuleNotFoundError, ImportError):
            _curie_home = _Path.home() / ".curie"
        self._thread_count_store = _ThreadCountStore(
            _curie_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Curie is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("CURIE_HOME", str(_Path.home() / ".curie"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${CURIE_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a curie MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Curie is thinking…' marker message.
        ...
        "Curie is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Curie is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Curie is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Curie is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``curie setup``.

    The setup wizard at ``curie_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.curie/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. curie-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.curie/.env")
    print_info("Restart the gateway: curie gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``curie cron`` running as a
    separate process from ``curie gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Curie plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `curie setup` to install Google Chat support.",
        ...
        # the "Curie is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Curie is thinking…' marker message "
