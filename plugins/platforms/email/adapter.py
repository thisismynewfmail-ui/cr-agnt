"""
Email platform adapter for the Curie gateway.

Allows users to interact with Curie by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "curie-agent" "version" "{_curie_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `curie config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `curie gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Curie Agent")
        ...
        msg_id = f"<curie-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# curie_cli/gateway.py, and the _send_email dispatch in
