"""
Microsoft Teams platform adapter for Curie Agent.
...
Requires:
    ...
    manually: ``<curie-venv>/bin/pip install microsoft-teams-apps aiohttp``)
...
def _suppress_third_party_dotenv() -> Iterator[None]:
    ...
    Curie owns dotenv loading; third-party import side effects must not.
    """
    try:
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Curie"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        headers = {"User-Agent": "Mozilla/5.0 (compatible; CurieAgent/1.0)"}
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        curie_action = data.get("curie_action", "")
        session_key = data.get("session_key", "")

        if not curie_action or not session_key:
        ...
        choice = choice_map.get(curie_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        actions = [ExecuteAction(
            title="Allow Once", verb="curie_approve",
            data={**btn_data_base, "curie_action": "approve_once"}, style="positive",
        )]
        if not smart_denied and allow_session:
            actions.append(ExecuteAction(
                title="Allow Session", verb="curie_approve",
                data={**btn_data_base, "curie_action": "approve_session"},
            ))
            if allow_permanent:
                actions.append(ExecuteAction(
                    title="Always Allow", verb="curie_approve",
                    data={**btn_data_base, "curie_action": "approve_always"},
                ))
        actions.append(ExecuteAction(
            title="Deny", verb="curie_approve",
            data={**btn_data_base, "curie_action": "deny"}, style="destructive",
        ))
        body = [
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Curie\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.curie/.env")
...
def _install_hint() -> str:
    ...
    ``feature_install_command(venv_pip=True)`` targets the actual Curie
