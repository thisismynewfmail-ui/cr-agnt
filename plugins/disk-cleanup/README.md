...
Auto-tracks and cleans up ephemeral files created during Curie Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$CURIE_HOME` and `/tmp/curie-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CURIE_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `CURIE_HOME` or `/tmp/curie-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$CURIE_HOME/disk-cleanup/` is itself excluded
- `$CURIE_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
