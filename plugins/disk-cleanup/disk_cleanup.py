"""disk_cleanup — ephemeral file cleanup for Curie Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under CURIE_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly CURIE_HOME and /tmp/curie-*
Never touches: ~/.curie/logs/ or any system directory.
"""
...
try:
    ...
    def get_curie_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("CURIE_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".curie").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$CURIE_HOME/logs/``."""
    return get_curie_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$CURIE_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under CURIE_HOME or ``/tmp/curie-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    curie_home = get_curie_home()
    try:
        path.resolve().relative_to(curie_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/curie-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("curie-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "curie-agent", "backups", "profiles", ".worktrees",
...
# Paths under $CURIE_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so CURIE_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        curie_home = get_curie_home()
        for parent in ("cron", "cronjobs"):
            base = curie_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside CURIE_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under CURIE_HOME, but never recurse into known
    # durable state trees.  Some installs place the Curie checkout, venv,
    # and desktop build under CURIE_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    curie_home = get_curie_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in curie_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    curie_home = get_curie_home()
    try:
        rel = path.resolve().relative_to(curie_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "curie-agent",
        ...
        # Path isn't under CURIE_HOME (e.g. /tmp/curie-*) — fall through.
        pass
