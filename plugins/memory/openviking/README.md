...
- OpenViking server running and reachable from Curie

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Curie can identify older servers that expose the legacy status-only health
...
connection values into Curie, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.curie/.env`; for a named profile use
...
OpenViking's server config is separate from Curie:
...
Curie-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Curie lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Curie sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Curie also sends `User-Agent: openviking-memory-curie/<version>` on
OpenViking requests. This standard harness identifier contains the Curie
...
does not ask for a peer ID. Without a configured peer, Curie sends neither
...
order remains environment, linked OpenViking config, then Curie YAML. To use
...
on the old implicit `curie` peer now use user memory for new writes. Without
...
Set `agent: curie` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `curie-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Curie
...
Use the same OpenViking profile and credentials as Curie for manual recovery.
...
Curie built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Curie action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Curie
...
file URIs, such as
`viking://user/default/peers/curie/memories/preferences/mem_abc123.md` (any
