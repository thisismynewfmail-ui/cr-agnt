"""curie-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $CURIE_HOME/config.yaml (profile-scoped):
  plugins:
    curie-memory-store:
      db_path: $CURIE_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    try:
        ...
        return cfg_get(all_config, "plugins", "curie-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, curie_home):
        """Write config to config.yaml under plugins.curie-memory-store."""
        from pathlib import Path
        config_path = Path(curie_home) / "config.yaml"
        try:
            import yaml
            ...
            existing["plugins"]["curie-memory-store"] = values
    ...
    def get_config_schema(self):
        ...
        _default_db = f"{display_curie_home()}/memory_store.db"
        return [
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _curie_home = str(get_curie_home())
        _default_db = _curie_home + "/memory_store.db"
        db_path = self._config.get("db_path", _default_db)
        # Expand $CURIE_HOME in user-supplied paths so config values like
        # "$CURIE_HOME/memory_store.db" or "~/.curie/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$CURIE_HOME", _curie_home)
            db_path = db_path.replace("${CURIE_HOME}", _curie_home)
