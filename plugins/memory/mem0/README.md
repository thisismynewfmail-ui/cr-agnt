...
```bash
curie memory setup    # select "mem0"
```
...
Behavioral settings live in `$CURIE_HOME/mem0.json` (set them via `curie memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.curie/.env`.
...
| `user_id` | `curie-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Curie session and call `mem0_search` — it connects to your server.
...
```bash
curie memory setup mem0 --mode oss \
...
```bash
curie memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$CURIE_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.curie/mem0_qdrant"}}
  }
}
```
...
```bash
curie memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$CURIE_HOME/mem0.json`).
