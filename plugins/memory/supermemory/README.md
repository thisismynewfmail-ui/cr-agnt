...
- Hosted: API key from [app.supermemory.ai/integrations?connect=curie](http://app.supermemory.ai/integrations?connect=curie)
...
Before running `curie memory setup`, add the local endpoint to
...
Then run `curie memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$CURIE_HOME/supermemory.json`
...
| `container_tag` | `curie` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `curie-{identity}` → `curie-coder`). |
...
`https://api.supermemory.ai`. Curie resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: curie`, and document writes stamp
`metadata.sm_source: curie`. This is a **functional routing key, not telemetry**:
it groups Curie-written memories into a dedicated "Curie" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Curie profile:

```json
{
  "container_tag": "curie-{identity}"
}
```

For a profile named `coder`, this resolves to `curie-coder`. The default profile resolves to `curie-default`. Without `{identity}`, all profiles share the same container.
