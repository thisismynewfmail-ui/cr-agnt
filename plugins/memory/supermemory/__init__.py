...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=curie"
...
def _load_supermemory_config(curie_home: str) -> dict:
    config = _default_config()
    config_path = Path(curie_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, curie_home: str) -> None:
    config_path = Path(curie_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str,
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "curie"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Curie writes into the "Curie" Space in the Supermemory
        ...
        merged = {"sm_source": "curie", **(metadata or {})}
...
def _resolve_container_tag_for_setup(curie_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(curie_home)
...
def _probe_supermemory_connection(api_key: str, curie_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(curie_home)
    base_url = _resolve_base_url(config["base_url"])
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(curie_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `curie memory setup`.
        # All other options are documented for $CURIE_HOME/supermemory.json
    ...
    def save_config(self, values, curie_home):
        ...
        _save_supermemory_config(sanitized, curie_home)
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, curie_home)
    ...
    def post_setup(self, curie_home: str, config: dict) -> None:
        ...
        from curie_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(env_writes, curie_home=curie_home)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        from curie_constants import get_curie_home
        self._curie_home = kwargs.get("curie_home") or str(get_curie_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._curie_home)
