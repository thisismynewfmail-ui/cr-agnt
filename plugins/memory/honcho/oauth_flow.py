...
``curie://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Curie OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "curie-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``curie-desktop`` / ``curie-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Curie.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local CURIE_HOME override can't reach.
