...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/curie>
...
```bash
curie memory setup honcho   # configure Honcho directly (works on a fresh install)
curie memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
curie config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.curie/.env
```

> `curie honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `curie memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `curie model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$CURIE_HOME/honcho.json` | Profile-local (isolated Curie instances) |
| 2 | `~/.curie/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Curie profile: `curie` (default) or `curie_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `curie honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Curie session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`curie-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`curie`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-curie-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening curie in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Curie session ID (timestamp + hex). Every `curie` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Curie profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`curie`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Curie profile: `curie` (default) or `curie_<profile>` (e.g. `curie -p coder` -> host key `curie_coder`). Older `curie.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `CURIE_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `curie-agent`) |
...
|---------|-------------|
| `curie memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `curie honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `curie memory setup`) |
| `curie honcho status` | Show resolved config for active profile |
| `curie honcho enable` / `disable` | Toggle Honcho for active profile |
| `curie honcho mode <mode>` | Change recall or observation mode |
| `curie honcho peer --user <name>` | Update user peer name |
| `curie honcho peer --ai <name>` | Update AI peer name |
| `curie honcho tokens --context <N>` | Set context token budget |
| `curie honcho tokens --dialectic <N>` | Set dialectic max chars |
| `curie honcho map <name>` | Map current directory to a session name |
| `curie honcho sync` | Create host blocks for all Curie profiles |
