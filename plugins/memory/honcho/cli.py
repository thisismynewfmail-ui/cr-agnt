...
Handles: curie honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "curie" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Curie profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'curie honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'curie honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `curie update` -- no output, no exceptions.
    """
    try:
        from curie_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Curie profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $CURIE_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_curie_home() / "honcho.json"
...
def _resolve_effective_identity_mapping(
    ...
    pin = False
    for val in (
        curie_host.get("pinUserPeer"),
        curie_host.get("pinPeerName"),
        cfg.get("pinUserPeer"),
        cfg.get("pinPeerName"),
    ):
        if val is not None:
    ...
    if "userPeerAliases" in curie_host:
        aliases_src = curie_host.get("userPeerAliases")
    ...
    if "runtimePeerPrefix" in curie_host:
        prefix_src = curie_host.get("runtimePeerPrefix")
...
def _scrub_identity_mapping(curie_host: dict) -> None:
...
def _gateway_platforms() -> list[str] | None:
    ...
    guarded (matching the idiom curie_cli already uses for gateway refs).
    """
    try:
...
def _apply_runtime_prefix(
    curie_host: dict, current_prefix: str, prefix_from_root: bool, label: str
) -> None:
    ...
    if new_prefix and not (prefix_from_root and new_prefix == current_prefix):
        curie_host["runtimePeerPrefix"] = new_prefix


def _echo_identity_mapping(curie_host: dict) -> None:
    """Show the resulting keys so the operator can verify what was written."""
    aliases = curie_host.get("userPeerAliases")
    prefix = curie_host.get("runtimePeerPrefix")
    print("  resolved →")
    print(f"    pinUserPeer       = {bool(curie_host.get('pinUserPeer'))}")
...
def _configure_raw_identity_mapping(
    ...
    _scrub_identity_mapping(curie_host)
    curie_host["pinUserPeer"] = pin
    if pin:
        return
    aliases = (
    ...
    while True:
        ...
        if "=" in entry:
            ...
            if rid and peer:
                aliases[rid] = peer
    if aliases:
        curie_host["userPeerAliases"] = aliases
    _apply_runtime_prefix(
        curie_host, current_prefix, prefix_from_root,
...
def _ensure_sdk_installed() -> bool:
    ...
    # data-volume target instead of writing to /opt/curie (NS-605).
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Curie persistent cross-session memory.")
    ...
    if is_local:
        ...
        current_host_key = curie_host.get("apiKey", "")
        masked = (
        ...
        if new_local_key:
            curie_host["apiKey"] = new_local_key
    ...
    if not is_local:
        ...
        existing_oauth = OAuthCredential.from_host_block(curie_host)
        ...
        if use_device:
            ...
            try:
                ...
                print("\n  Cancelled. Re-run 'curie honcho setup' to try again.\n")
                return
            except (AuthorizationTimeout, DeviceCodeExpired):
                print("\n  Device code expired before approval.")
                print("  Re-run 'curie honcho setup' to get a new code.\n")
                return
            except AccessDenied:
                print("\n  Sign-in was denied on the approval page.")
                print("  Re-run 'curie honcho setup' to retry, or choose an API key instead.\n")
                return
                ...
                return
            print(" approved")
            _apply_grant_to_host(curie_host, cred)
            ...
            if not cfg.get("apiKey"):
                ...
                print("  Run 'curie honcho setup' again once you have a key.\n")
                return

    # --- 3. Identity ---
    current_peer = curie_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        curie_host["peerName"] = new_peer

    current_ai = curie_host.get("aiPeer") or cfg.get("aiPeer", "curie")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        curie_host["aiPeer"] = new_ai

    current_workspace = curie_host.get("workspace") or cfg.get("workspace", "curie")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        curie_host["workspace"] = new_workspace

    # --- 3b. Gateway identity mapping ---
    # These keys only affect the Curie GATEWAY (Telegram/Discord/Slack/...),
    ...
    (
        ...
        aliases_from_root,
        prefix_from_root,
    ) = _resolve_effective_identity_mapping(cfg, curie_host)
    ...
    if gw_platforms is None:
        ...
        run_mapping = _prompt(
            "Running the Curie gateway (Telegram/Discord/etc.)? (y/N)",
            default="n",
    ...
    if run_mapping:
        peer_target = curie_host.get("peerName") or current_peer or "user"
        ...
        if shape == "single":
            ...
            _scrub_identity_mapping(curie_host)
            curie_host["pinUserPeer"] = False
            if prior_aliases:
                curie_host["userPeerAliases"] = prior_aliases
            _apply_runtime_prefix(
                curie_host, current_prefix, prefix_from_root,
            ...
            if merged:
                curie_host["userPeerAliases"] = merged
            _apply_runtime_prefix(
                curie_host, current_prefix, prefix_from_root,
            ...
            _configure_raw_identity_mapping(
                curie_host, current_pin, current_aliases, current_prefix,
    ...
    # --- 4. Observation mode ---
    current_obs = curie_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        curie_host["observationMode"] = new_obs
    else:
        curie_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(curie_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        curie_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        curie_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = curie_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        curie_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = curie_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        curie_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                curie_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(curie_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            curie_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        curie_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        curie_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        curie_host["dialecticReasoningLevel"] = new_reasoning
    else:
        curie_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = curie_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        curie_host["sessionStrategy"] = new_strat

    curie_host["enabled"] = True
    curie_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        curie_config = load_config()
        curie_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(curie_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: curie config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    curie honcho status     -- show full config")
    print("    curie honcho mode       -- change recall/observation mode")
    print("    curie honcho tokens     -- tune context and dialectic budgets")
    print("    curie honcho peer       -- update peer names")
    print("    curie honcho map <name> -- map this directory to a session name\n")
...
def _headless() -> tuple[bool, bool]:
    """(is_remote, can_open_browser) — degrades safely if curie_cli internals move."""
    try:
        from curie_cli.auth import _can_open_graphical_browser, _is_remote_session
...
def _apply_grant_to_host(curie_host: dict, cred) -> None:
    """Store an OAuth grant on the host block; the wizard's final save persists it."""
    curie_host["apiKey"] = cred.access_token
    curie_host["oauth"] = cred.oauth_block()
    # Default the peer prompt to the name entered at consent.
    if cred.consent_peer_name:
        curie_host["peerName"] = cred.consent_peer_name


def _active_profile_name() -> str:
    """Return the active Curie profile name (respects --target-profile override)."""
    if _profile_override:
...
def _all_profile_host_configs() -> list[tuple[str, str, dict]]:
    ...
    for p in profiles:
        ...
        # ("curie.work") stay readable per the README's back-compat
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: curie honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'curie honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'curie honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: curie honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        curie = hosts.get(_host_key(), {})
        user = curie.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = curie.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = curie.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = curie.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Curie' identity in Honcho. Seed with 'curie honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: curie honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print("\n  Set with: curie honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = curie.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = curie.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = curie.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Curie asks Honcho's AI peer a question")
        ...
        print("\n  Set with: curie honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'curie honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'curie honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    curie honcho identity --show        — show both peer representations")
        print("    curie honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Curie + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Curie\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Curie persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  curie honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'curie honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'curie honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  curie honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'curie'")
        print("  with Honcho configured and no prior session history.")
        print("  (Curie calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    curie honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'curie honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Curie, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'curie honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    curie honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  curie honcho identity <path/to/SOUL.md>")
    ...
    print("    Curie:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Curie:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Curie:   Honcho observes every message and updates representations")
    ...
    print("    Curie:   per-session by default — each run gets its own session")
    print("              Map a custom name:  curie honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. curie honcho setup              — configure API key (required)")
        print("  2. curie honcho migrate            — re-run this walkthrough")
    else:
        print("  1. curie honcho status             — verify Honcho connection")
        print("  2. curie                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. curie honcho identity --show    — verify AI peer representation")
        print("  4. curie honcho tokens             — tune context and dialectic budgets")
        print("  5. curie honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'curie memory setup'...\n")
        from curie_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``curie honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``curie honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to curie memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Curie Honcho",
    )
