...
``_check_spotify_available()`` — when the user has not run ``curie auth
spotify``, the tools remain registered (so they appear in ``curie
...
The Spotify auth flow (``curie auth spotify``), CLI plumbing, and docs
