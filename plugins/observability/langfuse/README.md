...
This plugin ships bundled with Curie but is **opt-in** — it only loads when
...
curie tools  # → Langfuse Observability
...
Set these in `~/.curie/.env` (or via `curie tools`):
...
curie chat -q "hello"              # then check Langfuse for a "Curie turn" trace
```

Generation observations include the Curie system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `CURIE_LANGFUSE_MAX_CHARS`).

## Optional tuning

```bash
CURIE_LANGFUSE_ENV=production       # environment tag
CURIE_LANGFUSE_RELEASE=v1.0.0       # release tag
CURIE_LANGFUSE_SAMPLE_RATE=0.5      # sample 50% of traces
CURIE_LANGFUSE_MAX_CHARS=12000      # max chars per field (default: 12000)
CURIE_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
CURIE_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`CURIE_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
...
```bash
curie plugins disable observability/langfuse
```
