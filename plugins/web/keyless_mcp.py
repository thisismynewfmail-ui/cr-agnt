...
two endpoints so a fresh Curie install with **zero web credentials** still
...
def provider_tier(name: str) -> str:
    ...
    ``curie tools`` picker's Free/Paid rows). ``free`` forces the keyless
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "backend via `curie tools` for reliable service."
            ),
        }
    ...
    try:
        ...
        return {
            ...
            "error": (
                ...
                "via `curie tools` for reliable service."
            ),
        }
