...
``{title, url, description, position}`` rows every other Curie web
provider produces.
...
prefers Curie-managed xAI Grok OAuth (via ``curie auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.curie/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``curie tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `curie auth` to sign in with "
