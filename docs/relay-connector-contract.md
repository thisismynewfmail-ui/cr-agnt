...
This document is the formal interface between the **Curie gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `curie gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`curie gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `curie` CLI.
