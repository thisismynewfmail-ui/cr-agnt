...
`curie config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.curie/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `CURIE_HOME`, secret scope, and
