...
The canonical transcript store is SQLite via `SessionDB` (from `curie_state`). The
...
Counts consecutive restarts via a JSON file (`{CURIE_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `curie update`, `curie gateway restart`,
or `/restart`.
