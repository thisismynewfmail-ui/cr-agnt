...
Curie gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Curie's native NeMo Relay SDK
...
| --- | --- | --- |
| Gateway gauges | `/v1/metrics` | `curie.gateway.up/state/busy/drainable/active_agents/background_work/background_delegations/restart_requested`, `curie.platform.up/degraded` with bounded `error_code` attributes |
...
`curie.gateway.active_agents`, `curie.gateway.background_work`, and
`curie.gateway.background_delegations` are complementary. `active_agents`
...
```bash
curie monitoring status
```

The OpenTelemetry SDK is an optional extra (`pip install 'curie-agent[otlp]'`),
...
a plain `curie gateway run`: the exporter lives in the gateway process, so
...
`curie.gateway.up`, `curie.platform.up`, and `curie.platform.degraded`.
...
absent_over_time(curie_gateway_up[5m])

# Locally owned bridge is explicitly down.
curie_platform_up == 0

# Scheduler thread is stale even though the gateway may still be alive.
curie_cron_scheduler_heartbeat_age_seconds > 180

# Ticker loops but has not completed a successful tick recently.
curie_cron_scheduler_last_success_age_seconds > 300

# One or more jobs are beyond their existing scheduler grace window.
curie_cron_jobs_overdue > 0

# Catch-up counter increased, proving at least one stale occurrence was
# collapsed and run once after a delay.
increase(curie_cron_scheduler_catch_up_occurrences[15m]) > 0
```

Cron execution lifecycle records arrive as `curie.cron_execution` spans.
Alert or derive events from bounded attributes such as:

```text
curie.status = failed|unknown
curie.delivery_outcome = failed|not_configured
curie.error_class = auth_failed|rate_limited|timeout|network_error|
...
3. a cron lifecycle feed keyed only by opaque `curie.job_key`;
...
Curie Agent-owned Relay transport health remains in scope. A separate gateway
...
python scripts/observability/otel_capture_collector.py \
  --host 127.0.0.1 --port 4318 --log /tmp/curie_otel_capture.jsonl
...
python scripts/observability/gateway_health_export_probe.py \
  ...
  --log /tmp/curie_otel_capture.jsonl --wait 8
...
`curie.error_class = ...` list in this file's alert section and the enum's unit
...
```bash
curie monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `curie monitoring` CLI intentionally exposes `status` only. This first
release covers only Curie Agent-owned service-health and operational-diagnostic
signals, including Curie Agent-owned Relay transport health. Team Gateway's
