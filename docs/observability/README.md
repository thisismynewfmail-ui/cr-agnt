# Curie Observer Hooks

Curie observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Curie' planner, model providers, memory, tool registry,
...
Curie also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "curie.observer.v1"
```

Hook callbacks are fail-open. Curie catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Curie only builds
...
The native NeMo Relay SDK integration maps Curie session, turn, LLM, and tool
