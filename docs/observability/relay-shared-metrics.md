...
Curie includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Curie and does not require a Curie observability plugin. Curie remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Curie execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `curie-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Curie `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `CURIE_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `CURIE_NEMO_RELAY_ATOF_*` and `CURIE_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Curie does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Curie requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
Curie installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Curie process so configured interceptors can
...
Collection remains off unless Curie policy enables it:
...
opt-in. Set `CURIE_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Curie does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, Curie
...
retain the same `session_id` and add `curie.session.segment` plus
`curie.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Curie
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Curie process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Curie logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this Curie process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Curie
process and share the same policy boundary.

Run profiles in separate Curie processes when they require different trust
...
Curie core owns one Relay host and one isolated Relay session scope per Curie
session. Core lifecycle producers use
`curie_cli.observability.relay_runtime` to obtain the shared session handle or
run Relay scope, LLM, tool, and mark APIs in that session context. New product
marks do not require Curie plugin registration. Shared-metrics marks must
...
```text
Curie turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Curie shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Curie sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Curie used for the logical call, such as
...
New calls use `curie.model_route.count`. The previous
`curie.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `curie.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Curie
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Curie processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `curie.task_run`, parented to
the owning Curie session. The start counter contains only bounded execution
...
provider attempts for the same Curie API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Curie tool-call
...
cancellations. Active task ownership follows the task ID if Curie rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`curie.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Curie
...
tool or plugin names. Curie does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `curie.tool_approval` marks and
...
Successful skill mutations emit `curie.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `curie.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Curie derives reuse and patch-generation
...
```text
$CURIE_HOME/telemetry/shared_metrics/metrics.sqlite3
$CURIE_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Curie version,
...
schema field name, its current scope is one `CURIE_HOME`, so it is more
...
Deleting `$CURIE_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `CURIE_HOME`. To reset it, stop Curie
processes and remove `$CURIE_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Curie CLI turn against the deterministic local model server:
...
`$CURIE_HOME/telemetry/shared_metrics/outbox/` to the Curie telemetry ingest
...
Removing `$CURIE_HOME/telemetry/shared_metrics` still resets local identity,
