...
or change arbitrary Curie configuration.
...
small host fact requested by the RFC. Settings writes use Curie'
...
`CURIE_HOME`, so one globally loaded plugin context follows multiplexed
...
```text
<CURIE_HOME>/plugin-data/<plugin-data-namespace>/state.json
```
...
The implementation is covered with real temporary-Curie-home tests for:
...
- [Issue #64227](https://github.com/thisismynewfmail-ui/Cur-Agnt/issues/64227)
- [RFC PR #58542](https://github.com/thisismynewfmail-ui/Cur-Agnt/pull/58542) by Topher Ross
