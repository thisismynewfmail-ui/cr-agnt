...
When running Curie inside Docker, the default `network_mode: host` gives the
...
The Curie [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   CURIE_UID=$(id -u) CURIE_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=curie,curie-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://curie-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Curie trust model and vulnerability reporting
