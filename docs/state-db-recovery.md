...
`curie doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `curie doctor --fix` while the gateway
is still up. Next steps:

```bash
curie gateway stop
CURIE_HOME="$HOME/.curie" curie sessions recover --source "$HOME/.curie/state.db" --inspect-only
# if recoverable:
CURIE_HOME="$HOME/.curie" curie sessions recover --source "$HOME/.curie/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
curie gateway stop
CURIE_HOME="$HOME/.curie" curie sessions repair --check-only
CURIE_HOME="$HOME/.curie" curie sessions repair
```
