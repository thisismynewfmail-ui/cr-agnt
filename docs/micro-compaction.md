...
has to be thrown away or summarized. Curie has always done this in one batch:
...
Curie folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Curie's normal session flush is
...
With `micro_compact` unset or `false` Curie behaves exactly as it always has:
...
Defaults to `$CURIE_HOME/logs/agent.log`. It reports passes, outcome mix, net
