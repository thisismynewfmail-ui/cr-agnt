"""Windows UTF-8 bootstrap for Curie entry points.
...
should be imported at the very top of every Curie entry point
(``curie``, ``curie-agent``, ``curie-acp``, ``python -m gateway.run``,
...
def suppress_platform_ver_console() -> None:
    ...
    Mirrors ``curie_cli._subprocess_compat.suppress_platform_ver_console``
    ...
    ``curie_bootstrap``, never ``curie_cli.main``.
    """
    if not _IS_WINDOWS:
        return
    try:
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Curie modules.

    Curie ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Curie source root
    ...
    """
    root = src_root or os.environ.get("CURIE_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``CURIE_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("CURIE_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import curie_bootstrap``
# (or ``from curie_bootstrap import apply_windows_utf8_bootstrap``) at
