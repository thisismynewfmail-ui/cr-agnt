...
_CURIE_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (CURIE_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "or XAI_API_KEY). Off by default; enable in `curie tools` → "
            "X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (CURIE_KANBAN_TASK env "
    ...
    # Coding posture (base Curie — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in curie_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Curie toolsets (CLI + messaging platforms)
    #
    ...
    # the `curie send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "curie-cli": {
        ...
        "tools": _CURIE_CORE_TOOLS,
        "includes": []
    },

    "curie-cron": {
        # Mirrors curie-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `curie tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as curie-cli; gated by `curie tools`",
        "tools": _CURIE_CORE_TOOLS,
        "includes": []
    },
    ...
    "curie-email": {
        "description": "Email bot toolset - interact with Curie via email (IMAP/SMTP)",
        "tools": _CURIE_CORE_TOOLS,
        "includes": []
    },
    ...
    "curie-sms": {
        "description": "SMS bot toolset - interact with Curie via SMS (Twilio)",
        "tools": _CURIE_CORE_TOOLS,
        "includes": []
    },
    ...
    "curie-webhook": {
        ...
        "tools": _CURIE_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "curie-gateway": {
        ...
        "tools": [],
        "includes": ["curie-telegram", "curie-discord", "curie-whatsapp", "curie-slack", "curie-signal", "curie-bluebubbles", "curie-homeassistant", "curie-email", "curie-sms", "curie-mattermost", "curie-matrix", "curie-dingtalk", "curie-feishu", "curie-wecom", "curie-wecom-callback", "curie-weixin", "curie-qqbot", "curie-webhook", "curie-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``curie-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_CURIE_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``curie-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (curie-<name>).
        # Gives them _CURIE_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("curie-"):
            platform_name = name[len("curie-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_CURIE_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
