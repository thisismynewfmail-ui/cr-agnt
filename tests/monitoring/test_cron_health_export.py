...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("curie.gateway.up"), _M("curie.gateway.active_agents"),
        _M("curie.gateway.busy"), _M("curie.gateway.drainable"),
        _M("curie.gateway.restart_requested"),
        _M("curie.platform.up"), _M("curie.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("curie.cron.scheduler.heartbeat_age_seconds"),
        _M("curie.cron.scheduler.last_success_age_seconds"),
        _M("curie.cron.scheduler.catch_up_occurrences"),
        _M("curie.cron.jobs.enabled"), _M("curie.cron.jobs.running"),
        _M("curie.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Curie Agent-owned Relay transport health" in text
