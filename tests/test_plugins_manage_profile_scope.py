...
``set_curie_home_override`` / ``reset_curie_home_override``. Because
``_plugins_dir()`` keys off ``get_curie_home()``, the list action must then
...
def test_plugins_manage_profile_reads_that_profiles_dir(tmp_path, monkeypatch):
    ...
    import curie_cli.profiles as profiles
