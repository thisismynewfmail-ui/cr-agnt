"""Tests for curie_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Curie entry point
(curie, curie-agent, curie-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Curie entry point imports curie_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported curie_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("curie_bootstrap", None)
    import curie_bootstrap  # noqa: WPS433
    return curie_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Curie CLI
...
class TestEntryPointsImportBootstrap:
    """Every Curie entry point must import curie_bootstrap as its
    ...
    # Entry points that invoke Curie as a process.  Each one must
    # import curie_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "curie_cli/main.py",   # curie CLI (console_script)
        "run_agent.py",          # curie-agent (console_script)
        "acp_adapter/entry.py",  # curie-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import curie_bootstrap' and that
        ...
        half-finished ``curie update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``curie_bootstrap`` as a top-level module) leaves curie
        ...
        """
        # Resolve relative to the curie-agent repo root.  Tests live
        # at tests/test_curie_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # curie start even when curie_bootstrap hasn't been
        ...
        assert first_import_name == "curie_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'curie_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import curie_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Curie's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("CURIE_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["CURIE_PYTHON_SRC_ROOT"] = env
            elif "CURIE_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["CURIE_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/curie")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("CURIE_PYTHON_SRC_ROOT", None)
            else:
                os.environ["CURIE_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/curie", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Curie root.  The guard must relocate Curie to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/curie"])
        assert result[0] == "/opt/curie"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Curie root.
        assert result.index("/opt/curie") < result.index("/home/user/tg-ws-proxy")
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["CURIE_PYTHON_SRC_ROOT"] = "/env/curie"
            hb.harden_import_path()
            assert sys.path[0] == "/env/curie"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("CURIE_PYTHON_SRC_ROOT", None)
