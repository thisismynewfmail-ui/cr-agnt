...
Run from the PR worktree:
    ...
    cd ~/.curie/curie-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.curie/curie-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.curie/curie-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a curie-agent checkout"
)
...
# Isolated CURIE_HOME for the config write.
