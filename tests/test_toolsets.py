...
class TestToolsetConsistency:
    ...
    def test_curie_platforms_share_core_tools(self):
        """All curie-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on curie-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["curie-cli", "curie-telegram", "curie-discord", "curie-whatsapp", "curie-slack", "curie-signal", "curie-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_curie_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("curie-whatsapp")
...
class TestResolveToolsetMemo:
    ...
    def test_second_resolution_is_cached(self, monkeypatch):
        ...
        first = resolve_toolset("curie-cli")
        second = resolve_toolset("curie-cli")
        ...
        assert (
            "curie-cli", True, registry_id, generation
    ...
    def test_memo_result_matches_fresh_resolution(self):
        ...
        toolsets_mod._resolve_toolset_memo.clear()
        first = resolve_toolset("curie-cli", include_registry=False)
        second = resolve_toolset("curie-cli", include_registry=False)
        assert first == second
