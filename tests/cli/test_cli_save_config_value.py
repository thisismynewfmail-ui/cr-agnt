...
class TestSaveConfigValueAtomic:
    ...
    def config_env(self, tmp_path, monkeypatch):
        ...
        # save_config_value resolves the target live via get_curie_home(), so
        # point CURIE_HOME at the temp dir (the _curie_home import-time
    ...
    def test_model_write_runs_shared_cron_drift_warning(self, config_env, monkeypatch):
        warning = MagicMock()
        monkeypatch.setattr(
            "curie_cli.config.warn_unpinned_cron_jobs_after_model_config_change",
            warning,
        )
...
class TestSaveConfigValueTargetsUserConfig:
    """Regression: persisted runtime settings must land in CURIE_HOME/config.yaml
    ...
    install whose CURIE_HOME/config.yaml did not exist yet, save_config_value
    fell back to the checked-in cli-config.yaml. The toggle reported success, but
    startup read CURIE_HOME/config.yaml and never saw the setting."""

    def test_creates_user_config_when_absent(self, tmp_path, monkeypatch):
        # Fresh CURIE_HOME with NO config.yaml (managed/desktop first launch).
        curie_home = tmp_path / ".curie"
        curie_home.mkdir()
        monkeypatch.setenv("CURIE_HOME", str(curie_home))
