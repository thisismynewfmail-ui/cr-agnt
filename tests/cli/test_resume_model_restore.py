...
SessionDB.session_gateway_runtime (curie_state.py) — the round trip that
makes `curie --resume` reopen a session on the model/provider it actually
...
def _make_stub(**overrides):
    """Bare CurieCLI the way resume paths see it (no __init__)."""
    stub = object.__new__(cli_mod.CurieCLI)
