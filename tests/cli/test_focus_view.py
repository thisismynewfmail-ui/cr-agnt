...
class TestStatusBarSegment:
    ...
    def test_text_renderer_includes_the_badge_at_every_width_tier(self, width):
        ...
        host = CurieCLI.__new__(CurieCLI)
        host.model = "opus"
        ...
        with patch.object(CurieCLI, "_get_status_bar_snapshot", return_value=snapshot), \
             patch.object(CurieCLI, "_is_session_unlock_active", return_value=False):
            text = CurieCLI._build_status_bar_text(host, width=width)
