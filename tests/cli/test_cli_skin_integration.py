...
from cli import CurieCLI, _rich_text_from_ansi
...
class TestCliSkinPromptIntegration:
    ...
    def test_prompt_fragments_use_the_skin_symbol(self):
        ...
        The point is that the symbol comes from the active skin rather than
        from a hard-coded fallback, so the skin under test has to be one
        whose symbol actually differs from the default's.
        """
        cli = _make_cli_stub()
    ...
    def test_narrow_terminals_compact_voice_recording_prompt_fragments(self):
        ...
        with patch.object(CurieCLI, "_get_tui_terminal_width", return_value=50):
