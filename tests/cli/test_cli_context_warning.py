...
def _isolate(tmp_path, monkeypatch):
    """Isolate CURIE_HOME so tests don't touch real config."""
    home = tmp_path / ".curie"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal CurieCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = CurieCLI.__new__(CurieCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["curie-core"]
        obj.compact = False
...
class TestLowContextWarning:
    ...
    def test_warning_for_below_minimum_context(self, cli_obj):
        """Warning shown when context is below Curie' minimum."""
    ...
    def test_no_warning_at_boundary(self, cli_obj):
        """No warning at exactly Curie' minimum context length."""
    ...
    def test_no_warning_above_boundary(self, cli_obj):
        """No warning above Curie' minimum context length."""
