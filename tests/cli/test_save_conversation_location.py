...
Regression: the old implementation wrote ``curie_conversation_<ts>.json``
to the current working directory (CWD). Users who ran /save expected the
file to be discoverable via ``curie sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.curie/sessions/saved/`` and prints
...
def curie_home(tmp_path, monkeypatch):
    ...
    # Clear any cached curie_home computation
    import curie_constants
    if hasattr(curie_constants, "_curie_home_cache"):
        curie_constants._curie_home_cache = None
    return home
...
def test_save_conversation_writes_under_curie_home(curie_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.curie/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the CURIE_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "curie_constants"]:
    ...
    cli.CurieCLI.save_conversation(stub, "/save json")

    # File must NOT be in CWD
    cwd_leak = list(work.glob("curie_conversation_*.json"))
    assert not cwd_leak, f"snapshot leaked to CWD: {cwd_leak}"

    # File MUST be under ~/.curie/sessions/saved/
    saved_dir = curie_home / "sessions" / "saved"
    assert saved_dir.is_dir(), "expected saved/ subdirectory to be created"
    files = list(saved_dir.glob("curie_conversation_*.json"))
    ...
    assert "curie --resume 20260101_120000_abc123" in out, out


def test_save_conversation_empty_history_does_nothing(curie_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "curie_constants"]:
        sys.modules.pop(mod, None)
    import cli
...
def test_save_conversation_bare_shows_usage(curie_home, capsys):
    ...
    cli.CurieCLI.save_conversation(stub, "/save")
...
def test_save_conversation_bad_format_shows_usage(curie_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "curie_constants"]:
        sys.modules.pop(mod, None)
    import cli

    stub = _make_stub_cli([{"role": "user", "content": "hi"}])
    cli.CurieCLI.save_conversation(stub, "/save pdf")
