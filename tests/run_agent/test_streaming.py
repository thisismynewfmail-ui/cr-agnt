...
class TestStreamingFallback:
    ...
    def test_sse_connection_lost_retried_as_transient(self, mock_close, mock_create):
        ...
        # Should retry 3 times (default CURIE_STREAM_RETRIES=2 → 3 attempts)
...
class TestAnthropicStreamCallbacks:
    ...
    def test_anthropic_stream_parser_valueerror_retries_before_delivery(
        ...
        agent.api_mode = "anthropic_messages"
        agent._interrupt_requested = False
        monkeypatch.setenv("CURIE_STREAM_RETRIES", "1")
...
class TestPartialToolCallWarning:
    ...
    text was already delivered, the partial-stream stub used to silently set
    ``tool_calls=None`` and return ``finish_reason=stop``, losing the
    attempted action with zero signal.

    Fix (two parts):

    * The stub is tagged ``PARTIAL_STREAM_STUB_ID`` with
      ``finish_reason="length"``, which the conversation loop recognises and
      retries automatically (up to 4 times).  The action is not lost.
    * That retry is a transport detail, so it stays OUT of the transcript.
      The earlier implementation spliced "⚠ Stream stalled mid tool-call
      (write_file); the action was not executed.  Ask me to retry if you want
      to continue." into the assistant's content and fired it as a live
      delta — an apology that was untrue (the loop was already retrying),
      unactionable (there is nothing for the user to do), and persistent (it
      was saved to history, where the model read it back and apologised
      again).  Recovery is reported on the transient status line only.
    """
    ...
    def test_partial_tool_call_surfaces_warning(self, mock_close, mock_create):
        ...
        import os as _os
        _prev = _os.environ.get("CURIE_STREAM_RETRIES")
        _os.environ["CURIE_STREAM_RETRIES"] = "0"
        try:
            response = agent._interruptible_streaming_api_call({})
        finally:
            if _prev is None:
                _os.environ.pop("CURIE_STREAM_RETRIES", None)
            else:
                _os.environ["CURIE_STREAM_RETRIES"] = _prev

        from curie_constants import PARTIAL_STREAM_STUB_ID

        content = response.choices[0].message.content or ""
        assert "Let me write the audit:" in content, (
            f"Partial text not preserved in stub: {content!r}"
        )
        # The stub must be recognisable to the conversation loop's
        # auto-continuation path — that, not a warning, is what stops the
        # action from being lost.
        assert response.id == PARTIAL_STREAM_STUB_ID, (
            f"Stub is not tagged for auto-retry: id={response.id!r}"
        )
        assert response.choices[0].finish_reason == "length", (
            f"Stub must trip the continuation machinery, got "
            f"{response.choices[0].finish_reason!r}"
        )
        assert response.choices[0].message.tool_calls is None
        # No apology in the transcript, and none streamed to the user.
        assert "Stream stalled" not in content, (
            f"Transport apology leaked into assistant content: {content!r}"
        )
        assert "Ask me to retry" not in content, (
            f"Removed 'ask me to retry' prompt is back: {content!r}"
        )
        joined = "".join(fired_deltas)
        assert "Stream stalled" not in joined and "Ask me to retry" not in joined, (
            f"Transport apology streamed to the user: {joined!r}"
        )
...
class TestSilentRetryMidToolCall:
    ...
    def test_silent_retry_recovers_tool_call(
        ...
        import os as _os
        _prev = _os.environ.get("CURIE_STREAM_RETRIES")
        _os.environ["CURIE_STREAM_RETRIES"] = "2"
        try:
            response = agent._interruptible_streaming_api_call({})
        finally:
            if _prev is None:
        ...
        # The reconnect is reported on the status line, never spliced into
        # the assistant's text: the second attempt re-streams the same
        # preamble, so a delta here landed mid-sentence in the reply the
        # user was reading.
        joined = "".join(fired_deltas)
        assert "reconnecting" not in joined.lower(), (
            f"Reconnect marker leaked into the transcript as a stream "
            f"delta: {joined!r}"
        )
    ...
    def test_silent_retry_exhausted_falls_back_to_stub(
        ...
        """When all retry attempts fail with connection errors, fall back to
        the retryable partial-stream stub so the conversation loop — not the
        user — picks the turn back up."""
        from run_agent import AIAgent
        import httpx as _httpx
        ...
        import os as _os
        _prev = _os.environ.get("CURIE_STREAM_RETRIES")
        _os.environ["CURIE_STREAM_RETRIES"] = "1"
        try:
            response = agent._interruptible_streaming_api_call({})
        finally:
            if _prev is None:
        ...
        # After retries exhaust, the retryable stub must engage — carrying
        # the recovered text but none of the removed apology copy.
        from curie_constants import PARTIAL_STREAM_STUB_ID

        content = response.choices[0].message.content or ""
        assert response.id == PARTIAL_STREAM_STUB_ID, (
            f"Exhausted-retry fallback did not return a retryable stub: "
            f"id={response.id!r}"
        )
        assert response.choices[0].finish_reason == "length"
        assert "Stream stalled" not in content, (
...
class TestBedrockStreamLivenessWatchdog:
    ...
    def test_stalled_stream_bumps_streak_and_aborts(self, monkeypatch):
        ...
        monkeypatch.setenv("CURIE_STREAM_STALE_TIMEOUT", "0.5")
        monkeypatch.setenv("CURIE_STREAM_STALE_GIVEUP", "5")
    ...
    def test_successful_stream_resets_streak(self, monkeypatch):
        ...
        monkeypatch.setenv("CURIE_STREAM_STALE_TIMEOUT", "60")
...
class TestNoFinishReasonIsNotAStall:
    """Regression: a missing ``finish_reason`` is not evidence of a stall.

    Plenty of OpenAI-compatible servers (llama.cpp, vLLM forks, LiteLLM and
    OpenRouter passthroughs, bespoke gateways) close a perfectly complete
    stream without ever emitting ``finish_reason`` — they just stop, or send
    it on a chunk whose ``choices`` list is empty.  The mid-tool-call drop
    detector used to treat that absence, on its own, as proof the connection
    had died, so every large tool call those endpoints served was reported as
    "⚠ Stream stalled mid tool-call (write_file); the action was not
    executed" and re-run from scratch — up to four times — before failing.

    The detector now needs positive evidence: arguments that cannot be parsed
    *or* repaired, on a stream that also carried no usage record.
    """
    ...
    @patch("run_agent.AIAgent._create_request_openai_client")
    @patch("run_agent.AIAgent._close_request_openai_client")
    def test_complete_tool_call_without_finish_reason_executes(
        self, mock_close, mock_create,
    ):
        """Valid, complete tool-call JSON + no finish_reason → real tool call."""
        from curie_constants import PARTIAL_STREAM_STUB_ID

        big_body = "line of the report\n" * 400

        def _stream():
            yield _make_stream_chunk(content="Writing the report now. ")
            yield _make_stream_chunk(tool_calls=[
                _make_tool_call_delta(index=0, tc_id="call_1", name="write_file"),
            ])
            # Arguments arrive in fragments, as they do on the wire.
            payload = json.dumps({"path": "/tmp/report.txt", "content": big_body})
            for i in range(0, len(payload), 512):
                yield _make_stream_chunk(tool_calls=[
                    _make_tool_call_delta(index=0, arguments=payload[i:i + 512]),
                ])
            # …and then the server simply stops.  No finish_reason, no usage.
        ...
        assert response.id != PARTIAL_STREAM_STUB_ID, (
            "A complete tool call was misreported as a mid-stream drop"
        )
        tool_calls = response.choices[0].message.tool_calls
        assert tool_calls, f"Tool call was discarded: {response!r}"
        assert tool_calls[0].function.name == "write_file"
        args = json.loads(tool_calls[0].function.arguments)
        assert args["content"] == big_body, "Streamed arguments were corrupted"

    @patch("run_agent.AIAgent._create_request_openai_client")
    @patch("run_agent.AIAgent._close_request_openai_client")
    def test_parameterless_tool_with_empty_arguments_executes(
        self, mock_close, mock_create,
    ):
        """A no-argument tool sending no arguments is correct, not truncated."""
        from curie_constants import PARTIAL_STREAM_STUB_ID

        def _stream():
            yield _make_stream_chunk(tool_calls=[
                _make_tool_call_delta(index=0, tc_id="call_1", name="list_sessions"),
            ])
        ...
        assert response.id != PARTIAL_STREAM_STUB_ID, (
            "A parameterless tool call was misreported as a mid-stream drop"
        )
        tool_calls = response.choices[0].message.tool_calls
        assert tool_calls and tool_calls[0].function.name == "list_sessions"

    @patch("run_agent.AIAgent._create_request_openai_client")
    @patch("run_agent.AIAgent._close_request_openai_client")
    def test_required_arguments_never_delivered_still_reported(
        self, mock_close, mock_create,
    ):
        """The genuine drop this guard exists for must still be caught."""
        from curie_constants import PARTIAL_STREAM_STUB_ID
        ...
        assert response.id == PARTIAL_STREAM_STUB_ID, (
            "A tool call whose required arguments never arrived was executed "
            "with no arguments instead of being retried"
        )
        assert response.choices[0].message.tool_calls is None

    @patch("run_agent.AIAgent._create_request_openai_client")
    @patch("run_agent.AIAgent._close_request_openai_client")
    def test_repairable_arguments_without_finish_reason_execute(
        self, mock_close, mock_create,
    ):
        """Repairable JSON is recovered, not treated as a lost tool call.

        ``_repair_tool_call_arguments`` returns ``"{}"`` both when it gives up
        *and* when the repaired result really is an empty object.  The
        detector used to read that bare string as "gave up", so any tool call
        needing repair on a finish_reason-less stream was thrown away.
        """
        from curie_constants import PARTIAL_STREAM_STUB_ID
        ...
        def _stream():
            ...
            # Trailing comma — the classic local-backend malformation.
            yield _make_stream_chunk(tool_calls=[
                _make_tool_call_delta(
                    index=0, arguments='{"path": "/tmp/x", "content": "hi",}',
                ),
        ...
        assert response.id != PARTIAL_STREAM_STUB_ID
        tool_calls = response.choices[0].message.tool_calls
        assert tool_calls, "Repairable tool call was discarded"
        assert json.loads(tool_calls[0].function.arguments)["content"] == "hi"

    @patch("run_agent.AIAgent._create_request_openai_client")
    @patch("run_agent.AIAgent._close_request_openai_client")
    def test_usage_chunk_proves_clean_completion(self, mock_close, mock_create):
        """Unparseable args + a usage record → genuine output-cap truncation.

        A usage record proves the provider finished generating and closed the
        stream cleanly, so this is the max_tokens path (which boosts the
        budget and retries), not a network drop.
        """
        from curie_constants import PARTIAL_STREAM_STUB_ID
        ...
        def _stream():
            ...
            yield _make_stream_chunk(tool_calls=[
                _make_tool_call_delta(index=0, arguments='{"path": "/tmp/x", "content": "unterminated'),
            ])
            yield _make_empty_chunk(usage=SimpleNamespace(
        ...
        assert response.id != PARTIAL_STREAM_STUB_ID, (
            "A cleanly-closed stream was reported as a network drop"
        )
        assert response.choices[0].finish_reason == "length", (
            "Output-cap truncation should route to the max_tokens-boost path"
        )
