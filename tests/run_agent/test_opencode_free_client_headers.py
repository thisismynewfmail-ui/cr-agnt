...
@patch("run_agent.OpenAI")
def test_opencode_free_sends_curie_attribution(mock_openai):
    """Keyless requests still identify as Curie (attribution headers match
    ...
    headers = _zen_call_headers(mock_openai)
    assert headers.get("X-Title") == "Curie Agent"
    assert str(headers.get("User-Agent", "")).startswith("CurieAgent/")
