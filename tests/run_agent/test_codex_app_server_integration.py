...
class TestRunConversationCodexPath:
    ...
    def test_gateway_terminal_cwd_seeds_codex_thread_cwd(self, monkeypatch, tmp_path):
        ...
        of falling back to the Curie daemon process cwd."""
        from agent.transports.codex_app_server_session import (
            CodexAppServerSession, TurnResult,
        )
    ...
    def test_approvals_mode_off_auto_approves_codex_server_requests(
        ...
        """When the user disables Curie approvals, codex app-server approval
    ...
    def test_frozen_unlock_env_auto_approves_codex_server_requests(
        self, monkeypatch
    ):
        """--unlock / CURIE_UNLOCK_MODE (frozen into _UNLOCK_MODE_FROZEN at import
        ...
        fail closed when the user launched with unlock mode."""
        import tools.approval as _approval

        captured = self._capture_routing_agent(monkeypatch)
        monkeypatch.setattr(_approval, "_UNLOCK_MODE_FROZEN", True)
        with patch(
            "curie_cli.config.load_config",
            return_value={"approvals": {"mode": "manual"}},
        ):
    ...
    def test_session_unlock_auto_approves_codex_server_requests(
        self, monkeypatch
    ):
        """The /unlock session toggle should be honored at Codex session creation
...
class TestCodexToolProgressBridge:
    ...
    Curie tool-progress so gateways show verbose breadcrumbs on this route.
