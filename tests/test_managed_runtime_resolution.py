"""Guard: Curie-owned subprocesses must not resolve managed runtimes by bare PATH.

Curie installs runtimes for itself — ``uv`` at ``$CURIE_HOME/bin/uv``, Node at
``$CURIE_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Curie's own
...
* when a system copy also exists, the one Curie does not own wins — which is
...
``iter_curie_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Curie provisions into CURIE_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Curie-owned subprocess code: plugins ship their own
...
def _source_files() -> list[Path]:
    ...
    # (curie_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Curie-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$CURIE_HOME/bin (uv) and $CURIE_HOME/node are not on an "
        ...
        "  node/npm -> curie_constants.find_node_executable()\n"
        "  PATH env -> curie_constants.iter_curie_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(curie_constants, helper))


def test_managed_uv_helpers_exist():
    from curie_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
