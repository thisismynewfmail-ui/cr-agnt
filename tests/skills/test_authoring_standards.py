...
def test_required_frontmatter_fields(p):
    ...
    curie = (fm.get("metadata") or {}).get("curie") or {}
    if not (curie.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.curie.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    curie = (fm.get("metadata") or {}).get("curie") or {}
    dangling = [
        rs for rs in (curie.get("related_skills") or []) if rs not in _all_names()
    ]
