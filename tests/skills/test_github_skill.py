...
def test_frontmatter_required_fields():
    ...
    curie = fm["metadata"]["curie"]
    assert curie["tags"]
    assert "related_skills" in curie
...
def test_author_credits_human_first():
    fm, _ = _frontmatter_and_body()
    assert not fm["author"].startswith("Curie Agent"), "human contributor must be credited first"
...
def test_related_skills_resolve_in_repo():
    ...
    for name in fm["metadata"]["curie"]["related_skills"]:
        hits = (
