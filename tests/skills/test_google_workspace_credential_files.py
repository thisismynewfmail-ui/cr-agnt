...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        curie_home.mkdir()
        (curie_home / "google_token.json").write_text("{}")
        (curie_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.curie/google_token.json" in container_paths
            assert "/root/.curie/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()
