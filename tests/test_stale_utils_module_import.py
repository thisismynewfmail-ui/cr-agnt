...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.curie/curie-agent/utils.py)
...
2. ``curie update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``curie_cli/main.py`` (the ``curie update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``curie_constants`` after the
