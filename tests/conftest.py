"""Shared fixtures for the curie-agent test suite.
...
2. **Isolated CURIE_HOME.** CURIE_HOME points to a per-test tempdir so
   code reading ``~/.curie/*`` via ``get_curie_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".curie"`` instead of the canonical
   ``get_curie_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No CURIE_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox CURIE_HOME before ANY test module is imported ──────────────────
# `curie_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_curie_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.curie/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes CURIE_HOME, but fixtures run
...
# the REAL Curie root — capture it BEFORE the sandbox rewires CURIE_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.curie (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("CURIE_KANBAN_HOME", "").strip()
_PRE_SANDBOX_CURIE_HOME = os.environ.get("CURIE_HOME", "")


def _curie_home_points_at_production(value: str) -> bool:
    """True when a pre-set CURIE_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    CURIE_HOME=~/.curie``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``curie_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.curie`` — the escape vector that landed pytest
    ...
    CURIE_HOME is honored now.
    """
    if not value:
        return True
    try:
        resolved = Path(value).expanduser().resolve()
        real_root = (Path.home() / ".curie").resolve()
    except Exception:
        return True
...
if _curie_home_points_at_production(os.environ.get("CURIE_HOME", "")):
    _SESSION_CURIE_HOME = tempfile.mkdtemp(prefix="curie-test-home-")
    os.environ["CURIE_HOME"] = _SESSION_CURIE_HOME
    atexit.register(shutil.rmtree, _SESSION_CURIE_HOME, True)
...
# like a real CLI") — which used to disarm curie_state's live-DB guard in
# the child at the same moment the child lost the CURIE_HOME redirect.
# CURIE_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# curie_state._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export CURIE_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["CURIE_TEST_ISOLATION"] = os.environ.get("CURIE_HOME", "") or "1"

#: CURIE_HOME as it stood when conftest was imported - i.e. before any test
...
CURIE_HOME_AT_CONFTEST_IMPORT = os.environ.get("CURIE_HOME", "")
...
# CURIE_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_CURIE_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``curie_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a Curie agent session inherits them and
    # curie_constants home-resolution helpers prefer them over monkeypatched
    ...
    "CURIE_MAX_ITERATIONS",
    "CURIE_SESSION_PLATFORM",
    "CURIE_SESSION_CHAT_ID",
    "CURIE_SESSION_CHAT_NAME",
    "CURIE_SESSION_CHAT_TYPE",
    "CURIE_SESSION_THREAD_ID",
    "CURIE_SESSION_SOURCE",
    "CURIE_SESSION_KEY",
    "CURIE_GATEWAY_SESSION",
    "CURIE_CRON_SESSION",
    "_CURIE_GATEWAY",
    "CURIE_PLATFORM",
    "CURIE_MODEL",
    "CURIE_INFERENCE_MODEL",
    "CURIE_INFERENCE_PROVIDER",
    "CURIE_TUI_PROVIDER",
    "CURIE_MANAGED",
    "CURIE_MANAGED_DIR",
    "CURIE_DEV",
    ...
    "CURIE_TIMEZONE",
    "CURIE_REDACT_SECRETS",
    "CURIE_BACKGROUND_NOTIFICATIONS",
    "CURIE_EXEC_ASK",
    "CURIE_HOME_MODE",
    "CURIE_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.curie/kanban.db instead of the per-test CURIE_HOME.
    "CURIE_KANBAN_DB",
    "CURIE_KANBAN_BOARD",
    "CURIE_KANBAN_HOME",
    "CURIE_KANBAN_WORKSPACES_ROOT",
    "CURIE_KANBAN_LOGS_ROOT",
    "CURIE_KANBAN_TASK",
    "CURIE_KANBAN_WORKSPACE",
    "CURIE_KANBAN_RUN_ID",
    "CURIE_KANBAN_CLAIM_LOCK",
    "CURIE_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    # otherwise-unrelated config tests away from the default "curie" host.
    "CURIE_HONCHO_HOST",
    ...
    "CURIE_DASHBOARD_OAUTH_CLIENT_ID",
    "CURIE_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and CURIE_HOME to per-test tempdirs so code that
    reads ``~/.curie/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral CURIE_* vars that could change test semantics.
    for name in _CURIE_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("CURIE_HONCHO_HOST", "curie")

    # 3. Redirect CURIE_HOME to a per-test tempdir. Code that reads
    #    ``~/.curie/*`` via ``get_curie_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.curie/*`` via
    #    ``Path.home() / ".curie"`` instead of ``get_curie_home()``
    #    is a bug to fix at the callsite.
    fake_curie_home = tmp_path / "curie_test"
    fake_curie_home.mkdir()
    (fake_curie_home / "sessions").mkdir()
    (fake_curie_home / "cron").mkdir()
    (fake_curie_home / "memories").mkdir()
    (fake_curie_home / "skills").mkdir()
    monkeypatch.setenv("CURIE_HOME", str(fake_curie_home))
    ...
    # curie_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("CURIE_TEST_ISOLATION", str(fake_curie_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("CURIE_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. curie_state computes ``DEFAULT_DB_PATH = get_curie_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from curie_state import ...``) that
    ...
    curie_state_mod = sys.modules.get("curie_state")
    if curie_state_mod is not None and hasattr(curie_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            curie_state_mod, "DEFAULT_DB_PATH", fake_curie_home / "state.db"
        )
    ...
    monkeypatch.setenv("CURIE_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.curie/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import curie_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path CURIE_HOME can't leak forward. Paths
...
@pytest.fixture(autouse=True)
def _isolate_curie_home(_hermetic_environment):
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from curie_cli import kanban_db as _kb_mod
    except Exception:
        return
...
# invocation), kanban writes silently pollute the real ~/.curie. This autouse
...
# allow-list because test-level fixtures legitimately move CURIE_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session CURIE_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``CURIE_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Curie root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_CURIE_HOME and not _curie_home_points_at_production(
        _PRE_SANDBOX_CURIE_HOME
    ):
        # CURIE_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_curie_root().resolve()
    # No pre-existing CURIE_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".curie").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.curie`` captured at import time. Hermetic tests that legitimately
    move CURIE_HOME to sibling tempdirs are unaffected.

    Only patches when ``curie_cli.kanban_db`` is *already imported* — a
    ...
    """
    _kdb = sys.modules.get("curie_cli.kanban_db")
    if _kdb is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import curie_cli.kanban_db`
...
# ``curie_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Curie root. This fixture wires the
# test-side knobs:
...
#     installs where CURIE_HOME is not ~/.curie) into the guard's
...
# so subprocess children that import curie_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("curie_state")
    ...
    if _PRE_SANDBOX_CURIE_HOME and not _curie_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_CURIE_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Curie home override redirects every later
    # ``get_curie_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from curie_constants import get_curie_home_override, set_curie_home_override

        if get_curie_home_override() is not None:
            set_curie_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal curie config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``curie-gateway`` process
...
# correlated with a ``tests/curie_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> curie-gateway``
...
def _wal_is_usable() -> bool:
    """True when Curie will actually put a database into WAL mode here.

    Curie refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Curie can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Curie managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``curie_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_curie_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_curie_home`` fixture
    redirects ``CURIE_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.curie/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``curie_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["CURIE_VOICE_TTS"] = "1"``. The
...
#      ``curie_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``curie_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``curie_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``curie_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Curie runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.curie``.
#
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Curie falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Curie uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart curie-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting curie/python patterns are also blocked.
    """
    ...
    def _matches_curie_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _CURIE_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_curie_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch curie-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m curie_cli.main".
                if (
                    "curie" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live curie-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting curie/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `curie update` (or the
        # equivalent `python -m curie_cli.main update`).  These commands
        ...
        if "update" in low and (
            # curie update / curie update --gateway / setsid bash -c ... curie update
            ("curie" in low and "update" in low.split())
            or
            # python -m curie_cli.main update --gateway
            ("curie_cli" in low and "update" in low.split())
            or
            # venv/bin/curie update  (absolute path variant used in tests)
            (".venv/bin/curie" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`curie update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_CURIE_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import curie_cli.voice as _voice
    except Exception:
