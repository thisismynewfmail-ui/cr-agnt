"""Cross-session CURIE_SESSION_ID leak via the shared bash snapshot.
...
``CURIE_SESSION_ID`` into the snapshot, so every LATER session ``source``d that
stale value and its ``echo $CURIE_SESSION_ID`` reported a FOREIGN session's id
— overriding the correct per-command Popen env injected by
``_inject_session_context_env``.

The fix strips the per-session bridged vars (CURIE_SESSION_* / UI /
...
def test_export_snippet_shape():
    snippet = _export_dump_excluding_session_vars('"$__curie_snap_tmp"')
    ...
    assert "${!CURIE_SESSION_*}" in snippet
    assert "${!CURIE_CRON_AUTO_DELIVER_*}" in snippet
    assert "${!CURIE_BROWSER_CONTROL_*}" in snippet
    assert "CURIE_UI_SESSION_ID" in snippet
    assert "grep -vE" not in snippet
    assert '"$__curie_snap_tmp"' in snippet
    ...
    assert snippet.rstrip().endswith('> "$__curie_snap_tmp"')
    ...
    try:
        def run_as(sid):
            ...
            def worker():
                ...
                out["r"] = env.execute('echo "[$CURIE_SESSION_ID]"')
        ...
        if os.path.exists(snap):
            with open(snap) as f:
                assert "CURIE_SESSION_ID" not in f.read()
    finally:
        env.cleanup()
