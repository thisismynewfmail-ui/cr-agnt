...
The hardline list is a floor below unlock: a small set of commands so
catastrophic they should never run via the agent, regardless of --unlock,
gateway /unlock, approvals.mode=off, or cron approve mode.
...
_HARDLINE_BLOCK = [
    ...
    # rule, which --unlock / approvals.mode=off / cron approve-mode bypass).
    "rm -rf //",
    "rm -rf /.",
    "rm -rf /./",
    "rm -rf /..",
    ...
    # guard: catastrophic disk/home wipe under --unlock / approvals.mode=off).
    'rm -rf "/"',
    "rm -rf '/'",
    'rm -rf "/*"',
    'rm -rf "/etc"',
    ...
    # learned to recognize `(` / `{` (issue: (reboot) walked past --unlock).
    "(reboot)",
    "( reboot )",
    "(shutdown -h now)",
    "(poweroff)",
...
# boundary, blocking `curie send` message bodies, multi-line
# `git commit -m` messages, and heredoc text that merely MENTION
# shutdown/reboot commands.
_QUOTED_NEWLINE_DATA_ALLOW = [
    # curie send with a multi-line message body (the reported symptom)
    'curie send -t telegram -s "spark1" "console output:\nsudo reboot\ndone"',
    'curie send -t telegram "line1\nshutdown -h now\nline3"',
...
_QUOTED_NEWLINE_THREATS_BLOCK = [
    ...
    'curie send -t telegram "$(sudo reboot)"',
    'echo "`shutdown -h now`"',
...
def test_quoted_newline_data_not_blocked_by_full_guard_chain(clean_session):
    ...
    quoted message (unlock on, so only the unconditional floor can block)."""
    enable_session_unlock("hardline_test")
    command = 'curie send -t telegram "status:\nsudo reboot happened at 3am"'
...
# stay flagged (unlock can still bypass this tier — that's what unlock is for).
...
def clean_session(monkeypatch):
    ...
    try:
        disable_session_unlock("hardline_test")
        yield
    finally:
        disable_session_unlock("hardline_test")
...
def test_unlock_env_var_cannot_bypass_hardline(clean_session, monkeypatch):
    """CURIE_UNLOCK_MODE=1 must not bypass the hardline floor."""
    monkeypatch.setenv("CURIE_UNLOCK_MODE", "1")

    for cmd in ['rm -rf /', 'rm -rf "/"', 'rm -rf "$HOME"', "rm -rf ${HOME}",
                "shutdown -h now", "mkfs.ext4 /dev/sda", "reboot"]:
        r1 = check_dangerous_command(cmd, "local")
        assert r1["approved"] is False, f"unlock leaked hardline on {cmd!r} (check_dangerous_command)"
        assert r1.get("hardline") is True

        r2 = check_all_command_guards(cmd, "local")
        assert r2["approved"] is False, f"unlock leaked hardline on {cmd!r} (check_all_command_guards)"
        assert r2.get("hardline") is True


def test_root_collapse_forms_cannot_bypass_hardline(clean_session, monkeypatch):
    """Shell-equivalent spellings of "rm -rf /" stay blocked under unlock.

    "//", "/.", "/./", "/..", "//*" all collapse to the root filesystem in
    the shell. They previously matched only the softer DANGEROUS_PATTERNS
    rule, which unlock bypasses — leaving the hardline floor open to a full
    root wipe under --unlock / approvals.mode=off / cron approve-mode.
    """
    monkeypatch.setenv("CURIE_UNLOCK_MODE", "1")
    ...
    for cmd in ["rm -rf //", "rm -rf /.", "rm -rf /./", "rm -rf /..", "rm -rf //*"]:
        ...
        assert result["approved"] is False, f"unlock leaked hardline on {cmd!r}"
        assert result.get("hardline") is True
...
def test_subshell_brace_group_cannot_bypass_hardline(clean_session, monkeypatch):
    ...
    the floor, even under unlock. `(reboot)` / `{ shutdown -h now; }` walked
...
def test_quoted_paren_brace_prose_not_blocked_under_unlock(clean_session, monkeypatch):
...
def test_line_continuation_root_wipe_cannot_bypass_hardline(clean_session, monkeypatch):
    """A line-continuation root wipe must stay blocked even under unlock.

    `rm -rf \\<newline>/` runs as `rm -rf /`. Unlock bypasses the regular
    ...
    assert result["approved"] is False, "unlock leaked a line-continuation root wipe"
    assert result.get("hardline") is True
    assert "BLOCKED (hardline)" in result["message"]


def test_session_unlock_cannot_bypass_hardline(clean_session):
    """Gateway /unlock (session-scoped) must not bypass the hardline floor."""
    enable_session_unlock("hardline_test")
...
def test_approvals_mode_off_cannot_bypass_hardline(clean_session, monkeypatch, tmp_path):
    """config approvals.mode=off (unlock-equivalent) must not bypass hardline."""
    # _get_approval_mode() reads from curie config; simplest path: monkeypatch the helper.
...
def test_recoverable_dangerous_commands_still_pass_unlock(clean_session, monkeypatch):
    """Unlock still bypasses the regular DANGEROUS_PATTERNS list.

    This confirms we haven't broken the unlock escape hatch — only narrowed it.
    """
    monkeypatch.setenv("CURIE_UNLOCK_MODE", "1")

    # These are dangerous but NOT hardline — unlock should still pass them.
    for cmd in ["rm -rf /tmp/x", "chmod -R 777 .", "git reset --hard", "git push --force"]:
        ...
        # And unlock bypasses the dangerous check
        result = check_dangerous_command(cmd, "local")
        assert result["approved"] is True, f"unlock should have bypassed {cmd!r}"
...
def test_hardline_list_is_small():
    ...
    DANGEROUS_PATTERNS where unlock can still bypass it.
    """
