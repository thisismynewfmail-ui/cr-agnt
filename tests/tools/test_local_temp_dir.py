...
Curie exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_curie_cache_not_tmp(tmp_path, monkeypatch):
    """With no overrides at all, the default temp root is real storage under
    CURIE_HOME (cache/terminal), NOT tmpfs /tmp."""
    import curie_constants  # noqa: F401 — resolves CURIE_HOME per call
    ...
    assert result == str(tmp_path / ".curie" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "curie_bg_dead1.log").exists()
