...
def test_output_path_rejects_curie_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    curie_home = tmp_path / "curie-home"
    curie_home.mkdir()
    monkeypatch.setattr(file_safety, "_curie_home_path", lambda: curie_home)
    monkeypatch.setattr(file_safety, "_curie_root_path", lambda: curie_home)

    target = curie_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    curie_home = tmp_path / "curie-home"
    token_dir = curie_home / "mcp-tokens"
