...
try:
    ...
    pytest.skip("curie-agent tools not importable (missing deps)", allow_module_level=True)
...
class TestCwdHandling:
    ...
    def test_home_path_replaced_for_modal(self, monkeypatch):
        ...
        monkeypatch.setenv("TERMINAL_CWD", "/home/dakota/github/curie-agent")
...
class TestDockerHostBindApproval:
    ...
    def _isolate_approval_state(monkeypatch):
        ...
        hermetic CURIE_HOME fixture runs), so on a dev machine whose real
        ...
        Same import-time freeze applies to ``_UNLOCK_MODE_FROZEN``: it reads
        CURIE_UNLOCK_MODE off the environment when the module is imported at
        collection time, before conftest's per-test env blanking runs. A test
        run launched from a --unlock Curie session (or any shell exporting
        CURIE_UNLOCK_MODE=1) freezes True and every guard auto-approves.
