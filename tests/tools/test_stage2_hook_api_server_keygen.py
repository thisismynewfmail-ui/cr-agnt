...
was gated on ``[ -f "$CURIE_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'CURIE_HOME="{home}"\n'
        # In tests we run unprivileged; as_curie is a passthrough then.
        'as_curie() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Curie loads $CURIE_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/curie/.env.example -> $CURIE_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $CURIE_HOME/.env from it on first boot"
    )
