...
def _isolate_approval_state(monkeypatch):
    ...
    # Not a unlock session (the shared gate checks this first).
    monkeypatch.setattr(approval, "is_current_session_unlock_enabled", lambda: False)
    monkeypatch.setattr(approval, "_UNLOCK_MODE_FROZEN", False, raising=False)
...
class TestRequestToolApproval:
    ...
    def test_unlock_session_bypasses_gate(self, monkeypatch):
        """A --unlock session skips the plugin approval gate (parity with the
        dangerous-command path, via the shared _run_approval_gate)."""
        monkeypatch.setattr(approval, "is_current_session_unlock_enabled", lambda: True)
        monkeypatch.setattr(
            approval, "prompt_dangerous_approval",
            lambda *a, **k: pytest.fail("unlock must not prompt"),
        )
