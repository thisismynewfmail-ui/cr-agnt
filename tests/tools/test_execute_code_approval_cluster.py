...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  3. tools.approval.check_execute_code_guard — the entry-point guard decision
     matrix (isolated backends, unlock/off, cron-deny, headless-local,
     gateway approve/deny/timeout/missing-notify, smart mode).
  4. tools.code_execution_tool._scrub_child_env — broad CURIE_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: CURIE_GATEWAY_SESSION set, a bound session
    ...
    # unlock inherited from the developer's live environment.
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
    monkeypatch.setattr(A, "_UNLOCK_MODE_FROZEN", False)
...
def test_guard_session_unlock_bypasses(gw_session):
    A.enable_session_unlock(gw_session)
    try:
        # Even with a denier registered, unlock short-circuits before the prompt.
        _register_resolver(gw_session, "deny")
        assert A.check_execute_code_guard("import os", "local")["approved"] is True
    finally:
        A.disable_session_unlock(gw_session)
...
def test_env_scrub_curie_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "CURIE_HOME": "/h", "CURIE_PROFILE": "p",
        "CURIE_CONFIG": "/c.yaml", "CURIE_ENV": "/e",
        "CURIE_DELEGATED_CHILD_CONTEXT": "1",
        # other CURIE_* → dropped (broad prefix removed)
        "CURIE_BASE_URL": "https://x", "CURIE_INTERACTIVE": "1",
        "CURIE_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "CURIE_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "CURIE_BASE_URL", "CURIE_INTERACTIVE", "CURIE_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped CURIE_* vars."""
    import logging
