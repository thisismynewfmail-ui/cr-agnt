...
must resolve identically on every surface that consults them:
  ...
  - the codex app-server surface: ``agent/codex_runtime.py`` feeds
    ...
    ``is_approval_bypass_active() == (mode == "off")`` when no unlock
    source is active.
...
def test_mode_and_timeout_parity_across_surfaces(
    curie_home, tui_server, yaml_text, expected_mode, expected_timeout
):
    approval_mod = _approval_module()

    _write_config(curie_home, yaml_text)
    ...
    # whenever no unlock source is active in this process.
    if not approval_mod._UNLOCK_MODE_FROZEN:
        with patch.object(
            approval_mod, "is_current_session_unlock_enabled", return_value=False
        ):
...
def test_tui_loader_delegates_to_core(curie_home, tui_server):
