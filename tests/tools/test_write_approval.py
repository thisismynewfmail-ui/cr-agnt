...
and the shared slash-command handlers (curie_cli/write_approval_commands.py).
...
@pytest.fixture
def curie_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="curie_wa_test_")
    home = os.path.join(d, ".curie")
    os.makedirs(home)
    monkeypatch.setenv("CURIE_HOME", home)
    yield home
...
def test_default_gate_is_off(curie_home):
...
def test_invalid_subsystem_is_off(curie_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(curie_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(curie_home, capsys):
...
def test_load_on_disk_store_honors_configured_limits_and_permissions(curie_home, monkeypatch):
    ...
    monkeypatch.setattr("curie_cli.config.load_config", _boom)
...
def test_handle_approve_all(curie_home):
...
def test_handle_approval_on(curie_home):
    from curie_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(curie_home):
    from curie_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_memory_inline_approve_writes(curie_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(curie_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_invalid_params_rejected_before_staging(curie_home):
