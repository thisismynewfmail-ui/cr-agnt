...
Covers:
- :class:`CurieTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.CurieMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _CURIE_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# CurieTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = CurieTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("CURIE_HOME", str(tmp_path))
        storage = CurieTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (CurieMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: CurieTokenStorage, **context_attrs):
    ...
    provider = _CURIE_PROVIDER_CLS.__new__(_CURIE_PROVIDER_CLS)
    provider._curie_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CURIE_HOME", str(tmp_path))
        storage = CurieTokenStorage("mgr-srv")
        ...
        with patch.object(
            _CURIE_PROVIDER_CLS.__bases__[0], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CURIE_HOME", str(tmp_path))
        storage = CurieTokenStorage("flow-srv")
        ...
        with patch.object(
            _CURIE_PROVIDER_CLS.__bases__[0],
            "async_auth_flow",
