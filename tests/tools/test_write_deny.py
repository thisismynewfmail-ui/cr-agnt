...
class TestWriteDenyExactPaths:
    ...
    def test_curie_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.curie/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "curie_root"
        ...
        # Sanity check: CURIE_HOME does point to the profile dir, not the root.
        from curie_constants import get_curie_home, get_default_curie_root
...
class TestWriteAllowed:
    ...
    def test_curie_control_files_requested_writable(self):
