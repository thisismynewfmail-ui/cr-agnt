...
from tools.code_execution_tool import (
    ...
    _uses_curie_python_environment,
...
class TestExecuteCodeModeIntegration(unittest.TestCase):
    ...
    def test_strict_mode_runs_in_tmpdir(self):
        ...
        path stages in curie_sandbox_*, the session kernel in
        curie_kernel_* — either satisfies strict mode's isolation promise."""
        result = self._run("import os; print(os.getcwd())", mode="strict")
        self.assertEqual(result["status"], "success")
        cwd = result["output"].strip()
        self.assertTrue(
            "curie_sandbox_" in cwd or "curie_kernel_" in cwd,
    ...
    def test_project_mode_interpreter_is_venv_python(self):
        ...
        # The curie-agent venv is always active during tests, so this also
    ...
    def test_project_mode_can_still_import_curie_tools(self):
        """Regression: curie_tools still importable from non-tmpdir CWD.

        This is the PYTHONPATH fix — without it, switching to session CWD
        breaks `from curie_tools import terminal`.
        """
        import tempfile
        with tempfile.TemporaryDirectory() as td:
            code = (
                "from curie_tools import terminal\n"
                "r = terminal('echo x')\n"
    ...
    def test_strict_mode_can_still_import_curie_tools(self):
...
# ---------------------------------------------------------------------------
# _python_environment_prefix / _uses_curie_python_environment
# ---------------------------------------------------------------------------
...
class TestPythonEnvironmentPrefix(unittest.TestCase):
    ...
    def test_failure_is_not_cached(self):
        ...
        A sticky cached failure would silently drop the curie root from
...
class TestUsesCuriePythonEnvironment(unittest.TestCase):
    """Unit tests for _uses_curie_python_environment."""
    ...
    def test_true_for_current_interpreter(self):
        ...
        self.assertTrue(_uses_curie_python_environment(sys.executable))
    ...
    def test_true_for_current_interpreter_without_probe(self):
        ...
        must never drop the curie root for the interpreter Curie itself runs.
        """
        with patch("subprocess.run",
                   side_effect=subprocess.TimeoutExpired(cmd=[], timeout=5)) as mock_run:
            self.assertTrue(_uses_curie_python_environment(sys.executable))
    ...
    def test_false_for_different_prefix(self):
        ...
        with patch("tools.code_execution_tool._python_environment_prefix",
            ...
            self.assertFalse(_uses_curie_python_environment("/other/python"))

    def test_false_when_prefix_is_empty(self):
        """If prefix cannot be determined (error path), treat as external."""
        with patch("tools.code_execution_tool._python_environment_prefix",
                   return_value=""):
            self.assertFalse(_uses_curie_python_environment("/bad/python"))

    def test_true_when_prefix_matches_sys_prefix(self):
        curie_prefix = os.path.realpath(sys.prefix)
        with patch("tools.code_execution_tool._python_environment_prefix",
                   return_value=curie_prefix):
            self.assertTrue(_uses_curie_python_environment("/same/env/python"))


# ---------------------------------------------------------------------------
# PYTHONPATH composition — curie root included only for same-env interpreters
# ---------------------------------------------------------------------------

class TestPythonPathComposition(unittest.TestCase):
    """Verify curie root inclusion in PYTHONPATH depends on env match.

    Patches ``_uses_curie_python_environment`` directly so these tests are
    ...
    def _capture_pythonpath(self, same_env: bool) -> tuple:
        ...
        def _fake_popen(cmd, **kwargs):
            ...
            # [python, <staging_dir>/curie_kernel_runner.py] (session
    ...
    def test_curie_root_included_when_same_env(self):
        """When interpreter is in the Curie env, curie root is in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=True)
        parts = pythonpath.split(os.pathsep)
        self.assertIn(self._curie_root(), parts,
                      "curie root must be in PYTHONPATH for same-env interpreters")

    def test_curie_root_excluded_when_external_env(self):
        """When interpreter is external, curie root must NOT be in PYTHONPATH."""
        pythonpath, _ = self._capture_pythonpath(same_env=False)
        parts = pythonpath.split(os.pathsep)
        self.assertNotIn(self._curie_root(), parts,
                         "curie root must not leak into an external interpreter's PYTHONPATH")
