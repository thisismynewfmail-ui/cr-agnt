...
class TestApprovalModeParsing:
    ...
    def test_config_bool_false_maps_to_off(self):
        with mock_patch("curie_cli.config.load_config_readonly", return_value={"approvals": {"mode": False}}):
...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("CURIE_SESSION_KEY", session_key)
        monkeypatch.setenv("CURIE_EXEC_ASK", "1")
        monkeypatch.delenv("CURIE_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestDetectDangerousRm:
    ...
    def test_nonrecursive_verification_artifact_cleanup_is_not_dangerous(self):
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
            for prefix in ("curie-verify-", "curie-ad-hoc-"):
    ...
    def test_symlinked_temp_dir_only_exempts_canonical_target(self, tmp_path):
        ...
        basename = "curie-verify-example.py"
    ...
    def test_verification_cleanup_exemption_rejects_broader_deletions(self):
        commands = (
            "rm -rf /tmp/curie-verify-example.py",
            "rm -f /tmp/curie-verify-example.py /tmp/other.py",
            "rm -f /tmp/nested/../curie-verify-example.py",
            "rm -f /tmp/a/../../tmp/curie-verify-example.py",
            "rm -f /var/tmp/curie-verify-example.py",
            "rm -f /tmp/curie-verify-*",
            "rm -f /tmp/curie-verify-$(touch>/tmp/pwned).py",
            "rm -f /tmp/curie-ad-hoc-`touch>/tmp/pwned`.py",
            "rm -f /tmp/curie-verify-example.py; touch /tmp/pwned",
        )
        with mock_patch("tempfile.gettempdir", return_value="/tmp"):
...
class TestWindowsShellDestructiveCommands:
    def test_windows_destructive_requires_approval(self):
        cases = [
            (r"cmd /c del /f /q C:\tmp\curie-victim\file.txt", "Windows cmd destructive delete"),
            (r"cmd.exe /k rmdir /s /q C:\tmp\curie-victim", "Windows cmd destructive delete"),
            ...
            (r"powershell Remove-Item -Recurse -Force C:\tmp\curie-victim",
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"CURIE_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.curie/.env",
            "echo x | tee $CURIE_HOME/.env",
            'echo x | tee "$CURIE_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestCurieConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.curie/config.yaml (#14639). config.yaml IS the security policy
    (approvals.mode/unlock live there, mtime-keyed cache reloads mid-session),
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.curie/config.yaml",
            "echo '  mode: off' >> ~/.curie/config.yaml",
            "echo x | tee ~/.curie/config.yaml",
            "echo x | tee $CURIE_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.curie/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Curie absolute config.yaml is
        # handled by the project patterns, not the Curie-home rule.
        for cmd in (
            "cat ~/.curie/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.curie/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Curie-home prefixes must fold to ~/ and
    ~/.curie/ in dangerous-command detection.
    ...
    SSH, and Curie config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_curie_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/CURIE_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("CURIE_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.curie/curie-agent && source venv/bin/activate && python -m curie_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc
        for variant in (
            "python -m curie_cli.main gateway run --replace &",
            "nohup python -m curie_cli.main gateway run --replace",
        ):
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart curie-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestWebhookApprovalExclusion:
    ...
    def test_webhook_platform_returns_false(self, monkeypatch):
        ...
        monkeypatch.setenv("CURIE_SESSION_PLATFORM", "webhook")
    ...
    def test_all_unattended_platforms_return_false(self, monkeypatch):
        ...
        for platform in _UNATTENDED_APPROVAL_PLATFORMS:
            monkeypatch.setenv("CURIE_SESSION_PLATFORM", platform)
    ...
    def test_non_webhook_gateway_session_returns_true(self, monkeypatch):
        ...
        monkeypatch.delenv("CURIE_CRON_SESSION", raising=False)
        monkeypatch.setenv("CURIE_GATEWAY_SESSION", "1")
        monkeypatch.setenv("CURIE_SESSION_PLATFORM", "telegram")
    ...
    def test_cron_session_returns_false_regardless_of_platform(self, monkeypatch):
        ...
        monkeypatch.setenv("CURIE_CRON_SESSION", "1")
        monkeypatch.setenv("CURIE_SESSION_PLATFORM", "telegram")
    ...
    def _isolate(self, monkeypatch):
        """Neutralize host leakage: unlock frozen at import time + real config."""
        import tools.approval as approval_mod

        monkeypatch.setattr(approval_mod, "_UNLOCK_MODE_FROZEN", False)
    ...
    def test_webhook_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.setenv("CURIE_SESSION_PLATFORM", "webhook")
        monkeypatch.setenv("CURIE_SESSION_KEY", "test-webhook-session")
    ...
    def test_api_server_dangerous_command_denies_by_default(self, monkeypatch):
        ...
        monkeypatch.delenv("CURIE_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("CURIE_INTERACTIVE", raising=False)
        monkeypatch.setenv("CURIE_SESSION_PLATFORM", "api_server")
        monkeypatch.setenv("CURIE_SESSION_KEY", "test-api-session")
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Curie security config via IFS.
            "sed${IFS}-i ~/.curie/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep curie) bypasses the pkill/killall name-matching
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof curie)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        dangerous, _, desc = detect_dangerous_command("kill -TERM $(pidof curie_cli.main)")
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()
        assert detect_dangerous_command("kill -9 `pidof curie`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Curie service
    label achieves the same effect as `curie gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_against_curie_label_detected(self):
        for cmd in (
            "launchctl stop ai.curie.gateway",
            "launchctl kickstart -k system/ai.curie.gateway",
            "launchctl bootout system/ai.curie.gateway",
            "launchctl unload ~/Library/LaunchAgents/ai.curie.gateway.plist",
        ):
            dangerous, _, desc = detect_dangerous_command(cmd)
            assert dangerous is True, cmd

    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Curie labels, are
    ...
    def test_spliced_detection_does_not_flag_prose_or_other_services(self):
        ...
        a curie-gateway identifier, so quoted prose and non-gateway curie
    ...
    def test_label_built_before_verb_detected(self):
        ...
        "curie"/"ai.curie" to appear AFTER the verb and missed this
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("CURIE_GATEWAY_SESSION", "CURIE_CRON_SESSION",
                      "CURIE_UNLOCK_MODE",
                      "CURIE_SESSION_KEY", "CURIE_INTERACTIVE")
        }
        os.environ.pop("CURIE_UNLOCK_MODE", None)
        os.environ.pop("CURIE_INTERACTIVE", None)
        # CURIE_CRON_SESSION takes priority over CURIE_GATEWAY_SESSION in
        ...
        os.environ.pop("CURIE_CRON_SESSION", None)
        os.environ["CURIE_GATEWAY_SESSION"] = "1"
        os.environ["CURIE_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_import_error_allows_when_fail_open_or_disabled(self, enabled, fail_open):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("curie_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"CURIE_INTERACTIVE": "1"}, clear=False):
...
GATEWAY_LIFECYCLE_LAUNCHCTL = (
    ...
    "launchctl unload ~/Library/LaunchAgents/ai.curie.gateway.plist",
    "launchctl load ~/Library/LaunchAgents/ai.curie.gateway.plist",
    "launchctl stop ai.curie.gateway",
    "launchctl restart ai.curie.gateway",
    "launchctl bootout gui/501/ai.curie.gateway",
    "launchctl remove ai.curie.gateway",
    "launchctl disable gui/501/ai.curie.gateway",
)
...
class TestLifecycleGuardLaunchctlParity:
    ...
    under ``_CURIE_GATEWAY == "1"``, the ``cron.lifecycle_guard`` block is
    ...
    def test_unrelated_labels_are_not_blocked(self):
        ...
        services, including other Curie ones, stay runnable."""
        from cron.lifecycle_guard import contains_gateway_lifecycle_command

        for cmd in (
            "launchctl bootout gui/501/com.example.unrelated",
            "launchctl remove ai.curie.update-checker",
