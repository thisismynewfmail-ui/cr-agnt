        ...
        async def resolve(config=None, env_model=None):
            with ExitStack() as st:
                ...
                if config is not None:
                    st.enter_context(patch(
                        "curie_cli.config.load_config", return_value=config,
                    ))
...
class TestVisionConfig:
    ...
    async def test_temperature_and_timeout_come_from_config_with_defaults(self, tmp_path):
        ...
        # Approximately, not exactly: the configured value is a budget for the
        # whole call, and what reaches the provider is whatever is left of it
        # when the attempt starts. On the first attempt that is the whole
        # thing minus the microseconds since the budget was opened.
        assert kwargs["timeout"] == pytest.approx(77.0, abs=1.0)
        ...
        assert kwargs["timeout"] == pytest.approx(
            vision_tools.DEFAULT_VISION_TIMEOUT_SECONDS, abs=1.0
        )
        assert "max_tokens" not in kwargs
    ...
    def test_worker_count_tracks_host_cpus_with_env_override(self):
        ...
        def resolve(host_cpus, env_value=None):
            with (
                ...
                patch("curie_cli.config.load_config", side_effect=Exception),
            ):
                if env_value is None:
                    os.environ.pop("CURIE_VISION_MAX_CONCURRENCY", None)
                else:
                    os.environ["CURIE_VISION_MAX_CONCURRENCY"] = env_value
