...
A match blocks unconditionally — BEFORE the --unlock / /unlock / mode=off bypass —
...
def clean_env(monkeypatch):
    """Non-interactive, non-gateway, non-cron, non-unlock baseline."""
    for var in ("CURIE_UNLOCK_MODE", "CURIE_GATEWAY_SESSION",
                "CURIE_CRON_SESSION", "CURIE_INTERACTIVE",
                "CURIE_EXEC_ASK"):
        monkeypatch.delenv(var, raising=False)
    monkeypatch.setattr(mod, "_UNLOCK_MODE_FROZEN", False)
...
class TestDenyBeatsUnlock:
    def test_deny_blocks_under_unlock_env(self, deny_config, clean_env, monkeypatch):
        deny_config(["git push --force*"])
        monkeypatch.setattr(mod, "_UNLOCK_MODE_FROZEN", True)
    ...
    def test_deny_blocks_under_session_unlock(self, deny_config, clean_env, monkeypatch):
        deny_config(["*curl*|*sh*"])
        monkeypatch.setattr(mod, "is_current_session_unlock_enabled", lambda: True)
    ...
    def test_non_matching_command_still_bypassed_by_unlock(
            self, deny_config, clean_env, monkeypatch):
        deny_config(["git push --force*"])
        monkeypatch.setattr(mod, "_UNLOCK_MODE_FROZEN", True)

        # Dangerous but not denied — unlock passes it through unchanged.
        result = mod.check_dangerous_command("rm -rf build/", "local")
        assert result["approved"] is True

    def test_empty_deny_list_preserves_unlock_behavior(
            self, deny_config, clean_env, monkeypatch):
        deny_config([])
        monkeypatch.setattr(mod, "_UNLOCK_MODE_FROZEN", True)
