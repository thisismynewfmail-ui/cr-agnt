...
def _mode_manual(monkeypatch):
    ...
    The test conftest redirects CURIE_HOME to an empty tempdir, so the
...
def _clean_state():
    ...
    saved = {}
    for k in ("CURIE_INTERACTIVE", "CURIE_GATEWAY_SESSION", "CURIE_EXEC_ASK", "CURIE_UNLOCK_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["CURIE_INTERACTIVE"] = "1"
        session_key = os.getenv("CURIE_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No CURIE_INTERACTIVE or CURIE_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
