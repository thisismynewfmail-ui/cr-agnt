...
def test_normal_curie_session_maps_to_standard_mode():
...
def test_any_explicit_curie_bypass_maps_to_unrestricted_mode():
    from tools.computer_use import tool as computer_use
...
def test_gateway_session_key_unlock_maps_to_unrestricted_mode():
    """Gateway /unlock keys bypass off the gateway session_key contextvar,
    not the DB session_id the tool path passes. Mode resolution must consult
    both namespaces or /unlock is silently dead on messaging platforms."""
...
def test_unlock_toggle_immediately_releases_mode_dependent_backend():
    from tools import approval

    with patch("tools.computer_use.release_computer_use_session") as release:
        approval.enable_session_unlock("session-a")
        approval.disable_session_unlock("session-a")
