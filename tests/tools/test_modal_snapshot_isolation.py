...
def _restore_tool_modules():
    original_curie_home = os.environ.get("CURIE_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_curie_home is None:
            os.environ.pop("CURIE_HOME", None)
        else:
            os.environ["CURIE_HOME"] = original_curie_home
        _reset_modules(("tools", "curie_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    curie_cli = types.ModuleType("curie_cli")
    curie_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["curie_cli"] = curie_cli
    curie_home = tmp_path / "curie-home"
    os.environ["CURIE_HOME"] = str(curie_home)
    sys.modules["curie_cli.config"] = types.SimpleNamespace(
        get_curie_home=lambda: curie_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="curie-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": curie_home / "modal_snapshots.json",
