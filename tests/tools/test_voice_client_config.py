...
Real config files in a temp CURIE_HOME, real resolution chains (no mocked
...
def voice_home(tmp_path, monkeypatch):
    """Isolated CURIE_HOME + reloaded config modules; yields a config writer."""
    home = tmp_path / ".curie"
    home.mkdir()
    ...
    for var in (
        ...
        "DEEPINFRA_API_KEY", "CURIE_LOCAL_STT_LANGUAGE",
    ):
