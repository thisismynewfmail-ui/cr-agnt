...
def test_root_session_no_compression(db):
    ...
    assert prov["currentCurieSessionId"] == "root1"
    assert prov["rootCurieSessionId"] == "root1"
    assert prov["parentCurieSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_curie_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentCurieSessionId"] == "old"
    assert prov["rootCurieSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousCurieSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootCurieSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"curie"}
    assert "sessionProvenance" in meta["curie"]
    assert meta["curie"]["sessionProvenance"]["currentCurieSessionId"] == "root1"
