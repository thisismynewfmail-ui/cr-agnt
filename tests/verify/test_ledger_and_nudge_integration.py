...
- ``curie verify`` records into the evidence ledger (pass and fail),
- a passing run satisfies the verify-on-stop guard,
- the verify-on-stop nudge names ``curie verify --json`` when the workspace
...
def _workspace(tmp_path, *, scripts=None, manifest_recipe=None):
    ...
    if manifest_recipe is not None:
        curie_dir = project / ".curie"
        curie_dir.mkdir()
        (curie_dir / "environment.json").write_text(
...
def test_record_verify_run_marks_workspace_passed(curie_home):
    project = _workspace(curie_home)
    ...
    assert status["evidence"]["canonical_command"] == "curie verify"


def test_record_verify_run_records_failure(curie_home):
    project = _workspace(curie_home)
...
def test_cli_passing_run_writes_ledger_evidence(curie_home, capsys):
    project = _workspace(curie_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_cli_failing_run_writes_failed_evidence(curie_home, capsys):
    project = _workspace(curie_home, manifest_recipe={"name": "Fake", "test": ["false"]})
    code = run_verify_command(make_args(project))
    assert code == 1
    status = verification_status(session_id=None, cwd=project)
    assert status["status"] == "failed"
...
def test_cli_partial_run_records_targeted_scope(curie_home, capsys):
...
def test_cli_run_uses_curie_session_id_env(curie_home, capsys, monkeypatch):
    monkeypatch.setenv("CURIE_SESSION_ID", "sess-42")
    project = _workspace(curie_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# closed loop: edit -> stop guard nudge -> curie verify -> guard satisfied
# ---------------------------------------------------------------------------


def test_passing_verify_run_satisfies_stop_guard(curie_home, capsys):
    project = _workspace(curie_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# nudge wording: recipe-aware `curie verify --json` suggestion
# ---------------------------------------------------------------------------


def test_nudge_mentions_curie_verify_when_recipe_has_start(curie_home):
    project = _workspace(curie_home, scripts={"test": "vitest", "dev": "vite"})
    changed = str(project / "src" / "app.ts")
    ...
    assert "curie verify --json" in nudge
    # The cheap verify commands are still listed first.
    assert "npm run test" in nudge


def test_nudge_mentions_curie_verify_when_manifest_exists(curie_home):
    # No start script, but a saved .curie/environment.json qualifies.
    project = _workspace(
        curie_home,
        scripts={"test": "vitest"},
...
def test_nudge_keeps_plain_wording_without_recipe_start(curie_home):
    # Verify commands but no start script and no manifest: today's wording.
    project = _workspace(curie_home, scripts={"test": "vitest"})
    changed = str(project / "src" / "app.ts")
    mark_workspace_edited(session_id="s1", cwd=project, paths=[changed])
    nudge = build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    assert nudge is not None
    assert "curie verify" not in nudge


def test_nudge_recipe_detection_failure_is_silent(curie_home, monkeypatch):
...
def test_detect_path_merges_project_facts_commands(curie_home, capsys):
    project = _workspace(curie_home)  # package.json with no scripts
...
def test_manifest_recipe_is_not_merged(curie_home, capsys):
...
def test_merge_skips_commands_recipe_already_has(curie_home, capsys):
    project = _workspace(curie_home, scripts={"test": "vitest"})
    code = run_verify_command(make_args(project, detect_only=True))
    assert code == 0
    payload = json.loads(capsys.readouterr().out)
