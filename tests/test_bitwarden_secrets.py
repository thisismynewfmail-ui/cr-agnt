...
binary download are exercised manually by `curie secrets bitwarden
...
def curie_home(tmp_path, monkeypatch):
    """Point Curie at an isolated home directory."""
    home = tmp_path / ".curie"
    home.mkdir()
    monkeypatch.setenv("CURIE_HOME", str(home))
    # Some modules cache get_curie_home; clear if needed.
    import curie_constants
    if hasattr(curie_constants, "_CURIE_HOME_CACHE"):
        curie_constants._CURIE_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(curie_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from curie_cli.env_loader import _apply_external_secret_sources
