...
def progress(tmp_path):
    ...
    status = tmp_path / "curie-update-status"
...
# Stands in for `curie update`, and reports the stage that was on screen
...
n="$(cat "$CURIE_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$CURIE_TEST_CALLS"
for f in "$TMPDIR"/curie-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$CURIE_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$CURIE_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "curie-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    curie = install_root / "venv" / "bin" / "curie"
    curie.write_text(FAKE_CURIE)
    curie.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "CURIE_TEST_CAPTURE": str(capture),
        "CURIE_TEST_CALLS": str(calls),
        "CURIE_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".curie-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`curie update` is the longest wait in the hand-off and the one the
