...
The ``curie update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m curie_cli.main gateway run`` — image name
    ``pythonw``, not ``curie.exe`` — so the ``taskkill /IM curie.exe`` guard
    ...
    # The curie.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM curie.exe' in text
