...
#
# Installs a real, earlier Curie the way a user does, applies ONE update route,
# and requires the checkout to land on this commit with a working `curie`.
#
...
# Keep sandbox state out of the default .curie-sandbox so a run never clobbers
...
SANDBOX_DIR_NAME=".curie-sandbox-e2e-$ROUTE"
export CURIE_DEV_SANDBOX_DIR="$SANDBOX_DIR_NAME"

SANDBOX_ROOT="$REPO_ROOT/$SANDBOX_DIR_NAME"
INSTALL_DIR="/home/curie/.curie/curie-agent"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/curie-agent.git"
...
LOG_DIR="${CURIE_E2E_LOG_DIR:-$(mktemp -d -t curie-install-e2e-logs.XXXXXX)}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED curie accept FLAG on `curie update`?
#
...
# that silently rots. `curie update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "curie update --help 2>&1" | grep -qF -- "$flag"
}
...
require_curie_works() {
  local when="$1" out
  out="$(in_sandbox "curie --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "curie --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "curie runs $when"
}

# ── install the earlier Curie ─────────────────────────────────────────────
...
require_curie_works 'after install'

# ── apply exactly one update route ─────────────────────────────────────────
case "$ROUTE" in
  update)
    step 'ROUTE: curie update'
    ...
    # curie which it accepts; older ones read the prompt from stdin, so close it.
    if update_supports --yes; then
      update_cmd="curie update --yes"
    else
      update_cmd="curie update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "curie update failed ($update_cmd)"
    fi
    require_landed_on_target 'curie update'
    require_curie_works 'after curie update'
    ;;
  installer)
    ...
    require_curie_works 'after installer re-run'
    ;;
esac
