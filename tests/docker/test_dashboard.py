"""Harness: dashboard opt-in via CURIE_DASHBOARD.

Today (tini): dashboard starts once when CURIE_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``curie`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without CURIE_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "curie dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without CURIE_DASHBOARD"
    )
...
# whenever `CURIE_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``CURIE_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        ...
        "CURIE_DASHBOARD=1",
        "CURIE_DASHBOARD_HOST=0.0.0.0",
        "CURIE_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"CURIE_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and CURIE_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``CURIE_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate
