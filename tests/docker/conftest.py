...
Override the image with ``CURIE_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("CURIE_TEST_IMAGE", "curie-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``CURIE_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("CURIE_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged curie user
# ---------------------------------------------------------------------------
#
# Background: every Curie runtime path inside the container drops to UID
# 10000 (the ``curie`` user) via ``s6-setuidgid curie``. ``docker exec``
...
# fails (PermissionError swallowed) for curie, so a test that ran as root
...
# helpers below run every probe under ``-u curie`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: curie).
