"""Runtime smoke test for Docker $CURIE_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the curie user after container boot.
...
registered later runs its log service as the dropped curie user and
...
def test_logs_gateways_seeded_and_curie_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by curie after boot."""
    ...
    assert "logs=curie" in r.stdout, (
        f"logs/ not owned by curie: {r.stdout}"
    )
    assert "gateways=curie" in r.stdout, (
        f"logs/gateways/ not owned by curie: {r.stdout}"
    )
