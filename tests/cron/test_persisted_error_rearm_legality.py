...
def cron_store(tmp_path, monkeypatch):
    ...
    (curie_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("CURIE_HOME", str(curie_home))
    monkeypatch.setattr(J, "CURIE_DIR", curie_home)
    monkeypatch.setattr(J, "CRON_DIR", curie_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", curie_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", curie_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_curie_now", lambda: now):
            due = J.get_due_jobs()
