...
def test_run_job_cron_execute_code_deny_does_not_pollute_later_gateway_execute_code(
    ...
    monkeypatch.setenv("CURIE_MODEL", "test-model")
    ...
    monkeypatch.setattr("curie_state.get_shared_session_db", _DummySessionDB)
    monkeypatch.setattr("run_agent.AIAgent", _FakeCronAgent)
    monkeypatch.setattr(
        "curie_constants.resolve_reasoning_config", lambda *_args, **_kwargs: None
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(cron_scheduler, "_get_curie_home", lambda: tmp_path)
    ...
    assert os.environ.get("CURIE_CRON_SESSION") is None
    ...
    assert get_session_env("CURIE_CRON_SESSION") == "1"
    monkeypatch.delenv("CURIE_CRON_SESSION")
