...
the shared /blueprint command handler (curie_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated CURIE_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("curie://blueprint/morning-brief?")
