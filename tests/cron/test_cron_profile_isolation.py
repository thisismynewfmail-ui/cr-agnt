...
profile's CURIE_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.curie/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active CURIE_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever CURIE_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    CURIE_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import curie_constants

    monkeypatch.setattr(
        curie_constants, "_get_platform_default_curie_home", lambda: root
    )
    monkeypatch.setenv("CURIE_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile CURIE_HOME (<root>/profiles/<name>), the cron store
    ...
    assert curie_constants.get_curie_home().resolve() == profile_home.resolve()
    assert curie_constants.get_default_curie_root().resolve() == root.resolve()

    # cron/jobs.py computes CURIE_DIR from get_curie_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.CURIE_DIR.resolve() == profile_home.resolve()
        assert (
