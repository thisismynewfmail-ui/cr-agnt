...
child at the same moment the child lost the ``CURIE_HOME`` redirect. That
pairing is exactly how fixture rows (dm:123 / chat-1 / wx-chat) landed in a
developer's production state.db.

``CURIE_TEST_ISOLATION`` is Curie's own marker: the hermetic conftest
exports it (value = the isolation root) before any test module imports, it
inherits into children by default, and ``curie_state`` honors it as a
...
def test_conftest_exports_the_marker():
    ...
    assert os.environ.get("CURIE_TEST_ISOLATION"), (
        "CURIE_TEST_ISOLATION must be exported by tests/conftest.py so "
...
def test_marker_alone_reports_test_context(monkeypatch):
    ...
    monkeypatch.setenv("CURIE_TEST_ISOLATION", "/tmp/some-isolation-root")
    assert curie_state._running_under_pytest() is True
...
def test_no_signals_reports_production(monkeypatch):
    ...
    monkeypatch.delenv("PYTEST_CURRENT_TEST", raising=False)
    monkeypatch.delenv("PYTEST_VERSION", raising=False)
    monkeypatch.delenv("CURIE_TEST_ISOLATION", raising=False)
    assert curie_state._running_under_pytest() is False
...
def test_child_with_rebuilt_env_keeping_marker_refuses_production_db():
    ...
    stripped, CURIE_HOME lost) but which keeps the marker must still refuse
    to open the production state.db."""
    env = _minimal_env(CURIE_TEST_ISOLATION="/tmp/pytest-isolation-root")
...
def test_child_bypass_env_disarms_guard_even_with_marker():
    ...
    DB: CURIE_STATE_DB_GUARD_BYPASS=1, not marker-stripping."""
    env = _minimal_env(
        CURIE_TEST_ISOLATION="/tmp/pytest-isolation-root",
        CURIE_STATE_DB_GUARD_BYPASS="1",
    )
    result = _spawn_probe(env)
