...
from curie_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(curie_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <curie_home>/plugins/<name> and opt it in."""
    plugin_dir = curie_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CURIE_HOME", str(tmp_path / "curie_empty"))
