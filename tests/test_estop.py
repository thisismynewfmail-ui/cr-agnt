"""Global emergency stop (`curie pause` / `curie resume`) — agent/estop.py.
...
already in flight is never touched. Removing the sentinel (`curie resume`)
...
def curie_home(tmp_path, monkeypatch):
    """Point CURIE_HOME at a temp dir and reset estop module log state."""
...
def test_engage_creates_sentinel_and_is_engaged(curie_home):
    assert estop.is_engaged() is False
    estop.engage()
    assert (curie_home / "ESTOP").exists()
    assert estop.is_engaged() is True


def test_disengage_removes_sentinel(curie_home):
    estop.engage()
    assert estop.disengage() is True
    assert not (curie_home / "ESTOP").exists()
    assert estop.is_engaged() is False
    # Disengaging when not engaged is a no-op that reports False.
    assert estop.disengage() is False


def test_reason_and_timestamp_stored(curie_home):
    ...
    raw = json.loads((curie_home / "ESTOP").read_text(encoding="utf-8"))
    assert raw["reason"] == "runaway cron fan-out"


def test_get_state_none_when_disengaged(curie_home):
    assert estop.get_state() is None


def test_corrupt_sentinel_still_engages(curie_home):
    """A hand-touched/corrupt ESTOP file must still pause (fail safe)."""
    (curie_home / "ESTOP").write_text("not json", encoding="utf-8")
    assert estop.is_engaged() is True
    state = estop.get_state()
    assert state is not None
...
def test_paused_reply_none_when_disengaged(curie_home):
    assert estop.paused_reply() is None


def test_paused_reply_surfaces_reason_and_resume_hint(curie_home):
    ...
    assert "curie resume" in notice


def test_paused_reply_without_reason(curie_home):
    estop.engage()
    notice = estop.paused_reply()
    assert notice is not None
    assert "paused" in notice.lower()
...
def test_check_paused_logs_once_per_engagement(curie_home, caplog):
...
def test_cron_tick_skips_dispatch_when_engaged(curie_home, monkeypatch):
...
def test_cron_tick_resumes_after_disengage(curie_home, monkeypatch):
    from cron import scheduler
...
def test_kanban_dispatch_blocked_when_engaged(curie_home):
...
@pytest.mark.asyncio
async def test_gateway_new_turn_gets_paused_reply(curie_home):
...
@pytest.mark.asyncio
async def test_gateway_internal_events_bypass_estop(curie_home):
...
# ── CLI: curie pause / curie resume ───────────────────────────────────────


def test_cli_pause_engages_with_reason(curie_home, capsys):
    from curie_cli.subcommands.pause import cmd_pause
...
def test_cli_pause_idempotent(curie_home, capsys):
    from curie_cli.subcommands.pause import cmd_pause
...
def test_cli_resume_disengages(curie_home, capsys):
    from curie_cli.subcommands.pause import cmd_pause, cmd_resume
...
def test_cli_resume_when_not_paused(curie_home, capsys):
    from curie_cli.subcommands.pause import cmd_resume
...
def test_builtin_subcommands_include_pause_resume():
    from curie_cli.main import _BUILTIN_SUBCOMMANDS

    assert "pause" in _BUILTIN_SUBCOMMANDS
    assert "resume" in _BUILTIN_SUBCOMMANDS


# ── curie status surfacing ─────────────────────────────────────────────────


def test_status_line_when_paused(curie_home):
    from curie_cli.status import _estop_status_line
...
def test_is_engaged_fails_safe_on_stat_error(curie_home, monkeypatch):
    """A stat failure must report ENGAGED (fail safe) — the pause has to
    hold even when CURIE_HOME is misbehaving, matching the module's
...
@pytest.mark.asyncio
async def test_gateway_slash_commands_bypass_estop(curie_home):
    ...
    try:
        ...
        return
    assert reply is None or "curie is paused" not in (reply or "").lower()
...
@pytest.mark.asyncio
async def test_gateway_pause_command_engages_and_resumes(curie_home):
    from gateway.run import GatewayRunner
...
def test_pause_command_registered_for_gateway():
    from curie_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command
...
def test_profile_gateway_honors_canonical_root_estop(tmp_path, monkeypatch):
    """fleet-analyst-class: CURIE_HOME is a profile dir; pause lives at root.

    A process launched with CURIE_HOME=~/.curie/profiles/fleet-analyst must
    still treat ~/.curie/ESTOP as engaged. Otherwise `curie pause` is not
