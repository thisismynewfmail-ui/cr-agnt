...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_curie_home", lambda: curie_home)
    monkeypatch.setattr(uninstall, "_is_default_curie_home", lambda home: False)
    ...
    assert str(curie_home) in output
    assert project_root.exists()
    assert curie_home.exists()
