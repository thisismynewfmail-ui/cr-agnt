...
#88251: ``curie update`` treated a failed desktop pack as non-fatal, printed
...
def desktop_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "curie_constants.with_curie_node_path", lambda: {}, raising=False
    )
    monkeypatch.setattr(
        "curie_constants.display_curie_home", lambda: str(tmp_path), raising=False
    )
...
def test_gateway_exit_code_file_tracks_desktop_rebuild(tmp_path, monkeypatch):
    monkeypatch.setattr(update_cmd, "get_curie_home", lambda: tmp_path)
