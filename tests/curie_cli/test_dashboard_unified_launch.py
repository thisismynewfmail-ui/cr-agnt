...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "curie_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # CURIE_HOME.  For a standard install (CURIE_HOME unset) that root is
        # the platform-native default (~/.curie), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from curie_constants import get_default_curie_root
        assert env.get("CURIE_HOME") == str(get_default_curie_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (CURIE_DESKTOP=1) must NOT
