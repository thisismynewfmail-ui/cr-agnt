...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    CURIE_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_curie_argv", lambda: ["curie"])
    ...
    assert pid == 4242
    assert captured["env"]["CURIE_HOME"] == str(profile)
    assert captured["env"]["CURIE_KANBAN_TASK"] == "t_spawn_tools"
