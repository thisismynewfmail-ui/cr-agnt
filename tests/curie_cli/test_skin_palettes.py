...
from curie_cli.skin_engine import _BUILTIN_SKINS
...
# Documented schema keys beyond the required set (see the SKIN YAML SCHEMA
# block in curie_cli/skin_engine.py). They are optional, so the completeness
# contract above does not demand them — but a skin that does ship them is not
# using "unknown keys", which is what the overlay audit below checks for.
...
LIGHT_AUTHORED = frozenset({
    # The Curie skins are authored for a light terminal: the bench palette is
    # enamel silver-white with graphite ink, and `curie-vga` / `curie-amber` /
    # `curie-blueprint` follow it. Each ships a hand-tuned `dark_colors` block
    # for the other polarity.
    "default",
    "curie-amber",
    "curie-blueprint",
    # Two Curie skins are deliberately dark-authored and so absent from this
    # set: `curie-nightbench` is the bench's dark half promoted to a skin, and
    # `curie-vga` is the VGA text-mode palette, whose iconic form is
    # bright-on-blue (Turbo Vision, Norton Commander, EDIT.COM). `curie-vga`
    # ships no light overlay because it cannot: of the sixteen colours a VGA
    # card put on the wire, the only green reaches 3.4:1 on white and so
    # cannot satisfy the strong-foreground floor. Rather than substitute a
    # colour from outside the sixteen, cross-polarity rendering is left to
    # the TUI's hue-preserving contrast lift — which is what it is for.
    "daylight",
    "warm-lightmode",
})
...
def test_overlay_keys_and_fill_polarity(skin, block, palette, is_light):
    unknown = palette.keys() - KNOWN_KEYS
