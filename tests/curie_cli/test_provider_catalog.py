"""Tests for the unified provider catalog (curie_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``curie model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from curie_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
