"""Tests for the top-level `./curie` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./curie` should use `curie_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "curie"
    called = []

    fake_main_module = types.ModuleType("curie_cli.main")

    def fake_main():
        called.append("curie_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "curie_cli.main", fake_main_module)
