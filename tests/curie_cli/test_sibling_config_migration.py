...
`curie update` migrated only the active profile's config.yaml; sibling
...
context-local CURIE_HOME override.
...
def _setup(monkeypatch, tmp_path, active_home: Path):
    ...
    monkeypatch.setattr(
        curie_constants, "get_process_curie_home", lambda: active_home
    )
    monkeypatch.setattr(
...
def test_override_is_reset_after_run(monkeypatch, tmp_path):
    ...
    before = get_curie_home_override()
    update_cmd._migrate_sibling_profile_configs()
    assert get_curie_home_override() == before
