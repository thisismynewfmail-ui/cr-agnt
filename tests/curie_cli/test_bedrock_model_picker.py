...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (CURIE_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via CURIE_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in CURIE_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from curie_cli.providers import CURIE_OVERLAYS
        assert "bedrock" in CURIE_OVERLAYS
