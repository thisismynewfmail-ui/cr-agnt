"""Tests for `curie memory setup [provider]` routing.
...
``curie memory setup honcho``) without the interactive picker — which matters
because the per-provider ``curie <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "curie memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets CURIE_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"CURIE_DISABLE_LAZY_INSTALLS": "0"}), \
             patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("curie_cli.tools_config.shutil.which", side_effect=which_side_effect), \
             patch("curie_cli.tools_config.subprocess.run", fake_run):
