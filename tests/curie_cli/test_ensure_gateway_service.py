"""Tests for curie_cli.gateway.ensure_gateway_service — the zero-prompt
service install/start path used by `curie setup` and `curie import`.
...
def _patch_host(monkeypatch, *, container=False, systemd=True, macos=False, windows=False):
    monkeypatch.setattr("curie_constants.is_container", lambda: container)
