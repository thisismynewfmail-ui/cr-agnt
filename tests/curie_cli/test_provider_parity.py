...
the SAME provider universe as ``curie model`` (the CLI/TUI picker).
...
def test_every_curie_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `curie model` universe."""
    ...
    assert not missing, (
        "providers shown in `curie model` but not configurable in the desktop "
