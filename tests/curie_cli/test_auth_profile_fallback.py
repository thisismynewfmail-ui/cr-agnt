...
When ``CURIE_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.curie/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.curie            (has its own auth.json fixture)
    * Profile     -> tmp_path/.curie/profiles/coder   (active, CURIE_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Curie is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (CURIE_HOME != ~/.curie, no profiles)."""
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    curie_home = tmp_path / "classic"
    curie_home.mkdir()
