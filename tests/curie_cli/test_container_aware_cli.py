...
def container_env(tmp_path, monkeypatch):
    """Set up a fake CURIE_HOME with .container-mode file."""
    curie_home = tmp_path / ".curie"
    curie_home.mkdir()
    monkeypatch.setenv("CURIE_HOME", str(curie_home))
    monkeypatch.delenv("CURIE_DEV", raising=False)

    container_mode = curie_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=curie-agent\n"
        "exec_user=curie\n"
        "curie_bin=/data/current-package/bin/curie\n"
    )
    return curie_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("curie_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "curie-agent"
    assert info["exec_user"] == "curie"
    assert info["curie_bin"] == "/data/current-package/bin/curie"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from curie_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "curie"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "curie-agent" in cmd
    assert "/data/current-package/bin/curie" in cmd
    assert "chat" in cmd
