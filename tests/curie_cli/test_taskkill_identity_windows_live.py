...
- ``gateway.status.terminate_pid(force=True)`` requires a matching
  ...
  mismatched identity.
- ``curie_cli._subprocess_compat.pid_is_curie`` fails closed on foreign
  processes and identity mismatches.
- ``curie_cli.update_cmd._refuse_gateway_ancestor_tree_kill`` refuses to
...
class TestPidIsCurieLive:
    def test_foreign_real_process_is_refused(self):
        """A live non-Curie process (bare python sleeper in a temp-ish argv)
        must never be judged safe for taskkill."""
        from curie_cli._subprocess_compat import pid_is_curie

        proc = _spawn_sleeper()
        try:
            # sys.executable in CI lives under a uv/hostedtoolcache path with
            # no 'curie' token; if the checkout path itself contains one this
            # assertion is environment-dependent, so guard for it.
            if "curie" in sys.executable.lower():
                pytest.skip("interpreter path names curie; probe would match")
            assert pid_is_curie(proc.pid) is False
        finally:
            _cleanup(proc)

    def test_stale_fingerprint_is_refused_even_for_curie_argv(self):
        ...
        try:
            ...
            assert (
                pid_is_curie(proc.pid, expected_start_time=live + 12345) is False
            )
        finally:
            _cleanup(proc)
    ...
    def test_nonexistent_pid_is_refused(self):
        ...
        assert pid_is_curie(2**24) is False
...
class TestAncestorRefusalLive:
    def test_real_parent_chain_is_refused(self, capsys):
        ...
        from curie_cli.update_cmd import _refuse_gateway_ancestor_tree_kill
