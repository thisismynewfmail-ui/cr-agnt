...
Regression for the desktop "UNLOCK toggle does nothing / flips back off" bug:
...
gate re-reads config per command), but every live session's UNLOCK/approval
...
@pytest.fixture
def client(_isolate_curie_home):
    try:
...
class TestApprovalsSaveBroadcast:
    ...
    def test_own_profile_named_default_broadcasts(self, client, broadcast_calls):
        ...
        assert resp.status_code == 200
        assert broadcast_calls, (
            "?profile=default is this process's own CURIE_HOME; skipping "
            "the broadcast leaves live sessions painting stale UNLOCK state"
        )
    ...
    def test_other_profile_save_does_not_broadcast(self, client, broadcast_calls, monkeypatch, tmp_path):
        ...
        assert resp.status_code == 200
        assert not broadcast_calls, (
            "a profile-scoped save targets a different CURIE_HOME than this "
...
class TestGatewayBroadcastHelper:
    def test_broadcast_session_info_emits_for_live_sessions(self, _isolate_curie_home, monkeypatch):
    ...
    def test_approvals_slash_mirror_broadcasts(self, _isolate_curie_home, monkeypatch):
