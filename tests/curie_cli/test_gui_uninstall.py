"""Tests for curie_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import curie_cli.gui_uninstall as gu


def _make_agent(curie_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = curie_home / "curie-agent"
    (agent_root / "curie_cli").mkdir(parents=True)
    (agent_root / "curie_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(curie_home: Path) -> None:
    """Create the source-built GUI artifacts a `curie desktop` run produces."""
    desktop = curie_home / "curie-agent" / "apps" / "desktop"
    ...
    (curie_home / "curie-agent" / "node_modules").mkdir(parents=True)
    (curie_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(curie_home: Path) -> None:
    (curie_home / "config.yaml").write_text("x: 1\n")
    (curie_home / ".env").write_text("KEY=secret\n")
    (curie_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(curie_home)
    _make_gui_build(curie_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(curie_home)
    ...
    assert summary["curie_home"] == str(curie_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `curie desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    curie_home = tmp_path / ".curie"
    _make_agent(curie_home)
    icon = lde.icon_path(curie_home / "curie-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(curie_home)
    ...
    assert lde.icon_path(curie_home / "curie-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (curie_home / "curie-agent" / "curie_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".curie")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import curie_cli.uninstall as uninstall
