"""Tests for ``curie doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from curie_cli import doctor_live
from curie_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `curie doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("curie_cli.doctor.CURIE_HOME", tmp_path / "home")
        monkeypatch.setattr("curie_cli.doctor.PROJECT_ROOT", tmp_path / "root")
