"""``curie`` must survive git operations on the checkout (launcher layout).

The Windows ``curie`` command is a launcher derived from the venv console
script. Its canonical home is the managed binary dir ``CURIE_HOME\\bin`` —
OUTSIDE the git checkout — because the earlier in-checkout home
(``curie-agent\\bin``) was swept by ``curie update``'s autostash
(``git stash push --include-untracked``) and, with the desktop updater's
``--keep-stash``, never restored: ``curie`` stopped resolving in every new
...
existing install's PATH to the canonical layout from the ``curie update``
tail. Platform verdict, PATH values, and registry I/O are injected
parameters (same pattern as ``curie_constants.venv_bin_dir``), so these
...
from curie_cli._install_repair import (
...
def _make_managed(tmp_path, monkeypatch, *, relocatable: bool = False):
    """Fake managed layout: CURIE_HOME/curie-agent/venv/Scripts + launchers."""
    home = tmp_path / "curie"
    root = home / "curie-agent"
...
def test_source_checkout_untouched(tmp_path, monkeypatch):
    """A checkout NOT under CURIE_HOME gains nothing anywhere."""
    home = tmp_path / "curie-home"
    home.mkdir()
    monkeypatch.setenv("CURIE_HOME", str(home))
    root = tmp_path / "src" / "curie-agent"
    scripts = root / "venv" / "Scripts"
    scripts.mkdir(parents=True)
    for name in _WINDOWS_BIN_LAUNCHERS:
...
def test_profile_session_still_heals_the_shared_bin(tmp_path, monkeypatch):
    """Under ``curie -p <name>`` CURIE_HOME points inside profiles/<name>;
    ...
    monkeypatch.setenv("CURIE_HOME", str(home / "profiles" / "work"))
...
# ---------------------------------------------------------------------------
# migrate_windows_bin_path — the `curie update` tail migration
# ---------------------------------------------------------------------------
...
def test_migration_moves_path_to_home_bin_and_strips_legacy(managed_install):
    ...
    (root / "bin").mkdir()
    (root / "bin" / "curie.exe").write_bytes(b"legacy copy")
    ...
    assert (root / "bin" / "curie.exe").read_bytes() == b"legacy copy"
...
def test_normalize_windows_path_equivalences():
    assert (
        _normalize_windows_path(r"C:\Users\Me\AppData\Local\curie\bin")
        == _normalize_windows_path("c:/users/me/appdata/local/CURIE/BIN/")
    )
...
def test_repo_gitignores_the_legacy_bin_dir():
    ...
    launchers at ``<checkout>/bin``. ``curie update`` autostashes with
    ...
    result = subprocess.run(
        ["git", "-C", str(repo_root), "check-ignore", "-q", "bin/curie.exe"],
        capture_output=True,
    )
    assert result.returncode == 0, (
        "bin/curie.exe is not gitignored — curie update's autostash "
