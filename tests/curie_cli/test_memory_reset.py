"""Tests for the `curie memory reset` CLI command.
...
- Profile-scoped reset (uses CURIE_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake CURIE_HOME with memory files."""
    curie_home = tmp_path / ".curie"
    memories = curie_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("CURIE_HOME", str(curie_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nCurie repo is at ~/.curie/curie-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `curie memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        curie_home = tmp_path / ".curie"
        (curie_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("CURIE_HOME", str(curie_home))
