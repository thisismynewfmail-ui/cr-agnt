...
import pytest
from curie_cli.config import DEFAULT_CONFIG, load_config, save_config
...
def _make_chat_args(**overrides):
    return Namespace(
        ...
        unlock=overrides.get("unlock", False),
...
class TestNonInteractiveSetup:
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `curie` should not prompt for input when no provider exists and stdin is headless."""
        from curie_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("curie_cli.main._has_any_provider_configured", return_value=False),
            patch("curie_cli.main.cmd_setup") as mock_setup,
        ...
        out = capsys.readouterr().out
        assert "curie config set model.provider custom" in out
