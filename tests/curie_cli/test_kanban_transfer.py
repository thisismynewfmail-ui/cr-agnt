"""Tests for kanban board export / import (``curie_cli.kanban_transfer``).
...
from curie_cli import kanban_transfer as kt
from curie_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("CURIE_KANBAN_HOME", str(root))
        for var in ("CURIE_KANBAN_DB", "CURIE_KANBAN_WORKSPACES_ROOT",
                    "CURIE_KANBAN_ATTACHMENTS_ROOT", "CURIE_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Curie"):
