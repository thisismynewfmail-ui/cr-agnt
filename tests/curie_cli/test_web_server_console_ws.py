"""Dashboard Curie Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_curie_home):
...
def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from curie_cli.console_engine import ConsoleResult, CurieConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.2)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(CurieConsoleEngine, "execute", slow_execute)
