...
``curie*.exe`` shims aside (``curie.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``curie`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("curie", "curie-agent", "curie-acp", "curie-gateway"):
...
# ---------------------------------------------------------------------------
# curie_cli.main._run_quarantined_install
# ---------------------------------------------------------------------------
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "curie.exe" in names, "curie.exe must be restored after a no-op install"
    assert "curie-acp.exe" in names
    assert "curie-gateway.exe" in names
    assert (scripts / "curie.exe").read_bytes() == b"MZ-old-curie"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "curie.exe").read_bytes() == b"MZ-new-curie"
...
# ---------------------------------------------------------------------------
# curie_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "curie.exe" in names, "curie.exe must be restored after a no-op recovery install"
    assert (scripts / "curie.exe").read_bytes() == b"MZ-old-curie"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "curie.exe").write_bytes(b"MZ-new-curie")
