"""ensure_curie_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".curie"
    second = tmp_path / "b" / ".curie"

    monkeypatch.setenv("CURIE_HOME", str(first))
    cfg.ensure_curie_home()

    # Profile switch: CURIE_HOME moves → the new path is ensured too.
    monkeypatch.setenv("CURIE_HOME", str(second))
    cfg.ensure_curie_home()
