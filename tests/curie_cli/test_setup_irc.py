"""Tests for IRC gateway configuration via `curie setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Curie install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Curie install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#curie")
            monkeypatch.setenv("IRC_NICKNAME", "curie-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `curie setup gateway` experience with IRC present."""
