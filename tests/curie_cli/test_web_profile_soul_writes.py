...
def client(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CURIE_DASHBOARD_SESSION_TOKEN", "soul-test-token")
    from curie_cli import web_server

    # ``_SESSION_TOKEN`` is minted once, at import time, on purpose: the token
    # is meant to die with the process. So setting the env var only works for
    # whichever test module imports ``web_server`` first — any later one gets
    # 401 on every request, and the failure lands here rather than in the
    # module that already imported it. Patch the resolved constant too.
...
def profile_dir(tmp_path, monkeypatch) -> Path:
    """Create a real profile directory under the test CURIE_HOME."""
    monkeypatch.setenv("CURIE_HOME", str(tmp_path))
...
class TestSoulWriteDurability:
    ...
    def test_existing_soul_survives_an_interrupted_save(
        ...
        # CURIE_HOME patch the client/profile_dir fixtures installed.
