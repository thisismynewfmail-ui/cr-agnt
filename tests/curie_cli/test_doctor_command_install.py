"""Tests for the Command Installation check in curie doctor."""
...
import curie_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal CURIE_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".curie"
    ...
    curie_bin = venv_bin_dir / "curie"
    curie_bin.write_text("#!/usr/bin/env python\n# entry point\n")
...
class TestDoctorCommandInstallation:
    ...
    def test_fix_repairs_wrong_symlink(self, monkeypatch, tmp_path):
        home, project, curie_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create a symlink pointing to wrong target
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "curie"
        wrong_target = tmp_path / "wrong_curie"
        ...
        assert cmd_link.resolve() == curie_bin.resolve()
