"""Tests for curie_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $CURIE_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Curie-managed node dir.
"""
...
def _make_curie_node(curie_home: Path) -> Path:
    """Create a fake $CURIE_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = curie_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    curie_home = fake_home / ".curie"
    _make_curie_node(curie_home)
    ...
    removed = uninstall.remove_node_symlinks(curie_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    curie_home = fake_home / ".curie"
    node_bin = _make_curie_node(curie_home)
