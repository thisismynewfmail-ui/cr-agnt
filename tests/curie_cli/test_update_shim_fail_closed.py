...
The bug class: on Windows, when `curie.exe`/sibling shims could not be
...
import curie_cli._install_repair as ir
import curie_cli.update_cmd as update_cmd


def _make_shims(scripts_dir: Path, names=("curie", "curie-gateway")) -> list[Path]:
...
def test_strict_quarantine_refuses_before_install(windows, tmp_path, monkeypatch):
    ...
    # curie.exe cannot be renamed; curie-gateway.exe can
    real_rename = Path.rename

    def deny_curie(self, target):
        if self.name == "curie.exe":
            raise PermissionError(13, "held open")
        return real_rename(self, target)

    monkeypatch.setattr(Path, "rename", deny_curie)
    monkeypatch.setattr(cli_main, "_curie_exe_shims", lambda d: shims)
    ...
    assert install_ran == []
    assert "curie.exe" in exc_info.value.failed_shims
    # The successful rename (curie-gateway.exe) was rolled back.
    assert (scripts / "curie-gateway.exe").exists()
    assert not list(scripts.glob("curie-gateway.exe.old.*"))


def test_non_strict_keeps_warn_and_try(windows, tmp_path, monkeypatch):
    scripts = tmp_path / "venv" / "Scripts"
    shims = _make_shims(scripts, names=("curie",))
    monkeypatch.setattr(
        Path, "rename",
...
def test_recovery_install_cmd_fail_closed(windows, tmp_path, monkeypatch):
    ...
    _make_shims(scripts, names=("curie",))
