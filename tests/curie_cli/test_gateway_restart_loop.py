...
Covers:
- Defense 1: gateway stop/restart refuse when _CURIE_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        "curie gateway stop",
        "curie gateway uninstall",
        "curie  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "CURIE GATEWAY RESTART",           # uppercase
    ])
    def test_curie_gateway_commands(self, text):
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.curie.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.curie/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l curie-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.curie.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.curie.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.curie.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.curie/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.curie.gateway-apollo:/a.plist' "
            "'ai.curie.gateway-cronus:/c.plist' 'ai.curie.gateway-plutus:/p.plist' "
            "'ai.curie.gateway:/Users/botuser/Library/LaunchAgents/ai.curie.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated curie note on the next line"
        )
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.curie.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.curie.gateway',
        "launchctl kick'start' -k gui/501/ai.curie.gateway",
        "launchctl kick\\start -k gui/501/ai.curie.gateway",
        'launchctl "kickstart" -k gui/501/ai.curie.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.curie.gateway',
        "launchctl dis\\able gui/501/ai.curie.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.curie."gateway"',
        # Same class on the systemctl and curie-CLI branches.
        'systemctl re"start" curie-gateway',
        'curie gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.curie.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.curie.update-checker',
        'systemctl re"start" curie-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "curie cron list",
        "curie update",
        "curie config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `curie gateway start` is benign — starting a gateway from inside a
        ...
        "curie gateway start",
        "curie gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway curie
        # services must not be falsely blocked (the old `.*curie` matched any
        # curie token).
        "launchctl unload ai.curie.update-checker.plist",
        "launchctl restart ai.curie.daemon",
        ...
        "systemctl restart curie-meta.service",
        "systemctl restart curie-cron-helper",
        ...
        "echo after the curie gateway restarted cleanly",
        "the curie gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "curie skill view gateway-notes && echo curie gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `curie` is a path component
        # there, not a command.
        "cat '/docs/curie gateway restart-notes.md'",
        "less /home/user/notes/curie gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "curie gateway restart",
        "curie gateway restart; echo done",
        "curie gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;curie gateway restart",
        "true && curie gateway stop",
        "echo $(curie gateway restart)",
        "echo `curie gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.curie.gateway"])',
        'subprocess.run(["curie", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.curie.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["curie", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: curie gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `curie -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry CURIE_HOME/CURIE_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("CURIE_PROFILE", "zeus")
        monkeypatch.delenv("CURIE_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "curie -p zeus gateway stop",
        "curie -p zeus gateway restart",
        "curie --profile zeus gateway restart",
        "curie --profile zeus gateway stop",
        "curie --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "curie -v -p zeus gateway restart",
        "curie -p zeus -v gateway restart",
        "curie --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "curie -p venus gateway stop",
        "curie -p venus gateway restart",
        "curie --profile venus gateway restart",
        "curie --profile=venus gateway stop",
        "curie -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "curie -p zeus gateway start",
        "curie -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("curie gateway restart")
        assert _contains_gateway_lifecycle_command("curie gateway stop")

    def test_curie_home_derived_profile(self, monkeypatch):
        # Without CURIE_PROFILE the guard falls back to the CURIE_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("CURIE_PROFILE", raising=False)
        monkeypatch.delenv("CURIE_PROFILE_NAME", raising=False)
        import curie_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("curie -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("curie -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("curie -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("curie gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_curie_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade curie then run curie gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_curie_gateway_once.sh). The script must live under
        # CURIE_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("CURIE_HOME", str(tmp_path / ".curie"))
        scripts_dir = tmp_path / ".curie" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_CURIE_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _CURIE_GATEWAY=1.

    Issue #37453: systemctl --user restart curie-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart curie-gateway",
        "systemctl --user restart curie-gateway",
        "systemctl stop curie-gateway.service",
        "curie gateway restart",
        "curie gateway uninstall",
        "launchctl kickstart gui/501/ai.curie.gateway",
        "launchctl bootout gui/501/ai.curie.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.curie.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.curie/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.curie.gateway.restart-once.plist",
        "pkill -f curie.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart curie-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.curie.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-curie label: label-independent detection is the point
        # (#62891 second reproduction used `ai.curie.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.curie.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _CURIE_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _CURIE_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_CURIE_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="curie gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["curie gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.curie.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.curie.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-curie systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run curie gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("curie gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfecurie gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under CURIE_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("curie gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/curie\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 curie gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_curie_home()` falls back to Path.home(), which raises
        RuntimeError when neither CURIE_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart curie-gateway' .",
        "grep -rn 'curie gateway restart' ./logs",
        "rg 'curie gateway restart' ../archive",
        "grep -c 'systemctl stop curie-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM curie_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell curie gateway restart"',
        'sqlite3 ./db ".system systemctl restart curie-gateway"',
        'psql ./x -c "\\! systemctl restart curie-gateway"',
        "grep -r 'curie gateway restart' . | sh",
        "grep -r 'curie gateway restart' ./logs | bash",
        "grep -r 'curie gateway restart' . | sudo sh",
        "grep -r 'x' . ; curie gateway restart",
        "grep -r 'x' . && systemctl restart curie-gateway",
        'grep -r "$(curie gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart curie-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell curie gateway restart"',
        'sqlite3 db ".system systemctl restart curie-gateway"',
        'psql -c "\\! systemctl restart curie-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    curie_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run curie gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CURIE_HOME", str(tmp_path / ".curie"))
        (tmp_path / ".curie").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run curie gateway restart nightly",
        ))
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop curie-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart curie-gateway' /var/log/syslog",
        "rg 'curie gateway restart' /home/user/.curie/logs/",
        "journalctl -u curie-gateway --grep 'systemctl restart curie-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'curie_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell curie gateway restart"',
        'psql -c "\\! systemctl restart curie-gateway"',
        "grep 'systemctl restart curie-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart curie-gateway",
        'grep "$(systemctl restart curie-gateway)" f',
        "grep 'restart' log; systemctl restart curie-gateway",
        'sqlite3 db "SELECT 1"; curie gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "curie gateway restart",
        "sudo systemctl stop curie-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart curie-gateway%'\""
        )
