...
When flags like --unlock, -w, -s exist on both the parent parser and the 'chat'
subparser, placing the flag BEFORE the subcommand (e.g. 'curie --unlock chat')
...
def _build_parser():
    """Build the curie argument parser from the real code.
    ...
    chat.add_argument("--unlock", action="store_true",
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("curie_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "curie_cli.banner", fake_banner)
...
class TestUnlockEnvVar:
    """Verify --unlock sets CURIE_UNLOCK_MODE regardless of flag position.
    ...
    def _simulate_cmd_chat_unlock_check(self, args):
        """Replicate the exact check from cmd_chat in main.py."""
        if getattr(args, "unlock", False):
            os.environ["CURIE_UNLOCK_MODE"] = "1"

    def test_unlock_before_chat_sets_env(self):
        parser = _build_parser()
        args = parser.parse_args(["--unlock", "chat"])
        self._simulate_cmd_chat_unlock_check(args)
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `curie gateway run --accept-hooks` failed
    ...
    # One driver subprocess parses ALL argvs: curie_cli.main is a very heavy
    # import (previously 11 separate `python -m curie_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `curie <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `curie -t web chat` silently dropped the
    ...
    Uses the real `curie_cli._parser.build_top_level_parser()` rather than
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `curie <flag> <value> chat`:\n  "
