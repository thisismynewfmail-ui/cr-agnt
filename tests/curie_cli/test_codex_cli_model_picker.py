...
Covers:
 - Normal path (tokens already in Curie auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Curie
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `curie auth openai-codex`. The old
...
@pytest.fixture()
def curie_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Curie auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(curie_auth_only_env):
    """openai-codex appears when tokens are already in Curie auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Curie
    auth store."""
    curie_home = tmp_path / ".curie"
    curie_home.mkdir()
