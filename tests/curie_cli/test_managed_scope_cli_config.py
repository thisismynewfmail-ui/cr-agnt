...
curie_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `curie config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_curie_home = get_curie_home()`` at import time (module
    singleton), so monkeypatching CURIE_HOME after import doesn't move it.
    ...
    this only matters for tests that swap CURIE_HOME.)
    """
    import cli

    cli._curie_home = home
