...
def test_cmd_chat_rejects_noninteractive_gpt55_pro_even_with_unlock(
    main_mod, fake_cli, codex_config, monkeypatch, capsys
):
    monkeypatch.setattr(sys, "stdin", _NonInteractiveStdin())

    with pytest.raises(SystemExit) as excinfo:
        main_mod.cmd_chat(_chat_args(model="openai/gpt-5.5-pro", unlock=True))
...
def test_top_level_oneshot_rejects_noninteractive_gpt55_pro_startup_override(
    ...
    monkeypatch.setattr(sys, "stdin", _NonInteractiveStdin())
    monkeypatch.setattr(sys, "argv", ["curie", "-z", "hello", "-m", "openai/gpt-5.5-pro"])
