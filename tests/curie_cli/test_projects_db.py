"""Tests for the per-profile Projects store (curie_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Curie Agent", folders=["/tmp/curie"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "curie-agent"
    assert proj.name == "Curie Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/curie"
    assert [f.path for f in proj.folders] == ["/tmp/curie"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "curie-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' CURIE_HOME.
