"""Regression tests for #34107 — Docker UID/GID handling in ensure_curie_home.

When Curie runs in Docker with ``CURIE_UID=1000`` / ``CURIE_GID=911``,
the entrypoint chowns the top-level ``CURIE_HOME`` once at startup. But
...
The fix is a ``_chown_to_curie_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_curie_uid_gid
# ---------------------------------------------------------------------------


class TestResolveCurieUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("CURIE_UID", "1000")
        monkeypatch.setenv("CURIE_GID", "911")
        from curie_cli.config import _resolve_curie_uid_gid
...
class TestChownToCurieUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_curie_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("CURIE_UID", raising=False)
        monkeypatch.delenv("CURIE_GID", raising=False)
        from curie_cli import config as cfg
