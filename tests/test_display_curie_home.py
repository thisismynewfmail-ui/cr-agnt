"""display_curie_home() renders POSIX separators on every platform.

Maintainer catch (#95681 arc): on Windows with a custom CURIE_HOME under
the user profile (e.g. AppData/Local/curie), ``"~/" +
str(home.relative_to(Path.home()))`` produced the mixed-separator chimera
``~/AppData\\Local\\curie`` — which then leaked into every consumer that
appends sub-paths (the skill_manage schema showed the agent
``~/AppData\\Local\\curie/skills/``). The ``~/`` shorthand implies POSIX
...
class TestDisplayCurieHomePosix(unittest.TestCase):
    def test_nested_home_renders_forward_slashes(self):
        ...
        import curie_constants as hc

        fake_userhome = Path.home()
        nested = fake_userhome / "AppData" / "Local" / "curie"
        with patch.object(hc, "get_curie_home", return_value=nested):
            out = hc.display_curie_home()
        self.assertEqual(out, "~/AppData/Local/curie")
        self.assertNotIn("\\", out)

    def test_default_home_unchanged(self):
        import curie_constants as hc

        with patch.object(hc, "get_curie_home",
                          return_value=Path.home() / ".curie"):
            out = hc.display_curie_home()
        self.assertEqual(out, "~/.curie")

    def test_outside_home_falls_back_to_absolute(self):
        import curie_constants as hc

        outside = Path("/opt/curie-custom") if os.name != "nt" else Path("C:/opt/curie-custom")
        with patch.object(hc, "get_curie_home", return_value=outside):
            out = hc.display_curie_home()
