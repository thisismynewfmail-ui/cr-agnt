...
Uses a throwaway CURIE_HOME so it never touches ~/.curie. No real names.
"""
...
def _fresh_home():
    """Point CURIE_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_curie_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["CURIE_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER CURIE_HOME is set.
...
# a different get_curie_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"CURIE_HOME (throwaway): {HOME}")
