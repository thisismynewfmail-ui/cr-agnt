...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate CURIE_HOME under tmp_path."""
    monkeypatch.setenv("CURIE_HOME", str(tmp_path / ".curie"))
    (tmp_path / ".curie").mkdir()

    # Force the constants/image cache helpers to re-read CURIE_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("curie_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_curie_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under CURIE_HOME — gateway cleanup
