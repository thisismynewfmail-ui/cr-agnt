...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated CURIE_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="curie_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".curie"))
    prev_home = os.environ.get("CURIE_HOME")
    os.environ["CURIE_HOME"] = os.path.join(test_home, ".curie")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("curie_"):
            del sys.modules[mod]
    ...
    try:
        ...
        if prev_home is None:
            ...
            os.environ["CURIE_HOME"] = prev_home
