...
def isolated_home(monkeypatch):
    """Temp CURIE_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="curie_test_31179_")
    curie_home = os.path.join(test_home, ".curie")
...
def _fresh_modules():
    """Drop cached curie modules so each test reloads against current env."""
    _drop_reload_targets()
