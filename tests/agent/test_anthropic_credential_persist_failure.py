...
``~/.curie/.anthropic_oauth.json`` for ``curie_pkce`` ones.  Those singletons
...
def curie_home(tmp_path, monkeypatch):
    """Real on-disk CURIE_HOME so ``load_pool()`` re-reads what we persisted."""
    home = tmp_path / "curie"
    ...
    monkeypatch.setattr(
        "curie_cli.auth.is_provider_explicitly_configured", lambda pid: True
    )
    return home
...
def test_curie_oauth_writer_raises_instead_of_swallowing(curie_home, monkeypatch):
    ...
    with pytest.raises(CredentialPersistError):
        AA._write_curie_oauth_credentials(
            _ROTATED_ACCESS, _ROTATED_REFRESH, _EXPIRED_MS + 3_600_000
        )
...
def test_pool_claude_code_fails_closed_and_reload_cannot_resurrect(
    curie_home, claude_credentials, monkeypatch
):
    monkeypatch.setattr(AA, "refresh_anthropic_oauth_pure", _rotating_refresh)
    _break_durable_write(monkeypatch)
...
# ---------------------------------------------------------------------------
# Pool path: curie_pkce
# ---------------------------------------------------------------------------


def test_pool_curie_pkce_fails_closed_and_reload_cannot_resurrect(
    ...
    reloaded = [
        e for e in load_pool("anthropic").entries() if e.source == "curie_pkce"
    ]
    assert reloaded
    assert reloaded[0].refresh_token == _STALE_REFRESH
    assert reloaded[0].last_status == STATUS_DEAD
