...
path via ``curie chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp CURIE_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point CURIE_HOME at a temp dir and turn moa.save_traces on."""
    curie_home = tmp_path / ".curie"
    curie_home.mkdir()
    monkeypatch.setenv("CURIE_HOME", str(curie_home))

    # save_moa_turn reads config via curie_cli.config.load_config; stub it to
