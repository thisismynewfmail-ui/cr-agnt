...
manipulation. Curie keeps canonical conversation history in state.db and may
...
def fake_homes(tmp_path, monkeypatch):
    """Point CURIE_HOME at a temp profile dir.

    Uses the real env-var resolution chain (get_curie_home /
    get_default_curie_root) instead of monkeypatching private helpers —
    ...
    CURIE_HOME=<root>/profiles/<name> makes get_default_curie_root()
