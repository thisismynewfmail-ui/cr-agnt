"""Regression tests for thisismynewfmail-ui/Cur-Agnt#2765.
...
service that didn't inherit ``~/.curie/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "curie memory status" in msg
