"""Tests for the cross-Curie-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Curie profile's
...
Reference: May 2026 incident — a curie-security profile session
accidentally edited skills under both ~/.curie/profiles/curie-security/skills/
AND ~/.curie/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Curie root with two profiles, monkeypatch the
...
def _set_active_home(monkeypatch, curie_home: Path):
    """Point file_safety._curie_home_path at a specific profile dir."""
    import agent.file_safety as fs
    monkeypatch.setattr(fs, "_curie_home_path", lambda: curie_home)
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_curie, monkeypatch):
        _set_active_home(monkeypatch, fake_curie["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"


    def test_falls_back_to_default_on_resolution_failure(self, fake_curie, monkeypatch):
        """If CURIE_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_curie_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_curie, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_curie["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_curie["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "curie-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_curie, monkeypatch):
        """Inverse direction — default-profile session reaching into a named profile."""
        _set_active_home(monkeypatch, fake_curie["default_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_curie["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "default"
        assert result["target_profile"] == "curie-security"


    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_curie, monkeypatch, area):
        _set_active_home(monkeypatch, fake_curie["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_curie["default_home"] / area / "foo.txt"
...
class TestGetCrossProfileWarning:
    ...
    def test_in_profile_returns_none(self, fake_curie, monkeypatch):
        from agent.file_safety import get_cross_profile_warning
        assert get_cross_profile_warning(
            str(fake_curie["root"] / "skills" / "a" / "SKILL.md")) is None

    def test_cross_profile_returns_none_guard_retired(self, fake_curie, monkeypatch):
        from agent.file_safety import get_cross_profile_warning
        target = fake_curie["root"] / "profiles" / "security" / "skills" / "x" / "SKILL.md"
