...
def test_stream_ceiling_structurally_outlives_the_default_host_ceiling():
    ...
    ceiling, so the worker can always outlive the host that is waiting on it
    and leave an abandoned provider daemon streaming a summary nobody can
    commit.  The published host deadline (tested below) is what actually
    closes that window; this test pins the gap it has to close.

    The reporter's ``auxiliary.compression.timeout: 600`` sizes the gap.  The
    4x multiplier alone put it at 2400s — a 30-minute orphan window — so the
    ceiling now grants a flat grace instead of multiplying once the timeout is
    already generous, halving the exposure without ever dropping the worker's
    budget below the host's.
    """
    ...
    assert aux._aux_stream_total_ceiling(600.0) == 1200.0
    assert (
...
def test_protected_provider_daemon_inherits_the_host_deadline():
    ...
    assert seen["thread"] == "curie-protected-aux-provider"
