...
import curie_state
from curie_state import SessionDB, SessionTurnLeaseLostError
    ...
    def flaky_acquire(*args, **kwargs):
        ...
        if attempts["n"] == 1:
            raise sqlite3.OperationalError(
                "database is locked (another Curie process held the "
...
def test_non_expired_turn_lease_from_dead_pid_is_reclaimed(
    ...
    monkeypatch.setattr(
        curie_state, "psutil", SimpleNamespace(pid_exists=pid_exists)
    )
