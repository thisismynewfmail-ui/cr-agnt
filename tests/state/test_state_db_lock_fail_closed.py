...
work on a file several Curie processes share (gateway service, the Desktop
app's `curie serve` backend, CLI sessions, the TUI slash worker):

* `curie_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `curie_state._cross_process_repair_lock`   — writable_schema surgery / VACUUM
...
from curie_state import SessionDB, repair_state_db_schema
...
def test_repair_lock_fails_closed_when_lock_file_is_unopenable(tmp_path):
    ...
    with curie_state._cross_process_repair_lock(db_path) as holding:
