    ...
    with patch.dict(
        "sys.modules",
        {
            "curie_constants": MagicMock(
                get_curie_home=MagicMock(return_value="/tmp/curie_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'curie-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'curie-release' patched"
    }
