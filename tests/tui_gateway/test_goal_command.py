...
def curie_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves CURIE_HOME.
...
@pytest.fixture()
def server(curie_home, monkeypatch):
    ...
    # Pin config resolution to the isolated CURIE_HOME. Sibling test
    ...
    # module-level ``_curie_home = get_curie_home()`` snapshot freezes
    ...
    monkeypatch.setattr(mod, "_curie_home", curie_home)
...
def test_moa_bare_returns_usage(server, session, curie_home):
    _write_moa_config(curie_home, """
moa:
