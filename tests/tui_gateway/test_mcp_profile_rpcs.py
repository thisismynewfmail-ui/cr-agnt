...
``CURIE_HOME`` with named profile dirs — no mocks of the config/mcp layer — and
...
@pytest.fixture
def curie_root(tmp_path, monkeypatch):
    """A temp CURIE_HOME root with two named profiles: 'work' and 'other'.

    Pointing CURIE_HOME at a dir outside ~/.curie makes it the profile ROOT
    (get_default_curie_root's Docker/custom branch), so named profiles live at
    ``<root>/profiles/<name>/`` and the launch/default profile is ``<root>``.
    """
    root = tmp_path / "curie_home"
...
def test_add_lands_in_named_profile_only(curie_root):
    root = curie_root
    resp = _call(
        "mcp.servers.add",
        {
...
def test_list_reflects_the_scoped_profile(curie_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_writes_env_and_header_to_right_profile(curie_root):
    root = curie_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_set_api_key_stdio_references_env_block(curie_root):
    root = curie_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_remove_scoped_to_profile(curie_root):
    root = curie_root
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_duplicate_and_missing_errors(curie_root):
    _result(
        _call(
            "mcp.servers.add",
...
def test_add_requires_transport(curie_root):
    resp = _call("mcp.servers.add", {"profile": "work", "name": "empty", "config": {}})
    assert "error" in resp
    assert resp["error"]["code"] == 4063


def test_default_profile_add_when_profile_omitted(curie_root):
    root = curie_root
    _result(
        _call(
            "mcp.servers.add",
    ...
    # Omitted profile → launch/default profile == CURIE_HOME root config.yaml.
    default_cfg = _read_yaml(root / "config.yaml")
