...
def server():
    ...
    # e.g. curie_cli.active_sessions would bind the mocked get_curie_home
    ...
    with patch.dict("sys.modules", {
        "curie_constants": MagicMock(get_curie_home=MagicMock(return_value="/tmp/curie_test")),
        "curie_cli.env_loader": MagicMock(),
        "curie_cli.banner": MagicMock(),
        "curie_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `CURIE_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("CURIE_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("CURIE_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_sync_session_key_after_compress_reanchors_active_session_lease(
    ...
    fake_approval = types.SimpleNamespace(
        disable_session_unlock=lambda *_args, **_kwargs: None,
        enable_session_unlock=lambda *_args, **_kwargs: None,
        is_session_unlock_enabled=lambda *_args, **_kwargs: False,
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/curie-agent-dev": {"name": "curie-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "curie-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_scopes_skill_lookup_to_session_profile(server, tmp_path):
    ...
    profile_home rather than the gateway process's ambient CURIE_HOME
    ...
    # The gateway's own CURIE_HOME (the test-isolation tempdir, no
...
def test_skin_live_switch_end_to_end(server, tmp_path, monkeypatch):
    """Real config + skin files: activating a skin (as `curie config set` does)
    ...
    monkeypatch.setattr(skin_engine, "get_curie_home", lambda: tmp_path)
    ...
    # Activate midnight, as `curie config set display.skin midnight` would.
