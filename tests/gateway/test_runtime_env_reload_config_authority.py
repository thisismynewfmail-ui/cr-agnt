...
CURIE_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env CURIE_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``curie config get terminal.backend`` still says local.
    """
    curie_home = tmp_path / ".curie"
    curie_home.mkdir()
    (curie_home / "config.yaml").write_text(
    ...
    (curie_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
