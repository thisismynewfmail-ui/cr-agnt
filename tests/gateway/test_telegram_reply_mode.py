...
class TestDMTopicFallbackReplyToMode:
    ...
    Regression tests for https://github.com/thisismynewfmail-ui/Cur-Agnt/issues/23994:
    reply_to_mode 'off' was ignored when sending via Curie-created DM topic
...
class TestDMTopicSyntheticSendRouting:
    ...
    Regression tests for https://github.com/thisismynewfmail-ui/Cur-Agnt/issues/87051:
    ...
    response in a different chat lane than the Curie topic the session runs
    in. The no-anchor branch must prefer the Curie topic's
    ``message_thread_id`` whenever it resolves.
    """

    def test_no_anchor_prefers_curie_topic_thread_id(self):
