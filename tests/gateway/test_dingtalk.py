...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Curie"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^curie"]}
        )
        assert adapter._message_matches_mention_patterns("curie please help") is True
        assert adapter._message_matches_mention_patterns("please curie help") is False
