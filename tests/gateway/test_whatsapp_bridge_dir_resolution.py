...
(/opt/curie/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable CURIE_HOME location instead.
"""
import importlib
...
def test_readonly_install_mirrors_to_curie_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable CURIE_HOME."""
    ...
    monkeypatch.setattr(
        "curie_constants.get_curie_home", lambda: curie_home
    )
    ...
    expected = curie_home / "scripts" / "whatsapp-bridge"
