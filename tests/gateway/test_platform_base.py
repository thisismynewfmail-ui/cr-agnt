...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    curie_home.mkdir()
    monkeypatch.setattr(base, "_CURIE_HOME", curie_home)
    monkeypatch.setattr(base, "_CURIE_ROOT", curie_home)
    path = curie_home / "cache" / "bws_cache.enc.json"
    path.parent.mkdir()
...
class TestMediaInsideSerializedJson:
    ...
    def test_media_in_embedded_serialized_reply_not_extracted(self):
        ...
        content = (
            '{"content":"previous reply MEDIA:/Users/ex/.curie/media/'
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("CURIE_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("CURIE_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("CURIE_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("CURIE_KANBAN_HOME", str(tmp_path / "curie"))
        monkeypatch.setenv("CURIE_MEDIA_TRUST_RECENT_FILES", "0")
        board_root = tmp_path / "curie" / "kanban" / "boards" / "research"
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("CURIE_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("CURIE_MEDIA_TRUST_RECENT_FILES", "1")
        monkeypatch.setenv("CURIE_MEDIA_TRUST_RECENT_SECONDS", "600")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.curie/mcp-tokens/ must
        ...
        fake_home = tmp_path / "home"
        curie_dir = fake_home / ".curie"
        (curie_dir / "mcp-tokens").mkdir(parents=True)
        secret = curie_dir / rel
        secret.write_text('{"access_token": "live-bearer-abc123"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr(
            "gateway.platforms.base._CURIE_HOME",
            curie_dir,
        )
        monkeypatch.setattr(
            "gateway.platforms.base._CURIE_ROOT",
            curie_dir,
        )
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the CURIE_HOME root (google_token.json)
        ...
        fake_home = tmp_path / "home"
        curie_dir = fake_home / ".curie"
        curie_dir.mkdir(parents=True)
        token = curie_dir / "google_token.json"
        token.write_text('{"access_token": "***", "refresh_token": "***"}')
        monkeypatch.setenv("HOME", str(fake_home))
        monkeypatch.setattr("gateway.platforms.base._CURIE_HOME", curie_dir)
        monkeypatch.setattr("gateway.platforms.base._CURIE_ROOT", curie_dir)

        assert BasePlatformAdapter.validate_media_delivery_path(str(token)) is None


    def test_denylist_blocks_non_cache_file_under_curie_home(self, tmp_path, monkeypatch):
        """A non-credential file the agent wrote directly under ~/.curie
        ...
        curie_dir = fake_home / ".curie"
        curie_dir.mkdir(parents=True)
        artifact = curie_dir / "adhoc_report.pdf"
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting CURIE_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose CURIE_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.curie) emits
        ...
        curie_root = denied_root / ".curie"
        prof_cache = curie_root / "profiles" / "myprof" / "cache" / "images"
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._CURIE_ROOT", curie_root
        )
...
class TestDockerContainerMediaPathTranslation:
    ...
    def test_cache_dir_container_path_translates_to_host_cache(self, tmp_path, monkeypatch):
        """MEDIA:/root/.curie/cache/images/... (the agent_visible_image path
        under docker) must translate to the HOST cache file, not the sandbox
        home copy."""
        curie_home = tmp_path / ".curie"
        cache = curie_home / "cache" / "images"
    ...
    def test_container_credential_path_never_translates_through_home(self, tmp_path, monkeypatch):
        """/root/.curie/* outside a cache mount (the sandbox's credential
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "CURIE_HUMAN_DELAY_MODE": "natural",
            "CURIE_HUMAN_DELAY_MIN_MS": "oops",
            "CURIE_HUMAN_DELAY_MAX_MS": "still-bad",
        }
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"CURIE_MEDIA_DELIVERY_STRICT": "1",
                                     "CURIE_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CURIE_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Curie
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../curie/cache/video/abc.mp4``
    — a host filesystem path with no actionable information.
    """

    SENSITIVE_PATH = "/home/jayne/.curie/cache/media/sensitive_host_path_abc123.bin"
...
class TestDockerProfileSandboxMediaTranslation:
    ...
    def test_home_credential_surface_still_refused(self, monkeypatch):
        """The /root/.curie exclusion survives profile scoping: translating
        ...
        for task in ("default", f"session:{self.SESSION_KEY}"):
            secrets = self._sandbox_dir(task) / "home" / ".curie"
            secrets.mkdir(parents=True, exist_ok=True)
            (secrets / "auth.json").write_text("{}")

        assert (
            BasePlatformAdapter.validate_media_delivery_path(
                "/root/.curie/auth.json", session_key=self.SESSION_KEY
            )
            is None
        )
