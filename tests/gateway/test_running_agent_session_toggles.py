"""Regression tests: /unlock and /verbose dispatch mid-agent-run.
...
(PR #12334). A small allowlist bypasses that and actually dispatches:
  ...
  * /unlock — toggles the session unlock flag; useful to pre-approve a
...
def _make_runner():
    ...
    # compares seconds_since_activity against CURIE_AGENT_TIMEOUT. Return a
...
@pytest.mark.asyncio
async def test_unlock_dispatches_mid_run(monkeypatch):
    """/unlock mid-run must dispatch to its handler, not hit the catch-all."""
    runner = _make_runner()
    runner._handle_unlock_command = AsyncMock(return_value="⚡ UNLOCK mode **ON** for this session")

    result = await runner._handle_message(_make_event("/unlock"))

    runner._handle_unlock_command.assert_awaited_once()
    assert result == "⚡ UNLOCK mode **ON** for this session"
...
async def test_fresh_ancient_turn_remains_controllable(monkeypatch):
    ...
    runner._handle_verbose_command = AsyncMock(return_value="tool progress: new")
    monkeypatch.setenv("CURIE_AGENT_TIMEOUT", "1800")
