...
closure the PR changed, against a real temp ``CURIE_HOME``.
"""
...
def _fake_switch_result():
    ...
    from curie_cli.model_switch import ModelSwitchResult
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "curie_cli.model_switch.list_picker_providers",
    ...
    monkeypatch.setattr(
        "curie_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_curie_home", curie_home)
    _stub_picker_dependencies(monkeypatch)
    # save_config writes to ``get_curie_home() / config.yaml`` — point it here.
    monkeypatch.setattr("curie_constants.get_curie_home", lambda: curie_home)
    monkeypatch.setattr("curie_cli.config.get_curie_home", lambda: curie_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_curie_home", default_home)
    _stub_picker_dependencies(monkeypatch)
