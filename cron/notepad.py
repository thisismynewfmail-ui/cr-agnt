...
Write path is the CLI (``curie cron notepad <job_id> set <key> <value>``),
...
NOTEPAD_FILE = get_curie_home().resolve() / "cron" / "notepad.db"
...
def render_notepad_section(job_id: str) -> str:
    ...
    return (
        ...
        f"`curie cron notepad {job_id} set <key> <value>` "
        f"(also: get/delete/list; `curie cron notepad {job_id} delete "
