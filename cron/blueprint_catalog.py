...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``curie://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``curie://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"curie://blueprint/{quote(blueprint.key)}{qs}"
