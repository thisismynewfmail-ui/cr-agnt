...
Curie Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``CURIE_NIX_BUILD=1`` on the
Curie package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("CURIE_NIX_BUILD") == "1"

_BLOCK_MESSAGE = (
    "Building wheels or sdists for curie-agent is not supported.\n"
    "Curie is distributed via the shell installer, Docker image, or Nix.\n"
    "See: https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/getting-started/installation.md\n"
    "\n"
    "If you are developing, use an editable install instead:\n"
    "  uv sync          # or: uv pip install -e .\n"
    "\n"
    "If you are building with Nix (uv2nix), this error should not fire —\n"
    "the Curie Nix derivation sets CURIE_NIX_BUILD=1. If it does, file a bug."
)
