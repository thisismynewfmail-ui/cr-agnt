#!/usr/bin/env python3
"""
SQLite State Store for Curie Agent.
...
# transitive dependency via curie_state_common). run_agent keeps its own
# predating copy — curie_state cannot import run_agent (circular) — guarded
...
from curie_startup_watchdog import report_startup_progress
from curie_cli.sqlite_runtime import (
    is_sqlite_wal_reset_vulnerable as _is_sqlite_wal_reset_vulnerable,
)
from typing import Any, Callable, Dict, Iterator, List, Optional, Set, Tuple, TypeVar, cast

from curie_state_common import (  # noqa: F401  (re-exported for back-compat)
    _BRANCH_CHILD_SQL,
...
from curie_state_portability import SessionPortabilityMixin
from curie_state_schema import SessionSchemaMixin
from curie_state_search import SessionSearchMixin
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``curie -c``/``--resume`` to continue the most recent session in
...
DEFAULT_DB_PATH = get_curie_home() / "state.db"
...
#
# The ceilings above bound Curie's SQLite descriptors, which is only ever part
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.curie`` even when a test fixture later
    redirects ``CURIE_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_curie_home()`` fresh so a runtime
       ``CURIE_HOME`` redirect takes effect regardless of import order.
    """
    if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH:
        return DEFAULT_DB_PATH
    return get_curie_home() / "state.db"
...
# ~/.curie/state.db accumulated pytest fixture rows — sessions with
...
# FTS write corruption").  The hermetic conftest redirects CURIE_HOME per
# test, but any escape (a session-scoped fixture running before the autouse
# fixture, a subprocess child launched without CURIE_HOME, a stale worktree
# without the re-pin, or a developer shell that exports CURIE_HOME to the
...
_STATE_DB_GUARD_BYPASS_ENV = "CURIE_STATE_DB_GUARD_BYPASS"

#: Additional production roots to refuse (beyond the platform default
#: ``~/.curie``).  The test conftest injects the pre-sandbox production
#: root here so custom-``CURIE_HOME`` deployments are covered too.
_STATE_DB_GUARD_EXTRA_DENY_ROOTS: Tuple[Path, ...] = ()


def _real_platform_state_root() -> Optional[Path]:
    """Resolve the REAL platform-default Curie root for the guard.

    Deliberately avoids ``Path.home()`` / ``curie_constants``: tests
    routinely monkeypatch ``Path.home`` to a tempdir, and ``curie_state``
    ...
    try:
        if sys.platform == "win32":
            ...
            root = (
                Path(base) / "curie"
                if base
                else Path(os.path.expanduser("~")) / "AppData" / "Local" / "curie"
            )
        else:
            root = Path(os.path.expanduser("~")) / ".curie"
        return root.resolve()
    except Exception:
        return None
...
#: redirects ``CURIE_HOME`` to the per-session tmp isolation root.  Its
...
#: is OURS: it declares "this process tree is running under Curie test
#: isolation", and it inherits into subprocess children by default — so a
#: child that received the patched ``CURIE_HOME`` also received the marker,
#: and a child that resolves a production DB while carrying it is, by
#: definition, an isolation escape (#82770).
_TEST_ISOLATION_MARKER_ENV = "CURIE_TEST_ISOLATION"
...
def _has_pytest_ancestor() -> bool:
    ...
    ``CURIE_HOME`` redirect: that child aims at the production DB *and*
...
def _in_test_context() -> bool:
    ...
    so a real ``curie`` invocation pays for at most one walk.
    """
...
def _is_production_state_db(resolved: Path, root: Path) -> bool:
    """True when *resolved* is a DB file of the real Curie home *root*.
    ...
    under ``~/.curie/curie-agent/...``) so hermetic tests using unusual
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    pytest and for hermetic (tmp ``CURIE_HOME``) paths.
    ...
    a rebuilt environment loses ``PYTEST_*`` and ``CURIE_HOME`` together,
    ...
    for root in _production_state_roots():
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Curie root {root}). "
                "Tests must run against a temporary CURIE_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "CURIE_HOME. If this test genuinely needs the live "
...
# curie_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _apply_wal_size_limit(conn: sqlite3.Connection) -> None:
    ...
    3.0 GB ``state.db``: ``curie sessions optimize`` (FTS merge + VACUUM)
    ...
    ``curie_cli/kanban_db.py`` already bounds its WAL growth with
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``curie_cli.kanban_db.connect`` so
...
def _wal_reset_repair_hint() -> str:
    ...
    ``curie update`` can actually do for this install (#75153).
    """
    try:
        ...
        if method in {"git", "unknown"}:
            return f"Curie-managed installs can repair the embedded runtime with `{cmd}`"
        if method == "docker":
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Curie"
    )
...
def _log_wal_reset_bug_once(
    ...
    *,
    ...
    # Check whether this is a Curie-managed install (uv-managed venv)
    ...
    logger.warning(
        ...
        "%s. See `curie doctor`. This warning fires once per "
...
def _log_journal_mode_upgrade_once(db_label: str, previous_mode: str) -> None:
    ...
    ``curie_cli/managed_uv._default_live_venv``, which treats a database
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see curie_cli/kanban_db.py) would
...
def apply_database_pragmas(
    ...
    * ``synchronous`` — durability level: ``OFF``/``NORMAL``/``FULL``/``EXTRA``
    ...
    """
    try:
        # Local import avoids a circular import with curie_cli.config.
        from curie_cli.config import cfg_get, load_config_readonly
...
def classify_persistence_error(exc_or_str) -> str:
    ...
    * ``"corrupt"`` — the database file itself is structurally damaged
      ...
      (``curie doctor`` / automatic schema surgery).
...
# threads inside ONE interpreter, yet a normal Curie host runs several
# independent processes against the same ``state.db``: the gateway service,
# the Desktop app's own ``curie serve`` backend, interactive CLI sessions,
...
# included because rollback-journal (DELETE) mode — Curie's fallback on
...
def _db_fingerprint(db_path: Path) -> "Optional[str]":
    ...
    RESERVED lock (see ``curie_cli.sqlite_safe_read`` rule 1). This function
    ...
    try:
        ...
        try:
            ...
            # Scaffold/embed installs ship curie_state without curie_cli. No
...
def _backup_db_file(db_path: Path) -> "Tuple[Optional[Path], Optional[str]]":
    ...
    connection's POSIX advisory locks (see ``curie_cli.sqlite_safe_read``).
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Curie home tree (``get_curie_home()``) — the safe repair scope:
      Curie owns those files, and the OS makes ``chmod`` fail on files the
    ...
    to check). Shared by :class:`SessionDB` and ``curie_cli.kanban_db``.
    """
    raw = str(db_path)
    ...
    try:
        home: Optional[Path] = Path(get_curie_home()).resolve()
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Curie needs read-write access to open the database. "
    ...
    try:
        ...
        for fts_table in ("messages_fts", "messages_fts_trigram", "messages_fts_cjk"):
            try:
                ...
                # (curie_state.py:645-723). The supported degraded-runtime
                # path (SessionDB._is_fts5_unavailable_error + the
                # regression suite in tests/test_curie_state.py:600-632)
        ...
        probe_session_id = f"_curie_fts_health_probe_{time.time_ns()}"
        try:
...
def _live_writer_holds_db(db_path: Path) -> bool:
    ...
    bug and on NFS/SMB, Curie deliberately runs ``state.db`` in
        ...
        if not holding_lock:
            ...
            # later BEGIN EXCLUSIVE acquisition.
                ...
                report["error"] = (
                    ...
                    "Stop the gateway (curie gateway stop) and retry."
                )
                logger.error("state.db repair skipped: %s", report["error"])
            else:
...
# (``~/.curie/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``curie sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("CURIE_FTS5_CJK_SO")
    if env:
        return Path(env).expanduser()
    return get_curie_home() / "lib" / "libfts5_cjk.so"


def _cjk_fts_config_enabled() -> bool:
    """config.yaml ``sessions.cjk_fts`` (default on), via its env bridge."""
    return os.getenv("CURIE_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
_STATE_DB_CORRUPT_MSG = (
    ...
    "Stop the gateway, then run `curie sessions recover --source <state.db> "
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages as JSON lines under CURIE_HOME/sessions.
...
# (see curie_cli/sqlite_safe_read.py for the module built around this rule).
...
#
# The registry itself lives in curie_state_registry.py — a bounded
# module owning acquisition, generation identity, refcounting,
# retirement, and teardown.  These re-exports keep the historical
# import path (``from curie_state import get_shared_session_db``)
# working for every call site and test that imports from here.
#
# Routing rules (see curie_state_registry for the full lifecycle):
...
from curie_state_registry import (  # noqa: F401  (re-export)
...
def _connect_tracked_db(path, tracking_path=None, **kwargs):
    ...
    (scaffold/embed installs that ship curie_state without curie_cli). A
    ...
    try:
        ...
        logger.debug(
            "curie_cli.sqlite_safe_read unavailable; opening %s untracked "
    ...
    # patch curie_state.sqlite3.connect keep control of connection creation;
...
def is_zeroed_state_db(
    ...
    Prefer ``curie_cli.backup.is_zeroed_sqlite_file`` when available; this
    ...
    if size < 0:
        return False
    from curie_cli.sqlite_safe_read import has_live_connection, read_header_bytes_preopen
    ...
    with quarantine_cross_process_lock(path) as acquired:
        if not acquired:
            logger.error(
                ...
                "state-snapshots via `curie snapshot list` / "
                "`curie snapshot restore <id>`.",
                path,
            )
            return None
        return _do_quarantine()
...
# ── Read-only health/stats probes (curie doctor, dashboards) ──────────
...
_CURIE_CMDLINE_MARKERS = ("curie_cli.main", "curie_cli/main", "curie serve",
                           "curie-agent", "curie gateway", "curie chat")


def _looks_like_curie(cmdline: str) -> bool:
    """Heuristic: does this cmdline look like a Curie process?
    ...
    We only flag processes that look like Curie, not every system daemon.
    """
    lower = cmdline.lower()
    return any(marker in lower for marker in _CURIE_CMDLINE_MARKERS)
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple curie processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Curie
    ...
    # `curie update` leaves mixed-version processes sharing the DB until
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            def _handle_quarantine_if_zeroed(already_locked: bool = False):
                if (
                    ...
                    msg = (
                        ...
                        f"Restore from {snaps} via `curie snapshot list` / "
                        f"`curie snapshot restore <id>` if available. "
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`curie db optimize`),
            ...
            # ``curie_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `curie update` to rebuild the venv with a "
                ...
                if cjk_present:
                    ...
                    if live:
                        ...
                        logger.warning(
                            ...
                            "run `curie sessions optimize-storage` on a host "
        ...
        while True:
            ...
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Curie process held the "
    ...
    def _db_file_was_replaced(self) -> bool:
        ...
        if recorded_app:
            ...
            # replace. A copied Curie DB that minted its own id is nonzero.
    ...
    def _halt_db_corrupt(self, exc: BaseException) -> None:
        ...
        logger.error(
            ...
            "gateway and run `curie sessions recover --source %s "
            "--inspect-only`.",
            self.db_path,
            exc,
            self.db_path,
            ...
            try:
                ...
                for pid_str in os.listdir("/proc"):
                    ...
                    try:
                        ...
                        # so check whether this is a Curie process —
                        # only flag uninspectable holders that look like
                        # another Curie instance, not every system daemon.
                        cmdline = _read_proc_cmdline(pid)
                        if cmdline is not None and _looks_like_curie(cmdline):
    ...
    def _reap_inactive_orphan_desktop_holders(
        ...
        if not sys.platform.startswith("linux") or psutil is None:
            return []
        try:
            from curie_cli.dashboard_procs import _is_ephemeral_port_zero_backend
        except Exception:
            return []
    ...
    def close(self):
        ...
        if getattr(self, "_shared_registry_owned", False):
            from curie_state_registry import release
            ...
            if self._conn:
                if self._db_corrupt:
                    ...
                    logger.warning(
                        ...
                        "then run `curie sessions recover --source %s "
                        "--inspect-only`.",
                        self.db_path,
                        self._db_corrupt_reason,
                        self.db_path,
    ...
    #
    # `optimize_fts_storage()` (the `curie sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`curie sessions optimize-storage`) ──
    #
    ...
    def _own_profile_name(self) -> Optional[str]:
        ...
        try:
            ...
            root = get_default_curie_root().resolve()
    ...
    # offline repair path behind ``curie sessions repair-routing``.
    ...
    def _merge_model_config_json(
        ...
        *,
        ...
        ``set_session_unlock``, ``archive_and_compact``,
    ...
    def set_session_unlock(self, session_id: str, enabled: bool) -> None:
        """Persist the per-session UNLOCK bypass flag into ``model_config``.

        Merges ``unlock_mode`` into the existing ``model_config`` JSON (same
        merge discipline as ``update_session_runtime_lock`` so lineage
        markers like ``_branched_from`` / ``_delegate_from`` survive). The
        CLI resume paths read this flag back so a ``/unlock ON`` toggle — or a
        ``--unlock`` launch — survives ``curie --resume`` into a fresh
        process. No-op when the session row doesn't exist yet; the
        creation-time ``model_config`` carries the flag for ``--unlock``
        launches.
        """
        if not session_id:
            return
        ...
        def _do(conn):
            merged = self._merge_model_config_json(
                conn, session_id, {"unlock_mode": bool(enabled)}
            )
            if merged is _MODEL_CONFIG_ROW_MISSING:
                return
            conn.execute(
    ...
    @staticmethod
    def session_unlock_enabled(session_meta: Optional[Dict[str, Any]]) -> bool:
        """Read the persisted UNLOCK flag off a session row dict.
        ...
        if not isinstance(raw, dict):
            return False
        return bool(raw.get("unlock_mode"))
    ...
    # of truth: curie_state_common._RECOVERABLE_END_REASONS, interpolated
    ...
    def _rows_to_conversation(
        ...
        *,
        ...
        if repair_alternation and messages:
            # Lazy import: curie_state already depends on agent.* (see
    ...
    def search_sessions(
        ...
        ``curie -c``/``--resume`` so the "last" session is the last one in
    ...
    def latest_conversation_boundary(
        ...
        uses — never the key alone.  ``X-Curie-Session-Key`` accepts any
    ...
    def delete_session_if_empty(
        ...
        and ``curie sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``CURIE_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Curie, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Curie session.

        A Curie session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Curie session is already bound to any Telegram DM topic.
    ...
    def vacuum(self) -> int:
        ...
        with self._lock:
            ...
            # TRUNCATE: a manual `curie sessions vacuum` runs in a transient
