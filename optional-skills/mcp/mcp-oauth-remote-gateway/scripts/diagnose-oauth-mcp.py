...
Usage:
  ...
  <server_name> matches the files in $CURIE_HOME/mcp-tokens/<server>.json etc.
...
def _curie_home():
    # Prefer Curie' own resolver (profile-safe); fall back to env then ~/.curie.
    try:
        from curie_constants import get_curie_home
        return str(get_curie_home())
    except Exception:
        return os.environ.get("CURIE_HOME") or os.path.expanduser("~/.curie")


def _tokens_dir():
    return os.path.join(_curie_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "curie-diag", "version": "1.0"}},
        },
        headers={
