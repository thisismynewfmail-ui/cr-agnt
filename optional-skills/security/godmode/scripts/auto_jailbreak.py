...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("CURIE_HOME", os.path.expanduser("~/.curie")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CURIE_HOME", Path.home() / ".curie")) / "skills" / "red-teaming" / "godmode"
...
CURIE_HOME = Path(os.getenv("CURIE_HOME", Path.home() / ".curie"))
CONFIG_PATH = CURIE_HOME / "config.yaml"
PREFILL_PATH = CURIE_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Curie models — already uncensored, just needs clean prompt
    "curie": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "curie" in model_lower or "nous" in model_lower:
        return "curie"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Curie config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.curie/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Curie for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Curie for changes to take effect.")
