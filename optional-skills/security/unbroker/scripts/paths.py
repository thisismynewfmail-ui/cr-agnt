...
All per-subject data lives under PDD_DATA_DIR (default: $CURIE_HOME/unbroker),
which is the same trust boundary Curie uses for .env and OAuth tokens.
"""
...
def curie_home() -> Path:
    return Path(os.environ.get("CURIE_HOME") or (Path.home() / ".curie"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else curie_home() / "unbroker"
