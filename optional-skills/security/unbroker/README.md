...
The design is **Curie-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
curie skills install official/security/unbroker
```

Then start a new Curie session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Curie home directory (`0600`, optionally
