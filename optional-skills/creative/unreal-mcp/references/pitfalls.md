...
### 1. Start order: editor first, Curie second

Curie probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Curie session.
...
restart the editor, restart the Curie session.
...
server fails to bind (Output Log) or Curie' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.curie/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Curie

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Curie' connection
lives in `~/.curie/config.yaml` via `curie mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Curie — just don't
mistake it for the Curie setup step.
...
explicitly warns against overlapping calls. Curie executes same-turn tool
...
### 9. Timeouts: Curie gives up before Unreal does

Curie' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Curie runs elsewhere (SSH backend,
