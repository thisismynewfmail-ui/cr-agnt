...
the actual `setup.sh` that creates the project workspace, configures Curie
...
2. **Create profiles** — `curie profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.curie/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `curie kanban create` assigned to the director
...
```bash
curie profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.curie/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.curie/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Curie
...
Each profile gets a `SOUL.md` at `~/.curie/profiles/<name>/SOUL.md` that
...
```bash
curie kanban create "Direct production of <video title>" \
    --assignee director \
...
the Curie `.env` (`${CURIE_HOME:-$HOME/.curie}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _curie_env="${CURIE_HOME:-$HOME/.curie}/.env"
    if grep -q "^${var}=" "$_curie_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_curie_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_curie_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY curie ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY curie OPENROUTER_API_KEY || exit 1
# ...
```
