...
curie kanban watch --tenant <project-slug>

# Snapshot of the board
curie kanban list --tenant <project-slug>
curie kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
curie kanban stats --tenant <project-slug>

# Visual dashboard (browser)
curie dashboard

# Inspect a specific task (includes comments + events)
curie kanban show <task-id>

# Follow a single task's event stream
curie kanban tail <task-id>
```

Verify available subcommands with `curie kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `curie kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `curie kanban block <id>` then `curie kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `curie kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `curie kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `curie profile list` (profile exists?), `curie status` (gateway/dispatcher up?), and verify tenant. |
...
curie kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
curie kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(curie kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
curie kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
curie kanban block <task-id>
curie kanban archive <task-id>

# Diagnose what happened
curie kanban show <task-id>      # task body, comments, recent events
curie kanban tail <task-id>      # follow the live event stream
curie kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `curie kanban create` for the director
...
while true; do
    clear
    curie kanban list --tenant <slug>
    echo "---"
    curie kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `curie kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `curie profile list` and start
