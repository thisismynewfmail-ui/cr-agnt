...
Maps each role archetype to the Curie skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public curie-agent
...
## Curie skills relevant to video production

### Visual / rendering skills (`curie-agent/skills/creative/`)
...
### Design / pre-production skills (`curie-agent/skills/creative/`)
...
### Audio / media skills (`curie-agent/skills/creative/` + `skills/media/`)
...
These are **not** Curie skills but external CLIs / APIs that profiles invoke.
...
## Built-in Curie tools for media review

These are native Curie tools — not invoked via terminal but through their own
...
key is present in `${CURIE_HOME:-~/.curie}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Curie profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${CURIE_HOME:-~/.curie}/.env` → environment variable.
...
When a new Curie-public video skill ships:
