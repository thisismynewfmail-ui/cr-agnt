...
## 5. Curie Configuration

### Config location

`$CURIE_HOME/config.yaml` (defaults to `~/.curie/config.yaml` when `CURIE_HOME` is unset)
...
Restart the Curie session for changes to take effect. The MCP connection is
