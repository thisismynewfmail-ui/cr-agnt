...
CURIE_HOME_DIR="${CURIE_HOME:-$HOME/.curie}"
CURIE_CFG="${CURIE_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Curie config has twozero_td MCP entry ──
if [[ ! -f "$CURIE_CFG" ]]; then
    echo -e " ${FAIL} Curie config not found at ${CURIE_CFG}"
    manual_steps+=("Create ${CURIE_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$CURIE_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Curie config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Curie config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${CURIE_CFG} manually"); }
    manual_steps+=("Restart Curie session to pick up config change")
fi
