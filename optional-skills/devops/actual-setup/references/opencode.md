...
is typically only in `~/.curie/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Curie,
...
`curie-custom-providers` skill, pitfall 2). Curie hits this because it forwards
