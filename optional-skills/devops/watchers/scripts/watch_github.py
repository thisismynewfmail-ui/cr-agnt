...
Usage (via cron with --no-agent):
    ...
    curie cron create curie-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$CURIE_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name curie-issues --repo thisismynewfmail-ui/Cur-Agnt --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Curie .env file
(``${CURIE_HOME:-~/.curie}/.env``) to avoid the 60 req/hr
