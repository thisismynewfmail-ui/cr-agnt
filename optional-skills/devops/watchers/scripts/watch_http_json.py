...
Usage (via cron with --no-agent):
    ...
    curie cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$CURIE_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Curie-Watcher/1.0"})
