#!/usr/bin/env python3
"""Canvas LMS API CLI for Curie Agent.
...
def _check_config():
    ...
    if missing:
        curie_env = os.path.join(
            os.environ.get("CURIE_HOME", os.path.expanduser("~/.curie")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {curie_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Curie Agent"
    )
