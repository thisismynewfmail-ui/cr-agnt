#!/bin/sh
# shellcheck shell=sh
# /opt/curie/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching CURIE_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `curie` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `curie <args>` (subcommand passthrough)
#
# Drop to curie via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${CURIE_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${CURIE_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export CURIE_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset CURIE_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid curie "$@"; exec "$@"; }
...
# an arbitrary non-root, non-curie UID, the bootstrap was skipped and the
# baked image dirs (owned by the curie build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u curie)" ]; then
    cat >&2 <<EOF
[curie] ERROR: container started with --user $cur_uid (an arbitrary, non-curie UID) — not supported.
...
# to the curie user's home before dropping privileges so libraries that
...
_curie_orig_cwd="${CURIE_ORIG_CWD:-$PWD}"
...
# the user's command so `curie chat` starts in the Docker -w
