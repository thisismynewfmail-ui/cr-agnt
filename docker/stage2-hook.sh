...
#
# Wired into the image as /etc/cont-init.d/01-curie-setup by the
...
CURIE_HOME="${CURIE_HOME:-/opt/data}"
INSTALL_DIR="/opt/curie"

# Drop to curie via s6-setuidgid, but skip it when already non-root.
as_curie() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid curie "$@"; }
...
# starts non-root. The baked install tree under /opt/curie is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $CURIE_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass CURIE_UID/CURIE_GID — or the PUID/PGID aliases — which the
...
# `id -u curie` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# curie UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u curie)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-curie UID).
...
and the baked /opt/curie install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e CURIE_UID=\$(id -u) -e CURIE_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the curie user to that UID/GID at boot and chowns the data
...
# --- Bootstrap CURIE_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid curie mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom CURIE_HOME paths whose parents only
# root can create (e.g. `CURIE_HOME=/home/curie/.curie` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for CURIE_UID/CURIE_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  CURIE_UID/
# CURIE_GID still win when both are set.  See #15290, salvages #25872.
CURIE_UID="${CURIE_UID:-${PUID:-}}"
CURIE_GID="${CURIE_GID:-${PGID:-}}"

if [ -n "${CURIE_UID:-}" ] && validate_uid_gid "$CURIE_UID" && [ "$CURIE_UID" != "$(id -u curie)" ]; then
    echo "[stage2] Changing curie UID to $CURIE_UID"
    usermod -u "$CURIE_UID" curie
fi
if [ -n "${CURIE_GID:-}" ] && validate_uid_gid "$CURIE_GID" && [ "$CURIE_GID" != "$(id -g curie)" ]; then
    echo "[stage2] Changing curie GID to $CURIE_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$CURIE_GID" curie 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised curie user (UID 10000)
...
# curie process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes curie. Idempotent across container
...
#   - curie is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G curie 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] curie already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" curie 2>/dev/null; then
        echo "[stage2] Added curie to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group curie failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When CURIE_UID is remapped or the top-level $CURIE_HOME isn't owned by
# the runtime curie UID, restore ownership to curie — but ONLY for the
# directories curie actually writes to. The full $CURIE_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of curie-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_curie_uid=$(id -u curie)
...
chown_curie_tree() {
    ...
    chown -R curie:curie "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_curie_owner() {
    target="$1"
    find "$target" \( ! -user curie -o ! -group curie \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$CURIE_HOME" 2>/dev/null)" != "$actual_curie_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $CURIE_HOME (targeted) to curie ($actual_curie_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $CURIE_HOME: chown the directory itself (not its contents)
    # so curie can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$CURIE_HOME"; then
        :
    else
        chown curie:curie "$CURIE_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $CURIE_HOME failed (rootless container?) — continuing"
    fi
    # Curie-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by curie (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$CURIE_HOME/$sub" ] && tree_has_non_curie_owner "$CURIE_HOME/$sub"; then
            chown_curie_tree "$CURIE_HOME/$sub"
        fi
    done
fi
...
# curie user. Hosted/container instances keep mutable state under
# $CURIE_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# CURIE_DISABLE_LAZY_INSTALLS=1. Keeping /opt/curie root-owned and
...
# $CURIE_HOME/lazy-packages dir on the data volume (Dockerfile sets
# CURIE_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to curie
...
# Always reset ownership of $CURIE_HOME/profiles to curie on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> curie …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as curie and walks
...
if [ -d "$CURIE_HOME/profiles" ] && tree_has_non_curie_owner "$CURIE_HOME/profiles"; then
    chown_curie_tree "$CURIE_HOME/profiles"
fi

# Always reset ownership of $CURIE_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged curie runtime even
...
# gate as profiles/).
if [ -d "$CURIE_HOME/cron" ] && tree_has_non_curie_owner "$CURIE_HOME/cron"; then
    chown_curie_tree "$CURIE_HOME/cron"
fi

# Always ensure logs/gateways is curie-owned (#45258). Formerly healed by
...
# top-level $CURIE_HOME is mis-owned, so a warm volume with curie-owned
# CURIE_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid curie mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as curie.
if [ -d "$CURIE_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$CURIE_HOME/logs/gateways"; then
        :
    else
        chown curie:curie "$CURIE_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# curie pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged curie gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $CURIE_HOME is
...
# and cron/.
if [ -d "$CURIE_HOME/platforms/pairing" ] && tree_has_non_curie_owner "$CURIE_HOME/platforms/pairing"; then
    chown_curie_tree "$CURIE_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$CURIE_HOME/pairing" ] && tree_has_non_curie_owner "$CURIE_HOME/pairing"; then
    chown_curie_tree "$CURIE_HOME/pairing"
fi

# Reset ownership of curie-owned top-level state files on every boot.
# The targeted data-volume chown above only covers curie-owned
# *subdirectories*; loose state files living directly under $CURIE_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> curie …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged curie runtime then hits
...
# sweep so host-owned files in a bind-mounted $CURIE_HOME are never
...
# entries of curie_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$CURIE_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$CURIE_HOME/$f"; then
            :
        else
            chown curie:curie "$CURIE_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the curie runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$CURIE_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$CURIE_HOME/config.yaml"; then
        :
    else
        chown curie:curie "$CURIE_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$CURIE_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as curie user ---
# Run as curie via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $CURIE_HOME values
...
as_curie mkdir -p \
    ...
    "$CURIE_HOME/sessions" \
    "$CURIE_HOME/logs" \
    "$CURIE_HOME/logs/gateways" \
    "$CURIE_HOME/hooks" \
    "$CURIE_HOME/memories" \
    "$CURIE_HOME/skills" \
    "$CURIE_HOME/skins" \
    "$CURIE_HOME/plans" \
    "$CURIE_HOME/workspace" \
    "$CURIE_HOME/home" \
    "$CURIE_HOME/pairing" \
    "$CURIE_HOME/platforms/pairing" \
    "$CURIE_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/curie/.install_method (see Dockerfile), NOT written here into
# $CURIE_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $CURIE_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.curie:/opt/data) and sometimes shared with a
...
# 'curie update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $CURIE_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/curie stamp, so deleting the data-dir copy is safe).
if [ -f "$CURIE_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$CURIE_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$CURIE_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$CURIE_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$CURIE_HOME/$dest"; then
            :
        else
            as_curie cp "$INSTALL_DIR/$src" "$CURIE_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/curie/.env.example being present
...
# not generate one. Curie loads $CURIE_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$CURIE_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$CURIE_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $CURIE_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$CURIE_HOME/.env" ] && ! refuse_symlinked_path "clean" "$CURIE_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$CURIE_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$CURIE_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$CURIE_HOME/.env"; then
        :
    else
        if [ ! -f "$CURIE_HOME/.env" ]; then
            ...
            (umask 077 && as_curie touch "$CURIE_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$CURIE_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$CURIE_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$CURIE_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $CURIE_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $CURIE_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi
...
if [ -f "$CURIE_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$CURIE_HOME/.env"; then
        :
    else
        chown curie:curie "$CURIE_HOME/.env" 2>/dev/null || true
        chmod 600 "$CURIE_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `curie` user — without this it gets EACCES on every
...
# curie group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp curie /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted curie group access to the Fly Machines API socket"
    else
...
# $CURIE_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `curie update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set CURIE_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$CURIE_HOME/config.yaml" ]; then
    s6-setuidgid curie "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$CURIE_HOME/auth.json" ] && [ -n "${CURIE_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$CURIE_HOME/auth.json"; then
        :
    else
        printf '%s' "$CURIE_AUTH_JSON_BOOTSTRAP" > "$CURIE_HOME/auth.json"
        chown curie:curie "$CURIE_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$CURIE_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via CURIE_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$CURIE_HOME/auth.json" ] && [ -n "${CURIE_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$CURIE_HOME/auth.json"; then
        :
    else
        s6-setuidgid curie "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$CURIE_HOME/auth.json" \
...
# can set CURIE_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# CURIE_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$CURIE_HOME/gateway_state.json" ] && \
        [ "${CURIE_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$CURIE_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$CURIE_HOME/gateway_state.json"
        chown curie:curie "$CURIE_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$CURIE_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_curie "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/curie/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Curie spawns for the
...
# main-wrapper.sh propagates it into the supervised ``curie`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-curie, dashboard, etc.).
