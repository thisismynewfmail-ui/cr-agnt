...
# Curie WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: CURIE_TINI_SHIM_TARGET overrides the `/init` path (and
# CURIE_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${CURIE_TINI_SHIM_TARGET:-/init}"
WRAPPER="${CURIE_TINI_SHIM_WRAPPER:-/opt/curie/docker/main-wrapper.sh}"
