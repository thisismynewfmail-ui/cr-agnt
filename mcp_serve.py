"""
Curie MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Curie-specific extra)

Usage:
    curie mcp serve
    curie mcp serve --verbose
...
logger = logging.getLogger("curie.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using CURIE_HOME."""
    try:
        from curie_constants import get_curie_home
        return get_curie_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("CURIE_HOME", Path.home() / ".curie")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from curie_constants import get_curie_home
        directory_file = get_curie_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("CURIE_HOME", Path.home() / ".curie")
...
class EventBridge:
    ...
    This is the Curie equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline_with_db(self, db) -> None:
        ...
        try:
            from curie_constants import get_curie_home
            db_file = get_curie_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("CURIE_HOME", Path.home() / ".curie")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Curie MCP server with all tools registered."""
    ...
    mcp = MCPServer(
        "curie",
        instructions=(
            "Curie Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Curie MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
