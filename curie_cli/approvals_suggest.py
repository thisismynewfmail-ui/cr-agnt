"""``curie approvals suggest`` — mine approval history into allowlist proposals.

Curie has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.curie/state.db``): every assistant ``terminal`` tool
...
always, smart-approve, or unlock) before it ran.  Frequently re-approved
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, curie self-management, …) are eligible.
...
def normalize_command(command: str) -> str:
    """Fold user/curie home prefixes and collapse whitespace.

    Reuses tools.approval's home-folding machinery so proposals are portable
    across machines/users (``/home/alice/x`` -> ``~/x``).
    """
    from tools.approval import (
        _rewrite_resolved_curie_home,
        _rewrite_resolved_user_home,
    )

    folded = _rewrite_resolved_user_home(_rewrite_resolved_curie_home(command))
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  curie approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.curie/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``curie approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``curie approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from curie_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: curie approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `curie approvals <subcommand> -h` for details."
    )
    return 1
