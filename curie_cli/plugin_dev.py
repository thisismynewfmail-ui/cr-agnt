"""Runtime-backed validation behind ``curie plugins doctor``.
...
def _doctor_runtime(plugin_path: Path):
    ...
    test framework. Registration code executes under a temporary CURIE_HOME
    ...
    try:
        ...
        # removes it instead of stranding a curie-plugin-doctor-* directory
        # until interpreter GC (or never, on a hard exit).
        temporary_home = stack.enter_context(
            tempfile.TemporaryDirectory(prefix="curie-plugin-doctor-")
        )
        ...
        stack.enter_context(
            patch.dict(
                ...
                {
                    "CURIE_HOME": str(home),
                    "CURIE_BUNDLED_PLUGINS": str(bundled),
                    "CURIE_ENABLE_PROJECT_PLUGINS": "0",
                },
                clear=False,
            )
        )
    ...
    modules_before = {
        ...
        if name == "curie_plugins" or name.startswith("curie_plugins.")
    }
    ...
    try:
        ...
        if not manifests:
            raise _DoctorLoadError(
                f"Curie discovery found no valid plugin manifest under {copied}"
            )
        if len(manifests) != 1:
            raise _DoctorLoadError(
        ...
        for name in list(sys.modules):
            if (
                ...
                and (name == "curie_plugins" or name.startswith("curie_plugins."))
            ):
...
def _holds_plugin(path: Path) -> bool:
    ...
    confusing error: ``curie plugins doctor`` with no argument defaults to
...
def resolve_plugin_path(target: str | os.PathLike[str] | None = None) -> Path:
    ...
    if _is_plugin_id(raw):
        user_root = get_curie_home() / "plugins"
        ...
        try:
            ...
            pass
        candidates.append(Path.cwd() / ".curie" / "plugins" / raw)
...
def _check_manifest_v2(report: "DoctorReport", manifest: Any) -> None:
    ...
    from curie_cli.plugins import SUPPORTED_MANIFEST_VERSION

    mv = getattr(manifest, "manifest_version", 1)
    if mv > SUPPORTED_MANIFEST_VERSION:
        report.warning(
            f"manifest_version {mv} is newer than this Curie supports "
    ...
    if missing:
        report.warning(
            ...
            + ", ".join(missing)
            + " — Curie never auto-installs plugin dependencies; "
    ...
    if schema:
        from curie_cli.plugins import _CONFIG_SCHEMA_TYPES
...
def doctor_plugin(target: str | os.PathLike[str] | None = None) -> DoctorReport:
    """Validate one plugin through Curie' real scanner and registration path."""
    try:
