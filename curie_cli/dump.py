"""
Dump command for curie CLI.

Outputs a compact, plain-text summary of the user's Curie setup
...
from curie_cli.config import get_curie_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.curie/.env.
    ...
    exports. ``curie debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.curie_build_sha`` by the Dockerfile's
    ``CURIE_GIT_SHA`` build-arg (see ``curie_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``curie dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from curie_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(curie_home: Path) -> int:
...
def _cron_summary(curie_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = curie_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"curie_home:      {display_curie_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.curie/.env: a managed
        ...
        # A credential added via `curie auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `curie auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["curie-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(curie_home)}")
    lines.append(f"  skills:             {_count_skills(curie_home)}")
