...
Pure logic: walks a Curie config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `curie doctor` and a future `curie migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Curie config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Curie config and return retirement issues.
...
def apply_migration(
    ...
    # None``, so the next `curie migrate xai` reports nothing to migrate
