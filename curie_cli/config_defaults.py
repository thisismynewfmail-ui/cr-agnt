"""Default configuration data for Curie Agent.

Pure-data leaf module: DEFAULT_CONFIG and OPTIONAL_ENV_VARS, extracted
verbatim from curie_cli/config.py. Must not import from curie_cli.config.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["curie-cli"],
    # SQLite journal mode used by every Curie database opener. WAL is the
    ...
    # Soft file-descriptor limit for long-running Curie server processes.
    ...
    "session": {
        # Per-terminal `curie -c`: each CLI session drops a breadcrumb file
        # under $CURIE_HOME/terminal-sessions/<terminal-id>, and a bare
    ...
    "agent": {
        ...
        # all normalized by curie_cli.config.resolve_turn_limit.
        "max_turns": None,
        ...
        # budget. CLI one-shot equivalent: `curie chat --run-budget N`.
        ...
        # an interactive `curie gateway restart` must never block for hours
        ...
        # the Curie-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Curie
        ...
        # identity slot (SOUL.md). Empty by default. The CURIE_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Curie adds a coding operating brief
        ...
        # `curie doctor`. Delivered via the status channel only —
        ...
        # `curie status` / fleet monitoring). Retries never stop — this is a
        ...
        # ``CURIE_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        # Root directory for Curie' terminal session temp files (background
        # logs/pid/exit files, code-execution sandboxes, etc.). When empty,
        # Curie uses TMPDIR/TMP/TEMP if set, otherwise a managed dir on real
        # storage at CURIE_HOME/cache/terminal (auto-pruned after 72h) — NOT
        ...
        # bot_relay from a short-lived `curie -p <bot> chat -Q` recipient
        ...
        #             CURIE_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force CURIE_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Curie auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Curie sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Curie
        ...
        # bundled Curie image, which drops to the `curie` user via
    ...
    "web": {
        ...
        # `curie tools`, not a ring member). Set by the `curie tools`
    ...
    "browser": {
        ...
        #   Browser Use mode (default) — "lightpanda" makes Curie spawn
        ...
        # `curie doctor` say so. Also settable via AGENT_BROWSER_ENGINE.
        "engine": "auto",
        ...
        # browser tools) runs on a Curie-managed SNAPSHOT of the user's
        ...
        # fresh session launches — driven by Curie' packaged Chromium. Only
        ...
        # deletes the snapshot store (~/.curie/browser-profile/) so copied
        ...
        # approval does it run `curie browser close-profile` (terminates the
        ...
        "camofox": {
            # When true, Curie sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Curie should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``curie chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.curie/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: curie sweeps the checkpoint base at startup
        ...
        # ``curie checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .curie.md, or .cursorrules before Curie applies
    ...
    # Single-query (``curie -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Curie truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Curie' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); curie =
                                      # Curie' compression threshold triggers
        ...
        "codex_responses_native": False,  # Opt in to OpenAI's server-side compaction
                                      ...
                                      # route/model is unaffected. Curie' local
    ...
    "auxiliary": {
        ...
        "vision": {
            ...
            "timeout": 7200,       # seconds — two hours, and it is a budget for
                                   # the WHOLE tool call rather than for each
                                   # attempt inside it (the size-retry and the
                                   # empty-response retry share this number,
                                   # so the worst case is what it says).
                                   # A vision payload is a whole image; a
                                   # reasoning model reading a dense scan or a
                                   # page of small print can legitimately think
                                   # for a long time, and the old two-minute
                                   # cap cancelled it mid-answer and reported a
                                   # timeout instead. A dead stream is still
                                   # caught in ~60s by the no-progress
                                   # watchdog, so this only bounds a call that
                                   # is genuinely still working.
                                   # The tool-execution deadline follows this
                                   # key (agent/tool_executor.py
                                   # _TOOL_OWNED_BUDGETS), so raising it here
                                   # is enough — the two cannot drift.
        ...
        "title_generation": {
            ...
            "base_url": "",
            "api_key": "",
            # Seconds — one hour. Naming a conversation is a small job, but
            # it runs on whatever auxiliary model is configured, and a
            # reasoning model spends its thinking budget before emitting the
            # first token. At thirty seconds that produced "⚠ Auxiliary title
            # generation failed: Request timed out." on a call that was still
            # working, and the session was left untitled — which is also why
            # it never appeared in a title-filtered session list.
            "timeout": 3600,
            "extra_body": {},
            "reasoning_effort": "",  # per-task thinking level: none|minimal|low|medium|high|xhigh|max|ultra (empty = provider default)
            "language": "",
        ...
        # Invoked by ``curie kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``curie kanban decompose`` and the kanban
        ...
        # ``curie profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `curie model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `curie` (and `curie chat`) launches by default:
        ...
        # REPL and `--tui` (or CURIE_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `curie --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `curie -c` muscle memory.  Default off so existing
        # users aren't surprised.  CURIE_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `curie --tui` drops a one-time hint
        ...
        # model — see curie_cli/focus_view.py.
        "focus_view": False,
        ...
        # can set `path`/`paths` to CURIE_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/curie`. Per-platform overrides go under
        ...
        # unlock, stash, battery, title, total_tokens.
        ...
        # CLI, TUI, and desktop app. Manage with `curie pets`. Disabled until
        ...
        "pet": {
            ...
            # get_curie_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # protocol ping (see curie_cli/web_server.py — an event-loop stall
        ...
        # CURIE_TUI_WS_ORPHAN_REAP_GRACE_S env var remains an internal
        ...
        # (CURIE_TUI_SESSION_TTL_S, default 6h) are closed with
        ...
        # ``CURIE_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``CURIE_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``CURIE_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``CURIE_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``CURIE_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/curie``) —
    ...
    # `curie ui` — the bench console (Textual, mouse-driven).
    # Colour comes from the active skin (`curie skin`), not from here; these
    # are the console's own preferences, remembered between runs.
    "ui": {
        # Which of the console's two skins it opens in.
        #   "bench" — the instrument panel: enamel ground, stencilled labels,
        #             signal-coloured lamps, in whatever `display.skin` says.
        #   "dos"   — a phosphor terminal: one hue on dark glass, CP437
        #             frames, inverse-video bands, Norton's key bar. Its own
        #             settings are the `dos` block below.
        # Chosen on the console's PANEL pane, under DISPLAY. This is not
        # `display.skin`: that names the colour scheme the CLI and the TUI
        # share, and this names which interface the console draws.
        "skin_mode": "bench",
        # The DOS mode's own settings. Ignored while `skin_mode` is "bench",
        # but still applied to the preview on the PANEL pane, so a tube can be
        # chosen before the mode is switched on.
        "dos": {
            # Which monitor. One of: amber (P3, the 1980s office), green (P1,
            # the older tube), white (P4, paper-white).
            "phosphor": "amber",
            # The brightness control, 0-4 (OFF, LOW, NORMAL, HIGH, BURN). It
            # does two things at once, as a real one does: mixes the phosphor
            # into the glass (the haze) and drives the strokes toward the
            # tube's peak (the bloom). Higher is brighter and softer.
            "glow": 2,
            # Draw every other raster line dark, in the chrome and in the
            # animated state figures.
            "scanlines": True,
            # Blink the composer's cursor, as a DOS prompt's did. Its own
            # switch rather than part of the mode, because a blinking cursor
            # is exactly the thing some readers need to be able to turn off.
            "block_cursor": True,
        },
        # Which set of animated state indicators the panel draws with.
        # One of: bench (interference), scope (Lissajous), cells (cellular
        # automata), orbit (phyllotaxis), sweep (radar), bars (spectrum),
        # raster (a CRT scanning spot — the DOS mode's own figure).
        # Chosen on the console's PANEL pane; every set draws every state,
        # and every set picks up the DOS mode's glow and scanlines.
        "indicators": "bench",
        # Start with the microphone open, dictating into the composer.
        # Needs the audio extra (`curie update --ensure voice`).
        "voice_input": False,
        # Read finished replies aloud through the configured TTS provider.
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Curie appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.curie/cache/piper-voices/
    ...
    # "Hey Curie" hands-free wake word. Always-on, on-device hotword
    ...
    "wake_word": {
        ...
        "phrase": "hey curie",       # for "sherpa" this IS the detected phrase (any text works); for other engines it's a cosmetic label — detection is keyed by the model/keyword below
        ...
        "openwakeword": {
            # "hey_curie" (the bundled, works-out-of-the-box default) OR a
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.curie/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.curie/cache/delegation/
    ...
    # response. If not, Curie feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Curie auto-pauses the goal and
    ...
    "moa": {
        ...
        # <curie_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # goes to ~/.curie/skills/ unless create_dir (below) redirects it.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Where agent-created skills (skill_manage action=create) are written.
        # Empty = the profile-local skills dir (~/.curie/skills/). When set,
        ...
        # against CURIE_HOME. The directory is scanned for skills alongside
        # the local dir. e.g. "/opt/brain/skills"
        "create_dir": "",
        # Project-local skill discovery: when a session starts inside a git
        # checkout, ``<root>/.curie/skills/`` and ``<root>/.agents/skills/``
        ...
        # with ``curie skills trust`` (run from inside it). Set to false to
        ...
        # Managed by ``curie skills trust`` / ``curie skills untrust``.
        "trusted_project_dirs": [],
        # Substitute ${CURIE_SKILL_DIR} and ${CURIE_SESSION_ID} in SKILL.md
        ...
        # (`curie skills install`). Runs ALONGSIDE the built-in skills
        ...
        # ~/.curie/skills/.curator_ledger.jsonl with before/after file
        # hashes; file contents are stored content-addressed (deduped) under
        # ~/.curie/.curator_backups/blobs/. Enables `curie curator ledger`
        # and single-mutation `curie curator rollback <entry-id>`.
    ...
    # cron daemon.
    #
    # See `curie curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `curie curator run --consolidate` overrides
        ...
        # normally restored on every `curie update`, so pruning them only
        ...
        # skills are kept forever. When > 0, `curie curator purge` deletes
        ...
        # skipped), snapshot ~/.curie/skills/ into
        # ~/.curie/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `curie curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for curie-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "▮ *Curie Agent*" header.
    ...
    #   off    — skip all approval prompts (equivalent to --unlock)
    #
    ...
    # dangerous command. -q runs export CURIE_INTERACTIVE=1 (for interactive
    ...
    "approvals": {
        ...
        # --unlock / /unlock / mode=off bypass — making this the user-editable
        ...
        # modal; CURIE_TUI_NO_CONFIRM=1 force-skips that modal regardless of
    ...
    # replace Curie' built-in platform hint for a single messaging platform
    ...
    # ``curie plugins enable|disable`` and intentionally omitted here so an
    ...
    # stored approval from ~/.curie/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or CURIE_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .curie config) always require human
        # approval — even under auto-approve/unlock. Extra patterns are
        ...
        # longer trigger the startup banner. Add via `curie doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``curie_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Curie to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $CURIE_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via CURIE_CRON_MAX_PARALLEL env var.
        ...
        # CURIE_CRON_SCRIPT_TIMEOUT. Keep this in sync with
        ...
        # CURIE_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
        ...
        # overridable via CURIE_CRON_MEDIA_SEND_TIMEOUT env var. Keep in
    ...
    # todos to ready, and fires `curie -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `curie` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `curie kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     curie-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Curie tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.curie/logs/.
    ...
    # update model picker lists without shipping a curie-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `curie model` invocation; network failures
    ...
    # #8731).
    #
    # Provider keys accept the Curie provider id (as used elsewhere in
    ...
    "gateway": {
        ...
        # internal CURIE_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # bridges these keys to the internal CURIE_STARTUP_WATCHDOG /
        # CURIE_STARTUP_WATCHDOG_TIMEOUT_S env vars AND applies them to the
        ...
        # producing ~/.curie/sessions/sessions.json entirely.
        ...
        # the CURIE_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.curie.gateway` that defenses 1-2 don't
        ...
        # `curie update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``CURIE_GATEWAY_MAX_STARTS`` / ``CURIE_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.curie/.env,
        ...
        # behavior: files must live under the Curie cache, under
        ...
        # user. Bridged to CURIE_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Curie
        # cache (~/.curie/cache/{documents,images,audio,video,screenshots})
        ...
        # to CURIE_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # CURIE_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to CURIE_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.curie/state.db.
    ...
    "sessions": {
        ...
        # the default of ``curie sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.curie/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `curie sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `curie update` prints a one-line notice with
        ...
        # ~/.curie/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to CURIE_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # CURIE_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `curie sessions export`. Checked
    ...
    # ``curie doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `curie doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``curie update`` behaviour.
    "updates": {
        ...
        #     DBs) into <CURIE_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``curie backup``-style zip
        #     of CURIE_HOME into <CURIE_HOME>/backups/, restorable with
        #     ``curie import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `curie update` does with uncommitted local changes to the
        ...
        # When `curie update` finds the source checkout parked on a feature
        ...
        #                         merge. `curie update --switch-branch`
    ...
    "lsp": {
        ...
        #              ``<CURIE_HOME>/lsp/bin/`` on first use.
    ...
    # `curie tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.curie/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.curie/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.curie/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Curie may use this encrypted
            ...
            # ~/.curie/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.curie/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Curie disables it for our users unless they
        # explicitly opt in here. When false (default), Curie sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Curie also calls
        ...
        "no_overlay": None,
        # cua-driver permission mode for each Curie computer-use runtime.
        ...
        # the explicit per-session UNLOCK toggle so a config line can never
        # silently bypass approvals.
        "permission_mode": "standard",
        # Absolute or ~ path to the reviewed cua-driver capability
        # manifest used when permission_mode is "bounded". Curie passes the
    ...
    #
    # Configure with `curie egress setup`.  Disabled by default — the rest of
    # Curie works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.curie/bin/ on
    ...
    # Curie Desktop (Electron app) launch options. These only affect
    # `curie desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the CURIE_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
        ...
        # Ignored on macOS/Windows. Bridged to the CURIE_DESKTOP_PASSWORD_STORE
    ...
    # Curie downloads official llama.cpp release binaries, then spawns and
    ...
    "local_runtime": {
        ...
        # (Curie still finds an external llama-server you run yourself).
        "enabled": False,
        # Pinned llama.cpp release tag (rolling bNNNN). Bumped by Curie
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Curie mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'curie model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Curie auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Curie' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Curie, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @curie:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. CURIE_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #curie)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: curie-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'curie-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Curie API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Curie API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Curie API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # CURIE_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to CURIE_TOOL_PROGRESS_MODE for backward
    ...
    # CURIE_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "CURIE_PREFILL_MESSAGES_FILE": {
