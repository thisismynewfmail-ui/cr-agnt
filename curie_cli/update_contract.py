...
``curie update`` mutation (CLI apply, CLI --check, dashboard update
endpoint). The decision layers:

1. **Baked provenance marker** (``/etc/curie/image-provenance.json``,
   written by the image build — see :mod:`curie_cli.image_provenance`):
