"""
curie fallback — manage the fallback provider chain.
...
Subcommands:
  curie fallback [list]   Show the current fallback chain (default when no subcommand)
  curie fallback add      Pick provider + model via the same picker as `curie model`,
                           then append the selection to the chain
  curie fallback remove   Pick an entry to delete from the chain
  curie fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.curie/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from curie_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `curie model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  curie fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/features/fallback-providers.md")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `curie model`, then append the selection to the chain."""
    from curie_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `curie model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `curie fallback list` to view, or `curie fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``curie fallback [subcommand]``."""
