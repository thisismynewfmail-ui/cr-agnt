"""Per-terminal session breadcrumbs for ``curie -c`` / ``--continue``.

Each CLI session writes a tiny breadcrumb file
``$CURIE_HOME/terminal-sessions/<terminal-id>`` containing
``{"session_id": ..., "cwd": ..., "ts": ...}``.  A bare ``curie -c`` then
...
def _breadcrumbs_dir() -> Path:
    ...
    return get_curie_home() / "terminal-sessions"
