"""``curie worktree`` — audit and reclaim accumulated git worktrees/branches.

Attended counterpart of the silent startup pruner (see
``curie_cli/worktree_gc.py`` for the policy and shared invariants). Usage:

    curie worktree list             # audit: verdict + reason per tree
    curie worktree prune            # reap safe trees + merged branches
    curie worktree prune --dry-run  # show the plan, change nothing
    curie worktree prune --trees-only / --branches-only
"""
...
def cmd_worktree(args) -> int:
    ...
    if action == "list":
        ...
        print(
            ...
            f"{_fmt_size(reapable_mb)} reclaimable now via `curie worktree prune`."
        )
