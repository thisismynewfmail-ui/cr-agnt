"""On-demand supply-chain audit for Curie Agent installs.

Scans three surfaces a Curie user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Curie venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.curie/plugins``
...
def _discover_plugins(curie_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.curie/plugins``.
...
def _discover_components(
    *,
    ...
    home = curie_home or Path(get_curie_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `curie security audit`."""
    ...
    components = _discover_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, curie_home=home
    )
