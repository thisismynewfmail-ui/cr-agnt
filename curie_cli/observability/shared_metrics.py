"""Durable aggregation and local export for Curie shared metrics."""
...
_PACKAGE_SCHEMA_VERSION = "curie.shared_metrics.v2"
...
class SharedMetricsStore:
    ...
    def __init__(
        ...
        root = get_curie_home() / "telemetry" / "shared_metrics"
        ...
        return [
            {
                ...
                "resource": {
                    "curie_version": row["curie_version"],
    ...
    def _ensure_schema(self) -> None:
        with self._connection(busy_timeout_ms=_SCHEMA_BUSY_TIMEOUT_MS) as connection:
            # Serialize first-run creation and upgrades across Curie processes.
            with write_txn(connection):
    ...
    def _add_send_columns(connection: sqlite3.Connection) -> None:
        ...
        branch, so bumping would make an older Curie — a second profile on an
    ...
    def _create_package_in_transaction(
        ...
        resource = {
            "curie_version": period_row["curie_version"],
