"""Bounded product contract for the first Curie shared-metrics slice."""
...
SCHEMA_KEY = "curie.metrics.schema_version"
SCHEMA_VERSION = "curie.metrics.event.v2"
MODEL_CALL_SCOPE = "curie.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "curie.task_run"
TOOL_CALL_SCOPE = "curie.tool_call"
CLIENT_ACTIVE_MARK = "curie.client.active"
TOOL_APPROVAL_MARK = "curie.tool_approval"
SKILL_LIFECYCLE_MARK = "curie.skill.lifecycle"
SKILL_LOAD_MARK = "curie.skill.load"
SUBSCRIBER_NAME = "curie.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "curie.client.active"
LEGACY_MODEL_CALL_METRIC = "curie.model_call.count"
MODEL_ROUTE_METRIC = "curie.model_route.count"
TASK_STARTED_METRIC = "curie.task_run.started"
TASK_FINISHED_METRIC = "curie.task_run.finished"
TOOL_CALL_METRIC = "curie.tool_call.count"
TOOL_APPROVAL_METRIC = "curie.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "curie.skill.lifecycle.count"
SKILL_LOAD_METRIC = "curie.skill.load.count"
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Curie installation method."""
    normalized = str(value or "").strip().lower()
    if normalized == "nix":
        return "nixos"
...
def client_resource(
    ...
    *,
    ...
    normalized_version = str(curie_version or "").strip()
    ...
    return {
        ...
        "curie_version": normalized_version,
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("curie_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Curie logical scope."""
    metadata = getattr(event, "metadata", None)
    if (
    ...
    if relay_metadata - {"otel.status_code"} or metadata.get(
        ...
        return None
    call_role = metadata.get("curie.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from curie_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Curie terminal state to bounded task outcome dimensions."""
...
def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Curie registry toolset metadata to a low-cardinality category."""
    ...
    if toolset.startswith(
        ("discord", "email", "feishu", "curie-yuanbao", "slack", "sms")
    ):
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Curie tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Curie."""
