"""``curie slack ...`` CLI subcommands.

Today only ``curie slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ curie slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Curie. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Curie deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Curie on Slack's older Assistant messaging
    ...
    """
    from curie_cli.commands import slack_app_manifest
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Curie in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Curie in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Curie agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``curie_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$CURIE_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Curie")
    ...
    """
    name = getattr(args, "name", None) or "Curie"
    description = getattr(args, "description", None) or "Your Curie agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "curie slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"curie slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "curie slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "curie slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Curie app\n"
