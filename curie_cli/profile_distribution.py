"""Profile distributions — shareable, packaged Curie profiles via git.

A distribution is a Curie profile published as a git repository (or
...
* ``curie profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``curie skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``curie profile``, not a parallel tree):

    curie profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "curie-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            curie_requires=str(data.get("curie_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.curie_requires:
            out["curie_requires"] = self.curie_requires
        if self.author:
...
def check_curie_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Curie {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``curie profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Curie profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Curie distribution."
        )

    # Version check up-front so we fail fast
    check_curie_requires(manifest.curie_requires, curie_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.curie).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="curie_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `curie profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`curie profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="curie_dist_update_") as tmp:
        plan = plan_install(
