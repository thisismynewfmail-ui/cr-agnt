...
Used by ``curie kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``curie_cli/goals.py`` — same aux
...
CURIE_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("CURIE_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Curie Agent board.
...
def _profile_author() -> str:
    """Mirror of ``curie_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("CURIE_PROFILE")
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=CURIE_KANBAN_SPECIFY_MAX_TOKENS,
