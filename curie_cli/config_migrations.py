...
``curie_cli.config.migrate_config``. Each step is a function
...
``curie_cli.config`` imports :func:`run_migrations` lazily (inside
``migrate_config``), and every step function here resolves its helpers
(``read_raw_config``, ``_persist_migration``, ``get_env_value``, …) lazily
through the live ``curie_cli.config`` module object at call time via
:func:`_cfg`. There is deliberately NO module-level import of
``curie_cli.config`` here, so no circular import can form — and, just as
importantly, tests that monkeypatch helpers on ``curie_cli.config`` (e.g.
``patch("curie_cli.config.read_raw_config", ...)``) keep working, because
...
#: CURIE_TOOL_PROGRESS*) forever is not worth it. Below-floor configs are
...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_curie_home()}/config.yaml and run `curie setup` to "
...
def _cfg():
    """Return the live ``curie_cli.config`` module (lazy, cycle-free)."""
    from curie_cli import config
...
def _migrate_to_21(results: Dict[str, Any], quiet: bool) -> None:
    ...
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    get_curie_home = _c.get_curie_home
    ...
    if "enabled" not in plugins_cfg:
        ...
        # Scan ``$CURIE_HOME/plugins/`` for currently installed user plugins.
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `curie plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `curie curator status` has no
    ...
    #   3. Creates `~/.curie/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_curie_home() — old profiles that predate this
    ...
    try:
        curator_dir = get_curie_home() / "logs" / "curator"
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `curie config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `curie config set`"
                )
...
# behavior set it to true explicitly via `curie config set`.
...
def _migrate_to_34(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # rendered TEXT into agent.system_prompt. When #81946 made
    ...
    # defaults on after updating"), so this step reset the selection once.
    #
    # The whole personality mechanism has since been removed — SOUL.md is the
    # only description of the agent's voice — and ``_migrate_to_40`` strips
    # both dead keys outright. This step survives only to keep the registry's
    # version ladder contiguous for configs older than v34: rewriting history
    # by renumbering later migrations would re-run them on installs that have
...
def _migrate_to_38(results: Dict[str, Any], quiet: bool) -> None:
    ...
    from curie_cli.relay_plugin_cutover import legacy_relay_plugin_keys
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # provider surface (`video_generate`, `curie tools` → Video Generation).
    ...
    if changed:
        ...
        if not quiet:
            print(
                ...
                "lists — video generation now lives under `curie tools` → "
                "Video Generation (Nous Subscription or FAL)."
            )


def _migrate_to_40(results: Dict[str, Any], quiet: bool) -> None:
    # ── Version 39 → 40: retire the config-side personality overlay ──
    # Two independent sources of voice is one too many. ``SOUL.md`` is read
    # fresh on every message and is the agent's only character sheet; the
    # config carried a competing one — ``display.personality`` selecting from
    # a built-in table ("kawaii", "pirate", "noir", …), extensible through
    # ``agent.personalities`` — and whichever won, the other was silently
    # ignored. The table, both keys, and the ``/personality`` command on every
    # surface are gone.
    #
    # Strip the dead keys so they cannot linger as confusing no-ops in a
    # hand-edited config. ``agent.system_prompt`` is deliberately left alone:
    # it is the user-authored manual overlay, still honoured, and never
    # written by us.
    _c = _cfg()
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    ...
    if isinstance(raw_display, dict) and "personality" in raw_display:
        old_name = str(raw_display.pop("personality", "") or "").strip()
        config["display"] = raw_display
        removed.append(
            f"display.personality (was '{old_name}')" if old_name
            else "display.personality"
        )

    raw_agent = config.get("agent")
    if isinstance(raw_agent, dict) and "personalities" in raw_agent:
        count = raw_agent.pop("personalities", None)
        config["agent"] = raw_agent
        n = len(count) if isinstance(count, dict) else 0
        removed.append(
            f"agent.personalities ({n} custom entr{'y' if n == 1 else 'ies'})"
        )

    # Top-level ``personalities:`` — the shape DEFAULT_CONFIG declared before
    # the key moved under ``agent``. Older configs can carry either.
    if "personalities" in config:
        count = config.pop("personalities", None)
        n = len(count) if isinstance(count, dict) else 0
        removed.append(
            f"personalities ({n} custom entr{'y' if n == 1 else 'ies'})"
        )
    ...
    _persist_migration(config)
    results["config_added"].append(
        "removed retired personality keys: " + ", ".join(removed)
    )
    if not quiet:
        print(
            "  ✓ Removed the retired personality overlay from config "
            f"({', '.join(removed)}). SOUL.md is now the single description "
            "of the agent's voice — edit it to change how the agent writes; "
            "it takes effect on the next message with no restart."
        )


def _migrate_to_41(results: Dict[str, Any], quiet: bool) -> None:
    # ── Version 40 → 41: give the small auxiliary calls room to think ──
    # ``auxiliary.title_generation.timeout`` was 30s and
    # ``auxiliary.vision.timeout`` was 120s. Both were sized for a fast
    # completion model, and both are now routinely served by a reasoning
    # model that spends its thinking budget before emitting a first token —
    # so the call was cancelled while still working and the user saw
    # "⚠ Auxiliary title generation failed: Request timed out." on a job that
    # would have finished. A dead stream is still caught in about a minute by
    # the no-progress watchdog, so a generous ceiling costs nothing.
    #
    # Only rewrite a value the user has not chosen themselves: if it differs
    # from the old default, it is a deliberate setting and it stays.
    _c = _cfg()
    read_raw_config = _c.read_raw_config
    _persist_migration = _c._persist_migration
    ...
    raised: list = []
    for task, stale_default in (("title_generation", 30), ("vision", 120)):
        section = auxiliary.get(task)
        if not isinstance(section, dict):
            continue
        if section.get("timeout") != stale_default:
            continue
        section["timeout"] = 3600
        raised.append(f"auxiliary.{task}.timeout {stale_default}s → 1h")
    ...
    _persist_migration(config)
    results["config_added"].append("; ".join(raised))
    if not quiet:
        print(
            "  ✓ Raised the auxiliary title and vision timeouts to one hour "
            f"({'; '.join(raised)}). A reasoning model can now finish a "
            "small job instead of being cancelled part-way and reported as "
            "a timeout."
        )
