"""Image-authored deployment provenance for immutable Curie runtimes.

The published image bakes ``/etc/curie/image-provenance.json`` outside both
``$CURIE_HOME`` and the mutable checkout.  A bind-mounted checkout (including
...
IMAGE_PROVENANCE_PATH = Path("/etc/curie/image-provenance.json")
