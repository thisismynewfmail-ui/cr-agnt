"""`curie checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.curie/checkpoints/``.  Actions:

    curie checkpoints               # same as `status`
    curie checkpoints status        # total size, project count, breakdown
    curie checkpoints list          # per-project checkpoint counts + workdir
    curie checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    curie checkpoints clear [-f]    # nuke the entire base (asks first)
    curie checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    curie checkpoints
    curie checkpoints prune --retention-days 3 --max-size-mb 200
    curie checkpoints clear -f
...
from curie_cli.sizefmt import format_bytes as _fmt_bytes
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: curie checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``curie checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `curie checkpoints` → status
