...
Profile-aware location: ``$CURIE_HOME/logs/dashboard-auth.log``.
...
from ``curie_constants`` or other curie_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$CURIE_HOME/logs/dashboard-auth.log``.

    Uses ``curie_constants.get_curie_home()`` (a leaf module — no import
    ...
    return get_curie_home() / "logs" / "dashboard-auth.log"
