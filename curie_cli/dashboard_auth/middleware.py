...
from curie_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /curie``, the
    ``login_url`` is prefixed (``/curie/login?next=...``) so the
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["curie.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # ``curie_session_at``. Verify it with the identical ``verify_session``
    ...
    # sends only ``curie_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from curie_cli.dashboard_auth.cookies import clear_session_cookies
        from curie_cli.dashboard_auth.prefix import prefix_from_request
