"""
Curie Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``curie desktop`` / ``curie gui``)
     Built inside the agent checkout under ``$CURIE_HOME/curie-agent/``:
       ...
       - ``$CURIE_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Curie.app`` or ``~/Applications/Curie.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Curie`` (NSIS per-user)
...
the app name ("Curie"), separate from ``$CURIE_HOME``:
  - macOS:   ``~/Library/Application Support/Curie``
  - Windows: ``%APPDATA%\\Curie``
  - Linux:   ``$XDG_CONFIG_HOME/Curie`` (default ``~/.config/Curie``)
...
``curie uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(curie_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return curie_home / "curie-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Curie"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Curie"
...
def source_built_gui_artifacts(curie_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``curie desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``curie-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(curie_home)
    ...
    return [
        ...
        curie_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Curie" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Curie.app"),
            home / "Applications" / "Curie.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Curie).
            local_base / "Programs" / "Curie",
            # Older / alternate layout some builds used.
            local_base / "curie-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Curie")
    else:
        ...
        # ``uninstall_gui``.
        from curie_cli.linux_desktop_entry import desktop_entry_path

        data = os.environ.get("XDG_DATA_HOME")
        data_base = Path(data) if data else (home / ".local" / "share")
        paths += [
            # The launcher entry `curie desktop` installs. Its icon is
            ...
            data_base / "applications" / "Curie.desktop",
            data_base / "icons" / "hicolor" / "scalable" / "apps" / "curie.png",
        ]
        ...
        for size in ("24x24", "32x32", "48x48", "256x256", "512x512", "1024x1024"):
            paths.append(data_base / "icons" / "hicolor" / size / "apps" / "curie.png")
    return paths


def agent_is_installed(curie_home: Path) -> bool:
    """Return True when a usable Python agent install exists under CURIE_HOME.
    ...
    if (agent_root / "curie_cli").is_dir():
        return True
...
def gui_is_installed(curie_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(curie_home):
        if p.exists():
            return True
...
def gui_install_summary(curie_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = curie_home if curie_home is not None else get_curie_home()
...
def uninstall_gui(
    curie_home: "Path | None" = None, *, remove_userdata: bool = True
) -> "list[Path]":
    ...
    Never touches ``curie-agent/curie_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$CURIE_HOME``.
    ...
    if sys.platform.startswith("linux"):
        ...
        # Reindex so Curie disappears from the launcher.
        try:
            from curie_cli.linux_desktop_entry import (
                desktop_entry_path,
        ...
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove curie' or "
            "'sudo dnf remove curie'). AppImage builds are a single file you "
