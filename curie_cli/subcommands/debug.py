...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Curie Agent. Use 'curie debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    curie debug share              Upload debug report (asks for confirmation)
    curie debug share --yes        Skip confirmation (for scripts/CI)
    curie debug share --lines 500  Include more log lines
    curie debug share --expire 30  Keep paste for 30 days
    curie debug share --local      Print report locally (no upload)
    curie debug share --no-redact  Disable upload-time secret redaction
    curie debug share --nous       Upload to Nous-internal storage (private)
    curie debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'curie debug share'",
    )
