...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``curie login`` was removed in favor of ``curie auth`` / ``curie model``
    (the runtime handler in ``curie_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``curie login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``curie --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``curie login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``curie model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `curie auth` to manage credentials, "
            "`curie model` to select a provider, or `curie setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `curie model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: curie-cli)"
    )
