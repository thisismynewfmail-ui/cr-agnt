"""``curie approvals`` subcommand parser.
...
def build_approvals_parser(subparsers, *, cmd_approvals: Callable) -> None:
    ...
    approvals_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "`curie approvals suggest` mines past approval decisions from "
    ...
    suggest_parser.add_argument(
        "--db",
        help="Path to an alternate session database (default: ~/.curie/state.db)",
    )
    ...
    test_parser = approvals_subparsers.add_parser(
        ...
        description=(
            ...
            "detection, allowlist, unlock/off bypass — and print the verdict, "
            ...
            "aren't parsed: curie approvals test -- rm -rf /tmp/x"
        ),
    )
