"""``curie sync`` subcommand parser — Skill Sync.

Cloned from ``curie_cli/subcommands/cron.py`` — same injected-handler shape
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    curie sync status                 show gate/opt-in/head state
    curie sync pull                   pull and materialize opted-in skills
    curie sync push                   push opted-in skills
    curie sync now                    reconcile: pull then push
    curie sync enable <skill>         opt a skill into sync
    curie sync disable <skill>        opt a skill out of sync
    curie sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    curie sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  curie sync status            what is synced, and from where\n"
            "  curie sync enable my-skill   include a skill in your sync\n"
            "  curie sync now               pull, then push\n"
            "  curie sync propose my-skill  share a skill with your team\n"
        ),
