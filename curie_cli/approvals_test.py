"""``curie approvals test`` — dry-run approval verdict for a command.
...
guard (``check_all_command_guards``) applies them:
    ...
    2. hardline blocklist (never bypassable, fires before unlock/off),
    3. sudo-stdin guard (unconditional),
    4. user ``approvals.deny`` rules (fire before unlock/off),
    5. unlock / ``approvals.mode: off`` bypass,
...
def evaluate_command(command: str, env_type: str = "local") -> dict:
    ...
    # 2. Hardline blocklist — never bypassable, even under unlock.
    ...
    if is_hardline:
        return result(
            ...
            detail="matches the hardline blocklist (never bypassable, "
                   "blocked even under --unlock / approvals.mode=off)",
        )
    ...
    # 4. User-defined approvals.deny rules — fire before unlock/off.
    ...
    if deny_pattern is not None:
        return result(
            ...
            detail="matches a user-defined approvals.deny rule in "
                   "config.yaml (blocked even under --unlock / mode=off)",
        )

    # 5. Unlock / approvals.mode=off bypass.
    if (approval._UNLOCK_MODE_FROZEN
            or approval.is_current_session_unlock_enabled()
            or approval._get_approval_mode() == "off"):
        return result(
            "allow",
            detail="approval bypass active (--unlock or approvals.mode: off); "
...
def approvals_test_command(args) -> int:
    """Handle ``curie approvals test <command...>``. Returns the exit code."""
    ...
    if words and words[0] == "--":
        words = words[1:]
    if not words:
        print("usage: curie approvals test [--env-type TYPE] [--json] -- <command...>")
        return EXIT_USAGE
    command = " ".join(words)
