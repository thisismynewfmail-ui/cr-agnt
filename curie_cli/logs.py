"""``curie logs`` — view and filter Curie log files.
...
Usage examples::
    ...
    curie logs                    # last 50 lines of agent.log
    curie logs -f                 # follow agent.log in real time
    curie logs errors             # last 50 lines of errors.log
    curie logs gateway -n 100    # last 100 lines of gateway.log
    curie logs gui -f            # follow gui.log (dashboard/pty/ws)
    curie logs desktop -f        # follow desktop.log (Electron app boot/backend)
    curie logs --level WARNING    # only WARNING+ lines
    curie logs --session abc123   # filter by session ID substring
    curie logs --component tools  # only tool-related lines
    curie logs --since 1h         # lines from the last hour
    curie logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_curie_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Curie runs — try 'curie chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_curie_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_curie_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_curie_home()}/logs/")
        return

    print(f"Log files in {display_curie_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'curie chat' to generate logs)")
