...
Curie is developed on Linux / macOS and tested natively on Windows too.
...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_CURIE_GATEWAY_BREAKAWAY"
...
# Electron exits, so the gateway never gets respawned after a `curie
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Curie' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Curie shells out to git from many non-interactive contexts — MCP catalog
...
def _text_names_curie(text: str) -> bool:
    """True when *text* names Curie at a path-segment / token boundary.

    A bare ``"curie" in text`` substring test would also match unrelated
    ...
    that *starts with* ``curie`` (``curie``, ``curie.exe``, ``curie_cli``,
    ``curie-agent``, ``curie-runtime``) or the hidden-dir form
    ``.curie``/``.curie-runtime``.
    """
    for token in re.split(r"[\\/\s=,;\"']+", text.lower()):
        if token.startswith("curie") or token.startswith(".curie"):
            return True
    return False


def _process_command_is_curie(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Curie code."""
    try:
        import psutil
        ...
        return _text_names_curie(f"{command} {executable}")
    except Exception:
        return False
...
def pid_is_curie(
    ...
    *,
    ...
    The PID must be valid, currently exist, and identify a Curie process. When
    ...
    if (
        ...
        return False
    try:
        return _process_command_is_curie(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # ``pid_is_curie`` guard is for BARE pids from state files or process
...
def bounded_probe_run(
    ...
    *,
    ...
    scan hit exactly this during ``curie update`` on slow-WMI machines
