"""Slash command definitions and autocomplete for the Curie CLI.
...
from curie_constants import INDICATOR_STYLES
...
class CommandDef:
    ...
    # Names a key in ``curie_cli.slash_exec.EXECUTORS`` — a pure formatter
    # producing the canonical, surface-independent core text.  Surfaces
    # resolve it via ``curie_cli.slash_exec.run_execute`` and apply only
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("snapshot", "Create or restore state snapshots of Curie config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Curie works on across turns until achieved", "Session",
    ...
    CommandDef("plan", "Write a markdown implementation plan to .curie/plans/ without executing anything", "Session",
               args_hint="[task]"),
    ...
    CommandDef("footer", "Toggle gateway runtime-metadata footer on final replies",
               ...
               desktop="terminal"),
    CommandDef("unlock", "Toggle UNLOCK mode (skip all dangerous command approvals)",
    ...
    CommandDef("wake", "Toggle the 'Hey Curie' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control how messages behave while Curie is working", "Configuration",
    ...
    CommandDef("reload-skills", "Re-scan ~/.curie/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Curie Agent to the latest version", "Info",
               busy_policy="dispatch", desktop="terminal"),
    CommandDef("version", "Show Curie Agent version", "Info", aliases=("v",),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`curie_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/curie`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Curie ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Curie
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``curie skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``curie skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``curie skills list`` and
...
# unrelated command landed. These claim their slots right after /curie,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /curie <command>).
...
# native slot, the alias spelling stays reachable via /curie reset).
...
# ``/curie <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/curie" decision, not a silent clamp.
#   - topup: the billing/balance surface; reached via /curie topup on Slack.
#     (the rehaul folded the old /credits + /billing surfaces into /topup.)
#   - moa: high-cost slash mode, available through /curie moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /curie debug on Slack.
#   - egress: Docker-only proxy status; reachable as /curie egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /curie init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /curie version on
...
#   - diff: git working-tree diff; reached via /curie diff on Slack so it
...
#     /curie update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /curie heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /curie refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
#   - pause: global emergency stop; reached via /curie pause [off] on
#     Slack. Added at the 50-cap — a native slot would clamp /platform.
#   - whoami: one-off identity lookup; reached via /curie whoami on Slack.
...
#     /curie platform on Slack. Demoted when /save became gateway-available
#     (session export is an interactive surface; platform is a rare
#     informational lookup) — without this entry /save tips the registry
#     past the 50-cap and silently clamps /platform, breaking parity.
_SLACK_VIA_CURIE_ONLY = frozenset({"topup", "moa", "debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine", "review", "pause", "whoami", "platform", "insights"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/curie <verb>`` subcommand.
    ...
    can still reach them via ``/curie <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/curie`` is always reserved as the first entry so the
    legacy ``/curie <subcommand>`` form keeps working for anything that
    ...
    # Reserve /curie as the catch-all top-level command.
    entries.append(("curie", "Talk to Curie or run a subcommand", "[subcommand] [args]"))
    seen.add("curie")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_CURIE_ONLY:
            # Intentionally Slack-via-/curie only (see _SLACK_VIA_CURIE_ONLY).
            return
    ...
    # everything except /curie, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://curie-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /curie handler.

    Maps both canonical names and aliases so /curie bg do stuff works
    the same as /curie bg do stuff.

    Plugin-registered slash commands are included so ``/curie <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    _PICKER_COMMANDS = frozenset({"model", "skin"})
    ...
    def _completion_text(cmd_name: str, word: str) -> str:
        ...
        However, commands that open pickers (model, skin) should
