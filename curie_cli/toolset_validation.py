...
the tool registry or launching Curie (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``curie-cli`` to the non-existent ``curie``. ``resolve_toolset('curie')``
...
def _platform_default_is_valid(
    ...
    # though their synthesized curie-<platform> name is not in TOOLSETS.
    try:
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``curie-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``curie`` instead of ``curie-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        if not isinstance(raw, list):
            ...
            warnings.append(
                ...
                f"{fallback_detail}. Run `curie tools` to configure explicitly."
            )
            if platform_valid_count == 0:
                warnings.append(
                    ...
                    f"Run `curie tools` to reconfigure."
                )
            continue
        names = raw
        for name in names:
        ...
        if platform_valid_count == 0:
            ...
            warnings.append(
                ...
                f"tools on this platform. Run `curie tools` to reconfigure."
            )

    if valid_count == 0:
        warnings.append(
            "platform_toolsets resolves to zero valid toolsets — the agent will "
            "have no tools. Run `curie tools` to reconfigure."
        )
    return warnings
