"""curie webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.curie/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _curie_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: curie gateway run
"""
...
def webhook_command(args):
    """Entry point for 'curie webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: curie webhook {subscribe|list|remove|test}")
        print("Run 'curie webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (curie gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: curie webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from curie webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (curie gateway run)")
