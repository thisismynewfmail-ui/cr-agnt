...
on Japanese locales, etc.).  Curie's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``curie_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Curie is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``CURIE_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("CURIE_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Curie-managed Git install directories so
    ...
    # already-running shells don't see the change — which means curie
    ...
    # install without Curie-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Curie's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Curie's
    ``curie config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Curie (User env var in Windows Settings, or
    export in a PowerShell profile) and Curie picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Curie-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\curie\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``curie`` from right after install) retain their old PATH.

    Any subprocess Curie spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Curie-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "curie", "git", "cmd"),
        os.path.join(local_appdata, "curie", "git", "bin"),
        os.path.join(local_appdata, "curie", "git", "usr", "bin"),
        # Curie venv Scripts directory — host of the curie.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes curie, but harmless to include.
        os.path.join(local_appdata, "curie", "curie-agent", "venv", "Scripts"),
