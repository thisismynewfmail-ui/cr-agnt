"""Curie update pipeline — extracted from ``curie_cli/main.py``.
...
change is that references to helpers/constants that STAY in ``curie_cli.main``
(and to moved-but-test-patched siblings) are routed through ``_m()`` — a lazy
``curie_cli.main`` reference — so existing call sites and test monkeypatches
that target ``curie_cli.main.<name>`` (``PROJECT_ROOT``, ``_is_windows``,
...
the test-patch surface still resolve on ``curie_cli.main``.
...
Imports are one-way: ``curie_cli.main`` imports this module, never the reverse
...
def _m():
    """Lazy ``curie_cli.main`` reference.

    Lets callers keep patching ``curie_cli.main.<helper>`` (the historical
    test surface) and have those patches reach this code path, and defers the
    import so ``curie_cli.main`` -> ``curie_cli.update_cmd`` stays one-way
    at import time.
    """
    from curie_cli import main
...
#: ``_purge_stale_curie_modules`` so any LATER import chain resolves against
...
_STALE_PURGE_PROTECTED = frozenset(
    {
        ...
        "curie_cli.curie_logging",
    }
)


def _purge_stale_curie_modules() -> None:
    """Evict every cached Curie module after the checkout changed in-place.

    ``curie update`` keeps running in the pre-pull Python process. The
    gateway auto-restart phase that follows does function-level
    ``from curie_cli.gateway import ...`` — executing NEW source inside an
    ...
    field failure: freshly-pulled ``curie_cli.gateway`` does
    ``from curie_cli.cli_output import line_input``, but ``cli_output`` was
    ...
    export. This is the class fix: drop EVERY cached module under the Curie
    ...
    try:
        ...
        if purged:
            logger.debug(
                "Purged %d stale Curie module(s) after checkout update", len(purged)
            )
    except Exception as exc:
        logger.debug("Could not purge stale Curie modules: %s", exc)


def _reload_updated_runtime_modules() -> None:
    """Reload update-sensitive modules after the checkout changes in-place.

    ``curie update`` keeps running in the pre-pull Python process. After a
...
def _reload_config_modules() -> None:
    ...
    ``curie update`` runs in the PRE-pull Python process. After ``git pull``
    ...
    This function force-reloads ``curie_cli.config_defaults``,
    ``curie_cli.config``, and ``curie_cli.config_migrations`` from disk
    so subsequent imports read the UPDATED code.

    It also reloads ``curie_cli._subprocess_compat`` and
    ``curie_cli.dashboard_procs`` so that post-update dashboard cleanup
    ...
    importlib.invalidate_caches()
    for mod_name in (
        "curie_cli.config_defaults",
        "curie_cli.config",
        "curie_cli.config_migrations",
        "curie_cli._subprocess_compat",
...
def _run_config_check_fresh() -> tuple:
    ...
    from curie_cli.config import check_config_version
...
def _run_migrate_config_fresh(*, interactive: bool = False, quiet: bool = False) -> dict:
    ...
    """
    _reload_config_modules()
    from curie_cli.config import migrate_config
...
def _migrate_sibling_profile_configs() -> list[tuple[str, int, int]]:
    ...
    shared checkout serves every profile, but ``curie update`` historically
    ...
    caller): scope config reads/writes via the context-local CURIE_HOME
    ...
    try:
        ...
        from curie_cli.profiles import _get_profiles_root, _PROFILE_ID_RE

        active_home = get_process_curie_home()
        ...
        for entry in sorted(root.iterdir()):
            ...
            token = set_curie_home_override(entry)
            try:
...
def _check_and_apply_config_migration(
    *,
    ...
    try:
        ...
        print("     Run 'curie config migrate' to check manually.")
        return
    ...
    if version_bump_only:
        ...
        try:
            ...
            print("     Run 'curie config migrate' to retry.")
        ...
        if assume_yes:
            ...
            try:
                ...
                print(
                    ...
                    "Run 'curie config migrate' manually to configure."
                )
                response = "n"
        ...
        if response in {"", "y", "yes", "auto"}:
            ...
            if (gateway_mode or assume_yes or response == "auto") and missing_env:
                print("  ℹ API keys require manual entry: curie config migrate")
        else:
            print()
            print("Skipped. Run 'curie config migrate' later to configure.")
    else:
    ...
    # context-local CURIE_HOME override (never os.environ — other
    ...
    # it and warn loudly.
    try:
        from curie_cli.backup import restore_cron_jobs_if_emptied
    ...
    try:
        from curie_cli.backup import restore_config_model_settings_if_rewritten
    ...
    try:
        from curie_cli.backup import restore_cron_jobs_all_profiles
    ...
    try:
        from curie_cli.backup import restore_config_model_settings_all_profiles
...
# Critical files that Curie must be able to import immediately after an
...
_UPDATE_CRITICAL_FILES = (
    "curie_cli/main.py",
    "curie_cli/config.py",
    "curie_cli/__init__.py",
    "curie_cli/web_server.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _prune_orphan_rescue_refs(
    ...
    ``refs/curie-update-backups/orphan-<branch>-<ts>-<sha>``. A rescue ref
    ...
    try:
        list_result = subprocess.run(
            git_cmd + [
                ...
                "--format=%(refname)",
                "--sort=refname",
                f"refs/curie-update-backups/orphan-{branch}-*",
            ],
            cwd=cwd,
            capture_output=True,
            text=True, encoding="utf-8", errors="replace",
        ...
        if max_age_days > 0:
            ...
            prefix = f"refs/curie-update-backups/orphan-{branch}-"
            for ref in refs:
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    On Windows that rewrite is the only reason the running ``curie.exe`` has
    ...
    Skipping is safe because Curie pins its editable finder to a *static*
...
def _validate_python_files_syntax(
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="curie-syntax-check-") as tmpdir:
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``curie`` startup; if any of them
...
def _critical_module_import_failures(
    ...
    ``_venv_core_imports_healthy``): ``curie update`` can be driven by a
    ...
    a later one.
    """
    from curie_constants import FIRST_PARTY_MODULE_ROOTS

    import secrets

    marker = f"__CURIE_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        ...
        # The root set is injected from curie_constants so this can't drift
        # from the hint the user is shown (they disagreed once already).
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('curie_') or %r:\n"
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``curie update --gateway`` so interactive prompts (stash restore,
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `curie update`.
    ...
    print("  Preview now:  curie curator run --dry-run")
    print("  Pause it:     curie curator pause")
    print(
        "  Docs:         https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/features/curator.md"
    )

def _print_fts_optimize_available_notice() -> None:
    """Advertise the opt-in v23 search-index optimization after `curie update`.
    ...
    try:
        ...
        return
    db_path = get_curie_home() / "state.db"
    ...
    if interrupted:
        ...
        print(
            "  A previous `curie sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    curie sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  curie sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``curie update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``curie update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: curie curator status)"
    )
...
def _reload_process_scan_modules() -> None:
    ...
    ``from curie_cli._subprocess_compat import bounded_probe_run``. If the
...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units* forwards the systemd unit names (no
    ...
    print("    curie dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    ``curie_cli.main`` re-export surface and guards the #49145 regression.
    """
...
def _stage_replacement(src: str, dst: str) -> str:
    ...
    untouched.
    """
    staging = f"{dst}.curie-update-staging"
    backup = f"{dst}.curie-update-old"
...
def _discard_staged(staged) -> None:
    ...
    the tree. The user then follows the "re-run `curie update`" advice with
...
def _commit_staged_replacements(staged) -> None:
    ...
    first-party modules (``run_agent.py``, ``cli.py``, ``curie_constants.py``
...
def _assess_parked_branch_switch(
    ...
    stale feature branch left behind by earlier tooling; ``curie update``
...
def _print_parked_branch_skip_warning(
    ...
    print(f"    git -C {cwd} checkout {target_branch} && curie update")
    print(
...
def _print_update_completion(message: str) -> None:
    ...
    action_id = os.environ.get("CURIE_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== curie-update completed {action_id} ===")
...
def _refuse_update_for_contended_shims(exc: BaseException) -> None:
    ...
    """
    print("✗ Cannot continue the update: live Curie launcher(s) could not be")
    print("  moved aside:")
    for name in getattr(exc, "failed_shims", []) or ["curie.exe"]:
        print(f"    {name}")
    print("  Another process is holding this install's venv — typically Curie")
    print("  Desktop, a gateway, or another curie REPL — and mutating the venv")
    print("  now would strand it half-updated.")
    print("  The dependency install has been deferred: close the process(es)")
    print("  above, then run any `curie` command to finish it automatically.")
...
def _should_zip_fallback_on_update_error(exc: BaseException) -> bool:
    ...
    A dependency-install failure (locked ``curie.exe`` / ``uv pip install``
...
def _zip_overlay_block_reason(
    ...
    two-phase replace creates ``*.curie-update-staging`` siblings inside the
...
_ZIP_STAGING_ARTIFACT_SUFFIXES = (".curie-update-staging", ".curie-update-old")
...
def _abort_zip_update_if_dirty_tree() -> None:
    ...
    print("  Stash or commit your changes, then rerun `curie update`.")
...
def _post_update_sqlite_runtime_status():
    ...
    from curie_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next Curie runtime."""
    ...
    print(
        "  Rebuild the Curie venv with a uv-managed Python, restart Curie, "
        "then verify with `curie doctor`."
    )
    return False
...
def _clear_stale_sqlite_sidecars(db_path: Path) -> None:
    ...
    database (a crashed writer, or a second Curie process the updater's drain
...
def _print_update_summary(
    *,
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not desktop_build_ok:
            print("  Run `curie desktop` to retry the desktop rebuild.")
        if not sqlite_runtime_ok:
            print(
                "  The Python runtime remediation did not complete. Run `curie "
                "update` again; if SQLite is unchanged, rebuild the Curie venv "
                "with a uv-managed Python, restart Curie, then verify with "
                "`curie doctor`."
            )
    else:
...
def _write_gateway_update_exit_code(ok: bool) -> None:
    path = get_curie_home() / ".update_exit_code"
    try:
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from curie_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    from curie_cli.sqlite_safe_read import LiveConnectionError, offline_file_access

    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (curie gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart Curie) and retry."
        )
        return False
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    """
    try:
        from curie_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    try:
        from curie_cli.backup import _sibling_profile_homes
...
def _update_via_zip(args, *, had_desktop_app_before_update: bool = False) -> bool:
    """Update Curie Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `curie update "
            f"--branch {branch}`, or update against main with `curie update`."
        )
        _m().sys.exit(1)
    _abort_zip_update_if_dirty_tree()
    zip_url = f"{OFFICIAL_REPO_WEB_URL}/archive/refs/heads/{branch}.zip"

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="curie-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"curie-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for curie-agent itself
        ...
        # GitHub ZIPs extract to curie-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"curie-agent-{branch}")
        ...
        try:
            for item in entries:
                ...
                # Curie.exe) exists only in the LIVE tree. Swapping `apps`
        ...
        print(
            "  Re-run `curie update` to retry; if the agent won't start, "
            f"reinstall from {OFFICIAL_REPO_WEB_URL}"
        )
        _m().sys.exit(1)
    finally:
    ...
    from curie_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    if uv_bin:
        ...
        from curie_cli.managed_uv import managed_python_env
    ...
    if not import_ok:
        ...
        print("  Re-run `curie update` to complete it.")
        _m().sys.exit(1)
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: curie skills list-modified  "
    ...
    try:
        from curie_cli.model_catalog import seed_cache_from_checkout
    ...
    try:
        from curie_cli.update_receipt import finalize_update_receipt
...
_AUTOSTASH_NAME_PREFIX = "curie-update-autostash-"

#: Age past which a leftover ``curie-update-autostash-*`` entry is called out
...
def _warn_orphaned_update_autostashes(git_cmd: list[str], cwd: Path) -> int:
    ...
    Curie never drops one automatically.
...
def _park_stashed_changes(stash_ref: str) -> None:
    ...
    Used by ``curie update --keep-stash`` (the desktop updater's mode): the
...
def _reject_unsafe_stash_restore(
    ...
    print()
    print("✗ Restored local changes made the Curie agent unexecutable.")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Curie behaves unexpectedly.")
    ...
    if restore.returncode != 0 and not has_conflicts and (
        ...
        # files makes curie completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Curie couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Curie couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Curie behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Curie couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Curie couldn't drop "
...
#: The official repository, as an ``owner/name`` slug. Everything the update
...
#: an install made under a previous name has that name in its ``origin`` and
#: is still the official checkout. Dropping the old slug here would not break
#: their ``git pull`` — it would quietly reclassify them as a fork and start
...
OFFICIAL_REPO_URLS = {
    url
    for slug in OFFICIAL_REPO_SLUG_ALIASES
    for url in (
...
OFFICIAL_REPO_WEB_URL = f"https://github.com/{OFFICIAL_REPO_SLUG}"
OFFICIAL_REPO_URL = f"{OFFICIAL_REPO_WEB_URL}.git"
OFFICIAL_REPO_RAW_BASE = f"https://raw.githubusercontent.com/{OFFICIAL_REPO_SLUG}"
...
def _remote_slug(url: Optional[str]) -> str:
    """Reduce a GitHub remote URL to its ``owner/name``, lowercased.

    Returns "" for a non-GitHub remote, which is the signal to leave it alone.
    """
    if not url:
        return ""
    ...
    if value.endswith(".git"):
        value = value[:-4]
    # scp-style (git@github.com:owner/name) before the path form, so the
    # colon is consumed by the branch that owns it.
    for marker in ("github.com:", "github.com/"):
        index = value.find(marker)
        if index >= 0:
            return value[index + len(marker):].lower()
    return ""


def _retarget_stale_origin(git_cmd: list[str], cwd: Path) -> bool:
    """Point a checkout named for a former repository at the current one.

    This is the half of a rename that clone URLs cannot fix. An install made
    before the move has the old address in ``origin``, and every update goes
    through ``origin`` — so it fetches a repository GitHub no longer serves,
    which answers 404, which git reports as "Repository not found" only
    *after* asking for a password, because a 404 is indistinguishable from a
    private repository the caller has not authenticated to. The update stops
    at a credential prompt naming a repository the user has never heard of.

    A remote that is neither the current name nor a former one is somebody's
    fork or mirror and is deliberate: left untouched. Returns whether the
    remote was rewritten.
    """
    origin_url = _get_origin_url(git_cmd, cwd)
    slug = _remote_slug(origin_url)
    if not slug or slug == OFFICIAL_REPO_SLUG.lower():
        return False
    former = {s.lower() for s in OFFICIAL_REPO_SLUG_ALIASES} - {OFFICIAL_REPO_SLUG.lower()}
    if slug not in former:
        return False

    print(f"⚠ This install points at {slug}, which has been renamed.")
    try:
        result = subprocess.run(
    ...
    if result is not None and result.returncode == 0:
        print(f"  ✓ Remote updated to {OFFICIAL_REPO_URL}")
        return True
    print(f"  ✗ Could not update the remote. Fix it by hand with:")
    print(f"      git -C {cwd} remote set-url origin {OFFICIAL_REPO_URL}")
    return False
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_curie_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_curie_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(
    ...
    *,
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Curie repository.")
...
def _invalidate_update_cache():
    ...
    ``curie update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from curie_constants import get_default_curie_root

    default_home = get_default_curie_root()
...
# ``fleet_restart_pending`` lives under CURIE_HOME (not next to the venv).
...
def _fleet_restart_pending_marker_path() -> Path:
    """CURIE_HOME breadcrumb for a pull that has not yet restarted the fleet."""
    return get_curie_home() / _FLEET_RESTART_PENDING_NAME
...
def _current_checkout_sha() -> str | None:
    ...
    try:
        from curie_cli.build_info import get_code_identity
...
def _warn_pending_fleet_restart(*, startup: bool = False) -> None:
    ...
    print(
        "⚠ A previous `curie update` pulled new code but did not "
    ...
    if startup:
        print(
            "  Run `curie update` or `curie gateway restart`.",
            file=stream,
        )
...
def _restart_systemd_gateway_units_best_effort(failed: list) -> None:
    """Best-effort ``systemctl restart`` of every curie-gateway/serve unit."""
...
def _run_pending_fleet_restart() -> bool:
    ...
    try:
        _m()._purge_stale_curie_modules()
    except Exception:
        pass
    try:
        from curie_cli.gateway import (
            find_gateway_pids,
            is_macos,
            is_windows,
    ...
    try:
        ...
        if is_windows():
            try:
                from curie_cli import gateway_windows
...
def _apply_pending_fleet_restart_catchup() -> None:
    """On an already-up-to-date ``curie update``, finish a skipped restart.
    ...
    if _run_pending_fleet_restart():
        ...
        return
    print("  ⚠ Fleet restart incomplete. Recover with: curie gateway restart")
    sys.exit(1)
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "curie.exe"
    lines = ["✗ Another curie.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Curie Desktop, exit any open `curie` REPLs, and")
    lines.append("  stop the gateway (`curie gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `curie update --force` if you've already")
...
def _classify_concurrent_instance(pid: int) -> str:
    ...
    from curie_cli._scan_venv_blockers import _is_pausable_gateway  # noqa: PLC0415
...
def _filter_non_gateway_concurrent_instances(
    ...
    ``curie update``. If every concurrent instance is a gateway, the pause
    ...
    TUI shell, a Curie Desktop backend child, an unrelated ``curie`` REPL)
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``curie tools``."""
    try:
        from curie_cli import tools_config

        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Curie Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    *,
    ...
    """Restore allowlisted ``curie tools`` dependencies into a rebuilt venv.
    ...
    try:
        ...
        logger.debug("Curie Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Curie Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    *,
    ...
    first use. ``curie update`` runs ``uv pip install -e .[all]`` which
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `curie update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `curie update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``hindsight-all``), NOT in Curie' editable-install extras or
    ...
    try:
        from curie_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from curie_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$CURIE_HOME/bin/uv``, but on Termux the official
    ...
    """
    from curie_cli.managed_uv import resolve_uv
...
def _npm_manifest_paths() -> tuple[Path, ...]:
    ...
    lockfile is then unchanged but `curie update` is exactly the step
...
def _npm_lockfile_changed(curie_root: Path) -> bool:
    ...
    # landed must NOT skip the reinstall — otherwise every later `curie
    ...
    try:
        ...
        cache_file = curie_root / f".npm_lock_hash_{cache_key}"
...
def _record_npm_lockfile_hash(curie_root: Path) -> None:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    curie update" — but the early return never reached the Node refresh,
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `curie update`.")
        print_completion(
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        from curie_constants import is_wsl
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `curie update`.")
            failed = []
            if any(
    ...
    # Curie profile using this checkout. Keep one per-checkout cache under the
    # shared Curie root rather than rerunning npm once per named profile.
    shared_curie_root = get_default_curie_root()
    ...
    # `curie update` case. Synchronous and can block ~11s on a true cold
    ...
    if not _m()._npm_lockfile_changed(shared_curie_root):
    ...
    def _partial_update_failure(*labels: str) -> list[str]:
        ...
        print("    deps). Fix npm and re-run `curie update`.")
        return list(labels)
    ...
    nixos_env = with_curie_node_path(_m()._nixos_build_env())
    ...
    # `curie update` comes from the *desktop* build, not this step; that
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_curie_root)
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.curie/logs/update.log`` only, never the terminal.

    During ``curie update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``curie update --check``: fetch and report without installing.
    ...
    from curie_cli.gitlock import clear_stale_git_locks, clear_stale_tmp_packs
    ...
    if is_shallow:
        ...
        if head_sha and target_sha and head_sha == target_sha:
            ...
            from curie_cli.banner import _github_compare_behind
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``curie update`` without requiring a reinstall.  The
    ...
    ``curie`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/curie.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v curie'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/curie, code at /usr/local/lib/curie-agent).
    fhs_link = Path("/usr/local/bin/curie")
    ...
    path_comment = (
        "# Curie Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal: install a ``curie-acp`` launcher next to the ``curie`` one.

    Mirrors the launcher block in ``scripts/install.sh`` so existing installs
    gain the ACP command on ``curie update`` without a reinstall.  ACP hosts
    (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving the
    ``curie-acp`` command name against the login-shell PATH; the console
    script of that name lives inside the install's venv, which is not on that
    PATH, so those hosts report Curie as not installed even when it is.

    The shim simply delegates to the sibling ``curie`` launcher with the
    ...
    No-op on Windows (install.ps1 stages the ``curie`` / ``curie-acp``
    launchers into the managed binary dir ``$CurieHome\bin`` and puts THAT
    ...
    ``curie_cli._install_repair.ensure_windows_bin_launchers`` re-stages
    them) and wherever a ``curie-acp`` is already present next to the
    ``curie`` command.  Unwritable directories (e.g. ``/usr/local/bin`` as
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        curie_cmd = bin_dir / "curie"
        acp_cmd = bin_dir / "curie-acp"
        try:
            if not (curie_cmd.is_file() or curie_cmd.is_symlink()):
                continue
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    - ``full``  — the quick snapshot PLUS a full zip of CURIE_HOME under
      ``backups/`` (restorable via ``curie import``; the #48200 wrong-path
    ...
    snapshot_id = None
    try:
        from curie_cli.backup import (
            _quick_snapshot_root,
            create_quick_snapshot,
            verify_sqlite_integrity,
        )

        # NOTE: this function later does `from curie_constants import
        # get_curie_home`, which makes the name function-local — the
        # module-level import is shadowed and unbound here. Alias explicitly.
        from curie_cli.config import get_curie_home as _get_home
        ...
        try:
            from curie_cli.backup import create_pre_update_snapshots_all_profiles
            ...
            if _sibling_snaps:
                ...
                try:
                    from curie_cli.update_receipt import record_step
    ...
    try:
        from curie_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Human-readable size
    from curie_cli.sizefmt import format_bytes

    size_str = format_bytes(size_bytes)

    # Render path using display_curie_home so the user sees ~/.curie/...
    try:
        from curie_constants import get_curie_home, display_curie_home

        home = get_curie_home()
        try:
            display_path = f"{display_curie_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  curie import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``curie update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``curie update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run curie from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.curie-bootstrap-complete`,
        ...
        managed_markers = (
            _m().PROJECT_ROOT / ".curie-bootstrap-complete",
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The curie.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m curie_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``curie
    ...
    try:
        ...
        # fetch cmdline/cwd lazily for plausible Python/uv/Curie candidates.
    ...
    for proc in proc_iter:
        ...
        if not is_holder and not (
            ...
            or name_low in {"uv.exe", "uvx.exe", "curie.exe"}
        ):
            continue
        ...
        # the `-m curie_cli.main gateway run` argv, so autostarted gateways
...
# ``cryptography`` is the canonical case: ``curie_cli.main`` used to import
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    - The ``curie.exe`` console shim we were launched from (#88838, #89599).
      The marker cannot help here — every future ``curie`` launch is also the
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `curie` launch will complete it")
...
def _curie_holder_subcommand(cmdline: str) -> str | None:
    """The actual Curie SUBCOMMAND a venv-holder argv runs, or None.

    Token-based, never substring (#90778: ``kanban --preserve-cache``
    contained \"serve\" and got labeled as the Desktop backend). Finds the
    ``curie_cli.main`` / ``curie(.exe)`` entry token, then returns the
    ...
    for i, token in enumerate(tokens):
        ...
        if low.endswith("curie_cli.main") and i > 0 and tokens[i - 1] == "-m":
            entry_idx = i
            break
        base = low.rsplit("\\", 1)[-1].rsplit("/", 1)[-1]
        if base in ("curie", "curie.exe"):
            entry_idx = i
            break
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    (#90778): a standalone ``curie dashboard`` must not be labeled as the
    ...
    """
    lines = [
        "✗ Other Curie processes are running from this install's venv:",
    ]
    hint_by_subcommand = {
        "serve": "  ← Curie backend (if the Desktop app is open, close it)",
        "dashboard": "  ← curie dashboard (stop it: curie dashboard stop, or close that terminal)",
        "gateway": "  ← gateway",
    }
    for pid, name, cmdline in matches[:6]:
        sub = _curie_holder_subcommand(cmdline)
    ...
    lines.append(
        "  Close the Curie desktop app / other Curie terminals, then re-run:"
    )
    lines.append("    curie update")
    lines.append("  (or use `curie update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _venv_launcher_ancestors(pids: list[int]) -> list[int]:
    ...
    # Never return ourselves or our own ancestry: a CLI ``curie update``
...
def _leftover_pausable_gateway_pids(
    ...
    """
    from curie_cli._scan_venv_blockers import _is_pausable_gateway
...
def _refuse_gateway_ancestor_tree_kill(
    ...
    A chat agent can launch plain ``curie update`` through its terminal tool.
    ...
    try:
        from curie_cli.gateway import _is_pid_ancestor_of_current_process
    ...
    print("  Otherwise, run `curie update` from a separate terminal.")
    return True
...
def _ledger_manual_serve_holders(
    ...
    Curie spawner). Returns the full ledger entries so the relauncher can
    rebuild the launch command from structured host/port/profile instead of
    parsing argv.
    """
    try:
        from curie_cli.process_identity import ledger_entries, spawner_is_dead
    except Exception:
        return []
...
def _relaunch_stopped_serves(token: dict) -> None:
    ...
    if skipped or failed:
        print(
            ...
            "restart them manually (curie serve --host <ip> --port <port>)."
        )
    try:
        from curie_cli.update_receipt import record_step
...
def _orphaned_desktop_backend_pids(
    ...
    shim before spawning curie-setup, and the update-in-progress marker
    ...
    the update with "Curie is still running" while the user stares at zero
    open windows (ryanc's 2026-08-09 01:59/02:17 failures).

    A holder qualifies only when BOTH hold:

    - its cmdline is a Curie backend (``curie_cli.main`` + ``serve`` /
      ``dashboard``), and
    ...
    managed-runtime descendants (the ``.curie-runtime`` interpreter child)
    ...
    def _is_backend(argv_low: str) -> bool:
        return "curie_cli.main" in argv_low and (
    ...
    for pid, _name, cmdline in matches:
        ...
        try:
            ...
            # value round-trips through pid_is_curie at kill time. (Reading
...
def _ledger_reapable_backend_pids(
    ...
    (``curie_cli.process_identity``). A holder qualifies when ALL of:
...
def _handoff_reapable_backend_pids(
    ...
    """PIDs of Curie ``serve``/``dashboard`` backends safe to reap during a
    ...
    run AND no live Desktop shim (``curie.exe``) is open, NOTHING legitimate
    ...
    - Only Curie backends (``curie_cli.main`` + ``serve``/``dashboard``)
    ...
    - Only runs when the CALLER has confirmed the hand-off context
      ...
      ``curie.exe`` shim) — outside that gate this function is never called
...
def _stop_process_trees(
    ...
    ``.curie-runtime`` interpreter child alive and holding the install open
    (#70026). Best effort; never raises.
    """
    from gateway.status import get_process_start_time
    from curie_cli._subprocess_compat import pid_is_curie, windows_hide_flags
    ...
    for entry in pids:
        ...
        try:
            ...
            if not pid_is_curie(
                ...
                logger.debug(
                    "Skipping taskkill of non-Curie or changed PID %s",
                    pid,
                )
                continue
            subprocess.run(
...
def _looks_like_desktop_control_plane(cmdline: str) -> bool:
    """True for this-install ``curie serve`` / ``curie dashboard`` argv.
    ...
    """
    if "curie_cli.main" not in (cmdline or "").lower():
        return False
    return _curie_holder_subcommand(cmdline) in ("serve", "dashboard")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    curie.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Curie...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m curie_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: curie gateway run")
...
def _cold_start_windows_gateway_after_update() -> bool:
    ...
    ``curie gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from curie_cli import gateway_windows
        from curie_cli.gateway import find_gateway_pids
    except Exception as exc:
        raise RuntimeError(
...
def _for_each_systemd_gateway_unit(
    ...
    *,
    ...
    """Process each ``curie-gateway*.service``/``curie-serve*.service`` unit
    ...
    for line in (list_units_stdout or "").strip().splitlines():
        ...
        # ``unit.startswith("curie-serve")`` alone would also accept the
        # unrelated ``curie-server.service`` — require the exact base unit
        # or the hyphenated profile family instead (review on #83595).
        if not (
            unit == "curie-gateway.service"
            or unit.startswith("curie-gateway-")
            or unit == "curie-serve.service"
            or unit.startswith("curie-serve-")
        ):
            continue
...
def _service_unit_supports_graceful_sigusr1_restart(svc_name: str) -> bool:
    ...
    Only ``curie-gateway*`` units run ``gateway/run.py``, which installs the
    SIGUSR1 handler. ``curie-serve*`` units (#83438) don't, so sending them
    ...
    (``curie-gateway-helper`` is fine — profile units are
    ``curie-gateway-<profile>`` — but ``curie-gatewayd``-style names are
    not) can't be sent a SIGUSR1 it doesn't handle.
    """
    return svc_name == "curie-gateway" or svc_name.startswith("curie-gateway-")
...
def _warn_incomplete_gateway_fleet_restart(failed_units: list) -> None:
    ...
    from curie_cli.gateway import is_macos
    ...
    print("    curie gateway status")
    if any(not name.startswith("ai.curie.") for name in ordered):
        print("    systemctl --user restart <unit>   # user-scope")
        print("    sudo systemctl restart <unit>     # system-scope")
    if any(name.startswith("ai.curie.") for name in ordered):
...
def _restart_launchd_gateway_after_update(
    *, supervision_verify: bool = True
    ...
    try:
        ...
        try:
            ...
            print(
                ...
                "Recover manually: curie gateway restart"
            )
    ...
    print(
        ...
        "    Check logs, then: curie gateway restart"
    )
    return [], [current_label]
...
def _restart_macos_launchd_gateways(
    ...
    ``ai.curie.gateway*`` LaunchAgent must reload it — restarting only the
...
def _surviving_gateway_pids_after_failed_restart():
    ...
    when ``curie_cli.gateway`` itself no longer imports, which is one of the
...
def _gateway_service_matches_profile(profile: str, service: object) -> bool:
    ...
    that ``curie-gateway-foobar.service`` was already restarted.  These are
    ...
    return name in {
        f"curie-gateway-{profile}",
        f"ai.curie.gateway-{profile}",
        f"gateway-{profile}",
    }


# Abort recovery lives in its own bounded module (review on #96235). Re-exported
# here because `curie_cli.main` and the update flow below address these names
# through `update_cmd`.
from curie_cli.update_abort_recovery import (  # noqa: E402
...
def _gateway_recovery_partition(
    ...
    only safe source here: re-importing ``curie_cli.gateway`` in the failing
    ...
    unrecoverable.  The fresh child runs a separate ``curie-serve*`` systemd
    ...
    ``curie serve``: it records no spawner, so it reads as ``manual-serve``.
        ...
        for runtime in getattr(plan, "runtimes", ()) or ():
            ...
            if kind == "gateway":
                ...
                if supervisor == "desktop":
                    ...
                    # `curie serve` sets neither CURIE_SPAWN nor
                    # CURIE_PARENT_PID, so it lands here as "manual-serve".
                    # Unit-backed serve runtimes are recovered by the
                    # fresh child's systemd pass, which enumerates
                    # `curie-serve*` from systemd instead of trusting this
                    ...
                    reason = (
                        ...
                        " unit pass when it owns a curie-serve* unit, else"
...
def _warn_gateway_restart_phase_aborted(exc: BaseException, pids) -> None:
    ...
    (e.g. importing ``curie_cli.gateway`` from the freshly pulled checkout)
...
def _refresh_windows_gateway_launchers() -> None:
    ...
    ``curie update`` never touched them, so installs created before the
...
def _refresh_bootstrap_cache_scripts(branch: str = "main") -> None:
    ...
    The Desktop GUI updater (``curie-setup.exe``) executes
    ``$CURIE_HOME/bootstrap-cache/install-<ref>.ps1`` (or ``.sh``) for its
    ...
    try:
        ...
        cache_dir = Path(_m().get_curie_home()) / "bootstrap-cache"
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    if relaunched or unmapped_relaunched:
        ...
        if not ready_pids:
            ...
            print("    Recover with: curie gateway restart")
            raise RuntimeError(
...
def _portable_git_candidates() -> list:
    ...
    The Curie-managed PortableGit tree lives under the SHARED root
    (``<root>/git/...``), not the profile-scoped CURIE_HOME
    (``<root>/profiles/<name>``), so a profile-scoped ``curie update`` must
    ...
    candidates = []
    try:
        for root in (get_default_curie_root(), Path(get_curie_home())):
            candidates.append(
...
def _locate_real_git() -> Optional[Path]:
    ...
    Curie-managed PortableGit copy; accept the first candidate that runs and
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``curie update`` sees a clean tree instead of autostashing every run.
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    ``install.ps1`` forever — so ``curie update``, which ships with the checkout
...
def _rebuild_desktop_after_update(
    ...
    # `curie desktop --build-only` subprocess re-imports the whole CLI stack
    ...
    desktop_build_cmd = [sys.executable, "-m", "curie_cli.main", "desktop", "--build-only"]
    ...
    #
    # Start the build subprocess with the Curie-managed Node on PATH: when
    # `curie update` runs inside the desktop updater chain (Desktop →
    # curie-setup → curie update), the shell PATH customizations are lost,
    # so a bare-PATH child would fail with `node: not found` before cmd_gui can
    # self-heal.
    from curie_constants import with_curie_node_path

    build_env = with_curie_node_path()
    ...
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `curie desktop` to retry)")
        tail = "\n".join((build_result.stdout or "").strip().splitlines()[-15:])
        if tail:
            print(tail)
        from curie_constants import display_curie_home as _dhh
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv that was ever touched by ``sudo pip`` / ``sudo curie`` contains
    root-owned files (classically ``*.dist-info/INSTALLER``). A later normal
    ``curie update`` then dies mid-mutation inside ``uv pip install -e .``
    ("Permission denied (os error 13)") with ``venv/bin/curie`` already
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Curie broken.")
    print("  This usually happens after running curie or pip with sudo. Offending paths:")
...
def _drain_or_signal_gateway_for_update(
    ...
    """Decide how ``curie update`` hands a running gateway over to new code.
    ...
    1. **Gateway is an ancestor of this process** — THREE-WAY DEADLOCK BREAK
       (#100179). When ``curie update`` runs INSIDE the gateway's own
       process tree (the curie-auto-update cron job is the canonical case),
       waiting for that gateway to exit is a circular wait::

           gateway  waits on all in-flight work units (#77184)
             └─ cron agent session waits on the `curie update` process
                  └─ `curie update` waits on the gateway to exit  ← back to A
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        from curie_cli.update_receipt import begin_update_receipt
    ...
    # running Curie runtime, its supervisor, and its running code version —
    ...
    # On Windows, abort early if another curie.exe is holding the venv shim
    ...
    if _m()._is_windows() and not getattr(args, "force", False):
        scripts_dir = _m()._venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _m()._detect_concurrent_curie_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `curie serve` backend) will keep .pyd
    ...
    # the curie.exe shim guard above, but its lock probe only checks the shim
    ...
    if _m()._is_windows() and not getattr(args, "force_venv", False):
        ...
        if _venv_holders:
            ...
            # the spawn ledger proves are orphaned Curie backends — the
            ...
            if _ledger_backends:
                print(
                    ...
                    "Curie backend process(es) hold the venv; stopping their trees"
                )
        ...
        if _venv_holders:
            ...
            if _orphan_backends:
                ...
                # its backend (and any .curie-runtime child) holding the
                # venv. Nothing will respawn an orphan, so reap the tree and
                # re-check instead of dead-ending with "Curie is still
        ...
        if _venv_holders:
            ...
            # `curie serve --host <ip>` powering a REMOTE Desktop holds the
        ...
        if _venv_holders:
            ...
            # the hand-off context those surviving Curie backends are leaks,
            ...
            try:
                ...
                if _scripts_dir is not None:
                    _no_live_shim = not _m()._detect_concurrent_curie_instances(_scripts_dir)
            except Exception:
                _no_live_shim = False
            if _handoff and _no_live_shim:
                ...
                if _handoff_backends:
                    print(
                        f"  ⚠ {len(_handoff_backends)} Curie backend process(es) "
    ...
    # process by design (a CLI `curie update` IS the venv python), and an
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                f"  curl -fsSL {OFFICIAL_REPO_RAW_BASE}/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    # A checkout made before the repository was renamed still has the old
    # address in origin, and every fetch below goes through it. Repoint it
    # first, or the update stalls on a credential prompt for a repository
    # that no longer exists.
    _m()._retarget_stale_origin(git_cmd, _m().PROJECT_ROOT)
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if handed_off_sync:
                print("→ Finishing the dependency install handed off by curie.exe...")
            elif not healthy:
            ...
            if handed_off_sync or not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Curie windows/gateways and re-run: curie update")
            else:
            ...
            if runtime_repaired is not None and not _m()._is_windows():
                ...
                print(
                    "  Any running Curie gateways, Desktop backends, or other "
        ...
        # orphan merge-conflict markers in curie_cli/config.py bricked
        # every user who ran ``curie update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``curie
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``curie update`` again later once a fix lands.")
                    else:
        ...
        # Before this guard, ``curie update`` printed "✓ Code updated!" and
        # reinstalled deps + rebuilt the desktop app against the stale tree —
        # no error, no warning, ``curie doctor`` healthy. Compare pre-pull
        ...
        if pre_pull_sha and post_pull_sha == pre_pull_sha:
            ...
            print(
                ...
                f"git -C {_m().PROJECT_ROOT} checkout {branch} && curie update"
            )
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
        ...
        # ``curie update`` can catch up even when git is already up to date.
        ...
        # the old bytecode (e.g. get_curie_home added to curie_constants).
        removed = _m()._clear_bytecode_cache(_m().PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # marker survives and the next ``curie`` launch finishes the install
        ...
        # symbols in curie_constants or lazy_deps. The dependency install
        ...
        if lazy_ok:
            ...
            print(
                "  ⚠ Lazy-refresh recovery incomplete — run `curie` again "
        ...
        if not import_ok:
            ...
            print("    Run `curie update` again — if it persists, reinstall:")
            print(f"    {OFFICIAL_REPO_WEB_URL}")
        ...
        if sys.platform == "darwin" and had_desktop_app_before_update:
            print()
            print(
                "  ℹ macOS: if Curie re-prompts for permissions you already "
                "granted (toggle shows ON), the stored grant is stale — run "
                "`tccutil reset ScreenCapture com.nousresearch.curie` (repeat "
        ...
        try:
            from curie_cli.macos_tcc_anchor import ensure_tcc_anchor
        ...
        # current — so copy it straight over ~/.curie/cache/model_catalog.json
        ...
        # seed_profile_skills() uses subprocess with an explicit CURIE_HOME so
        # it is not affected by sync_skills()'s module-level CURIE_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's CURIE_HOME env var points at the default or a named profile.
        try:
        ...
        try:
            from curie_cli.profiles import backfill_profile_envs
        ...
        # check `curie curator status`. Self-stamps after printing so it
        ...
        # Self-heal the curie-acp launcher for installs that predate it, so
        # ACP hosts (Zed, JetBrains, Buzz) can resolve Curie on PATH without
        # a reinstall.  No-op on Windows (the launcher migration below owns
        # that) and when already present.
        try:
            _ensure_acp_launcher()
        except Exception as e:
            logger.debug("curie-acp launcher self-heal failed: %s", e)

        # Migrate the Windows curie launchers to the managed binary dir
        # (the default Curie root's bin, next to the managed uv) and repair
        # them if they are missing. Earlier layouts put them inside the git
        # checkout (curie-agent\bin) or put venv\Scripts itself on PATH; the
        # in-checkout copies were swept by this command's own pre-update
        # autostash (git stash push --include-untracked) and, with
        # --keep-stash (the desktop updater), never restored — `curie`
        ...
        # the managed clone under the default Curie root).
        try:
            from curie_cli._install_repair import migrate_windows_bin_path
        ...
        # don't have it. Tying the refresh to ``curie update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from curie_cli.tools_config import install_cua_driver
                ...
                # driver) keeps the installed version — `curie update`
                # must stay fast; `curie computer-use install --upgrade`
        ...
        # When running as ``curie update --gateway`` (spawned by the gateway's
        ...
        # ``user/curie-serve`` and ``system/curie-serve`` are two different
        ...
        #
        # Purge stale cached Curie modules FIRST: the import below pulls
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... curie-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... curie-gateway*)
            ...
            try:
                from curie_cli.gateway import _get_restart_exit_wait_budget
            ...
            # Discover all curie-gateway* units (default + profiles) plus
            # curie-serve* units (the Desktop app's backend, #83438).
            if supports_systemd_services():
                try:
                    _ensure_user_systemd_env()
                except Exception:
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        print(
                            ...
                            f"Check the gateway with: curie gateway status"
                        )
                        continue
                    ...
                    def _restart_one_systemd_gateway_unit(svc_name: str) -> None:
                        ...
                        # curie-serve has no such handler (it isn't
                        ...
                        if _manage_cmd is None:
                            ...
                            print(
                                ...
                                f"    To let `curie update` restart it automatically, allow\n"
            ...
            # Restart EVERY ai.curie.gateway* LaunchAgent, not only the
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: curie gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: curie -p <profile> gateway run  for each profile)"
                        )

            if failed_or_stale_units:
                gateway_fleet_restart_incomplete = True
                if gateway_mode:
                    _exit_code_path = get_curie_home() / ".update_exit_code"
                    try:
            ...
            try:
                from curie_cli.update_receipt import record_gateway_restart
            ...
            # the reporter hit through tui_gateway (hosted by `curie serve`,
        ...
        # Warn if legacy Curie gateway unit files are still installed.
        # When both curie.service (from a pre-rename install) and the
        # current curie-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `curie update` surfaces the issue until the user migrates.
        try:
            from curie_cli.gateway import (
                has_legacy_curie_units,
                _find_legacy_curie_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_curie_units():
                print()
                print("⚠ Legacy Curie gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_curie_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (curie.service) fight the current")
                print("  curie-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    curie gateway migrate-legacy")
                print()
        ...
        #
        # Forward the systemd units restarted above (includes curie-serve*,
        ...
        # `curie serve` (no unit) is left running its pre-update
        ...
        print("  curie model              # Select provider and model")
        ...
        try:
            from curie_cli.update_receipt import (
        ...
        try:
            if _pre_update_plan is not None and _pre_update_plan.runtimes:
                ...
                if report_unaccounted_runtimes(_runtime_outcomes):
                    gateway_fleet_restart_incomplete = True
                try:
                    import curie_cli.update_receipt as _ur
        ...
        if gateway_fleet_restart_incomplete:
            ...
            # ``curie update`` still runs the catch-up restart.
            sys.exit(1)
        _clear_fleet_restart_pending_marker()
            ...
            if _called_process_error_is_python_dep_install(e):
                ...
                if _m()._is_windows():
                    ...
                    print(
                        ...
                        '"from curie_cli.main import main; main()" update --yes'
                    )
            try:
                from curie_cli.update_receipt import finalize_update_receipt
...
def _restart_phase_failure_is_incomplete(surviving, pre_restart_pids) -> bool:
    ...
    * ``surviving is None`` — the survivor probe could not determine state
      (typically the freshly-pulled ``curie_cli.gateway`` no longer imports,
