"""
Skills configuration for Curie Agent.
`curie skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.curie/config.yaml under:
...
from curie_cli.config import cfg_get, load_config, save_config
from curie_cli.colors import Colors, color
from curie_cli.platforms import PLATFORMS as _PLATFORMS
...
def save_disabled_skills(config: dict, disabled: Set[str], platform: Optional[str] = None):
    ...
    Essential skills (e.g. ``curie-agent``) are silently dropped from the
