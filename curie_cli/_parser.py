"""
Top-level argparse construction for the curie CLI.
...
# argparse runs (it sets ``CURIE_HOME`` and strips itself from ``sys.argv``),
...
# tests/curie_cli/test_top_level_value_flags_parity.py fails CI if the parser
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    the exact drift that made ``curie --reasoning high chat …`` misread
...
def _inherited_flag(parser, *args, **kwargs):
    """Register a flag that ``curie_cli.relaunch`` should carry over when
...
_EPILOGUE = """
Examples:
    curie                        Start interactive chat
    curie chat -q "Hello"        Single query mode
    curie --tui                  Launch the modern TUI (or set display.interface: tui)
    curie --cli                  Force the classic REPL (overrides display.interface: tui)
    curie -c                     Resume the most recent session
    curie -c "my project"        Resume a session by name (latest in lineage)
    curie --resume <session_id>  Resume a specific session by ID
    curie --resume latest        Resume the most recent session (same as -c)
    curie --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    curie setup                  Run setup wizard
    curie logout                 Clear stored authentication
    curie auth add <provider>    Add a pooled credential
    curie auth list              List pooled credentials
    curie auth remove <p> <t>    Remove pooled credential by index, id, or label
    curie auth reset <provider>  Clear exhaustion status for a provider
    curie model                  Select default model
    curie fallback [list]        Show fallback provider chain
    curie fallback add           Add a fallback provider (same picker as `curie model`)
    curie fallback remove        Remove a fallback provider from the chain
    curie config                 View configuration
    curie config edit            Edit config in $EDITOR
    curie config set model gpt-4 Set a config value
    curie gateway                Run messaging gateway
    curie -s curie-agent-dev,github-auth
    curie -w                     Start in isolated git worktree
    curie gateway install        Install gateway background service
    curie sessions list          List past sessions
    curie sessions browse        Interactive session picker
    curie sessions rename ID T   Rename/title a session
    curie logs                   View agent.log (last 50 lines)
    curie logs -f                Follow agent.log in real time
    curie logs errors            View errors.log
    curie logs --since 1h        Lines from the last hour
    curie debug share             Upload debug report for support
    curie console                Open the safe Curie command console
    curie update                 Update to latest version
    curie dashboard              Start web UI dashboard (port 9119)
    curie dashboard --stop       Stop running dashboard processes
    curie dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="curie",
        description="Curie Agent - AI assistant with tool-calling capabilities",
    ...
    # Mirrors `curie chat --model ... --provider ...` semantics.
    _inherited_flag(
        parser,
        "-m",
        "--model",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via CURIE_INFERENCE_MODEL env var."
        ),
    )
    _inherited_flag(
        parser,
        ...
        help=(
            ...
            "under model.provider — use `curie setup` or edit the file to change it."
        ),
    )
    _inherited_flag(
        parser,
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to CURIE_ACCEPT_HOOKS=1 or "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.curie/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Curie Agent",
    )
    ...
    # top-level parser: when the user writes `curie -m foo chat`, argparse
    ...
    # `-w/--worktree`, `--unlock`, etc. (see tests/curie_cli/
    ...
    _inherited_flag(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also CURIE_ACCEPT_HOOKS env var and "
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.curie/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Curie itself.",
    )
    chat_parser.add_argument(
        "--source",
        default=None,
