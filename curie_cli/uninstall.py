"""
Curie Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.curie/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Curie PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing curie-agent or curie PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Curie Agent" comment and following line
                if '# Curie Agent' in line or '# curie-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('curie' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing curie
                if 'curie' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
            ...
            if new_content != original_content:
                ...
                # This is the user's own shell rc, not a Curie-owned file, and
...
def remove_wrapper_script():
    """Remove the curie wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "curie",
        Path.home() / ".local" / "bin" / "curie-acp",
        Path.home() / ".local" / "bin" / "curie-agent",
        Path("/usr/local/bin/curie"),
        Path("/usr/local/bin/curie-acp"),
        Path("/usr/local/bin/curie-agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains curie_cli reference)
                content = wrapper.read_text(encoding="utf-8")
                if 'curie_cli' in content or 'curie-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(curie_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``curie`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Curie home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (curie_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``curie gateway run`` processes
    ...
    try:
        from curie_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from curie_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``CURIE_HOME`` and ``CURIE_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\curie\git\cmd``, ``%LOCALAPPDATA%\curie\git\bin``,
#      ``%LOCALAPPDATA%\curie\git\usr\bin``, ``%LOCALAPPDATA%\curie\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\curie\git\`` and Node to
#      ``%LOCALAPPDATA%\curie\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``curie dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\curie\gateway-service\`` and sometimes
...
def _curie_path_markers(curie_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Path-entry substrings that identify Curie-owned User-PATH entries.

    ``include_managed_bin`` adds the managed binary dir (``<root>\\bin``,
    holding the curie launchers and the managed uv) — only wanted when
    ...
    """
    root = str(curie_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare curie-agent install dir.
    markers = [root + "\\curie-agent", root + "\\git", root + "\\node", root + "\\venv"]
    if include_managed_bin:
        markers.append(root + "\\bin")
    # Also match if CURIE_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "curie".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-curie as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(curie_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Curie-owned entries from User-scope PATH in the registry.
    ...
    ``include_managed_bin`` adds ``<curie_home>\\bin`` (the managed binary
    dir holding the curie launchers and the managed uv) to the sweep. Only
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _curie_path_markers(curie_home, include_managed_bin=include_managed_bin)
            kept: list[str] = []
...
def remove_curie_env_vars_windows() -> list[str]:
    """Delete CURIE_HOME and CURIE_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("CURIE_HOME", "CURIE_GIT_BASH_PATH"):
                try:
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``curie`` launchers install.ps1 staged in the managed
    binary dir (the default Curie root's ``bin``, next to the managed uv).

    Every uninstall mode deletes the code checkout, so the launchers —
    which invoke ``<checkout>\\venv\\Scripts`` — would otherwise dangle:
    ``curie`` in a new terminal resolves to a launcher whose target is
    ...
    ``_install_repair._quarantine_running_curie_exe`` relies on), so
    ...
    *windows* is an injectable platform verdict for tests (same pattern as
    ...
    try:
        ...
        from curie_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from curie_constants import get_default_curie_root

        bin_dir = get_default_curie_root() / "bin"
    except Exception as e:
...
def _is_default_curie_home(curie_home: Path) -> bool:
    """Return True when ``curie_home`` points at the default (non-profile) root."""
    try:
        from curie_constants import get_default_curie_root
        return curie_home.resolve() == get_default_curie_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its CURIE_HOME directory.

    We shell out to ``curie -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current CURIE_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m curie_cli.main` so we don't depend on a `curie`
    #    wrapper that may be half-removed mid-uninstall.
    curie_invocation = [_sys.executable, "-m", "curie_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                curie_invocation + ["gateway", subcmd],
                capture_output=True,
    ...
    # 3. Wipe the profile's CURIE_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``curie uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$CURIE_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from curie_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(curie_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ▮ Curie Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Curie Chat GUI installation was found.")
        print(f"  Checked: {curie_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Curie agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(curie_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Curie agent at {curie_home / 'curie-agent'}")
        print(f"  • Your config, sessions, and secrets under {curie_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Curie agent is still installed. Run 'curie' to use the CLI,")
    print("or 'curie uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.curie/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.curie/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie CURIE_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_curie_home(curie_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.curie data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ▮ Curie Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {curie_home / 'config.yaml'}")
    print(f"  Secrets: {curie_home / '.env'}")
    print(f"  Data:    {curie_home / 'cron/'}, {curie_home / 'sessions/'}, {curie_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their CURIE_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Curie data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Curie code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, curie_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Curie PATH entries from shell configs / Windows User PATH")
    print("  • Curie wrapper scripts and Curie-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Curie config/data: {curie_home}")
        if _is_default_curie_home(curie_home):
        ...
        print(f"  • Keep Curie config/data: {curie_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``curie`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$CURIE_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in curie_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\curie\git\cmd, not %LOCALAPPDATA%.
        # The managed binary dir (curie\bin: launchers + managed uv) leaves
        ...
        sweep_managed_bin = full_uninstall and _is_default_curie_home(curie_home)
        removed_path_entries = remove_path_from_windows_registry(
            Path(os.path.expandvars(str(curie_home))),
        ...
        if removed_path_entries:
            ...
            log_info("No Curie-owned PATH entries in User environment")

        log_info("Removing CURIE_HOME / CURIE_GIT_BASH_PATH User env vars...")
        removed_env = remove_curie_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Curie-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing curie command...")
    ...
    #     would dangle — `curie` in a new terminal would resolve and then
    #     error on its missing venv target, worse than command-not-found.
    if _is_windows():
        log_info("Removing Windows curie launchers...")
        ...
        if removed_launchers:
            ...
            log_info("No Windows curie launchers found")

    # 3b. Remove node/npm/npx symlinks the installer left in ~/.local/bin
    #     (only when they still point into this Curie home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Curie-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(curie_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Curie-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(curie_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE CURIE_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from curie_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(curie_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.curie/, just remove the curie-agent subdir
            if curie_home in project_root.parents or project_root.parent == curie_home:
    ...
    #     under CURIE_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(curie_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(curie_home)
    ...
    # 5. Optionally remove ~/.curie/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile CURIE_HOME dirs live under
        ...
        try:
            if curie_home.exists():
                shutil.rmtree(curie_home)
                log_success(f"Removed {curie_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {curie_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {curie_home}")
    ...
    print()
    print("Thank you for using Curie Agent! ▮")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m curie_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import curie_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``curie_constants`` + ``curie_cli.colors``
    (and lazily ``curie_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m curie_cli.uninstall")
    parser.add_argument(
        "--mode",
