...
Pure display functions with no CurieCLI state dependency.
"""
import json
import logging
import os
...
def cprint(text: str):
    ...
    try:
        ...
        # absent stdout (pythonw.exe, CI, `curie ... > file`) raises
...
from curie_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

# Fallback art for when no skin supplies its own. The block letters spell
# CURIE-AGENT; the hero is a strip-chart recorder trace, which is what a
# bench instrument actually draws on paper.
CURIE_AGENT_LOGO = """[bold #8A3B12] ██████╗██╗   ██╗██████╗ ██╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #8A3B12]██╔════╝██║   ██║██╔══██╗██║██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#B4541A]██║     ██║   ██║██████╔╝██║█████╗  █████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║   [/]
[#B4541A]██║     ██║   ██║██╔══██╗██║██╔══╝  ╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║   [/]
[#8A6A21]╚██████╗╚██████╔╝██║  ██║██║███████╗      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║   [/]
...
CURIE_AGENT_LOGO_MEDIUM = """[bold #8A3B12] ██████╗██╗   ██╗██████╗ ██╗███████╗[/]
[bold #8A3B12]██╔════╝██║   ██║██╔══██╗██║██╔════╝[/]
[#B4541A]██║     ██║   ██║██████╔╝██║█████╗  [/]
[#B4541A]██║     ██║   ██║██╔══██╗██║██╔══╝  [/]
[#8A6A21]╚██████╗╚██████╔╝██║  ██║██║███████╗[/]
...
CURIE_AGENT_LOGO_COMPACT = """[bold #8A3B12]╒═══ C U R I E ═══╕[/]
[#8C7A5B]╘═ bench terminal ═╛[/]"""

CURIE_BENCH_HERO = """[#8C7A5B]╔════════════════════════════╗[/]
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "curie-cli-update-check",
            },
        )
...
def _check_via_local_git(repo_dir: Path) -> Optional[int]:
    ...
    if _is_official_ssh_remote(origin_url):
        ...
        # ahead checkout as behind nudges the user into `curie update`,
...
def check_for_updates() -> Optional[int]:
    """Check whether a Curie update is available.

    Two paths: if ``CURIE_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = curie_home / ".update_check"
    embedded_rev = os.environ.get("CURIE_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # CURIE_REVISION (that's nix-only). Returning None makes both the Rich
    # banner (build_welcome_banner) and the Ink badge (branding.tsx, guarded
    # on `typeof === 'number' && > 0`) show nothing. The dashboard's REST
    # `/api/curie/update/check` endpoint short-circuits docker the same way
    # (web_server.py); mirror that here so the banner/TUI surfaces agree.
    try:
        from curie_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $CURIE_HOME/curie-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = curie_home / "curie-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Curie git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$CURIE_HOME/curie-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``curie_cli/build_info.py``) and return it as a frozen
...
def _compute_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    if repo_dir is None:
        ...
        try:
            from curie_cli.build_info import get_build_sha
...
def format_banner_version_label() -> str:
    ...
    base = f"Curie Agent v{VERSION} ({RELEASE_DATE})"
...
def _format_update_notice(behind: int) -> str:
    ...
    from curie_cli.config import get_managed_update_command, recommended_update_command
    if behind > 0:
    ...
    # UPDATE_AVAILABLE_NO_COUNT: nix-built curie; we know an update
...
def _banner_snapshot_path() -> Path:
    return get_curie_home() / "cache" / "banner_snapshot.json"
...
def banner_snapshot_fingerprint() -> Optional[str]:
    ...
    try:
        ...
        for p in (get_config_path(), get_curie_home() / ".env"):
            try:
                st = p.stat()
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else CURIE_BENCH_HERO
    except Exception:
        _bskin = None
        _hero = CURIE_BENCH_HERO
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        if not (model or "").strip() or (model or "").strip().lower() == "unknown":
            ...
            left_lines.append(
                ...
                f"[dim {dim}]— run /model or curie setup[/]"
            )
        else:
    ...
    if os.getenv("CURIE_UNLOCK_MODE"):
        left_lines.append(f"[bold red]⚠ UNLOCK mode[/] [dim {dim}]— all approval prompts bypassed[/]")
    ...
    if not _has_native_mcp:
        try:
            from curie_cli.plugins import get_portable_mcp_server_names_nowait
    ...
    try:
        from curie_cli.codex_runtime_switch import get_current_runtime
        from curie_cli.config import load_config as _load_cfg
    ...
    console.print()
    # The wordmark is sized to the window rather than gated on one width.
    # A single >=95 threshold meant everything narrower — a split pane, a
    # phone-sized SSH session, a docked terminal — got no wordmark at all,
    # while anything at exactly 95 got art that Rich then wrapped mid-glyph
    # into three rows of debris. ``logo_for_width`` steps down through the
    # skin's sizes and returns "" only when even the smallest will not fit,
    # which at 30 columns is the correct answer.
    term_width = shutil.get_terminal_size().columns
    _logo = ""
    if _bskin is not None and hasattr(_bskin, "logo_for_width"):
        _logo = _bskin.logo_for_width(term_width)
    elif term_width >= 95:
        _logo = CURIE_AGENT_LOGO
    if _logo:
        console.print(_logo)
        console.print()
