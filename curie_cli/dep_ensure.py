...
from curie_constants import agent_browser_runnable, find_node_executable
from tools.environments.local import curie_subprocess_env

_IS_WINDOWS = platform.system() == "Windows"

_DEP_CHECKS = {
    # find_node_executable() rather than a bare which(): $CURIE_HOME/node is
...
def _has_curie_agent_browser() -> bool:
    ...
    # install.sh installs globally into $CURIE_HOME/node/bin/ via npm -g --prefix
...
def ensure_dependency(
    ...
    if shell == "powershell":
        ...
        cmd = [
            ...
            "-File", str(script),
            "-Ensure", dep,
            "-CurieHome", str(get_curie_home()),
        ]
    else:
        cmd = ["bash", str(script), "--ensure", dep]

    run_env = curie_subprocess_env(inherit_credentials=False)
