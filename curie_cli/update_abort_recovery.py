...
``curie update`` performs its fleet restart in the interpreter that started
...
itself lives in :mod:`curie_cli.update_restart_recovery`.
"""
...
def _serve_unit_recovery_available() -> bool:
    """Can a fresh process restart ``curie-serve*`` units on this host?"""
...
def _qualified_serve_skips(skip_units) -> list[dict]:
    ...
    ``curie-serve.service`` can exist in BOTH the user and the system
...
def _recover_gateway_restart_after_abort(
    ...
    *,
    ...
    ``curie update`` normally performs the fleet restart in the interpreter
    ...
    The same child additionally restarts active ``curie-serve*`` systemd
    units (#92145).  ``curie serve`` hosts ``tui_gateway.server`` and is
    ...
    """
    from curie_cli.update_cmd import _gateway_recovery_partition
    ...
    command = [
        sys.executable,
        "-m",
        "curie_cli.update_restart_recovery",
        "--stdin",
    ]
    env = os.environ.copy()
    env["CURIE_UPDATE_RESTART_RECOVERY"] = "1"
    for marker in ("_CURIE_GATEWAY", "CURIE_GATEWAY", "CURIE_GATEWAY_MODE"):
        env.pop(marker, None)
...
def _warn_stale_serve_runtimes(rows) -> None:
    ...
    terminal naming the responsible process. ``curie serve`` hosts
    ...
    print(
        "    Restart them before using Curie again, e.g."
        " `systemctl --user restart curie-serve.service`"
        " or by relaunching `curie serve` / the Desktop app."
    )
...
def _abort_recovery_is_complete(
    *,
    ...
    ``gateway_state.json`` / control socket — so a ``curie serve`` still
    ...
    * no ``curie-serve*`` unit failed to produce a verified replacement; and
