"""MCP picker — interactive `curie mcp picker` (also the default `curie mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``curie mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `curie plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from curie_cli.cli_output import prompt_yes_no
from curie_cli.curses_ui import curses_single_select
from curie_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Curie session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from curie_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `curie mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: curie mcp install <name>    Picker: curie mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Curie is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Curie — run `curie update` "
...
def show_catalog() -> None:
    """`curie mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`curie mcp picker` (and default `curie mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`curie mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `curie mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
