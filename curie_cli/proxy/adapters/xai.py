...
# xAI's public API is OpenAI-compatible for the endpoints Curie commonly
# uses. The Responses endpoint is included because Curie' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Curie-managed OAuth credentials."""

    auth_hint = "curie auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`curie auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`curie auth reset xai-oauth` or re-authenticate with "
                    "`curie auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `curie auth add xai-oauth --type oauth`."
            )
