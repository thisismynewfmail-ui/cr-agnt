"""
Profile management for multiple isolated Curie instances.

Each profile is a fully independent CURIE_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.curie/profiles/<name>/`` by default.

The "default" profile is ``~/.curie`` itself — backward compatible,
zero migration needed.

Usage::

    curie profile create coder          # fresh profile + bundled skills
    curie profile create coder --clone  # also copy config, .env, SOUL.md, skills
    curie profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    curie -p coder chat                 # or via flag
    curie profile use coder             # set as sticky default
    curie profile delete coder          # remove profile + alias + service
"""
...
from curie_cli.archive_safe import (
    archive_root_dirs,
    make_targz,
...
_PROFILE_DIRS = [
    ...
    # See curie_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `curie profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `curie
...
# `curie skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Curie infrastructure directories that only the default profile
         (``~/.curie``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_curie_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.curie) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "curie-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when CURIE_HOME equals the
...
# into the export. The set below identifies the *known Curie profile
# artifacts* at the root of CURIE_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``curie_constants``.
...
_RESERVED_NAMES = frozenset({
    "curie", "default", "test", "tmp", "root", "sudo",
})

# Curie subcommands that cannot be used as profile names/aliases
_CURIE_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the curie root, NOT to the current CURIE_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where CURIE_HOME points outside
    ``~/.curie``, profiles live under ``CURIE_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_curie_home() / "profiles"


def _get_default_curie_home() -> Path:
    """Return the default (pre-profile) CURIE_HOME path.

    In standard deployments this is ``~/.curie``.
    In Docker/custom deployments where CURIE_HOME is outside ``~/.curie``
    (e.g. ``/opt/data``), returns CURIE_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_curie_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Curie installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its CURIE_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_curie_home()
...
def profile_matches_home(name: str, home: "Path | None" = None) -> bool:
    ...
    ``home`` defaults to the process's current Curie home
    (:func:`curie_constants.get_curie_home`).  Used by single-profile
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, curie subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _CURIE_SUBCOMMANDS:
        return f"'{canon}' conflicts with a curie subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            curie_exe = shutil.which("curie") or "curie"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(curie_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Curie. If we
    ...
    try:
        ...
        # not be migrated. The next `curie doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``curie -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``curie profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``curie -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``curie profile list`` for an unrelated profile.
    """
...
def _seed_model_config(profile_dir: Path) -> None:
    ...
    try:
        ...
        source = get_curie_home() / "config.yaml"
        ...
        # Creation must not fail over this; `curie model` still sets it later.
        pass
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``CURIE_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Curie config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_curie_home()
...
def profiles_to_serve(
    ...
    """Return the ``(profile_name, curie_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own CURIE_HOME. When
    ...
    The returned ``curie_home`` is the path to pass to
    ``set_curie_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_curie_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``curie update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.curie)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.curie/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
        ...
        from curie_cli.auth import strip_cloned_single_use_oauth_grants
    ...
    # Keys pages, `curie -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Curie profile.\n"
    ...
    if not soul_path.exists():
        try:
            from curie_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `curie update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`curie profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Curie (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `curie profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `curie -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches CURIE_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``curie profile create
    ...
    only the essential skills (e.g. ``curie-agent``), reporting
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "CURIE_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_curie_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Curie *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Curie backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``CURIE_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `curie -p <canon> profile
    ...
    curie_markers = ("curie_cli.main", "curie-gateway", "tui_gateway")
    ...
    # something (e.g. Electron's `findOnPath('curie')` resolution) spawns the
    ...
    # interpreter, not "curie", so the checks below would otherwise miss it.
    ...
    # interpreter.
    _CURIE_CONSOLE_SCRIPT_NAMES = frozenset({"curie", "curie-agent", "curie-acp"})
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Curie process: either an entrypoint marker in argv, a
            # resolved executable named `curie`, or a python interpreter
            # directly exec'ing a `curie`-named console-script shim (argv[0]
            # is the interpreter, argv[1] is the shim's path).
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_curie = (
                any(marker in joined for marker in curie_markers)
                or exe_name == "curie"
                or exe_name.startswith("curie")
            )
            if not is_curie and len(argv) >= 2 and _python_interpreter_re.match(exe_name):
                # Match against the actual known console-script entry points
                # (pyproject.toml [project.scripts]: curie, curie-agent,
                # curie-acp) rather than a bare `startswith("curie")` --
                ...
                # "curie-notes.py" or "curie-unrelated-tool" as the shim,
                ...
                is_curie = script_stem in _CURIE_CONSOLE_SCRIPT_NAMES
            if not is_curie:
                continue
            ...
            # ...or by CURIE_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("CURIE_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.curie).\n"
            "To remove everything, use: curie uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_curie_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``CURIE_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from curie_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set CURIE_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("CURIE_HOME")
    try:
        os.environ["CURIE_HOME"] = str(profile_dir)
        from curie_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["CURIE_HOME"] = old_home
        elif "CURIE_HOME" in os.environ:
            del os.environ["CURIE_HOME"]
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        pid = int(data["pid"])
        # Cross-profile kill refusal (#89315): the record's curie_home stamp
        ...
        if recorded_gateway_home_conflicts(data, expected_home=profile_dir):
            print(
                f"✗ Refusing to stop PID {pid}: its recorded CURIE_HOME "
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.curie/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: curie profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from CURIE_HOME.

    Returns ``"default"`` if CURIE_HOME is not set or points to ``~/.curie``.
    Returns the profile name if CURIE_HOME points into ``~/.curie/profiles/<name>``.
    Returns ``"custom"`` if CURIE_HOME is set to an unrecognized path.
    """
    from curie_constants import get_curie_home
    curie_home = get_curie_home()
    resolved = curie_home.resolve()

    default_resolved = _get_default_curie_home().resolve()
...
def _inside_git_checkout(path: Path) -> bool:
    ...
    ``CURIE_HOME`` points inside a checkout but the process runs from
...
def _profile_export_directory() -> Path:
    ...
    export_dir = _get_default_curie_home() / "profile-exports"
    if not _inside_git_checkout(export_dir):
        return export_dir

    # A custom deployment may point CURIE_HOME at its source checkout.  Do
    ...
    # Per-uid temp name: a fixed /tmp/curie-profile-exports is a predictable
    # shared path another local user could pre-create (or symlink) before us.
    uid_suffix = f"-{os.getuid()}" if hasattr(os, "getuid") else ""
    candidates = (
        Path.home() / ".curie-profile-exports",
        Path(tempfile.gettempdir()) / f"curie-profile-exports{uid_suffix}",
    )
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      CURIE_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Curie file the
      user may have lying alongside CURIE_HOME (#58394).
...
def _scrub_export_secrets(staged: Path) -> None:
    ...
    ``curie sessions export --redact``. Runs on the *staged copy only* so the
    live profile is never rewritten. ``force=True`` ignores
    ``security.redact_secrets`` / ``CURIE_REDACT_SECRETS`` — share archives
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.curie itself — its parent is ~/ and its
        # directory name is ".curie", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: curie profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.curie itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.curie). "
            "Specify a different name: curie profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="curie_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"curie_{old_name}"
    legacy_old_host = f"curie.{old_name}"
    new_host = f"curie_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_curie_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("curie_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a CURIE_HOME path string.

    Called early in the CLI entry point, before any curie modules
    are imported, to set the CURIE_HOME environment variable.

    When CURIE_HOME is already set, the configured spelling IS the
