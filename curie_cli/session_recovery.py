"""Offline, non-destructive recovery for a damaged Curie session database.
...
def _copy_source_bundle(source: Path, snapshot_dir: Path) -> tuple[Path, list[str]]:
    ...
    locks -- the failure class ``curie_cli.sqlite_safe_read`` exists to
    ...
    keeps this path consistent with ``curie_state._backup_db_file``.
    """
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Curie process using this profile and retry. "
                "This includes the interactive `curie` CLI session this "
                ...
                "Curie process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `curie` session open, "
...
def _recover_via_lost_and_found(
    *,
    ...
    from curie_cli.session_lost_and_found import (
