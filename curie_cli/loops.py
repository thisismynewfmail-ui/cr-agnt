...
Design notes / invariants (same contract as ``curie_cli/goals.py``):
...
- This module has zero hard dependency on ``cli.CurieCLI``, the gateway
...
def _get_session_db() -> Optional[Any]:
    ...
    """
    try:
        from curie_cli.goals import _get_session_db as _goals_db
...
def save_loop(session_id: str, state: LoopState) -> None:
    ...
    db = _get_session_db()
    if db is None:
        from curie_cli.goals import _warn_dropped_write
