...
1. **Update receipt** — a machine-readable JSON record of what one
   ``curie update`` run discovered, did, skipped (and why), written to
   ``<CURIE_HOME>/logs/update_receipts/``. Silent-failure classes this
...
``curie_cli.build_info.get_code_identity()``: an image build reports
...
# Module-level current receipt. ``curie update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``curie update`` run."""
    ...
    def gateway_restart_result(
        ...
        *,
        ...
        if fresh_recovery is not None:
            ...
            # Serve/dashboard coverage (#92145). ``curie serve`` hosts
...
def _receipt_dir() -> Path:
    ...
    return get_curie_home() / "logs" / _RECEIPT_DIR_NAME
...
def finalize_pending_update_receipt(
    ...
    ``curie update`` has many early-termination paths (Windows
