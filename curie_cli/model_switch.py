...
- ``curie_cli.providers``        -- canonical provider identity + overlays
- ``curie_cli.model_normalize``  -- per-provider name formatting
...
from curie_cli.model_normalize import (
...
def _declared_model_ids(value: Any) -> list[str]:
    ...
    if isinstance(value, dict):
        for model_id in value:
            # Backward compat: pre-fix Curie wrote sentinel keys inside the
...
def _entry_models_discovered(entry: Any) -> bool:
    """True when the entry's ``models`` mapping was auto-discovered by Curie.

    The current shape is an entry-level ``models_discovered: true`` sibling of
    ``models``. Older Curie versions wrote an in-mapping
...
def _models_config_is_allowlist(value: Any, discovered: bool = False) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``curie model`` wizard — not a
    ...
    while ``curie model`` still live-probed the full ``/v1/models`` list.
    ...
    ``_entry_models_discovered``): a catalog Curie itself persisted after a
    ...
    *,
    ...
    try:
        ...
        for entry in providers:
            ...
            # A mapping Curie itself discovered (``models_discovered: true``
...
_CURIE_MODEL_WARNING = (
    ...
    "for use with Curie Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Hermes 3 / Hermes 4 chat families.
# The previous substring check (`"curie" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``curie-brain:qwen3-14b-ctx16k`` that just
# happen to carry "curie" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Hermes-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   curie-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_CURIE_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])curie[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
# ---------------------------------------------------------------------------
def is_nous_curie_non_agentic(model_name: str) -> bool:
    ...
    if not model_name:
        return False
    return bool(_NOUS_CURIE_NON_AGENTIC_RE.search(model_name))


def _check_curie_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Hermes 3/4 chat model."""
    if is_nous_curie_non_agentic(model_name):
        return _CURIE_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``curie config set model.aliases.xxx``
...
# switch (CURIE_HOME moves, so the path moves) and a config/key rotation
...
def _direct_alias_source_identity() -> Optional[tuple]:
    ...
    """
    try:
        from curie_constants import get_config_path
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from curie_cli.model_switch import
...
def resolve_startup_model_route(
    ...
    *,
    ...
    ``CurieCLI`` is constructed before the interactive ``/model`` pipeline
    ...
    if current_provider:
        try:
            ...
            if _is_routing_agg(_norm_prov(current_provider)):
                from curie_cli.models import _find_openrouter_slug
    ...
    try:
        from curie_cli.models import normalize_provider
...
def resolve_display_context_length(
    ...
    if config_context_length is not None and (
        ...
        try:
            from curie_cli.route_identity import should_clear_context_pin
...
async def resolve_display_context_length_async(
    ...
    ``curie_cli.route_identity.should_clear_context_pin_async``, which
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'curie model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'curie doctor' — config issues detected:"
        ...
        from curie_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from curie_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
        ...
        if not new_model:
            if pdef.base_url:
                from curie_cli.runtime_provider import _auto_detect_local_model
    ...
    # =================================================================
        ...
        try:
            from curie_cli.config import load_config
            from curie_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from curie_cli.providers import resolve_user_provider as _ruser
        ...
        if current_provider == "custom" and current_base_url:
            try:
                from curie_cli.models import should_use_ollama_native_catalog
    ...
    # curie_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    # kimi) the same way.
    from curie_cli.models import opencode_provider_family as _oc_family_fn
    if _oc_family_fn(target_provider) is not None and isinstance(base_url, str):
        from curie_cli.models import normalize_opencode_base_url
    ...
    curie_warn = _check_curie_model_warning(new_model)
    if curie_warn:
        warnings.append(curie_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from curie_cli.config import normalize_extra_headers
...
def _prefetch_provider_models_parallel(provider_slugs: list[str]) -> None:
    ...
    :param provider_slugs: Curie provider IDs to prefetch (e.g. ``["openrouter",
        "anthropic", "deepseek"]``).  Unknown providers are silently skipped.
    """
    from curie_cli.models import cached_provider_model_ids
    ...
    def _fetch_one(slug: str) -> None:
        try:
            ...
            if models:
                from curie_cli.models import update_provider_cache_entry
...
def _collect_authed_provider_slugs(
    ...
    from curie_cli.auth import PROVIDER_REGISTRY, _load_auth_store
    from curie_cli.providers import CURIE_OVERLAYS, ALIASES as _PROVIDER_ALIAS_TABLE
    from curie_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS, CANONICAL_PROVIDERS

    _excluded_set = {str(p).strip().lower() for p in excluded if p}
    slugs: list[str] = []
    seen: set[str] = set()

    # --- Section 1: Curie-mapped providers (PROVIDER_TO_MODELS_DEV) ---
    for curie_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        _alias_target = _PROVIDER_ALIAS_TABLE.get(curie_id)
        if (
            _alias_target
            and _alias_target != curie_id
        ...
        try:
            ...
            pass
        if _canonical != curie_id:
            continue
        if curie_id.lower() in seen:
            continue
        if curie_id.lower() in _excluded_set or mdev_id.lower() in _excluded_set:
            continue
        pdata = models_dev_data.get(mdev_id)
        if not isinstance(pdata, dict):
            continue
        pconfig = PROVIDER_REGISTRY.get(curie_id)
        if pconfig and pconfig.auth_type != "api_key":
            continue
        from curie_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(curie_id):
            continue
        ...
        if not has_creds:
            try:
                ...
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(curie_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        curie_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if has_creds:
            slugs.append(curie_id)
            seen.add(curie_id.lower())

    # --- Section 2: Curie-only providers (CURIE_OVERLAYS) ---
    _mdev_to_curie = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    for pid, overlay in CURIE_OVERLAYS.items():
        if pid.lower() in seen:
            continue
        curie_slug = _mdev_to_curie.get(pid, pid)
        if curie_slug.lower() in seen:
            continue
        if pid.lower() in _excluded_set or curie_slug.lower() in _excluded_set:
            continue
        has_creds = False
        ...
        if not has_creds:
            try:
                ...
                if pid in providers_store or curie_slug in providers_store:
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
            try:
                if _credential_pool_is_usable(curie_slug):
                    has_creds = True
            except Exception:
                pass
        if has_creds:
            slugs.append(curie_slug)
            seen.add(pid.lower())
            seen.add(curie_slug.lower())
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from curie_cli/models.py (OPENROUTER_MODELS,
    ...
    from curie_cli.config import coerce_provider_id, stringify_provider_map
    ...
    # Compared against curie_id / mdev_id (section 1), pid / curie_slug
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from curie_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by curie provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs so
    # newly added Portal models surface in the /model picker without
    # requiring a Curie release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from curie_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from curie_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Curie-mapped providers ---
    from curie_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from curie_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for curie_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if (
            ...
            continue
        # Resolve the canonical provider profile name.  Skip curie_ids
        # that are mere aliases resolving to a different canonical profile
        # (e.g. "kimi" and "moonshot" both → "kimi-coding").  Only process
        # entries whose curie_id matches the canonical profile name so
        ...
        # same curie_id).  Distinct canonical profiles that share a
        ...
        if slug.lower() in seen_slugs:
            continue
        if curie_id.lower() in _excluded or mdev_id.lower() in _excluded:
            continue
        ...
        # section 2 (CURIE_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Curie may not route yet.
        ...
        if not has_creds:
            try:
                from curie_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(curie_id)
                )
        ...
        # /model picker sees the SAME list `curie model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(curie_id)
        if not model_ids:
            model_ids = curated.get(curie_id, [])
            if curie_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(curie_id, model_ids)
    ...
    # --- 2. Check Curie-only providers (nous, openai-codex, copilot, opencode-go) ---
    from curie_cli.providers import CURIE_OVERLAYS
    from curie_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Curie provider ID.
    # CURIE_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Curie IDs ("copilot").
    _mdev_to_curie = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}
    ...
    for pid, overlay in CURIE_OVERLAYS.items():
        ...
        # Resolve Curie slug — e.g. "github-copilot" → "copilot"
        curie_slug = _mdev_to_curie.get(pid, pid)
        if curie_slug.lower() in seen_slugs:
            continue
        if pid.lower() in _excluded or curie_slug.lower() in _excluded:
            continue
        ...
        if getattr(overlay, "keyless", False):
            ...
            has_creds = _has_aws_sdk_creds_for_listing(curie_slug)
        elif overlay.auth_type == "vertex":
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or curie_slug in providers_store):
                    has_creds = True
            except Exception as exc:
        ...
        if not has_creds:
            try:
                if _credential_pool_is_usable(curie_slug):
                    ...
                    try:
                        from agent.credential_pool import load_pool
                        _pool = load_pool(curie_slug)
                        if _pool.has_credentials():
                            has_creds = True
                    except Exception:
                        pass
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", curie_slug, exc)
        ...
        # configured.
        if not has_creds and curie_slug == "anthropic":
            try:
                ...
                if (curie_creds and curie_creds.get("accessToken")) or \
        ...
        if curie_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(curie_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(curie_slug)
                model_ids = _ids if _ids else (curated.get(curie_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(curie_slug, []) or curated.get(pid, [])
        elif curie_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `curie model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from curie_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(curie_slug, []) or curated.get(pid, [])
                if curie_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(curie_slug, model_ids)
        total = len(model_ids)
        if curie_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": curie_slug,
            "name": get_label(curie_slug),
            "is_current": curie_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "curie",
        ...
        seen_slugs.add(curie_slug.lower())
        _record_builtin_endpoint(curie_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or CURIE_OVERLAYS (keeps /model in sync
    # with `curie model`).
    try:
        from curie_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Curie writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Curie's own writer uses (mirrors section-4 grouping).
            ...
            # ``curie model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``curie model``. Pin with
            ...
            if _probe_live:
                ...
                try:
                    from curie_cli.models import cached_fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Curie's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, curie_cli/config.py) already consume that dict.
        ...
        for grp in groups.values():
            ...
            #   ``curie model``. Pin a dict catalog with
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from curie_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`curie_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from curie_cli.models import fetch_openrouter_models
