"""Shell completion script generation for curie CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_curie_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Curie Agent bash completion
# Add to ~/.bashrc:
#   eval "$(curie completion bash)"

_curie_profiles() {{
    local profiles_dir="$HOME/.curie/profiles"
...
_curie_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_curie_profiles)" -- "$cur"))
        return
    fi
...
complete -F _curie_completion curie
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef curie
# Curie Agent zsh completion
# Add to ~/.zshrc:
...
_curie_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.curie/profiles" ]]; then
        profiles+=($HOME/.curie/profiles/*(N/:t))
    fi
...
_curie() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_curie_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'curie command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Curie Agent fish completion",
        "# Add to your config:",
        "#   curie completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __curie_profiles",
        "    echo default",
        "    if test -d $HOME/.curie/profiles",
        "        for d in $HOME/.curie/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c curie -f -s p -l profile"
        " -d 'Profile name' -xa '(__curie_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__curie_profiles)' -d 'Profile name'"
                )
