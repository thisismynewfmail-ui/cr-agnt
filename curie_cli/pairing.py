...
Usage:
    curie pairing list              # Show all pending + approved users
    curie pairing approve <platform> <request-id|code>  # Approve a pairing request
    curie pairing revoke <platform> <user_id> # Revoke user access
    curie pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle curie pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: curie pairing {list|approve|revoke|clear-pending}")
        print("Run 'curie pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: curie pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.curie/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'curie pairing list' to see pending requests.\n")
