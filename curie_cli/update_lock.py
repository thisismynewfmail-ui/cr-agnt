"""Cross-process mutual exclusion for in-flight Curie updates.

Three different surfaces can start an update of the same install tree:

* ``curie update`` from a terminal,
* the dashboard's Update button (``POST /api/curie/update`` →
  ``_spawn_curie_action(["update"])``, detached),
* the desktop's Update button, which hands off to the Tauri
  ``curie-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``curie update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <CURIE_HOME>/.curie-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``curie update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Curie is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``curie-setup`` binary under ``~/.curie`` is only refreshed by a full
  installer run (``copy_self_to_curie_home`` deliberately no-ops during
  ...
  ancestry check those users get exit 2 ("Curie is still running") on every
  GUI update forever, with no Curie process actually running.
"""
...
MARKER_NAME = ".curie-update-in-progress"

# Set by an orchestrating updater (the Tauri `curie-setup --update` flow) to
# its own pid before spawning `curie update` as a child stage. The parent
...
HANDOFF_PID_ENV = "CURIE_UPDATE_HANDOFF_PID"
...
# to show "Curie is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Curie home (never the context-local profile override):
    the Rust updater resolves ``$CURIE_HOME`` or the platform default, and the
    ...
    return get_process_curie_home() / MARKER_NAME
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``curie update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``curie-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Curie update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `curie update` as a stage): we run under ITS claim
