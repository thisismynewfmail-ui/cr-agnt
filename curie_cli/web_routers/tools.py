...
late-binding seam in :mod:`curie_cli.web_deps` (``late`` for callables,
...
from curie_cli.web_deps import late, LateState
from curie_cli.web_models import (
...
_log = logging.getLogger("curie_cli.web_server")
...
_spawn_curie_action = late("_spawn_curie_action")
...
async def get_toolsets(profile: Optional[str] = None):
    ...
    from curie_cli.platforms import platform_label
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `curie tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `curie computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``curie tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `curie tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``curie tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.curie/.env`` via ``save_env_value`` —
    the same store ``curie tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``curie tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``curie -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same CURIE_HOME the rest of the
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``curie computer-use permissions grant`` as a background action.
