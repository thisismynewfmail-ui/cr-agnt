"""curie hooks — inspect and manage shell-script hooks.

Usage::

    curie hooks list
    curie hooks test <event> [--for-tool X] [--payload-file F]
    curie hooks revoke <command>
    curie hooks doctor

Consent records live under ``~/.curie/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.curie/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``curie hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: curie hooks {list|test|revoke|doctor}")
        print("Run 'curie hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.curie/config.yaml.")
        print("See `curie hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.curie/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `curie hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `curie hooks test` and `curie hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Curie wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `curie hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `curie hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `curie hooks doctor`.")
