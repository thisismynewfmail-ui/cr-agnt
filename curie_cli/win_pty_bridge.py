"""Windows ConPTY bridge for the `curie dashboard` chat tab.

Drop-in counterpart to ``curie_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``curie_cli.web_server`` consumes: ``spawn``, ``read``,
