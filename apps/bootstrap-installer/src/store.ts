...
export const $curieHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.curie/logs/bootstrap-installer.log')
    $curieHome.set('~/.curie')
  ...
  try {
    const [logPath, curieHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_curie_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $curieHome.set(curieHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Curie" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Curie-Setup.exe --update);
...
export async function launchCurieDesktop(): Promise<void> {
  ...
  await invoke('launch_curie_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Curie repository' },
