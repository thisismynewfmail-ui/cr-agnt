...
//!
//! Resolution order:
//!   1. Dev shortcut: a sibling repo checkout via $CURIE_SETUP_DEV_REPO_ROOT
...
//! Electron app's APP_ROOT/../.. trick, because Curie-Setup.exe is meant
...
/// The official repository, as an `owner/name` slug. Stated once so the
/// bootstrap fetches its install script from the same place the installer it
/// downloads goes on to clone from: the two were written out separately, and
/// a rename only ever reaches one of them.
const OFFICIAL_REPO_SLUG: &str = "thisismynewfmail-ui/cru";
...
///
/// `pin` is the commit-or-branch from either Curie-Setup's build-time
...
pub async fn resolve(
    ...
    // 1. Dev shortcut.
    if let Ok(repo_root) = std::env::var("CURIE_SETUP_DEV_REPO_ROOT") {
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    ...
    let response = reqwest::Client::builder()
        ...
        .get(&url)
        .header("User-Agent", "curie-setup/0.0.1")
        .send()
        .await
...
mod tests {
    ...
    fn upgrade_cached_script_adds_bom_to_legacy_ps1() {
        ...
        let dir = std::env::temp_dir().join(format!("curie-bom-test-{}", std::process::id()));
    ...
    fn upgrade_cached_script_leaves_sh_untouched() {
        let dir = std::env::temp_dir().join(format!("curie-bom-sh-test-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
