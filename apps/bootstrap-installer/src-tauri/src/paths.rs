...
//!
//! Mirrors `curie_constants.get_curie_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\curie
//!   macOS:   ~/.curie
//!   Linux:   ~/.curie  (override via $CURIE_HOME)
//!
//! NOTE (macOS): Python's get_curie_home(), scripts/install.sh, and the
//! Electron desktop's resolveCurieHome() ALL use ~/.curie on macOS — there
...
/// Returns the canonical Curie home directory, respecting $CURIE_HOME if set.
pub fn curie_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("CURIE_HOME") {
    ...
    {
        // %LOCALAPPDATA%\curie — matches scripts/install.ps1's $CurieHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("curie");
        }
    }

    // macOS + Linux + fallback: ~/.curie (matches Python get_curie_home(),
    // install.sh, and the Electron desktop's resolveCurieHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".curie");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    curie_home().join("bootstrap-cache")
}
...
/// CURIE_HOME so it survives repo checkout deletion (unlike anything under
/// curie-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\curie\curie-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_curie`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under CURIE_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves CURIE_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    curie_home().join(".curie-update-in-progress")
}
...
pub fn copy_self_to_curie_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to CURIE_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_CURIE_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".curie-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under CURIE_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[curie-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("CURIE_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_curie_home() -> String {
    curie_home().to_string_lossy().into_owned()
}
