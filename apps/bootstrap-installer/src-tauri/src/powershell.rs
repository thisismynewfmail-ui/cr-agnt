...
/// read side does not see EOF until the last of them closes it. `curie update`
...
/// stranded with it.
///
/// Same bound `Invoke-CurieStep` grew in `scripts/desktop-update/windows.ps1`
...
///
/// `curie_home_override` propagates to the child as $CURIE_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, curie_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = curie_home_override {
        cmd.env("CURIE_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, curie_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = curie_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_curie_home() {
    ...
    /// child. `curie update` is this shape -- the Electron build alone is
    /// megabytes.
    #[cfg(unix)]
    #[tokio::test]
