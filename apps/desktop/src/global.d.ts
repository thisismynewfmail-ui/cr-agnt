import type { GatewayWsUrlResult } from '@curie/shared'
import type { TranslucencyState } from '@curie/shared/translucency'
...
declare global {
  interface Window {
    curieDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<CurieConnection>
      ...
      // Resume this session in the user's own terminal emulator (`curie --tui
      ...
      // Curie Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new CURIE_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: CurieApiRequest) => Promise<T>
      notify: (payload: CurieNotification) => Promise<boolean>
      ...
      readFileText: (filePath: string) => Promise<CurieReadFileTextResult>
      ...
      readPluginSource?: (filePath: string) => Promise<CurieReadFileTextResult>
      selectPaths: (options?: CurieSelectPathsOptions) => Promise<string[]>
      ...
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<CuriePreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<CuriePreviewWatch>
      ...
      watchDirectory?: (dir: string) => Promise<CuriePreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: CurieActiveWork) => void
      setTitleBarTheme?: (payload: CurieTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<CurieReadDirResult>
      ...
      // Local Desktop runtime-plugin root (<CURIE_HOME>/desktop-plugins),
      ...
      /** LOCAL `<CURIE_HOME>/logs` (profile-aware) — error card "Open Logs". */
      logsRoot?: () => Promise<string>
      // Local AGENT-plugin root (<CURIE_HOME>/plugins), same Electron-local
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<CurieGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<CurieGitBranch[]>
        ...
        baseBranchList: (repoPath: string) => Promise<CurieGitBaseBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<CurieRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: CurieReviewScope, baseRef?: null | string) => Promise<CurieReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: CurieReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<CurieReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<CurieRepoPullRequests>
          ...
          fetchPrComment: (repoPath: string, url: string) => Promise<CuriePrComment | null>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: CurieTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<CurieTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: CurieWindowState) => void) => () => void
      ...
      onPreviewFileChanged: (callback: (payload: CuriePreviewFileChanged) => void) => () => void
...
export interface CurieTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface CurieTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `curie update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  curieRoot?: string
...
export interface CurieConnection {
  baseUrl: string
  darwinMajor?: number
...
export interface CurieTitleBarTheme {
  background: string
  foreground: string
}

/** Turns in flight, so the main process can confirm before a quit kills them. */
export interface CurieActiveWork {
  count: number
  titles: string[]
}

export interface CurieWindowState {
  darwinMajor?: number
  isFullscreen: boolean
  isMinimized?: boolean
  isVisible?: boolean
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Curie Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Curie Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Curie Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the curie gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Curie Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface CurieApiRequest {
  path: string
  method?: string
  body?: unknown
...
export interface CurieNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface CuriePreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface CurieReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface CuriePreviewWatch {
  id: string
  path: string
}
...
export interface CurieGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface CurieGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
  isRemote: boolean
...
// preselect it.
export interface CurieGitBaseBranch {
  name: string
  isRemote: boolean
  isDefault: boolean
}
...
export interface CurieRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface CurieRepoStatus {
  branch: null | string
...
// turn began.
export type CurieReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface CurieReviewFile {
  path: string
  added: number
  removed: number
...
export interface CurieReviewList {
  files: CurieReviewFile[]
...
export interface CurieReviewPr {
  url: string
  state: string
  number: number
}
...
export interface CurieBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface CurieRepoPullRequests {
  ghReady: boolean
  prs: CurieBranchPullRequest[]
}
...
export interface CuriePrComment {
  author: string
  body: string
  diffHunk: string
  kind: 'issue' | 'review'
...
export interface CurieReviewShipInfo {
  ghReady: boolean
  pr: CurieReviewPr | null
}

export interface CurieReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface CurieReadDirResult {
  entries: CurieReadDirEntry[]
  error?: string
}

export interface CuriePreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface CurieSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
