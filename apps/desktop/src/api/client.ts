import { JsonRpcGatewayClient } from '@curie/shared'

import type { CurieApiRequest } from '@/global'
...
// "Timed out connecting to Curie backend" that hangs the UI (#48504).
//
...
export class CurieGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Curie gateway connection closed',
      connectErrorMessage: 'Could not connect to Curie gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Curie gateway is not connected',
...
// process to route them to that host (curie:api's registry branch). Null
...
export function curieApi<T>(request: CurieApiRequest): Promise<T> {
  return window.curieDesktop.api<T>({ ...connectionScoped(), ...request })
}
