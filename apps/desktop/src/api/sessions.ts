...
import { capabilityScoped, getApiRequestConnection, curieApi, type ProfileScope, profileScoped } from './client'
...
export async function listSessions(
  ...
  const result = await curieApi<PaginatedSessions>({
    ...profileScoped(),
    path:
...
export async function listSidebarSessions(req: SidebarSessionsRequest): Promise<SidebarSessionsResponse> {
  ...
  try {
    result = await curieApi<SidebarSessionsResponse>({
      ...profileScoped(),
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return curieApi<SessionSearchResponse>({
...
export function getSessionMessages(
  ...
  return curieApi<SessionMessagesResponse>({
    ...sessionScope,
...
export function renameSession(
  ...
  return curieApi<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
