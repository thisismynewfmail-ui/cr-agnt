import type { LocalCatalogModel, LocalHardware, LocalModelsStatus, LocalRuntimeJob } from '@/types/curie'

import { curieApi, profileScoped } from './client'
...
export function getLocalModelsStatus(): Promise<LocalModelsStatus> {
  return curieApi<LocalModelsStatus>({
    ...profileScoped(),
...
export function getLocalCatalog(): Promise<{ models: LocalCatalogModel[] }> {
  return curieApi<{ models: LocalCatalogModel[] }>({
    ...profileScoped(),
...
export function installLocalRuntime(backend?: string): Promise<{ backend: string; job_id: string; tag: string }> {
  return curieApi<{ backend: string; job_id: string; tag: string }>({
    ...profileScoped(),
...
export function quickstartLocalModels(modelId?: string): Promise<QuickstartResponse> {
  return curieApi<QuickstartResponse>({
    ...profileScoped(),
...
export function downloadLocalModel(modelId: string): Promise<{ already_downloaded?: boolean; job_id: null | string }> {
  return curieApi<{ already_downloaded?: boolean; job_id: null | string }>({
    ...profileScoped(),
...
export function getLocalRuntimeJob(jobId: string): Promise<LocalRuntimeJob> {
  return curieApi<LocalRuntimeJob>({
    ...profileScoped(),
...
export function getLocalModelsJobs(): Promise<{ jobs: LocalRuntimeJob[] }> {
  return curieApi<{ jobs: LocalRuntimeJob[] }>({
    ...profileScoped(),
...
export function searchHFModels(q: string, limit = 20): Promise<{ hits: HFSearchHit[] }> {
  return curieApi<{ hits: HFSearchHit[] }>({
    ...profileScoped(),
...
export function listHFRepoFiles(repo: string): Promise<{ files: HFFileGroup[] }> {
  return curieApi<{ files: HFFileGroup[] }>({
    ...profileScoped(),
...
export function downloadBrowsedModel(
  ...
  return curieApi<{ already_downloaded?: boolean; job_id: null | string; model_id: string }>({
    ...profileScoped(),
    body: { paths, repo },
    method: 'POST',
...
export function sideloadLocalModel(
  ...
  return curieApi<{ already_present?: boolean; model_id: string; ok: boolean }>({
    ...profileScoped(),
    body: { path },
    method: 'POST',
