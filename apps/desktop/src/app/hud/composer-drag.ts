...
function setWorkspaceTransfer(transferring: boolean): void {
  window.curieDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.curieDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.curieDesktop?.hud?.beginMove?.()
}
 ...
 * (see curie:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.curieDesktop?.hud?.endMove?.()
      }
