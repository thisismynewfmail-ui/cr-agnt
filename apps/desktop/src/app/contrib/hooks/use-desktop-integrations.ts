...
import { pathFromCurieDeepLink, resolveCurieOpenPath } from '@/lib/curie-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Curie" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.curieDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.curieDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.curieDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.curieDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // curie://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.curieDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future curieDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveCurieOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.curieDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (curie://index-network/intent/1, curie://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromCurieDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.curieDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.curieDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.curieDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.curieDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
