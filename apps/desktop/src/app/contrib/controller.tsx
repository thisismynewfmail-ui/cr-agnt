...
import { setUnlockEnabled } from '@/lib/unlock-session'
...
import { $currentCwd, $selectedStoredSessionId, $sessions, $unlockActive, sessionMatchesStoredId } from '@/store/session'
...
registry.registerMany([
  ...
  // The agent's write -> see loop: rescan <curie home>/desktop-plugins
  ...
  // The keybind panel's non-titlebar door (the keyboard icon is gone).
  {
    ...
    data: {
      ...
      run: () => window.dispatchEvent(new CustomEvent('curie:open-keybinds'))
    } satisfies PaletteContribution
  },
...
// UNLOCK (dangerous-command approval bypass) is a status-bar zap and a /unlock
...
registry.register(
  paletteToggle({
    ...
    label: 'Toggle unlock',
    icon: Zap,
    keywords: ['unlock', 'approvals', 'auto-approve', 'bypass', 'dangerous', 'commands'],
    get: () => $unlockActive.get(),
    set: enabled => void setUnlockEnabled(enabled).catch(() => undefined)
  })
)
    ...
    >
      <ContribWiring>
        ...
        <div
          ...
          // [data-curie-glass] rules in styles.css clear them so the tint
