...
import { getCurieConfigRecord, saveMcpServers } from '@/curie'
...
import { setCurieConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `curie://mcp/install` deep links. The payload is
 ...
 * confirming lets Curie spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getCurieConfigRecord())
      ...
      setCurieConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
