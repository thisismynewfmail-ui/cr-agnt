...
import { getLatestSessionMessages } from '@/curie'
...
import { useCurieConfig } from '../session/hooks/use-curie-config'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const { refreshCurieConfig, sttEnabled, voiceMaxRecordingSeconds } = useCurieConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('curie:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('curie:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshCurieConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshCurieConfig])
