import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@curie/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as CurieGateway | null
  const gatewayRef = useRef<CurieGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.curieDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<CurieGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as CurieGateway | null
      }),
    []
  )
