import { isGatewayReauthRequired, JsonRpcGatewayError, resolveGatewayWsUrl } from '@curie/shared'
import { useEffect, useRef } from 'react'

import { shouldApplyPostBootProgressError } from '@/components/boot-failure-reauth'
import type { CurieConnection } from '@/global'
import { CurieGateway } from '@/curie'
...
import type { RpcEvent } from '@/types/curie'
...
export function primaryRuntimeConnectionId(connection: Pick<CurieConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.curieDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: CurieGateway | null) => void
  refreshCurieConfig: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: CurieConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Curie...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Curie…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Curie backend'
        )
        ...
        // as an opaque "Could not connect to Curie gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshCurieConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshCurieConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startCurie re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new CurieGateway()
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting Curie…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshCurieConfig(),
          ...
          // retry it), not brick boot behind the "Curie couldn't start"
