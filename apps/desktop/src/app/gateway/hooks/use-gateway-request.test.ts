...
vi.mock('@/curie', async importOriginal => {
  const actual = await importOriginal<typeof CurieModule>()

  class FakeCurieGateway {
  ...
  return { ...actual, CurieGateway: FakeCurieGateway }
})

import type * as CurieModule from '@/curie'
import type { CurieGateway } from '@/curie'
import {
  $gateway,
...
const fakeGateway = { connectionState: 'open' } as unknown as CurieGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as CurieGateway, 'default')
  $gateway.set(primary as unknown as CurieGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.curieDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as CurieGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { curieDesktop: unknown }).curieDesktop = { getConnection }
    $gateway.set(dropped)
