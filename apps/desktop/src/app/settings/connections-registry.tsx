 ...
 * agent sources (local runtime + any number of remote gateways / Curie Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.curieDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.curieDesktop.oauthLoginConnectionConfig(editorUrl)
  ...
  // Fan out `curie update` to every eligible source; per-connection results
