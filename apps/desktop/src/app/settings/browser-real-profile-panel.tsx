...
import { type ProfileScope, saveCurieConfigRecord } from '@/curie'
 ...
 * the default browser's profile (cookies/logins) into a Curie-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useCurieConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        await saveCurieConfigRecord(next, profile)
        notify({
          kind: 'info',
