...
import { disconnectOAuthProvider, listOAuthProviders } from '@/curie'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/curie'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.curieDesktop?.terminal)
...
//      in curie_cli/provider_catalog.py) — the SAME provider identity
//      `curie model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Curie can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Curie can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Curie never deletes creds another tool owns behind a silent API call.
