...
const getCurieConfigSchema = vi.fn()
const saveCurieConfig = vi.fn()
const getElevenLabsVoices = vi.fn()

vi.mock('@/curie', () => ({
  getCurieConfigRecord: () => getCurieConfigRecord(),
  getCurieConfigSchema: () => getCurieConfigSchema(),
  saveCurieConfig: (config: unknown, profile?: string) => saveCurieConfig(config, profile),
...
beforeEach(() => {
  ...
  getCurieConfigSchema.mockResolvedValue({ fields: {} })
...
describe('ConfigSettings autosave', () => {
  it('sends a later revert instead of diffing it away against the stale page-load baseline', async () => {
    getCurieConfigRecord.mockResolvedValue({ checkpoints: { enabled: false }, other: 'untouched' })
    ...
    try {
      ...
      await waitFor(() => expect(saveCurieConfig).toHaveBeenCalledTimes(1))
      expect(saveCurieConfig.mock.calls[0][0]).toEqual({ checkpoints: { enabled: true } })

      // Revert: flip it back to its original value and let autosave fire again.
      toggle.click()
      await vi.advanceTimersByTimeAsync(700)

      await waitFor(() => expect(saveCurieConfig).toHaveBeenCalledTimes(2))
      ...
      expect(saveCurieConfig.mock.calls[1][0]).toEqual({ checkpoints: { enabled: false } })
    } finally {
      vi.useRealTimers()
    }
  })
