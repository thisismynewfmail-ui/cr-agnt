...
import { getElevenLabsVoices, getCurieConfigSchema, saveCurieConfig } from '@/curie'
...
import type { ConfigFieldSchema, CurieConfigRecord } from '@/types/curie'

import { curieConfigCacheWriter, useCurieConfigRecord } from '../hooks/use-config-record'
...
function ConfigSettingsInner({
  ...
  const [config, setConfig] = useState<CurieConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useCurieConfigRecord(scopeProfile)
  ...
  const writeConfigCache = useMemo(() => curieConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const {
    ...
    queryKey:
      scopeProfile == null ? ['curie-config-schema'] : ['curie-config-schema', normalizeProfileKey(scopeProfile)],
    queryFn: () => getCurieConfigSchema(scopeProfile),
    staleTime: 5 * 60 * 1000
  })
  ...
  // never touched — possibly changed out-of-band by `curie config set`
  // while this page sat open — is never resent with its stale value.
  const configBaselineRef = useRef<CurieConfigRecord | null>(null)
    ...
    const t = window.setTimeout(() => {
      ...
      saveQueueRef.current = saveQueueRef.current.then(async () => {
        try {
          ...
          const result = await saveCurieConfig(patch, scopeProfile)
  ...
  const applyConfig = (next: CurieConfigRecord) => {
    saveVersionRef.current += 1
    setConfig(next)
    setSaveVersion(saveVersionRef.current)
  }

  const updateConfig = (next: CurieConfigRecord) => {
