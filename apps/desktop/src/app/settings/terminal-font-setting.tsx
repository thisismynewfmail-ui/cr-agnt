...
import { saveCurieConfig } from '@/curie'
import { useI18n } from '@/i18n'
import { notifyError } from '@/store/notifications'
import type { CurieConfigRecord } from '@/types/curie'

import { setCurieConfigCache, useCurieConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: CurieConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig } = useCurieConfigRecord()
  ...
  const [staleConfig, setStaleConfig] = useState<CurieConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setCurieConfigCache, so
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="curie-terminal-font-families"
          ...
          <datalist id="curie-terminal-font-families">
