...
// User-installed plugins first — mirrors `curie plugins list --user`.
...
function reveal(file: string) {
  void window.curieDesktop?.revealPath?.(file)?.catch(() => undefined)
}

async function revealPluginsDir() {
  try {
    // Electron owns the local plugin root — deriving it from the backend's
    // curie_home breaks against a remote backend (#66899).
    const dir = await window.curieDesktop?.desktopPluginsRoot?.()
    ...
    const result = await window.curieDesktop?.openDir?.(dir)
...
// Agent plugins live under the BACKEND's curie home (profile-aware), so the
// path comes from the gateway — not from the renderer's local CURIE_HOME.
...
async function revealAgentPluginsDir(request: GatewayRequest) {
  try {
    ...
    const opened = await window.curieDesktop?.openDir?.(`${home}/plugins`)
            ...
            <SelectContent>
              {profiles.map(profile => (
                <SelectItem key={profile.name} value={profile.name}>
                  {profile.is_default ? 'Curie (default)' : profile.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
