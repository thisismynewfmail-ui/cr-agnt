...
import type { ToolsetConfig } from '@/types/curie'
...
beforeEach(() => {
  ...
  getCurieConfigRecord.mockResolvedValue({
    tts: {
      provider: 'edge',
  ...
  getCurieConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveCurieConfig).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveCurieConfig.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
