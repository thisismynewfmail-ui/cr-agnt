...
import type { SessionInfo } from '@/curie'
...
// zero Curie sessions, and those arrive continuously — so once the user dragged
...
export function useRepoWorktreeMap(
  repoPaths: string[],
  enabled: boolean
): [Record<string, CurieGitWorktree[]>, boolean] {
  const [map, setMap] = useState<Record<string, CurieGitWorktree[]>>({})
