...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/curie'
...
// highlight markers (sqlite snippet() delimiters — see curie_state_search.py).
...
export function ChatSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener('curie:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('curie:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])
  ...
  // A turn can `git worktree add/remove` in the terminal (e.g. you ask Curie to
