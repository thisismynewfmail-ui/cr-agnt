...
const preamble = () => `  var w = window;
  ...
  w.__curieActHolder = w.__curieActHolder || {};
  w.__curieAct = ${actEngineSource()};
  w.__curieWatch_fn = (${watchInPage.toString()});
  w.__curieWatchTag = ${WATCH_TAG};
  var holder = w.__curieActHolder;
  var act = function (a) { return w.__curieAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__curieWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__curieHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'CURIE-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
