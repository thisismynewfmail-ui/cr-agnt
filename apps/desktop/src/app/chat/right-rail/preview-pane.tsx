        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.curieDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.curieDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Curie to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.curieDesktop?.watchPreviewFile ||
      !window.curieDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.curieDesktop.onPreviewFileChanged(payload => {
    ...
    void window.curieDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.curieDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.curieDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:curie-preview')
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.curieDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.curieDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.curieDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.curieDesktop?.openExternal(currentUrl)
                : undefined
            }
