...
vi.mock('@/curie', () => ({
  saveCurieConfigRecord: (config: Record<string, unknown>, profile?: unknown) => mocks.save(config, profile)
}))

const promptCopy = {
  title: 'Stay signed in to your sites',
  body: 'Let Curie browse with a snapshot of your default browser profile.',
...
vi.mock('../../hooks/use-config-record', () => ({
  curieConfigCacheWriter: () => (config: Record<string, unknown>) => mocks.cache(config),
  useCurieConfigRecord: () => ({ data: mocks.loadedConfig })
}))
