...
import { getCurieConfig, getCurieConfigDefaults } from '@/curie'
...
interface CurieConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useCurieConfig({ activeSessionIdRef }: CurieConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshCurieConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getCurieConfig(), getCurieConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Curie' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshCurieConfig, sttEnabled, voiceMaxRecordingSeconds }
}
