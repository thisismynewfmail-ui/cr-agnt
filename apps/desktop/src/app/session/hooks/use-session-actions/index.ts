...
import type { SessionCreateResponse, SessionMessage, SessionResumeResponse, UsageStats } from '@/types/curie'
  ...
  const createBackendSessionForSend = useCallback(
    async (preview: string | null = null): Promise<string | null> => {
      ...
      try {
        ...
        const unlockArmed = $unlockActive.get()
        ...
        // User may have armed UNLOCK on the new-chat draft before the runtime
        // session existed — apply it to the freshly created session.
        if (unlockArmed) {
          await setSessionUnlock(requestGateway, created.session_id, true).catch(() => undefined)
        }
