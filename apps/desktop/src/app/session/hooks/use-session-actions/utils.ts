...
import type { SessionCreateResponse, SessionInfo, SessionResumeResponse, SessionRuntimeInfo } from '@/types/curie'
...
type SessionRuntimeStatePatch = Partial<
  Pick<
    ClientSessionState,
    'branch' | 'cwd' | 'fast' | 'model' | 'personality' | 'provider' | 'reasoningEffort' | 'serviceTier' | 'unlock'
  >
>
...
function publishRuntimeToComposer(state: SessionRuntimeStatePatch): void {
  ...
  if (state.unlock !== undefined) {
    setUnlockActive(state.unlock)
  }
}
...
export function applyRuntimeInfo(
  ...
  if (typeof info.unlock === 'boolean') {
    sessionState.unlock = info.unlock
  }
