import { skillInvocationText } from '@curie/shared'
import { type MutableRefObject, useCallback, useRef } from 'react'

import { getProfiles } from '@/curie'
...
import { setSessionUnlock } from '@/lib/unlock-session'
...
const renderWakeStatus = (status: WakeStatusResponse): string => {
  const lines = [
    ...
    `Phrase: "${status.phrase?.trim() || 'hey curie'}"`,
  ...
  return useCallback(
    async (rawCommand: string, options?: { sessionId?: string; recordInput?: boolean }) => {
      ...
      const actionHandlers: Record<DesktopActionId, (ctx: SlashActionCtx) => Promise<void>> = {
        ...
        // /unlock maps to the status-bar UNLOCK control — a per-session approval
        ...
        unlock: async ({ sessionHint }) => {
          const sid = sessionHint || activeSessionIdRef.current
          const next = !$unlockActive.get()

          if (!sid) {
            setUnlockActive(next)
            notify({ kind: 'success', message: next ? copy.unlockArmed : copy.unlockOff })

            return
          }

          try {
            const active = await setSessionUnlock(requestGateway, sid, next)
            appendSessionTextMessage(sid, 'system', copy.unlockSystem(active))
          } catch {
            notify({ kind: 'error', title: copy.unlockTitle, message: copy.unlockToggleFailed })
          }
        },
        // /wake must stay in the gateway process that owns the Desktop wake
        // lease. Sending it through slash.exec creates a separate CurieCLI in
