import type { BillingBlock } from '@curie/shared'
 ...
 * billing-specific toast — never the generic "Curie error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Curie reported an error',
