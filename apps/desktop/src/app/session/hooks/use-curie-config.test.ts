...
import { getCurieConfig } from '@/curie'
...
import { useCurieConfig } from './use-curie-config'

vi.mock('@/curie', () => ({
  getCurieConfig: vi.fn(),
  getCurieConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'curie.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getCurieConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getCurieConfig>>)

describe('useCurieConfig refreshCurieConfig', () => {
  beforeEach(() => {
  ...
  // `agent.reasoning_effort: high` to Curie' built-in medium.
  it('publishes the profile default effort even when a manual pick blocks the composer reseed', async () => {
    ...
    const { result } = renderHook(() => useCurieConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshCurieConfig()
    })
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useCurieConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getCurieConfig>>>()
    vi.mocked(getCurieConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useCurieConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshCurieConfig(true)
    })
    expect(getCurieConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getCurieConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getCurieConfig>>>()
    vi.mocked(getCurieConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useCurieConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshCurieConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getCurieConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getCurieConfig>>>()
    vi.mocked(getCurieConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshCurieConfig(true)
      refreshC = result.current.refreshCurieConfig(true)
    })
