...
const CURIE_PATHS_MIME = 'application/x-curie-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(CURIE_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(CURIE_PATHS_MIME)
  ...
  const getPath = window.curieDesktop?.getPathForFile
...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const terminalApi = window.curieDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (curie --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.curieDesktop?.readClipboard?.()) ?? ''
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[curie-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.curieDesktop?.terminal?.write(sessionId, `${command}\r`)
