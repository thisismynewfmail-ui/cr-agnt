...
beforeEach(() => {
  ...
  readDir.mockReset()
  ;(window as unknown as { curieDesktop: { readDir: typeof readDir } }).curieDesktop = { readDir }
})
...
function ok(entries: { name: string; path: string; isDirectory: boolean }[]): CurieReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { curieDesktop: unknown }).curieDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: CurieReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<CurieReadDirResult>(resolve => {
  ...
  it('discards a stale live refresh after the active registered connection changes', async () => {
    let resolveRefreshFromA: ((result: CurieReadDirResult) => void) | undefined
  ...
  it('discards a stale child read after the active registered connection changes', async () => {
    let resolveChildFromA: ((result: CurieReadDirResult) => void) | undefined
  ...
  it('discards a stale root read after the active registered connection changes', async () => {
    let resolveFirst: ((result: CurieReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<CurieReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.curieDesktop is missing', async () => {
    delete (window as unknown as { curieDesktop?: unknown }).curieDesktop
  ...
  it('keeps an unreadable root on screen while it re-probes', async () => {
    ...
    let releaseProbe: ((value: CurieReadDirResult) => void) | undefined
  ...
  it('keeps loaded rows on screen while the root refreshes', async () => {
    ...
    let releaseRefresh: ((value: CurieReadDirResult) => void) | undefined
  ...
  it('clears the rows when the same path is re-read from another backend', async () => {
    ...
    let releaseFromB: ((value: CurieReadDirResult) => void) | undefined
