...
import type { CurieReadDirEntry, CurieReadDirResult } from '@/global'
...
export type ProjectTreeEntry = CurieReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: CurieReadDirEntry) {
...
async function filterIgnored(entries: CurieReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<CurieReadDirResult> {
  if (!window.curieDesktop) {
