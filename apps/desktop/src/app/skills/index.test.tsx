...
import type * as CurieApi from '@/curie'
...
vi.mock('@/curie', async importOriginal => ({
  ...(await importOriginal<typeof CurieApi>()),
...
describe('SkillsView toolset management', { timeout: 60_000 }, () => {
    ...
    await act(async () => {
      window.dispatchEvent(
        new MessageEvent('message', {
          data: { type: 'curie-skill-pick', name: 'web-research', identifier: 'web-research' },
          origin: 'https://github.com/thisismynewfmail-ui/Cur-Agnt'
        })
      )
    })
  ...
  it('offers (connection, profile) scope rows on multi-connection desktops', async () => {
    ...
    ;(window as { curieDesktop?: unknown }).curieDesktop = { connections, getAgentRoster }
