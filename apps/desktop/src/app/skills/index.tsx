...
import type { OfficialSkillInfo, SkillInfo, ToolsetInfo } from '@/types/curie'
...
export function SkillsView({
  ...
  const registryBridge = window.curieDesktop?.connections
  const rosterBridge = window.curieDesktop?.getAgentRoster
  ...
  const scopeOptions: { key: string; label: string; value: string }[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Curie (default)' : p.name,
      value: p.name
    }))
                ...
                <MasterDetail pane={skillEditorPane} resizeId="capabilities-split" split="wide">
                  ...
                  >
                    ...
                    {/* The built-in optional-skills catalog, below the
                        installed list: every official skill Curie ships but
