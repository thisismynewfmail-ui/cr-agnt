...
const SKIN_KEY = 'curie-desktop-theme-v2'
const MODE_KEY = 'curie-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'curie-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'curie-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'curie-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.curieTheme = skinName
  root.dataset.curieMode = rendered
  ...
  window.curieDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('curie-boot-background', chromeBg)
    window.localStorage.setItem('curie-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.curieThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.curieDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Curie authoring/activating a skin from a
