/**
 * Live skin sync from the Curie backend.
 *
 * The backend resolves the active skin (built-in or `$CURIE_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Curie
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { CurieSkin } from '@curie/shared/skin'
 ...
 */
export function ingestBackendSkin(skin: CurieSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as CurieSkin)
