...
vi.mock('@/curie', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), curieApi: vi.fn() }))
...
const { deleteProfile, curieApi } = await import('@/curie')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its CURIE_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { curieDesktop: unknown }).curieDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(curieApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(curieApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(curieApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
