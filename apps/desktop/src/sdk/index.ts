/**
 * @curie/plugin-sdk — THE plugin language. The vscode-module model: plugin
 ...
 *    object as `window.__CURIE_PLUGIN_SDK__` and maps the import to it, so a
...
import { deleteProfile, getLogs, getStatus, curieApi, type CurieGateway } from '@/curie'
import {
  $gateway,
...
import type { PaginatedSessions, UsageStats } from '@/types/curie'
...
export interface PluginProfileRoute {
  ...
  profile: string
  /** Backend Curie profile served by that route. */
...
async function requestPluginProfile<T>(
  ...
  const getAgentRoster = window.curieDesktop?.getAgentRoster
...
async function pluginRouteStillRegistered(route: PluginProfileRoute): Promise<boolean> {
  const getProfileRoutes = window.curieDesktop?.getProfileRoutes
...
//
// ensureGatewayProfile awaits the store's dial, and CurieGateway.connect() has
...
export const host = {
  ...
  deleteProfile: async (profile: string | PluginProfileRoute): Promise<void> => {
    ...
    // restores on relaunch and re-creates the deleted profile (curie-agent#94235).
    dropTilesForProfile(
  ...
  connections: async () => {
    const bridge = window.curieDesktop?.connections

    if (!bridge) {
      throw new Error('This Desktop build has no connection registry. Update Curie Desktop.')
    }
  ...
  agents: async () => {
    const roster = window.curieDesktop?.getAgentRoster

    if (!roster) {
      throw new Error('This Desktop build cannot enumerate multi-source agents. Update Curie Desktop.')
    }
  ...
  newChat: (profile?: null | string | PluginProfileRoute, options: PluginNewChatOptions = {}): void => {
    if (options.workspaceMode === 'bots') {
      ...
      if (!openTab) {
        notify({ kind: 'error', message: 'Update Curie Desktop to open another Bot chat.' })
   ...
   *
   *  `isStaleTile` (curie-agent#90102): the caller's reconciliation probe
  ...
  profileRoutes: async () => {
    const desktop = window.curieDesktop
    const getProfileRoutes = desktop?.getProfileRoutes

    if (!getProfileRoutes) {
      throw new Error('Curie Desktop connection routing unavailable')
    }
  ...
  listPersistedSessions: async (
    ...
    return curieApi<PaginatedSessions>({
  ...
  setPersistedSessionHidden: async (
    ...
    return curieApi<{ ok: boolean; hidden: boolean }>({
      ...(route ? { connectionId: route.connectionId } : {}),
 ...
 *  "CURIE AGENT" with it, a `chat.empty` contribution writes its own name. */
...
export type { CurieGateway } from '@/curie'
...
export type { CurieOpenTarget } from '@/lib/curie-open-target'
...
/** Curie' reasoning levels + their compact labels, so a plugin surfacing a
...
export type { RpcEvent, StatusResponse } from '@/types/curie'
