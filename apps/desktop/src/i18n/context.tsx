...
import { getCurieConfigRecord, type CurieConfigRecord, saveCurieConfig } from '@/curie'
...
export interface I18nConfigClient {
  getConfig: () => Promise<CurieConfigRecord>
  saveConfig: (config: CurieConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.curieDesktop?.api) {
      return Promise.resolve({})
    }

    return getCurieConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.curieDesktop?.api) {
...
export function getConfigDisplayLanguage(config: CurieConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: CurieConfigRecord, locale: Locale): CurieConfigRecord {
