 ...
 *    a `CuriePlugin` registers automatically (vite glob — drop a folder in).
 *    `curie-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `curie-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<curie home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<curie home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: CuriePlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default CuriePlugin export — skipped`)
