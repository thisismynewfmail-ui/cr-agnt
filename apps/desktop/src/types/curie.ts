...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Curie doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in curie_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `curie model` uses — instead of
...
export type CurieConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `curie model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Curie simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (curie.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors curie_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Curie, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Curie but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `curie tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Curie); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** curie-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** curie-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/curie/update/check` — the backend's own update state.
