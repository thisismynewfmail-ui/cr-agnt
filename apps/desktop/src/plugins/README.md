...
Drop a `<name>/plugin.{ts,tsx}` here that default-exports a `CuriePlugin` and
...
`$CURIE_HOME/desktop-plugins/<name>/plugin.js` (the disk door) — see the
`curie-desktop-plugins` skill.
