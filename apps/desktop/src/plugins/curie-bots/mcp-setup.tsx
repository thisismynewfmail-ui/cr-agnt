...
import { Button, host, Input, useI18n } from '@curie/plugin-sdk'
...
// and the row falls back to the "run curie mcp / Settings" hint. profile is
...
export function McpSetupButton({ profile, entry, onDone, ensureProfile }: McpSetupButtonProps) {
  ...
  const beginOAuth = async () => {
    ...
    const mcpOauthBridge =
      typeof window !== 'undefined' && window.curieDesktop && window.curieDesktop.mcpOauth
        ? window.curieDesktop.mcpOauth
        : null
    ...
    try {
      ...
      } else if (typeof window !== 'undefined' && window.curieDesktop && window.curieDesktop.openExternal) {
        window.curieDesktop.openExternal(authUrl)
      } else {
