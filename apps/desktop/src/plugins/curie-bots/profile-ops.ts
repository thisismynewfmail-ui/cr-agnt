...
import { forgetSessionUnread, host, queryClient } from '@curie/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['curie-bots']
...
/** Permanently delete a bot's Curie profile, then remove plugin-local state
 ...
 * directory (curie-agent#52279). That is the "can't delete a bot" error. */
