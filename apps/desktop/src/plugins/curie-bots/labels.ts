...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Curie". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `curie profile rename
  ...
  // never generic "Curie" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Curie (the agent it is)
