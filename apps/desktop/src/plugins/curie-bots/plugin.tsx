/**
 * Curie Bot Mode — a "one chat per agent" roster for the Curie desktop.
 *
 * Left pane "Bots": one row per Curie profile (a bot = an agent profile) with
 ...
 *
 * Right tile "Routines": scheduled tasks (Curie cron jobs) scoped to the
 ...
 * bot-initiated sends use `curie -p <bot> chat --in ~ -c "Bot Chat"`.
 */

import { CHAT_EMPTY_AREA, COMPOSER_AREAS, host, PALETTE_AREA, translateNow } from '@curie/plugin-sdk'
import type { ChatEmptyProps, PluginContext } from '@curie/plugin-sdk'
...
export default {
  ...
  register(ctx: PluginContext) {
    ...
    // excluded (a bot doesn't @ itself); 'default' surfaces as @curie.
    ctx.register({
            ...
            try {
              const registry =
                typeof window !== 'undefined'
                  ? await Promise.resolve(window.curieDesktop?.connections?.list?.()).catch(() => null)
                  : null
    ...
    try {
      if (typeof window !== 'undefined') {
        unbindConnectionsChanged =
          window.curieDesktop?.connections?.onChanged?.(payload => {
    ...
    // A bot's chat before it has spoken: core's splash is Curie' wordmark and
      ...
      data: {
        handler: async (draft: ComposerDraftPayload): Promise<ComposerDraftPayload> => {
          ...
          const lines = mentionedBots.map(bot => {
            ...
            const title = String(
              botRosterMeta(bot, $botMeta.get())?.title || bot.ui_meta?.['curie-bots']?.title || bot.title || ''
            ).trim()
