...
describe('routing', () => {
  ...
  it('resolves @curie to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@curie take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @curie', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @curie,/)
    ...
    expect(peer).toMatch(/group chat with @curie/)
  })
