...
import { Button, host, Input } from '@curie/plugin-sdk'
import { useEffect, useRef, useState } from 'react'

import { useBots } from './i18n'

// ── skills hub section: the REAL hub page (docs) embedded as a picker ──────
// https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs?embed=picker hides the
// docs chrome and adds "+ Add to this Agent" per card, posting
// {type: 'curie-skill-pick', ...} to us (curie-agent#86243). We validate
...
export function HubSkillsSection({ forProfile, onInstalled }: HubSkillsSectionProps) {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!data || data.type !== 'curie-skill-pick' || !data.name) {
        return
      }
