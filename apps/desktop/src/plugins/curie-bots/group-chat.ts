...
import { atom, host } from '@curie/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'curie-bots-groups'
...
/** The v3 envelope stored under the default profile's `curie-bots-groups`
 ...
 *  clients (Curie Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Curie"
 *  (community report, Aug 21 2026: renamed default still read "Curie is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Curie (matching displayName and the
 *  @curie handle) so the main agent never loses its name in rooms. */
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`curie profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Curie' : trimmed
}
