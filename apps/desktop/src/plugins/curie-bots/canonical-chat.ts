...
import * as sdk from '@curie/plugin-sdk'
import { host } from '@curie/plugin-sdk'
...
// grandfather, or recover. Stored-id pins (ui_meta['curie-bots'].chat) were
 ...
 *  click path's tile-staleness probe (curie-agent#90102), which must
...
async function openStoredBotChat(
  ...
  if (!storedId || typeof host.openSession !== 'function') {
    throw new Error('This Curie Desktop version cannot open stored sessions')
  }
  ...
  // even though the session is fine (curie-agent#89617) — clicking Retry
  ...
  // until an app restart (curie-agent#93604). A resume is cheap and
  ...
  if (route && typeof host.requestProfile !== 'function') {
    throw new Error(
      getPluginCtx()?.i18n?.t('bot.remoteConnectionsUnsupported') ??
        'Update Curie Desktop to chat with bots on other connections.'
    )
  }
...
export async function ensureBotMetadata(bot: RosterRow): Promise<BotMeta> {
  ...
  const server = row?.ui_meta?.['curie-bots']
