/**
 * Routines: the Curie cron jobs scoped to the bot you're chatting with — the
...
// curie -p <bot> chat delegation wrapper so the run reaches that bot's
...
export function routinePrompt(
  ...
  return (
    ...
    `curie -p ${shellQuote(bot)} chat -c ${shellQuote(`Routine: ${title}`)} -q ${shellQuote(`[Scheduled routine] ${instruction}`)}\n\n` +
...
// Curie-native schedule string; Advanced exposes it raw.
...
/** Compose the Curie schedule string from picker state. */
