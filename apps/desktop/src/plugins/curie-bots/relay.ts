...
import { host, LruCache } from '@curie/plugin-sdk'
...
// to (local, remote URL, SSH, Curie Cloud, docker) must be able to find
...
// curie_cli/config_defaults.py and tui_gateway/methods_bot_relay.py and fails
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['curie-bots']?.title || profile?.display_name || ''),
