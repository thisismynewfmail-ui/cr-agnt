...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'curie:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.curieDesktop?.saveImageFromUrl) {
        if (await window.curieDesktop.saveImageFromUrl(src)) {
