...
const CURIE_REF_TYPES = WIRE_REFERENCE_KINDS
type CurieRefType = (typeof CURIE_REF_TYPES)[number]
...
const CURIE_DIRECTIVE_RE = referenceRe()
...
export const curieDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(CURIE_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...curieDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Curie directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.curieDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.curieDesktop?.readFileDataUrl(id)
