 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.curie.send(prompt)` (or declarative
 * `data-curie-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'curie-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'curie-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `curie.send(prompt)`.
 ...
 *  wires `data-curie-send` so declarative HTML works with zero script:
 *  `<button data-curie-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.curie={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-curie-send]"):null;' +
    'if(el)send(el.getAttribute("data-curie-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `curie serve` — renders live.)
