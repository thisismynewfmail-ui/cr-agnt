...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's CURIE_HOME/logs. On a
  ...
  const openLogs = useCallback(async () => {
    try {
      const root = await window.curieDesktop?.logsRoot?.()
      ...
      const result = await window.curieDesktop?.openDir?.(root)
