...
describe('buildToolView web-search query', () => {
  it('keeps the query separate from structured search results', () => {
    const view = buildToolView(
      part({
        args: { query: 'Curie Agent Desktop tool calls' },
        result: { web: [{ snippet: 'Desktop docs', title: 'Curie docs', url: 'https://example.com/docs' }] },
        toolName: 'web_search'
      }),
      ''
    )

    expect(view.searchQuery).toBe('Curie Agent Desktop tool calls')
    expect(view.searchHits).toEqual([
      { snippet: 'Desktop docs', title: 'Curie docs', url: 'https://example.com/docs' }
    ])
  })
})
  ...
  it('shows failed title when navigate returns success=false', () => {
    const view = buildToolView(
      part({
        ...
        args: { url: 'https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs' },
    ...
    expect(view.title).toBe('Failed to open github.com/thisismynewfmail-ui/Cur-Agnt/docs')
  })
  ...
  it('shows opened title on success', () => {
    const view = buildToolView(
      part({
        toolName: 'browser_navigate',
        args: { url: 'https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs' },
        result: { success: true, url: 'https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs', title: 'Docs' }
      }),
      ''
    )

    expect(view.status).toBe('success')
    expect(view.title).toBe('Opened github.com/thisismynewfmail-ui/Cur-Agnt/docs')
  })
})
