...
function failBoot() {
  $desktopBoot.set({
    error: 'Could not connect to Curie gateway',
    fakeMode: false,
    message: 'boot failed',
...
function stubDesktop(config: Record<string, unknown>, overrides: Record<string, unknown> = {}) {
  const original = window.curieDesktop
  Object.defineProperty(window, 'curieDesktop', {
    configurable: true,
    value: { getRecentLogs: async () => ({ lines: [] }), getConnectionConfig: async () => config, ...overrides }
  })

  return () => Object.defineProperty(window, 'curieDesktop', { configurable: true, value: original })
}
