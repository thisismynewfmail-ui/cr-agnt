...
function sessionsBotsTree() {
  ...
  registerPane('curie-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'curie-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('curie-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('curie-bots:pane')
    expect($hiddenStripTabs.get()).toContain('curie-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('curie-bots:pane')

    expect(setStripTabHidden('curie-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('curie-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('curie-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('curie-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('curie.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('curie-bots:pane')
    ...
    revealTreePane('curie-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('curie-bots:pane')
    expect(window.localStorage.getItem('curie.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'curie-bots:pane', title: 'curie-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'curie-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
