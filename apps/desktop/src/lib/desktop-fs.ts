import { curieApi } from '@/api/client'
import type {
  CurieConnection,
  CurieReadDirResult,
  CurieReadFileTextResult,
  CurieSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: CurieConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: CurieConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<CurieReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<CurieReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<CurieReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<CurieReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.curieDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: CurieSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
