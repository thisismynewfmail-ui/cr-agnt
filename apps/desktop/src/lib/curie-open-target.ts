 ...
 * `curie://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `curie://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `curie://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type CurieOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }

const CURIE_PROTOCOL = 'curie:'
...
export function normalizeCurieOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('curie://') || trimmed.startsWith(`${CURIE_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // curie://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // curie://index-network/intent/1 → /index-network/intent/1
...
export function resolveCurieOpenPath(target: CurieOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeCurieOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeCurieOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeCurieOpenString(target.path)
 ...
 * (`curie://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromCurieDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveCurieOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveCurieOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `curie://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromCurieDeepLink('open', name, params)
}
