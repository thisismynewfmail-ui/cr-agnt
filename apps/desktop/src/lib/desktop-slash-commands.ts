...
/** Why a known Curie command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/unlock', description: 'Toggle UNLOCK — auto-approve dangerous commands', surface: action('unlock') },
  {
    name: '/wake',
  ...
  { name: '/profile', description: 'Switch the active Curie profile', surface: action('profile') },
  {
    name: '/skin',
...
function isKnownCurieSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Curie' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownCurieSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Curie and were never asked for. It is
