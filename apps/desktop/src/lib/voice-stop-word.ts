...
//
// When someone is in a hands-free "Hey Curie" voice chat, the natural way to
...
// phrase (optionally addressed to Curie), so a real turn that merely contains
...
// Optional address prefixes so "curie stop" / "ok stop" / "hey curie, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey curie', 'hey curie,', 'curie', 'curie,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("curie") is not a stop command on its own.
      continue
    }
 ...
 * to Curie). Returns false for anything that merely contains "stop" as part of
