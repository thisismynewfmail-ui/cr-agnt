...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __curieWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__curieWatch || (win.__curieWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('curie-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('curie-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __curieWatchTag?: number }).__curieWatchTag
