...
import { LOCAL_CONNECTION_ID, registryBackendScopeKey } from '@curie/shared'
...
const TILES_KEY = 'curie.desktop.sessionTiles.v2'
const LEGACY_TILES_KEY = 'curie.desktop.sessionTiles.v1'
...
const BOT_CHAT_SCOPE_KEY = 'curie.desktop.botChatSessions.v1'
 ...
 *  (curie-agent#90102). The tile bucket is a Local Storage cache, and a
 ...
 * restore re-dials the profile's backend, whose ensure_curie_home() re-creates
 * the profile directory the delete just removed (curie-agent#94235). Same
...
export function dropTilesForProfile(
  ...
  const ownerMatches = (owner: SessionProfileRoute | undefined): boolean => {
    ...
    // would orphan a live conversation (curie-agent#94235). Tiles persisted
...
export const $focusedStoredSessionId = computed(
  ...
  (groupId, tree, selected, workspaceMode) => {
    ...
    // it to the sidebar group, whose active pane ('curie-bots:pane') is not a
...
// Dev hook for automation (mirrors __CURIE_LAYOUT_TREE__).
if ((import.meta.env.DEV || import.meta.env.VITE_PERF_PROBE === '1') && typeof window !== 'undefined') {
  ;(window as unknown as Record<string, unknown>).__CURIE_SESSION_TILES__ = {
