import { type ConnectionState, type GatewayEvent, registryBackendScopeKey, resolveGatewayWsUrl } from '@curie/shared'
import { atom } from 'nanostores'

import type { CurieConnection } from '@/global'
import { CurieGateway, setApiRequestConnection } from '@/curie'
...
const isOpen = (gateway: CurieGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: CurieConnection) => void
  /**
...
interface Secondary {
  ...
  connection: CurieConnection | null
  gateway: CurieGateway
...
interface GatewayRegistryState {
  ...
  primaryGateway: CurieGateway | null
  ...
  $gateway: ReturnType<typeof atom<CurieGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('curie.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<CurieGateway | null>(null),
...
export function setPrimaryGateway(gateway: CurieGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<CurieConnection, 'connectionId'> | null): void {
...
export function activeGateway(): CurieGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the curie module (null
...
function publishActiveConnection(connection: CurieConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new CurieGateway()
...
async function gatewayForProfile(
  profile: string,
  leaseRequest = false
): Promise<{ gateway: CurieGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Curie gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.curieDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Curie Desktop.')
  }
...
export async function ensureActiveGatewayOpen(): Promise<CurieGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Curie gateway is not connected" on the Sessions `+` action (#88880).
