import type { ConnectionState } from '@curie/shared'
...
import type { SessionInfo, UsageStats } from '@/types/curie'
...
const COMPOSER_MODEL_KEY = 'curie.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'curie.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'curie.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'curie.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'curie.desktop.composer.fast'
...
function composerScopeForConnection(connection: CurieConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'curie.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'curie.desktop.lastRoute'
...
function workspaceCwdKey(connection: CurieConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.curieDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.curieDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<CurieConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'curie.desktop.sessionOwnerHints.v1'
...
export const $unlockActive = atom(false)
 ...
 * CurieConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<CurieConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useCurieConfig.
...
export const setUnlockActive = (next: Updater<boolean>) => updateAtom($unlockActive, next)
