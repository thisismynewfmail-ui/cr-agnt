import { LOCAL_CONNECTION_ID } from '@curie/shared'
import { atom, batch, computed } from 'nanostores'

import type { CurieConnection } from '@/global'
import { getProfiles, curieApi, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/curie'
...
import type { ProfileInfo } from '@/types/curie'
...
// /api/profiles/active `current`). "default" is the root ~/.curie. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'curie.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'curie.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await curieApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new CURIE_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.curieDesktop.profile.set(name)
}
...
async function resolveConnectionForProfile(profile: string): Promise<CurieConnection | null> {
  const getConnection = window.curieDesktop?.getConnection
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<CurieConnection | null> {
  const getConnectionFor = window.curieDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'curie.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.curieDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.curieDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.curie) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.curieDesktop?.touchBackend?.(target).catch(() => undefined)
}
