...
import type { CurieBranchPullRequest } from '@/global'
import { scanSessionPullRequests, type SessionInfo } from '@/curie'
...
export const $pullRequestsByBranch = atom<Record<string, CurieBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'curie.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('curie.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: CurieBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
