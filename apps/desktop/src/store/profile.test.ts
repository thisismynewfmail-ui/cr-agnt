...
const { getProfiles } = await import('@/curie')
...
const remoteConn = (over: Partial<CurieConnection> = {}): CurieConnection =>
  ({ baseUrl: 'https://curie-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as CurieConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { curieDesktop: { getConnection } })
