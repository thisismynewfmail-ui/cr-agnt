...
import type { CurieReviewFile, CurieReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Curie' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'curie.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'curie.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'curie.desktop.reviewTreeMode'
const SELECTED_KEY = 'curie.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<CurieReviewFile[]>([])
...
export const $reviewShipInfo = atom<CurieReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['curieDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: CurieReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly CurieReviewFile[], path: string): CurieReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.curieDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.curieDesktop?.openExternal?.(url)
    }
