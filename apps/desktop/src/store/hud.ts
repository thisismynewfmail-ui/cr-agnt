 ...
 * composer with the reply scrolling above it, so Curie can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.curieDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.curieDesktop?.hud
...
export function resetHudLayout(): void {
  void window.curieDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.curieDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.curieDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
