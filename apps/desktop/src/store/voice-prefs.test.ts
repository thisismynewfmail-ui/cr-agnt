...
vi.mock('@/curie', () => ({
  getCurieConfigRecord: vi.fn(async () => ({})),
  saveCurieConfig: vi.fn(async () => undefined)
}))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye curie', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye curie')
  })
