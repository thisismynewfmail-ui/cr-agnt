...
const agentConn = (over: Partial<CurieConnection> = {}): CurieConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as CurieConnection

const localConn = (over: Partial<CurieConnection> = {}): CurieConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as CurieConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<CurieConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<CurieConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { curieDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<CurieConnection>(() => undefined))
