...
export function setZoomPercent(percent: number): void {
  window.curieDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.curieDesktop?.zoom) {
  void window.curieDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.curieDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
