 ...
 * `@curie/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-curie-glass]`
 * block in styles.css).
 */
...
} from '@curie/shared/translucency'
 ...
 * sets `curieDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.curieDesktop?.glassSupported === 'boolean'
    ? window.curieDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.curieDesktop?.translucencySupported === 'boolean'
    ? window.curieDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-curie-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-curie-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-curie-glass-on', glassLive)
  root.toggleAttribute('data-curie-glass', glassOn)
  root.toggleAttribute('data-curie-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-curie-glass-scope', scope)
  } else {
    root.removeAttribute('data-curie-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.curieDesktop?.setTranslucency?.(state)
  })
