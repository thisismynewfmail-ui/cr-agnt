 ...
 *
 * Local models ship on main behind `--local` (either `curie desktop --local`
 * or the flag on Curie.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.curieDesktop?.localModelsEnabled === true
)
