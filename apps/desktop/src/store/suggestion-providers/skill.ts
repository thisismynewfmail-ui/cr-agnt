 ...
 *  Working in `~/www/curie-agent`, the draft says "curie-agent" constantly
 *  — a "Use skill: curie-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`curie-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/curie-agent-dev`, `plugin:skill`). */
