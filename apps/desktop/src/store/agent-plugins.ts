...
/**
 * Feature store for backend (agent) plugins — the native Curie plugins plus
 ...
 * primitives `curie plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Curie. */
  portable?: boolean
}
 ...
 *  CURIE_HOME. Always refetches (it's a cheap local disk scan on the
