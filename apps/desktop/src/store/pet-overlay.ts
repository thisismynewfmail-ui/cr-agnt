 ...
 * app's bounds and stays visible while Curie is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'curie.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'curie.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.curieDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.curieDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.curieDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.curieDesktop?.petOverlay?.close()
}
