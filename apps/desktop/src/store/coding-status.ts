...
import type { CurieGitWorktree, CurieRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: CurieGitWorktree[] = []
...
export const $repoStatusByCwd = atom<Record<string, CurieRepoStatus | null>>({})
export const $repoWorktreesByCwd = atom<Record<string, CurieGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<CurieRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<CurieGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<CurieRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<CurieGitWorktree[]>>()
const $noRepoStatus = atom<CurieRepoStatus | null>(null)
const $noWorktrees = atom<CurieGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<CurieRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<CurieGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: CurieRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: CurieGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<CurieRepoStatus | null>
  seq: number
  target: string
}
