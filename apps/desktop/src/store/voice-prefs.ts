...
import { getCurieConfigRecord, saveCurieConfig } from '@/curie'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getCurieConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveCurieConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {
