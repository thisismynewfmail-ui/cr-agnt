 ...
 *
 * The archive is the CLI's own `curie profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/curie'
...
import type { ProfileDesktopOverlay } from '@/types/curie'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Curie profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.curieDesktop?.selectSavePath
...
export async function runImportProfileFlow(): Promise<null | string> {
  const paths = await window.curieDesktop?.selectPaths?.({
