...
import type { CurieGitBaseBranch, CurieGitBranch } from '@/global'
import { getCurieConfig, curieApi, type CurieGateway } from '@/curie'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/curie'
...
const PROJECT_SCOPE_KEY = 'curie.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Curie gateway is not connected')
  }
...
async function activeProjectsContext(): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== projectProfile()) {
    throw new Error('Active Curie profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await curieApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<CurieGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<CurieGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Curie
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getCurieConfig(context.profile))
...
// repo, returning where Curie should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<CurieGitBranch[]> {
  const git = desktopGit()
...
export async function listBaseBranches(repoPath: string): Promise<CurieGitBaseBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.curieDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.curieDesktop?.writeClipboard?.(path)
  }
}
