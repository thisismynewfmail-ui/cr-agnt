/**
 * windows-curie-path.ts
 *
 * Pure, dependency-injected pieces of Windows `curie` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvCurieCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `curie` shim shadowed the real
 *      curie.cmd/curie.exe; the shim then failed the --version probe and
 ...
 *      and Curie launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvCurieCommand() — unwrapWindowsVenvCurieCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `curie`) must
 * not shadow `curie.cmd`/`curie.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Curie launcher that drives `curie update`.
...
export interface ResolveVenvCurieCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportCurieCli: (python: string, opts?: { env?: Record<string, string> }) => boolean
...
/**
 * If `command` is a Windows venv `curie`/`curie.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/curie(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m curie_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportCurieCli(). A
 * venv whose update died mid-`pip install` still has python.exe + curie.exe
 ...
 *
 * Returns null when `command` is not a venv curie shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export function resolveVenvCurieCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvCurieCommandDeps
): {
  label: string
  command: string
  args: string[]
  ...
  if (!/^curie(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    ...
    rememberLog?.(
      `Ignoring venv Curie at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Curie Python at ${python}`,
    command: python,
