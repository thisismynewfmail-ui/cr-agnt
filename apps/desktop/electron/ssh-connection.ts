...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(CURIE_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Curie-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.curie/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'curie-desktop-ssh')
  }

  return path.join(os.homedir(), '.curie', 'desktop-ssh')
}
