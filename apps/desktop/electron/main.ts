...
import { buildDesktopBackendEnv, curieManagedNodePathEntries, normalizeCurieHomeRoot } from './backend-env'
import {
...
import { isCurieOwnedVenvDaemon } from './venv-holder-select'
...
import {
  ...
  resolveVenvCurieCommand
} from './windows-curie-path'
import {
...
const USER_DATA_OVERRIDE = process.env.CURIE_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.CURIE_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.CURIE_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with CURIE_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[curie] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[curie] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. CURIE_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[curie] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[curie] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via CURIE_DESKTOP_PASSWORD_STORE, bridged by the
// `curie desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[curie] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[curie] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `curie desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Curie sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[curie] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[curie] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[curie] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[curie] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[curie] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('curie:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Curie burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[curie] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[curie] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[curie] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[curie] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// CURIE_HOME — the user-facing root for everything Curie-related. Mirrors
// scripts/install.ps1's $CurieHome and scripts/install.sh's $CURIE_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\curie (matches install.ps1)
//   macOS / Linux: ~/.curie (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.curie directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\curie yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// CURIE_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// CURIE_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.curie / %LOCALAPPDATA%\curie.
function resolveCurieHome() {
  if (process.env.CURIE_HOME) {
    return normalizeCurieHomeRoot(process.env.CURIE_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'curie-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a CURIE_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\curie and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('CURIE_HOME')

    if (fromRegistry) {
      return normalizeCurieHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'curie')
    const legacy = path.join(app.getPath('home'), '.curie')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.curie setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.curie')
}

const CURIE_HOME = resolveCurieHome()

function pathWithCurieManagedNode(...entries) {
  const managed = curieManagedNodePathEntries(CURIE_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_CURIE_ROOT — the canonical mutable Curie install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_CURIE_ROOT = path.join(CURIE_HOME, 'curie-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_CURIE_ROOT, 'venv')
...
// boots, going straight to `resolveCurieBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_CURIE_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_CURIE_ROOT, '.curie-bootstrap-complete')
...
// active-profile.json records which Curie profile the desktop launches its
// local backend as. When set, startCurie() passes `curie --profile <name>
// dashboard …`, which deterministically pins CURIE_HOME (see
// _apply_profile_override in curie_cli/main.py) and bypasses the sticky
// ~/.curie/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors curie_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// curieDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under CURIE_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by curie_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(CURIE_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (curie_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.CURIE_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.CURIE_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.CURIE_DESKTOP_SKIP_QUIT_CONFIRM === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.CURIE_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.CURIE_DESKTOP_APP_NAME || 'Curie'
...
// 'curie:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.curie) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.curie')
}

// Seed the native About panel with the live Curie version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `curie update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveCurieVersion(),
...
// startCurie(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.CURIE_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.CURIE_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
//   * WSL2 IPC stall  = the renderer's `curie:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.CURIE_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startCurie() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning curie serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Curie backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  if (curieLog.length > 300) {
    curieLog.splice(0, curieLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-curie-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('curie:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the curie:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('curie:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_CURIE_ROOT
  })
}
...
// The Tauri updater writes CURIE_HOME/.curie-update-in-progress for the whole
...
// (`force_kill_other_curie`, taskkill /IM curie.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(CURIE_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--curie-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `curie update`
...
async function waitForUpdateToFinish() {
  ...
  const outcome = await waitForUpdateClearance(updateGateDeps(), {
    onWaitTick: async reason => {
      ...
      await advanceBootProgress(
        'backend.update-wait',
        'An update is finishing — Curie will start automatically when it completes…',
        12
      )
    },
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(CURIE_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      dialog.showErrorBox(
        'Curie update did not finish',
        `${result.message}\n\nDetails: ${path.join(CURIE_HOME, 'logs', 'desktop-update-handoff.log')}`
      )
    }
  } catch (err) {
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Curie to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `curie` — must not shadow `curie.cmd`/`curie.exe`.
...
function unwrapWindowsVenvCurieCommand(command, backendArgs) {
  return resolveVenvCurieCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `curie serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `curie` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'curie_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      // `curie --version` (~10.5s measured cold), and a false negative here
      ...
      execProbeSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, CURIE_HOME, ...(backend.env || {}) },
        timeout: PROBE_TIMEOUT_MS,
        stdio: 'ignore',
        // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
        // (see resolveCurieBackend step 4); execFileSync of a .cmd without
...
function isCurieSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'curie_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.CURIE_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Curie deps (notably pywinpty's Rust-built
      ...
      try {
        const out = execFileSync(
          ...
          hiddenWindowsChildOptions({
            ...
            // Bare interpreter startup — much lighter than the curie-import
...
// findGitBash — locate bash.exe on Windows. Resolves CURIE_GIT_BASH_PATH
...
// normal CURIE_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\curie\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'curie', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'curie', 'git', 'bin', 'git.exe'))
  }
...
function recentCurieLog() {
  return curieLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's curie root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_CURIE_ROOT.
// CURIE_DESKTOP_CURIE_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.CURIE_DESKTOP_CURIE_ROOT && path.resolve(process.env.CURIE_DESKTOP_CURIE_ROOT),
    !IS_PACKAGED && isCurieSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isCurieSourceRoot(ACTIVE_CURIE_ROOT) ? ACTIVE_CURIE_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_CURIE_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('curie:updates:progress', merged)
  }
}
      ...
      const req = https.get(
        url,
        {
          headers: {
            ...
            'User-Agent': 'curie-desktop-update-check'
          },
          timeout: 10_000
        },
        res => {
...
// Windows that binary owns ALL repo mutation — running `curie update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(CURIE_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `curie update` can write
// fresh entry points. On Windows this is the file the running backend
// `curie.exe` holds open; on POSIX it's never mandatory-locked.
function venvCurieShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'curie.exe')
    : path.join(updateRoot, 'venv', 'bin', 'curie')
}
...
// Kill only Curie-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `curie`,
...
function killCurieOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isCurieOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Curie-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `curie.exe` that
// spawned its own grandchildren (a `curie` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Curie backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Curie backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.curieBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Curie backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.curieBackendIdentity
...
// `curieProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `curie.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a curie
// REPL, a pty terminal, the gateway) keep `curie.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const curieProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\curie.exe, but the
  // backend is `python.exe -m curie_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (curieProcess && Number.isInteger(curieProcess.pid)) {
    initialPids.push(curieProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(curieProcess, {
  ...
  // `curie gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvCurieShimPath(updateRoot), CURIE_HOME)

  // Reap Curie-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killCurieOwnedVenvDaemons(updateRoot)

  const shim = venvCurieShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentCurieProcess = backendConnectionState.getProcess()

        if (currentCurieProcess && Number.isInteger(currentCurieProcess.pid)) {
          stragglers.push(currentCurieProcess.pid)
        }
...
// `curie update`). Instead we spawn the staged Curie-Setup binary with
// --update and quit, so it can run `curie update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `curie update` → relaunch),
      ...
      // in: the CURIE_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `curie update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `curie desktop`, never the Tauri installer that self-copies
      // curie-setup.exe into CURIE_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `curie` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `curie update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `curie update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, curieRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(CURIE_HOME)
    ...
    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Curie — this window will close and the updater will open. Don’t reopen Curie yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    preflightStateDb(CURIE_HOME, rememberLog)

    // Stop our own backend(s) and wait for the venv shim to unlock BEFORE we
    // spawn the updater. Without this the updater races a still-locked
    // curie.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running curie, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Curie install open ' +
        '(a second Curie window or a terminal running curie?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvCurieShimPath(updateRoot))
      }
    ...
    // Curie processes running from this venv.  The updater normally refuses
    ...
    // `curie update` will run (the venv shim is locked while we live).
    //
    ...
    // checkout, so each `curie update` refreshes the code that drives the
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          CURIE_HOME,
          CURIE_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithCurieManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `curie update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(CURIE_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Curie update is already running (PID <itself>)" —
      ...
      if (Number.isInteger(child.pid) && stagedUpdaterSupportsPrewrittenMarker(updater)) {
        writeUpdateMarker(CURIE_HOME, child.pid)
    ...
    if (!handoffOutcome.ok) {
      const message = `Update failed to start: ${handoffOutcome.message}. Curie will keep running — try again, or run \`curie update\` from a terminal.`
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvCurie = path.join(venvBin, IS_WINDOWS ? 'curie.exe' : 'curie')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Curie launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the curie
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.curie-bootstrap-complete')),
      hasVenvCurie: fileExists(venvCurie),
...
// Python-level pre-update snapshot inside `curie update`.
function preflightStateDb(curieHome, rememberLog) {
  const stateDbPath = path.join(curieHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(curieHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(curieHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(curieHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `curie update`, swaps/relaunches the app bundle, and writes
// .curie-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'curie update', percent: null })

    return { ok: true, manual: true, command: 'curie update', curieRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      CURIE_HOME,
      CURIE_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithCurieManagedNode(path.join(updateRoot, 'venv', 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Curie — this window will close. Don’t reopen Curie yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_CURIE_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isCurieSourceRoot(ACTIVE_CURIE_ROOT) &&
    fileExists(venvPython) &&
    canImportCurieCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_CURIE_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `curie
...
function resolveWebDist() {
  const override = process.env.CURIE_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set CURIE_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: curie desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: curie desktop --force-build`
  )
...
function resolveCurieCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Curie.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveCurieCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'curie_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_CURIE_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Curie at ${ACTIVE_CURIE_ROOT}`,
    command,
    args: ['-m', 'curie_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      curieHome: CURIE_HOME,
      pythonPathEntries: [ACTIVE_CURIE_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_CURIE_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveCurieBackend(backendArgs) {
  // 1. Explicit override -- CURIE_DESKTOP_CURIE_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.CURIE_DESKTOP_CURIE_ROOT && path.resolve(process.env.CURIE_DESKTOP_CURIE_ROOT)

  if (overrideRoot && isCurieSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Curie source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `curie` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isCurieSourceRoot.)
  if (!IS_PACKAGED && isCurieSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Curie source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_CURIE_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\curie\\curie-agent (Windows) or ~/.curie/curie-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Curie runtime at ${ACTIVE_CURIE_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  // 4. Existing `curie` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    CURIE_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.CURIE_DESKTOP_IGNORE_EXISTING !== '1') {
    let curieCommand = null
    const curieOverride = process.env.CURIE_DESKTOP_CURIE

    if (curieOverride) {
      const resolvedOverride = findOnPath(curieOverride)

      if (resolvedOverride) {
        curieCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(curieOverride, { isWsl: IS_WSL })) {
        curieCommand = curieOverride
      } else {
        rememberLog(`Ignoring Windows Curie override under WSL: ${curieOverride}`)
      }
    } else {
      curieCommand = findOnPath('curie')
    }

    if (curieCommand) {
      if (looksLikeDesktopAppBinary(curieCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Curie CLI: ${curieCommand}`)
        curieCommand = null
      }
    }

    if (curieCommand) {
      const unwrapped = unwrapWindowsVenvCurieCommand(curieCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `curie` shim
      ...
      const shellForProbe = isCommandScript(curieCommand)

      // CURIE_DESKTOP_CURIE is an explicit deployment override (used by
      ...
      if (shouldTrustCurieOverride(curieOverride) || verifyCurieCli(curieCommand, { shell: shellForProbe })) {
        ...
        // unwrapWindowsVenvCurieCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Curie CLI at ${curieCommand}`,
          command: curieCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Curie CLI at ${curieCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed curie_cli module via system Python.
  ...
  if (python) {
    ...
    // having curie_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\curie\curie-agent\venv.
    if (canImportCurieCli(python)) {
      return {
        kind: 'python',
        label: `installed curie_cli module via ${python}`,
        command: python,
        args: ['-m', 'curie_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: curie_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveCurieBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Curie Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveCurieBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Curie install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Curie recovery was handed off to Curie Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      curieHome: CURIE_HOME,
      logRoot: path.join(CURIE_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Curie install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Curie bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(CURIE_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startCurie() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // curie:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveCurieBackend(backend.args))
  }
  ...
  if (!isCurieSourceRoot(ACTIVE_CURIE_ROOT)) {
    throw new Error(
      `Curie install at ${ACTIVE_CURIE_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Curie' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\curie\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `curie` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Curie on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isCurieSourceRoot()
    // plus an importable curie_cli before it hands back the active runtime.
    // If we hit this, the user (or a deleted venv) broke the invariant; tell
    // them to re-run the install.
    throw new Error(
      `Curie venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Curie at ${ACTIVE_CURIE_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Curie runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----curie-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
          reject(new Error(`Unsupported Curie backend URL protocol: ${parsed.protocol}`))
                ...
                reject(
                  new Error(
                    `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                      'The endpoint is likely missing on the Curie backend.'
                  )
                )
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Curie backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Curie-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Curie-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('curie:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveCurieCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('curie:preview-file-changed', payload)
}
...
// waitForCurieReady how to read a 401 — rejected session vs gated route.
...
async function waitForCurie(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForCurieReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('curie:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('curie:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('curie:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('curie:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Curie...".
function sendPowerResume() {
  ...
  webContents.send('curie:power-resume')
}
...
// 'curie:power-battery' push below.
ipcMain.handle('curie:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('curie:power-battery', next)
    }
  }
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('curie:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('curie:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('curie:context-menu-spellcheck', {
...
//
// Hosted Curie gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``curie_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / curie #37247).
//     Both are set as HttpOnly cookies (``curie_session_at`` ~15 min,
//     ``curie_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startCurie → the renderer's
// single-shot boot() with no retry) surfaced as the "Curie couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Curie
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Curie Cloud agent…' : 'Sign in to Curie gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Curie backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in curie_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Curie Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:curie-remote-oauth`:
...
// convention (curie_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Curie surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.CURIE_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Curie gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for curie_session_at/rt that the portal
    ...
    return await new Promise<boolean>(resolve => {
      ...
      try {
        win = new BrowserWindow({
          ...
          height: 720,
          show: false,
          title: 'Renewing Curie Cloud session…',
          autoHideMenuBar: true,
          webPreferences: {
            contextIsolation: true,
            nodeIntegration: false,
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Curie Cloud sign-in.'))
...
// Discover the hosted (Curie Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Curie Cloud. Open Settings → Gateway, choose Curie Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Curie Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Curie Cloud session has expired. Sign in to Curie Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Curie Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every curie:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteCuriePath`/`remoteProfile` —
...
// a valid stored value (pins the root CURIE_HOME explicitly); null means "no
...
// contains "curie", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isCurieProcess(pid) {
  try {
...
//   1. Legacy ~/.curie/active_profile (explicit CLI choice via curie profile use)
//   2. Running gateway (gateway.pid with verified liveness + curie identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(CURIE_HOME, 'active_profile'),
    curieHome: CURIE_HOME,
    profilesRoot: path.join(CURIE_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.CURIE_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.CURIE_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Curie Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteCuriePath: (ssh || savedSsh)?.remoteCuriePath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by CURIE_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Curie Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Curie Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteCuriePath: input.sshRemoteCuriePath ?? existingBlock.remoteCuriePath,
...
async function buildRemoteConnection(
  ...
  if (authMode === 'oauth') {
    ...
    // refresh token (curie #37247), and the gateway middleware transparently
    ...
    try {
      ...
      throw gatewayTicketFailure(
        ...
        'Could not reach the remote Curie gateway while refreshing its WebSocket ticket. Try reconnecting.'
      )
    }
  ...
  if (!token) {
    throw new Error(
      'Remote Curie gateway is selected, but no session token is saved. ' +
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.CURIE_DESKTOP_REMOTE_TOKEN,
      url: process.env.CURIE_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      curiePath: result.curiePath,
      curieHome: result.curieHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { curiePath: result.curiePath, curieHome: result.curieHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteCuriePath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteCuriePath: sshConfig.remoteCuriePath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForCurie: (baseUrl, token) => waitForCurie(baseUrl, token, lease.signal, 'token'),
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        curieVersion: result.curieVersion || '',
        ...
        // per-profile pools also use bare keys. Only startCurie' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.curieVersion || 'curie (version unknown)'} at ${result.curiePath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteCurieVersion: result.curieVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (CURIE_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.CURIE_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startCurie() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Curie
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteCuriePath: input.sshRemoteCuriePath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteCuriePath || '')
          let curiePath
          let curieVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            curiePath = runtime.curiePath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.curiePath])
            curieVersion = inspection.version
            supported = inspection.supported
          } else {
            curiePath = await remoteLifecycle.locateCurie(ssh, sshConfig.remoteCuriePath || '')
            curieVersion = await remoteLifecycle.probeCurieVersion(ssh, curiePath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, curiePath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Curie on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteCuriePath: curiePath,
            remoteCurieVersion: curieVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startCurie())
  ...
  // connect to Curie gateway". Mirror the renderer's connect here so the test
...
function resetCurieConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  const curieProcess = backendConnectionState.invalidate()
  stopBackendChild(curieProcess)
...
// startCurie() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetCurieConnection() invalidates it.
  const curieProcess = backendConnectionState.getProcess()
  const dying = curieProcess && !curieProcess.killed ? curieProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('curie:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('curie:connections:changed', payload)
    }
  }
}
...
async function ensureBackend(profile) {
  ...
  if (route.backend === 'primary') {
    const connection = await startCurie()
  ...
  // spawning, so two children never share one profile's CURIE_HOME.
  ...
  entry.connectionPromise = spawnPoolBackend(key, entry).catch(async error => {
    ...
    rememberLog(
      `Curie backend for profile "${key}" failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
...
async function ensureRegistryBackend(connectionId, profile, managedUpdateCorrelation = '') {
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteCuriePath: source.remoteCuriePath,
  ...
  if (source.kind === 'local') {
    ...
    localEntry.connectionPromise = spawnPoolBackend(profileKey, localEntry, {
      ...
      rememberLog(
        `Curie backend for profile "${profileKey}" (forced-local) failed to start: ${error instanceof Error ? error.message : String(error)}`
      )
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. curie:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: curieLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForCurie(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.curiePath || !state?.curieHome) {
  ...
  return {
    ...
    curiePath: state.curiePath,
    curieHome: state.curieHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteCuriePath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.curiePath ? platform : await probeWindowsRemote(ssh, config.remoteCuriePath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          curiePath: runtime.curiePath,
          curieHome: runtime.curieHome,
          pythonPath: runtime.python
        }
      }
    }

    const curiePath = await remoteLifecycle.locateCurie(ssh, config.remoteCuriePath || '')
    const curieHome = await remoteLifecycle.probeRemoteCurieHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, curiePath, curieHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { curiePath: state.curiePath, curieHome: state.curieHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForCurie(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startCurie() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForCurie(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited CURIE_HOME env (see _apply_profile_override
  // step 3 in curie_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveCurieBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const curieCwd = resolveCurieCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(CURIE_HOME, 'profiles', key))
  )

  rememberLog(`Starting Curie backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: curieCwd,
        CURIE_DASHBOARD_SESSION_TOKEN: token,
        ...
        ...parentIdentityEnv,
        CURIE_WEB_DIST: webDist,
        ...(readyFile ? { CURIE_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Curie backend for profile "${profile}" failed to start: ${error.message}`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Curie backend for profile "${profile}" exited (${signal || code})`)
    releaseBackendChild(child)
    backendPool.delete(poolKey)
    ...
    if (!ready) {
      rejectStart?.(
        new Error(
          `Curie backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForCurie(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Curie backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Curie backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
// backend whose ensure_curie_home() recreates the deleted profile directory.
//
...
async function startCurie() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Curie Desktop is already running in another window.')
  }

  await reapOrphanedBackendsOnce()

  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startCurie() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Curie backend', 8)
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      if (!backendConnectionState.isCurrentAttempt(connectionAttempt)) {
        throw new Error('Curie backend start was superseded by a newer connection attempt.')
      }

      await advanceBootProgress('backend.remote', `Connecting to remote Curie backend at ${remote.baseUrl}`, 24)
      await waitForCurie(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Curie backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, curieLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.curie/active_profile file) and
    // resolves CURIE_HOME the same way `curie -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Curie runtime', 28)

        return resolveCurieBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Curie backend via ${backend.label}`, 84)
    rememberLog(`Starting Curie backend via ${backend.label}`)
    ...
    const curieProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin CURIE_HOME for the child so Python's get_curie_home()
          // resolves to the SAME location our resolveCurieHome() picked. Without
          // this pin, Python falls back to ~/.curie on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\curie, which differs from C:\Users\<u>\.curie.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets CURIE_HOME via setx; the desktop
    ...
    primaryOutputTail.attach(curieProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(curieProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, curieProcess)

    if (!processOwner) {
      stopBackendChild(curieProcess)
      await waitForBackendExit(curieProcess)
      releaseBackendChild(curieProcess)
      throw new Error('Curie backend start was superseded by a newer connection attempt.')
    }

    curieProcess.stdout.on('data', rememberLog)
    curieProcess.stderr.on('data', rememberLog)
    ...
    curieProcess.once('error', error => {
      releaseBackendChild(curieProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Curie backend error: ${error.message}`)
        rejectBackendStart?.(new Error('Curie backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Curie backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Curie backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    curieProcess.once('exit', (code, signal) => {
      releaseBackendChild(curieProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Curie backend exit (${signal || code})`)
      ...
      rememberLog(`Curie backend exited (${signal || code})`)
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Curie backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Curie backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentCurieLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Curie backend to launch', 86)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Curie backend to become ready', 90)
    await Promise.race([waitForCurie(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => curieProcess.exitCode === null && !curieProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Curie backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Curie backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('curie:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Curie is
...
// pushes pet state over IPC (curie:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via curie:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Curie app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Curie icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see curie:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Curie in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('curie:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
// window showing only the composer and its scrollback, so Curie can be driven
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('curie:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('curie:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[curie] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('curie:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `curie:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, curie:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('curie:hud:goto', sessionId)
...
function spawnQuickEntryWindow() {
  ...
  win.webContents.on('did-finish-load', () => {
    if (!win.isDestroyed() && quickEntryLastState) {
      win.webContents.send('curie:quick-entry:state', quickEntryLastState)
    }
  })
...
function showQuickEntryWindow() {
  ...
  quickEntryWindow.webContents.send('curie:quick-entry:shown')
}
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'curie desktop --force-build',
  ...
  startCurie().catch(error => rememberLog(error.stack || error.message))
...
ipcMain.handle('curie:connection', async (_event, profile) => {
...
ipcMain.handle('curie:connection:for', async (_event, payload) => {
  const { connectionId, profile } = payload && typeof payload === 'object' ? (payload as any) : ({} as any)
  const registry = readDesktopConnectionsRegistry()
  const id = String(connectionId || '').trim() || registry.primary
  // Same single-owner claim as 'curie:connection', keyed by the composite
...
ipcMain.on('curie:connection:active-route', (event, route) => {
...
// "Starting Curie…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('curie:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetCurieConnection({ soft: true }),
...
ipcMain.handle('curie:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('curie:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('curie:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('curie:window:openInstance', async () => {
  createInstanceWindow()

  return { ok: true }
})
ipcMain.handle('curie:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('curie:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = resolveCurieBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Curie is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `curie-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, CURIE_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('curie:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('curie:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('curie:zoom:get', event => {
...
ipcMain.on('curie:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('curie:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('curie:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startCurie() call restarts the
...
ipcMain.handle('curie:bootstrap:repair', async () => {
  // Forceful repair: force the next startCurie() through the full installer
  ...
  // "bypass active runtime" path inside startCurie() and fall through
...
ipcMain.handle('curie:boot-progress:get', async () => bootProgressState)
ipcMain.handle('curie:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('curie:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('curie:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('curie:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('curie:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('curie:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('curie:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('curie:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('curie:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('curie:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('curie:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('curie:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('curie:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('curie:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('curie:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteCuriePath: entry.remoteCuriePath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteCuriePath: connection.remoteCuriePath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteCurieProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root CURIE_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('curie:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// curie:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('curie:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('curie:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `curie update` to every eligible registered connection at once.
...
ipcMain.handle('curie:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/curie/update', {}, { timeoutMs: 15_000 })
...
// analogue of the curie:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('curie:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('curie:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startCurie() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('curie:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Curie Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('curie:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('curie:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('curie:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('curie:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of curie:profile:set: records the profile the
...
// the gateway (#79886).
ipcMain.handle('curie:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('curie:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new CURIE_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('curie:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('curie:requestMicrophoneAccess', async () => {
...
ipcMain.handle('curie:window:readBelow', async event => {
...
async function handleCurieApiRequest(request) {
  ...
  // backend calls ensure_curie_home() which recreates the profile directory,
...
ipcMain.handle('curie:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its CURIE_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleCurieApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleCurieApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('curie:ambient:claim', (_event, key) => !claimedAmbientCue(String(key ?? '')))

ipcMain.handle('curie:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('curie:focus-session', payload.sessionId)
    }

    // Plugin / session-less activation — serializable path (+ optional notifyId
    // for renderer callbacks). Same vocabulary as curie://index-network/….
    if (payload?.activate || payload?.notifyId) {
      mainWindow.webContents.send('curie:notification-activate', {
  ...
  notification.on('action', (_actionEvent, index) => {
    ...
    if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
      mainWindow.webContents.send('curie:notification-action', { sessionId: payload.sessionId, actionId: action.id })
...
ipcMain.handle('curie:data-url-read-max:get', () => ({
...
ipcMain.handle('curie:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('curie:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('curie:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('curie:readFileText', async (_event, filePath) => {
...
// `curie:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('curie:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('curie:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('curie:writeClipboard', (_event, text) => {
...
ipcMain.handle('curie:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('curie:readClipboard', () => clipboard.readText())

ipcMain.handle('curie:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))

ipcMain.handle('curie:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))
...
ipcMain.handle('curie:context-menu:edit', (event, command) => {
...
ipcMain.handle('curie:context-menu:copy-image', event => {
...
ipcMain.handle('curie:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('curie:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('curie:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))

  return capturePreviewContents(guest, payload?.rect, payload?.viewport)
})

ipcMain.handle('curie:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('curie:saveClipboardImage', async () => {
...
ipcMain.handle('curie:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('curie:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('curie:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('curie:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('curie:active-work', (event, payload) => {
  const id = event.sender.id
...
ipcMain.on('curie:titlebar-theme', (_event, payload) => {
...
ipcMain.on('curie:native-theme', (_event, mode) => {
...
ipcMain.on('curie:translucency:support', event => {
...
ipcMain.on('curie:keep-awake', (_event, on) => {
...
ipcMain.handle('curie:quick-entry:settings:get', async () => {
...
ipcMain.handle('curie:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('curie:quick-entry:submit', (_event, payload) => {
  hideQuickEntryWindow()
  ...
  mainWindow.webContents.send('curie:quick-entry:submit', {
...
ipcMain.on('curie:quick-entry:state', (_event, payload) => {
  quickEntryLastState = payload ?? null

  if (quickEntryWindow && !quickEntryWindow.isDestroyed()) {
    quickEntryWindow.webContents.send('curie:quick-entry:state', payload)
  }
})

ipcMain.on('curie:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('curie:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('curie:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `curie:window:openSession`).
...
ipcMain.handle('curie:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('curie:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('curie:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('curie:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveCurieCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('curie:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveCurieCwd()
}))

ipcMain.handle('curie:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('curie:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('curie:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('curie:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('curie:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('curie:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('curie:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: curieLog.slice(-200) }))
...
ipcMain.on('curie:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (curie:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('curie:updates:check', async () =>
...
ipcMain.handle('curie:updates:apply', async (_event, payload) =>
...
ipcMain.handle('curie:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('curie:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Curie version (the one `release.py` bumps in
// curie_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Curie version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveCurieVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'curie_cli', '__init__.py')
...
// Renderer-bundle skew: `curie update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Curie version and push it into the native About panel
// just before showing it, so an in-place `curie update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveCurieVersion()} — app build out of date, update the desktop app`
        : resolveCurieVersion(),
      copyright: 'Copyright © 2026 Nous Research'
    })
    app.showAboutPanel()
...
ipcMain.handle('curie:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveCurieVersion(),
    electronVersion: process.versions.electron,
    nodeVersion: process.versions.node,
    platform: process.platform,
    curieRoot: resolveUpdateRoot(),
...
// The About page's "Restart Curie" button (shown when bundleSwapPending):
...
ipcMain.handle('curie:app:relaunch', async () => {
...
// the actual removal via `curie uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (curie_cli/uninstall.py + curie_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_CURIE_ROOT
  ...
  const fallback = () => ({
    curie_home: CURIE_HOME,
    agent_installed: isCurieSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'curie_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, CURIE_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Curie agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import curie_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_CURIE_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `curie-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `curie-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('curie:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('curie:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('curie:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('curie:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// curie:// deep links (e.g. curie://blueprint/morning-brief?time=08:00,
// curie://mcp/install?name=NAME&config=B64 — the vendor "Add to Curie"
// button, or curie://plugin/install?repo=owner/repo). Dev
// (`CURIE_DESKTOP_DEV_SERVER`) registers curie-dev:// instead — bare
// Electron or a stale OS handler often owns curie:// on dev machines.
...
// ---------------------------------------------------------------------------
const CURIE_PROTOCOL = DEV_SERVER ? 'curie-dev' : 'curie'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['curie-dev', 'curie'] : ['curie']
...
function handleDeepLink(url) {
  ...
  // curie://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('curie:deep-link', payload)
...
ipcMain.handle('curie:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${CURIE_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(CURIE_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(CURIE_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${CURIE_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `curie://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startCurie), whose
...
app.whenReady().then(() => {
  ...
  // startCurie() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching curie:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Curie never keeps another app's chord hostage.
