...
function createTempGitRepo() {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), 'curie-update-count-'))
  ...
  try {
    ...
    git('config', 'user.name', 'Curie Test')
    git('config', 'user.email', 'curie@example.invalid')
...
test('compareApiUrl builds the GitHub compare URL for HTTPS origins', () => {
  assert.equal(
    compareApiUrl({
      currentSha: SHA_A,
      originUrl: 'https://github.com/thisismynewfmail-ui/Cur-Agnt.git',
      targetSha: SHA_B
    }),
    `https://api.github.com/repos/thisismynewfmail-ui/Cur-Agnt/compare/${SHA_A}...${SHA_B}`
  )
})

test('compareApiUrl handles SSH origin forms', () => {
  for (const originUrl of [
    'git@github.com:thisismynewfmail-ui/Cur-Agnt.git',
    'ssh://git@github.com/thisismynewfmail-ui/Cur-Agnt.git',
    'git@github.com:thisismynewfmail-ui/Cur-Agnt'
  ]) {
    assert.equal(
