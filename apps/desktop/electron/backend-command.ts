// Backend subcommand routing for the desktop-managed Curie process.
//
// The desktop app launches its own headless backend via `curie serve` — it
...
// app hasn't updated yet, or an older `curie` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Curie profile to pin via `--profile`.
 */
 ...
 * `-m curie_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `curie_cli/subcommands/dashboard.py` source registers
