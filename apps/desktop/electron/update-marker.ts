...
export function markerPath(curieHome) {
  return path.join(curieHome, '.curie-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(curieHome)
  let raw
 ...
 *
 * The Tauri-based curie-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(curieHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(curieHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Curie to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(curieHome, opts)
