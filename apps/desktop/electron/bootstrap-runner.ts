 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Curie Agent by spawning
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.curie-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(curieHome) {
  return path.join(curieHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.curie/curie-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(curieHome) {
  if (!curieHome) {
    return null
  }

  const candidate = path.join(curieHome, 'curie-agent', 'scripts', installScriptName())
...
function cachedScriptPath(curieHome, commit) {
  return path.join(bootstrapCacheDir(curieHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/${OFFICIAL_REPO_SLUGS[0]}/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(curieHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(curieHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, curieHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass CURIE_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          CURIE_HOME: curieHome || process.env.CURIE_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, curieHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, curieHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--curie-home', curieHome]
...
async function fetchManifest({ scriptPath, installerKind, emit, curieHome, activeRoot, installStamp, pinCommit }) {
  const isPosix = installerKind === 'posix'

  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, curieHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, curieHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(curieHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, curieHome, emit })
