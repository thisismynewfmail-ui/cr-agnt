// Unit tests for the pure Windows `curie` resolution helpers extracted from
// main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
// unwrapWindowsVenvCurieCommand(). These pin the two Windows resolution bugs
// that caused desktop reinstall loops:
//   1. buildPathExtCandidates() — PATHEXT extensions must be tried BEFORE the
//      empty extension, or an extensionless Git-Bash `curie` shim shadows
//      the real curie.cmd/curie.exe.
...
//   3. resolveVenvCurieCommand() — must probe the venv python via
//      canImportCurieCli() before trusting it, or a broken venv gets
...
test('chooseUpdaterArgs: gentle --update when both updater runtime files exist', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvCurie: true, hasVenvPython: true }, 'main'), [
    '--update',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: marker-only install uses --repair when the venv is gone', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvCurie: false, hasVenvPython: false }, 'main'),
    ['--repair', '--branch', 'main']
  )
})

test('chooseUpdaterArgs: partial updater runtimes use --repair', () => {
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvCurie: false, hasVenvPython: true }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
  assert.deepEqual(chooseUpdaterArgs({ hasBootstrapMarker: true, hasVenvCurie: true, hasVenvPython: false }, 'main'), [
    '--repair',
    '--branch',
    'main'
  ])
...
test('chooseUpdaterArgs: passes the branch through unchanged in both modes', () => {
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvCurie: true, hasVenvPython: true }, 'release/1.2'),
    ['--update', '--branch', 'release/1.2']
  )
  assert.deepEqual(
    chooseUpdaterArgs({ hasBootstrapMarker: false, hasVenvCurie: false, hasVenvPython: false }, 'release/1.2'),
    ['--repair', '--branch', 'release/1.2']
  )
})

function makeDeps(overrides: Partial<Parameters<typeof resolveVenvCurieCommand>[2]> = {}) {
  return {
    isWindows: true,
    isCommandScript: () => false,
    fileExists: () => true,
    directoryExists: () => false,
    canImportCurieCli: () => true,
    getVenvPython: (venvRoot: string) => `${venvRoot}/Scripts/python.exe`,
    getVenvSitePackagesEntries: () => [],
    buildDesktopBackendEnv: () => ({ FAKE_ENV: '1' }),
    curieHome: '/fake/curie-home',
...
test('resolveVenvCurieCommand: returns null off Windows', () => {
  const deps = makeDeps({ isWindows: false })

  assert.equal(resolveVenvCurieCommand('/root/venv/Scripts/curie.exe', [], deps), null)
})

test('resolveVenvCurieCommand: returns null for a .cmd/.bat script command', () => {
  const deps = makeDeps({ isCommandScript: () => true })

  assert.equal(resolveVenvCurieCommand('/root/venv/Scripts/curie.cmd', [], deps), null)
})

test('resolveVenvCurieCommand: returns null when the basename is not curie/curie.exe', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvCurieCommand('/root/venv/Scripts/python.exe', [], deps), null)
})

test('resolveVenvCurieCommand: returns null when the parent dir is not Scripts', () => {
  const deps = makeDeps()

  assert.equal(resolveVenvCurieCommand('/root/venv/bin/curie.exe', [], deps), null)
})

test('resolveVenvCurieCommand: returns null when the venv python does not exist on disk', () => {
  const deps = makeDeps({ fileExists: () => false })

  assert.equal(resolveVenvCurieCommand('/root/venv/Scripts/curie.exe', [], deps), null)
})

test('resolveVenvCurieCommand: probes the venv python before trusting it (returns null on failed probe)', () => {
  let probed = false

  const deps = makeDeps({
    canImportCurieCli: (python: string) => {
      probed = true
  ...
  const result = resolveVenvCurieCommand('/root/venv/Scripts/curie.exe', ['serve'], deps)
...
test('resolveVenvCurieCommand: returns the resolved python backend descriptor when the probe passes', () => {
  const deps = makeDeps()
  const result = resolveVenvCurieCommand('/root/venv/Scripts/curie.exe', ['serve', '--port', '0'], deps)
  ...
  assert.deepEqual(result.args, ['-m', 'curie_cli.main', 'serve', '--port', '0'])
...
test('resolveVenvCurieCommand: is case-insensitive on curie.exe and the Scripts dir name', () => {
  const deps = makeDeps()

  assert.ok(resolveVenvCurieCommand('/root/venv/Scripts/CURIE.EXE', [], deps))
  assert.ok(resolveVenvCurieCommand('/root/venv/SCRIPTS/curie.exe', [], deps))
})
