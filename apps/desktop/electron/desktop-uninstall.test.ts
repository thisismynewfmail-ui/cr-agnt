...
test('uninstallArgsForMode maps each mode to the module-runner argv', () => {
  assert.deepEqual(uninstallArgsForMode('gui'), ['-m', 'curie_cli.uninstall', '--mode', 'gui'])
  assert.deepEqual(uninstallArgsForMode('lite'), ['-m', 'curie_cli.uninstall', '--mode', 'lite'])
  assert.deepEqual(uninstallArgsForMode('full'), ['-m', 'curie_cli.uninstall', '--mode', 'full'])
})
...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Curie.app/Contents/MacOS/Curie', 'darwin'),
    '/Applications/Curie.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Curie.app/Contents/MacOS/Curie', 'darwin'),
    '/Users/x/Applications/Curie.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Curie\\Curie.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Curie'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\curie-desktop\\Curie.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\curie-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Curie.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_CurieXXXX/curie', 'linux', { APPIMAGE: '/home/x/Apps/Curie.AppImage' }),
    '/home/x/Apps/Curie.AppImage'
  )
})

test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  assert.equal(resolveRemovableAppPath('/opt/curie/linux-unpacked/curie', 'linux', {}), '/opt/curie/linux-unpacked')
  // A system-package install (/usr/bin) → null, left to apt/dnf.
  assert.equal(resolveRemovableAppPath('/usr/bin/curie', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Curie.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Curie.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.curie/curie-agent/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.curie/curie-agent',
    uninstallArgs: ['-m', 'curie_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/curie/linux-unpacked',
    curieHome: '/home/x/.curie'
  })
  ...
  assert.match(script, /'-m' 'curie_cli\.uninstall' '--mode' 'gui'/)
  assert.match(script, /rm -rf '\/opt\/curie\/linux-unpacked'/)
  assert.match(script, /export CURIE_HOME='\/home\/x\/\.curie'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.curie/curie-agent',
    agentRoot: '/home/x/.curie/curie-agent',
    uninstallArgs: ['-m', 'curie_cli.uninstall', '--mode', 'full'],
    appPath: null,
    curieHome: '/home/x/.curie'
  })

  // System python + source on PYTHONPATH so import curie_cli works while the
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.curie\/curie-agent'/)
  assert.match(script, /'\/usr\/bin\/python3' '-m' 'curie_cli\.uninstall' '--mode' 'full'/)
})
...
test('buildPosixCleanupScript omits the bundle rm when appPath is null', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: null,
    agentRoot: '/a',
    uninstallArgs: ['-m', 'curie_cli.uninstall', '--mode', 'lite'],
    appPath: null,
    curieHome: '/h'
  })
  ...
  assert.match(script, /'-m' 'curie_cli\.uninstall' '--mode' 'lite'/)
})
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\curie',
    uninstallArgs: ['-m', 'curie_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Curie',
    curieHome: 'C:\\Users\\x\\AppData\\Local\\curie'
  })

  assert.match(script, /@echo off/)
  assert.match(script, /set "PID=9988"/)
  // PYTHONPATH set so a system python can import curie_cli from source.
  assert.match(script, /set "PYTHONPATH=C:\\curie;%PYTHONPATH%"/)
  assert.match(script, /"C:\\Python313\\python.exe" "-m" "curie_cli\.uninstall" "--mode" "full"/)
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Curie" >nul 2>&1/)
