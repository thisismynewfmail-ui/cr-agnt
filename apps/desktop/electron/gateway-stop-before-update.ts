 ...
 *
 * So we delegate to `curie gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `curie update`'s
...
export function stopGatewayBeforeUpdate(
  ...
  return runGatewayLifecycleCommand(curieCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(curieCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(curieCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(curieCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(curieCliPath, args)
  }

  if (!existsSync(curieCliPath)) {
    return false
  }

  try {
    exec(curieCliPath, args, {
