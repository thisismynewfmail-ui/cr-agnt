 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `curie update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `curie desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Curie couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>curie desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>curie desktop --force-build</code>, then restart the app.</p>
