// Launching the Curie TUI in the user's OWN terminal emulator.
//
...
// `curie --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Curie runtime is often a venv Python invoked
//     as `python -m curie_cli.main`, not a `curie` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Curie
