...
export const ZOOM_STORAGE_KEY = 'curie:desktop:zoomLevel'
 ...
 * but forgot to emit 'curie:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('curie:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `curie:zoom:changed`, which never fired —
