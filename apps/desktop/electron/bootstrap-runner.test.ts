...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'curie-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/curie-runner-test',
    sourceRepoRoot: null,
    curieHome: '/tmp/curie-runner-test',
    logRoot: '/tmp/curie-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'curie-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'curie-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/curie-agent',
      curieHome: '/tmp/curie'
    }),
    ['--dir', '/tmp/curie-agent', '--curie-home', '/tmp/curie', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/curie-agent', '--curie-home', '/tmp/curie', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/curie',
      curieHome: '/tmp/home'
    }),
    ['--dir', '/tmp/curie', '--curie-home', '/tmp/home', '--branch', 'main']
  )
})
