...
export function resolveDesktopRemoteRoute({
  ...
  if (envUrl) {
    ...
    if (!envToken) {
      throw new Error(
        'CURIE_DESKTOP_REMOTE_URL is set but CURIE_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Curie backend.'
      )
    }
  ...
  if (!modeIsRemoteLike(config.mode)) {
    ...
    // resolves local and spawns a loopback `curie serve` the desktop never
