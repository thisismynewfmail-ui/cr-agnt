 ...
 *   1. CURIE_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\curie\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect CURIE_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.CURIE_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'curie', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'curie', 'git', 'usr', 'bin', 'bash.exe'))
  }
