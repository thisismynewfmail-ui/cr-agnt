 ...
 *   - the browser sees only a minimal "return to Curie" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Curie.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('curie:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('curie:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('curie:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
