 ...
 *     `X-Curie-Session-Token` header; WS uses `?token=`.
...
// curie_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - curie_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - curie_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / curie #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-curie_session_at', '__Secure-curie_session_at', 'curie_session_at']
const RT_COOKIE_VARIANTS = ['__Host-curie_session_rt', '__Secure-curie_session_rt', 'curie_session_rt']

// The Nous portal (NAS) does NOT use Curie gateway session cookies — it is a
...
// Keep this aligned with curie_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['curie', 'test', 'tmp', 'root', 'sudo'])
...
/** Which Curie profile the remote SSH dashboard should actually run as.
...
// Curie Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteCuriePath = String(entry.remoteCuriePath || '').trim()

  if (remoteCuriePath) {
    out.remoteCuriePath = remoteCuriePath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Curie installation. Preserve an explicit mapping
  // when it is a valid Curie profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_curie_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `CURIE_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process CURIE_HOME can belong to a different
 ...
 * `curie:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://curie.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `curie:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a curie session ACCESS-token cookie
 ...
 * the Curie gateway session cookies, so cloud sign-in / discovery liveness
