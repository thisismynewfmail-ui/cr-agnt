...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'curie serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/curie serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m curie_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/curie --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Curie Runtime\\curie.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('curie chat --query serve'), false)
