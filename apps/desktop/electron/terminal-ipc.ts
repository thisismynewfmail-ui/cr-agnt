...
// registry, and the curie:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // CURIE_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.CURIE_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Curie' TUI into
    ...
    env.TERM_PROGRAM = 'Curie'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a curie/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from CURIE_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.CURIE_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `curie:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('curie:terminal:start', async (event, payload = {}) => {
  ...
  ipcMain.handle('curie:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('curie:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('curie:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('curie:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('curie:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))
