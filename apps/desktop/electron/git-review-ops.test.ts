...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'curie-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'curie-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Curie Test'], { cwd: dir })
