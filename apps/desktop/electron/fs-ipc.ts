...
// hardening, CURIE_HOME resolution, and the git binary stay injected.
...
export function registerFsIpc({
  ...
  ipcMain.handle('curie:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

  ipcMain.handle('curie:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

  // Reveal a path in the OS file manager (Finder / Explorer / Files).
  ipcMain.handle('curie:fs:reveal', async (_event, targetPath) => {
  ...
  ipcMain.handle('curie:fs:openDir', async (_event, dirPath) => {
  ...
  // The LOCAL Desktop runtime-plugin root: `<CURIE_HOME>/desktop-plugins`,
  // resolved from the main-process CURIE_HOME (see resolveCurieHome) — NOT from
  // the connected backend. A remote backend reports its own `curie_home` over
  ...
  async function localPluginsRoot(dirName: string): Promise<string> {
    ...
    // profiles/<name>/, matching the profile-scoped curie_home the backend
    // reported before this resolver existed. 'default'/unset pins the global root.
    const profile = readActiveDesktopProfile()
    const base = profile && profile !== 'default' ? path.join(curieHome, 'profiles', profile) : curieHome
  ...
  ipcMain.handle('curie:fs:desktopPluginsRoot', async () => localPluginsRoot('desktop-plugins'))

  // The LOCAL logs root (`<CURIE_HOME>/logs`, profile-aware) — the error
  // card's "Open Logs" action reveals agent.log/gateway.log without the user
  // knowing where CURIE_HOME lives. Same Electron-local resolution as the
  // plugin roots: valid in every connection mode, created on demand.
  ipcMain.handle('curie:fs:logsRoot', async () => localPluginsRoot('logs'))

  // The LOCAL agent-plugin root (`<CURIE_HOME>/plugins`), same Electron-local
  ...
  ipcMain.handle('curie:fs:agentPluginsRoot', async () => localPluginsRoot('plugins'))

  ipcMain.handle('curie:plugin:probe', async (_event, payload) => {
  ...
  ipcMain.handle('curie:plugin:installDesktop', async (_event, payload) => {
    const identifier = String(payload?.identifier || payload?.repo || '').trim()
  ...
  ipcMain.handle('curie:fs:rename', async (_event, targetPath, newName) => {
  ...
  ipcMain.handle('curie:fs:writeText', async (_event, filePath, content) => {
  ...
  ipcMain.handle('curie:fs:trash', async (_event, targetPath) => {
    const target = String(targetPath || '').trim()
