 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `curie update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.curieHome, 'Curie home')
  const curiePath = validateRemoteValue(target.curiePath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(curiePath)

  const updateCommand =
    `env CURIE_HOME=${homeWord} ` +
    `CURIE_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'CURIE_UPDATE_ORIGIN_PROFILE=default ' +
    `CURIE_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `CURIE_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:CURIE_HOME=${psLiteral(home)}`,
    `$env:CURIE_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:CURIE_UPDATE_ORIGIN_PROFILE="default"',
    `$env:CURIE_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:CURIE_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:CURIE_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:CURIE_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:CURIE_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(curiePath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.curie-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Curie updated and every managed SSH profile is ready.'
        : restoreOk
