 ...
 * the three user-facing uninstall modes to the `curie uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m curie_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `curie_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'curie_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Curie.app/Contents/MacOS/Curie  → …/Curie.app
 *   Windows: …\Curie\Curie.exe                 → …\Curie  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Curie.app/Contents/MacOS/Curie → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Curie.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Curie.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Curie$/i.test(dir) || /[\\/]curie-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/curie
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import curie_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, curieHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import curie_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Curie install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "CURIE_HOME=${String(curieHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
