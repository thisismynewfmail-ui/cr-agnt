...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/curie', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/curie/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/curie'), 'https://gw.example.com/curie/auth/native/refresh')
})
