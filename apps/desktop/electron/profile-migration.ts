...
export interface MigrationDeps {
  ...
  /** Default profile home (`~/.curie`). Default's state.db and gateway.pid live here. */
  curieHome: string
  /** Named-profile root (`~/.curie/profiles`). Does not contain `default`. */
  profilesRoot: string
  ...
  isCurieProcess: (pid: number) => boolean
  now: () => number
...
/**
 * Production layout: default IS `curieHome`; named profiles are children of
 * `profilesRoot`. There is no `profiles/default` directory on a normal install.
 */
export function profileStateDbPath(name: string, curieHome: string, profilesRoot: string): string {
  return name === 'default' ? `${curieHome}/state.db` : `${profilesRoot}/${name}/state.db`
}

export function profileGatewayPidPath(name: string, curieHome: string, profilesRoot: string): string {
  return name === 'default' ? `${curieHome}/gateway.pid` : `${profilesRoot}/${name}/gateway.pid`
}

function resolveCurieHome(profilesRoot: string, curieHome?: string): string {
  if (curieHome) {
    return curieHome
  }

  // Tests that predate curieHome pass only profilesRoot.
...
/**
 * Return the profile names whose gateway.pid file points to a live curie process.
 ...
 *
 * `curieHome` is optional so existing call sites that only pass `profilesRoot`
 * still work: it is derived as the parent of `…/profiles`.
 */
export function findRunningGatewayProfiles(
  profilesRoot: string,
  allProfiles: string[],
  deps: Pick<MigrationDeps, 'existsSync' | 'readFileSync' | 'isCurieProcess'> & { curieHome?: string }
): string[] {
  const curieHome = resolveCurieHome(profilesRoot, deps.curieHome)
  const running: string[] = []

  for (const name of allProfiles) {
    const pidFile = profileGatewayPidPath(name, curieHome, profilesRoot)
...
export function decideMigration(
  ...
  for (const name of candidates) {
    const s = score(profileStateDbPath(name, deps.curieHome, deps.profilesRoot))
...
/** Default is always a candidate; it is `$CURIE_HOME`, not `$CURIE_HOME/profiles/default`. */
...
export function migrateActiveProfileIfMissing(desktopProfileConfigPath: string, deps: MigrationDeps): boolean {
  ...
  // launches `curie --profile default` and is worse than writing nothing
