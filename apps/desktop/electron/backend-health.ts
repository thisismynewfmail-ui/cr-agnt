...
export interface CurieReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForCurieReady`).
   */
...
export const REMOTE_UNSIGNED_OAUTH_MESSAGE =
  'Remote Curie gateway uses OAuth, but you are not signed in. ' +
 ...
 *  - readiness-probe exhaustion in waitForCurieReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed Curie Cloud instance
 ...
 * `isReauthRequired` so startCurie latches; `needsOauthLogin` alone only
...
export async function waitForCurieReady(baseUrl: string, options: CurieReadyOptions): Promise<void> {
  ...
  throw new Error(`Curie backend did not become ready: ${detail}`)
}
