 ...
 *
 * The pre-update handoff kills Curie-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `curie`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Curie-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Curie and holds venv shims mapped).
 */
export function isCurieOwnedVenvDaemon(
