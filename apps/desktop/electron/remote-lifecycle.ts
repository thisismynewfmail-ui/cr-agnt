 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Curie dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Curie install on the remote (login-shell probe),
 ...
 *     `CURIE_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForCurie in.
 *
 * The minted CURIE_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
import { OFFICIAL_REPO_SLUGS } from './update-remote'
...
const READY_RE = /^CURIE_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
const REMOTE_LOCK_DIR = '~/.curie/desktop-ssh'
...
// Resolve the remote curie executable. An EXPLICIT path is honored strictly
...
async function locateCurie(ssh, remoteCuriePath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The curie binary or wrapper script
    ...
    //     the Curie version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/thisismynewfmail-ui/Cur-Agnt/issues/74411
    return candidate
  }
  ...
  if (remoteCuriePath) {
    if (await isExecutable(remoteCuriePath)) {
      return resolveLauncher(remoteCuriePath)
    }

    const err: any = new Error(
      `The Curie path you set is not an executable on the remote host: "${remoteCuriePath}". ` +
        'Check the path (it must be the full path to the `curie` binary on the remote, e.g. ' +
        '~/curie-agent/.venv/bin/curie), or clear it to auto-detect.'
    )

    err.kind = 'curie-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v curie')}`)).trim()
  ...
  candidates.push('~/.local/bin/curie')
  candidates.push('/usr/local/bin/curie')
  candidates.push('~/.curie/curie-agent/venv/bin/curie')
  ...
  const err: any = new Error(
    'Curie is not installed on the remote host (could not find a `curie` executable). ' +
      `Install it on the remote with:  curl -fsSL https://raw.githubusercontent.com/${OFFICIAL_REPO_SLUGS[0]}/main/scripts/install.sh | sh  ` +
      '— or set the Curie path explicitly in the SSH connection settings.'
  )
...
// Probe the resolved binary's version string (first line of `<curie> --version`,
// e.g. "Curie Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH curie a
// connection uses, so a stale/unexpected install is visible.
async function probeCurieVersion(ssh, curiePath) {
  try {
    const out = (await ssh.exec(`${expandRemotePath(curiePath)} --version 2>&1`)).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Curie Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The CURIE_HOME the remote dashboard will use (explicit env wins, else
// ~/.curie). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteCurieHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${CURIE_HOME:-$HOME/.curie}"')).trim().split('\n').pop()

    return out || '~/.curie'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Curie home.')
...
marker=home/'.curie-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Curie checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, curieHome) {
  const home = assertSafeRemoteHome(curieHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Curie install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Curie update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Curie update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteCurieProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteCurieHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Curie profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Curie home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'curiePath', 'curieHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !curiePath) {
    return false
  }
  ...
  try {
    const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(curiePath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/curie`, leaving neither
      // launcher nor CURIE_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `curie_home=os.path.expanduser(${shq(curieHome)}) if ${shq(curieHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if curie_home:\n' +
      ' expected_entries.add(os.path.join(curie_home,"curie-agent","venv","bin","curie"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.curiePath)
  const expectedHome = lock.curieHome ? expandRemotePath(lock.curieHome) : "''"
  ...
  const executableMatch = lock.curieHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.curiePath === expected.curiePath &&
    lock.curieHome === expected.curieHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.curiePath)})
curie_home=os.path.expanduser(${py(lock.curieHome)})
expected_entries={expected_path,os.path.join(curie_home,"curie-agent","venv","bin","curie")}
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"curie-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(curiePath, profile, opts: any = {}) {
  const curie = expandRemotePath(curiePath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.curieHome || '~/.curie')}/.curie-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.curieHome || '~/.curie')}/.curie-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env CURIE_DESKTOP=1 ${curie} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} curie-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, curiePath) {
  const curie = expandRemotePath(curiePath)

  const out = await ssh.exec(
    `help="$(${curie} serve --help 2>&1)"; ` +
...
async function spawnRemoteDashboard(
  ssh,
  { curiePath, profile, token, ownershipId, curieHome = '~/.curie', assertInstallClear = async () => {} }
) {
  if (!(await remoteSupportsSshOwnership(ssh, curiePath))) {
    const err: any = new Error(
      'The remote Curie install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Curie on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(curiePath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        curieHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForCurie, a token-gated
...
async function connect(deps) {
  ...
  const curieHome = await probeRemoteCurieHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, curieHome)
  const curiePath = await locateCurie(ssh, remoteCuriePath)
  log(`located curie at ${curiePath}`)
  const curieVersion = await probeCurieVersion(ssh, curiePath)

  if (curieVersion) {
    log(`remote curie version: ${curieVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Curie Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.curiePath === curiePath &&
      lock.curieHome === curieHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    ownershipId,
    curieHome,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, curieHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForCurie(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
