...
const READY_RE = /^CURIE_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitCuriePath = '') {
  const explicit = psLiteral(explicitCuriePath)
  ...
  const script = [
    ...
    '$curieHome=$env:CURIE_HOME',
    'if(-not $curieHome){$curieHome=Join-Path $env:LOCALAPPDATA "curie"}',
    'Assert-NoReparse $curieHome $true',
    '$candidate=[IO.Path]::Combine($curieHome, "curie-agent\\venv\\Scripts\\curie.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "curie-agent\\.venv\\Scripts\\curie.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $curieHome "curie-agent\\venv\\Scripts\\curie.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "curie-agent\\.venv\\Scripts\\curie.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command curie.exe -ErrorAction SilentlyContinue',
    ...
    '$curie=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$curie=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $curie){throw "Curie is not installed on the remote Windows host."}',
    'Assert-NoReparse $curie $false',
    'if($explicit -and $curie -ne $explicit){throw "The configured Curie path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($curie), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;curieHome=$curieHome;curiePath=$curie;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(curieHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class CurieMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$home=${psLiteral(curieHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".curie-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[CurieMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, curieHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(curieHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitCuriePath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitCuriePath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'curie_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'curie_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'curie_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$home=${psLiteral(runtime.curieHome)}`,
    '$installRoot=$home',
    '$parent=Split-Path -Parent $home',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".curie-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};curiePath=${psLiteral(reservation.curiePath)};curieHome=${psLiteral(reservation.curieHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.curiePath === 'string' &&
    typeof lock.curieHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.curiePath === runtime.curiePath &&
    lock.curieHome === runtime.curieHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteCuriePath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.curieHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.curiePath])

  if (!inspection.supported) {
    const error: any = new Error('Update Curie on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.curiePath = inspection.path
  const curieVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located curie at ${runtime.curiePath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, curiePath: runtime.curiePath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForCurie(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Curie SSH"', 'powershell.exe -NoLogo')
