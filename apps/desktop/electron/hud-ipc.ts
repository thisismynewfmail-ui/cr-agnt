...
export function registerHudIpc({
  ...
  ipcMain.on('curie:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('curie:hud:windowing', event => {
  ...
  ipcMain.on('curie:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('curie:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('curie:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('curie:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('curie:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('curie:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('curie:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('curie:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('curie:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('curie:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('curie:hud:close', async () => {
    closeHudWindow()
