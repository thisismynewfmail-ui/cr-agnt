export type ParentWatchdogEnv = {
  CURIE_PARENT_PID: string
  CURIE_PARENT_START_MARKER?: string
  CURIE_PARENT_NONCE?: string
  /** Spawn tag consumed by curie_cli.process_identity (`v1:<install>:<purpose>:<spawner_pid>:<spawner_create_s>`).
...
export function parentWatchdogEnv(pid: number, startMarker: string | null, nonce: string): ParentWatchdogEnv {
  ...
  const env: ParentWatchdogEnv = { CURIE_PARENT_PID: String(pid) }
  env.CURIE_SPAWN = spawnTag(pid, startMarker)
  ...
  env.CURIE_PARENT_START_MARKER = startMarker
  env.CURIE_PARENT_NONCE = nonce

  return env
}

/** Build the `CURIE_SPAWN` tag mirrored by curie_cli/process_identity.py.
