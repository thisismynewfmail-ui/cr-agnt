// Profile-delete routing logic for the `curie:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_curie_home() and
...
/** Parse a `curie:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its CURIE_HOME.
 *
...
/**
 * Route the next `curie:api` request away from the primary/window backend
 ...
 * ensure_curie_home() recreates the directory the delete just removed.
 */
