 ...
 * runtime, remote gateways, Curie Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@curie/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteCuriePath || '').trim() !== String(activeSsh.remoteCuriePath || '').trim() ||
...
/** Turn `ls ~/.curie/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Curie Cloud
 * instances are platform-managed — we never run `curie update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteCuriePath: input.remoteCuriePath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Curie-managed names; an empty result stores nothing.
 ...
 * `remoteCuriePath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Curie Cloud' : 'Remote gateway'),
