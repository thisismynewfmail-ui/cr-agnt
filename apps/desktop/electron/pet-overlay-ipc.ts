...
export function registerPetOverlayIpc({
  ...
  ipcMain.handle('curie:pet-overlay:open', async (_event, request) => {
  ...
  ipcMain.handle('curie:pet-overlay:close', async () => {
    closePetOverlay()
  ...
  ipcMain.on('curie:pet-overlay:set-bounds', (_event, bounds) => {
  ...
  ipcMain.on('curie:pet-overlay:ignore-mouse', (_event, ignore) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('curie:pet-overlay:set-focusable', (_event, focusable) => {
    const petOverlayWindow = getPetOverlayWindow()
  ...
  ipcMain.on('curie:pet-overlay:state', (_event, payload) => {
    const petOverlayWindow = getPetOverlayWindow()

    if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
      petOverlayWindow.webContents.send('curie:pet-overlay:state', payload)
    }
  })
  // Overlay → main renderer: control messages (pop back in, composer submit).
  ipcMain.on('curie:pet-overlay:control', (_event, payload) => {
    const mainWindow = getMainWindow()
    ...
    mainWindow.webContents.send('curie:pet-overlay:control', payload)
  })
}
