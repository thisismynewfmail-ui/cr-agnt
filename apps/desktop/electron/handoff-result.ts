 ...
 * console. It writes CURIE_HOME/.curie-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Curie within 30 minutes.
...
export function handoffResultPath(curieHome: string): string {
  return path.join(curieHome, '.curie-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(curieHome)
  let raw: string
