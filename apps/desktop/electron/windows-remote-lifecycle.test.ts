...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command curie.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          curieHome: 'C:\\Users\\alice\\.curie',
          curiePath: 'C:\\Curie\\curie.exe',
          python: 'C:\\Curie\\python.exe'
        })
      }

      if (script.includes('.curie-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('curie_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\\.curie\\profiles\\research')
  assert.match(script, /\.curie-update-in-progress/)
...
test('Windows probe validates Curie and Python topology before selection', async () => {
  let script = ''
  ...
  const fallbackJoin = script.indexOf('Join-Path $curieHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Curie is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Curie is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Curie's\\python.exe" }, 'inspect', [
    'C:\\x y\\curie.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'curie_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Curie''s/)
  assert.match(script, /C:\\x y\\curie\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { curiePath: lock.curiePath, curieHome: lock.curieHome }
