...
//   CURIE_WIN_SSH_HOST   ssh alias/host of the Windows box
//   CURIE_WIN_SSH_USER   remote user
//   CURIE_WIN_SSH_CURIE absolute path to the remote curie.exe under test
const liveHost = process.env.CURIE_WIN_SSH_HOST || ''
const liveUser = process.env.CURIE_WIN_SSH_USER || ''
const configuredCurie = process.env.CURIE_WIN_SSH_CURIE || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Curie-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredCurie)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredCurie.replace('curie.exe', 'python.exe')}' -m curie_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredCurie.replace('curie.exe', 'python.exe')
        const terminate = `& '${python}' -m curie_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.curiePath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m curie_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m curie_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
