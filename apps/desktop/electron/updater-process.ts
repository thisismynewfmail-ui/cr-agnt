 ...
 *
 * The staged Tauri `curie-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `curie update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `curie update`, swaps/relaunches
 * the app, and writes .curie-update-result.json. With the app gone before
 * the update starts, the CURIE_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `curie update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into CURIE_HOME on *every* platform
 * (`curie-setup.exe` on Windows, `curie-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_curie_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `curie update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(curieHome, 'curie-setup.exe')
 ...
 *
 * `copy_self_to_curie_home` deliberately no-ops during `--update`
 ...
 * "Another Curie update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `curie update`, which
 ...
 * the installer is written to CURIE_HOME at install/repair time, making mtime
