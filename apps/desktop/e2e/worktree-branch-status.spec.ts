...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Curie E2E'], { cwd: repo })
...
function configureRepoCwd(curieHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(curieHome, mockUrl)
  fs.appendFileSync(path.join(curieHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(curieHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.curieHome, mock.url, repo)
