...
async function setupSeededMockBackend(): Promise<MockBackendFixture> {
  ...
  fs.appendFileSync(
    path.join(sandbox.curieHome, 'config.yaml'),
    '\ncompression:\n  threshold_tokens: 1\n',
    'utf8',
  )
  writeEnvFile(sandbox.curieHome)
  const builder = await RealSessionBuilder.start(sandbox.curieHome)
...
test('live verify-on-stop continuations stay out of the transcript', async ({}, testInfo) => {
  ...
  writeMockProviderConfig(sandbox.curieHome, mock.url)
  fs.appendFileSync(path.join(sandbox.curieHome, 'config.yaml'), '\nagent:\n  verify_on_stop: true\n', 'utf8')
