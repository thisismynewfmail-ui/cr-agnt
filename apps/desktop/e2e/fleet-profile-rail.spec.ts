 ...
 * second gateway, "Homelab", is a REAL second `curie serve` this spec spawns
 * with its own CURIE_HOME, profiles and session token, registered in the v2
...
function findCurieBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'curie')

  if (fs.existsSync(venv)) {
    return venv
  }

  const result = spawnSync('which', ['curie'], { encoding: 'utf8' })

  if (result.status === 0 && result.stdout.trim()) {
    return result.stdout.trim()
  }

  throw new Error('curie binary not found: create the repo venv (uv sync) or put curie on PATH')
}
...
/**
 * Spawn a second, fully real `curie serve` as the remote gateway. Its
 * session token is pinned through CURIE_DASHBOARD_SESSION_TOKEN so the
...
async function startRemoteGateway(root: string, mockUrl: string, profiles: string[]): Promise<RemoteGateway> {
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        CURIE_HOME: home,
        CURIE_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    if (child.exitCode !== null) {
      throw new Error(`remote curie serve exited early (${child.exitCode}):\n${log}`)
    }

    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-Curie-Session-Token': REMOTE_TOKEN },
      })
  ...
  if (Date.now() >= deadline) {
    throw new Error(`remote curie serve never became ready:\n${log}`)
  }
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    writeMockProviderConfig(sandbox.curieHome, mock.url)
    ...
    seedProfiles(sandbox.curieHome, ['research'])
