 ...
 * AIAgent loop, using the E2E mock provider configured in `curieHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(curieHome: string) {
  ...
  static async start(curieHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(curieHome)
