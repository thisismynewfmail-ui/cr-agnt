...
/**
 * E2E smoke tests for the packaged Curie desktop app.
 *
 ...
 * (credential stripping, isolated CURIE_HOME + userData, unique app name).
 *
...
test('window opens with the Curie title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      curieDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).curieDesktop?.hud?.open({ sessionId: null })
  )
