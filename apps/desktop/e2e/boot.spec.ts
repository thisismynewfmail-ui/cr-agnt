 ...
 * packaged binary) with a real `curie serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → curie serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Curie title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Curie')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { curieDesktop?: Record<string, unknown> }).curieDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → curie serve is spawned → WS connects → config
