/**
 * Shared E2E fixtures for the Curie desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → curie serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `curie desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `curie-e2e-${prefix}-${Math.random()}`))
  const curieHome = path.join(root, 'curie-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(curieHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(curieHome, 'config.yaml')
 ...
 */
export function writeEnvFile(curieHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(curieHome, '.env')
 ...
 */
function writeEmptyConfig(curieHome: string): void {
  const configPath = path.join(curieHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - CURIE_HOME → sandbox curie-home (isolated config/sessions)
 *  - CURIE_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - CURIE_DESKTOP_IGNORE_EXISTING=1 → don't pick up `curie` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - CURIE_DESKTOP_CURIE_ROOT → REPO_ROOT (dev checkout resolution)
 *  - CURIE_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    CURIE_HOME: sandbox.curieHome,
    CURIE_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    CURIE_DESKTOP_IGNORE_EXISTING: '1',
    CURIE_DESKTOP_CURIE_ROOT: REPO_ROOT,
    CURIE_DESKTOP_APP_NAME: `CurieE2E-${Date.now()}`,
    ...
    CURIE_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated CURIE_HOME + userData
 * @param env      - the process environment (already has CURIE_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.curieHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.curieHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via CURIE_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`curie serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.curieHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { CURIE_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Curie backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Curie.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Curie.app', 'Contents', 'MacOS', 'Curie')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'curie')
}
 ...
 * progress without spawning a real Curie backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * CURIE_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    CURIE_DESKTOP_BOOT_FAKE: '1',
    CURIE_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + curie-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).CURIE_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).CURIE_DESKTOP_CURIE
  delete (env as Record<string, string | undefined>).CURIE_DESKTOP_CURIE_ROOT
