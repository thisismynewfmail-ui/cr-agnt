...
How to build Curie Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Curie surface can also
...
- A **runtime home change** (switching the underlying `CURIE_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Curie — do not assume
