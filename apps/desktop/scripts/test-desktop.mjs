...
// no longer bundles the Curie Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Curie.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Curie'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Curie.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Curie'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default CURIE_HOME for non-sandboxed runs -- matches main.ts's
// resolveCurieHome(). On Windows it's %LOCALAPPDATA%\curie; elsewhere
// it's ~/.curie. The fresh-install sandbox launchFresh() sets its own
// CURIE_HOME and never touches this.
const DEFAULT_CURIE_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'curie')
  }
  return path.join(os.homedir(), '.curie')
})()
const VENV_ROOT = path.join(DEFAULT_CURIE_HOME, 'curie-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'curie-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.CURIE_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Curie-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Curie-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Curie-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Curie-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.CURIE_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.CURIE_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const curieHome = path.join(sandbox, 'curie-home')
  ...
  env.CURIE_DESKTOP_CWD = cwd
  env.CURIE_DESKTOP_IGNORE_EXISTING = '1'
  env.CURIE_DESKTOP_TEST_MODE = 'fresh-install'
  env.CURIE_DESKTOP_USER_DATA_DIR = userDataDir
  env.CURIE_HOME = curieHome
  delete env.CURIE_DESKTOP_CURIE
  delete env.CURIE_DESKTOP_CURIE_ROOT
  ...
  console.log(`  CURIE_HOME: ${curieHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(curieHome, 'curie-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Curie Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of curie_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'curie-agent', 'curie_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Curie
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + CURIE_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  CURIE_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
