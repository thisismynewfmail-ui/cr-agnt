...
//   · its own CURIE_HOME      → its own backend + sessions, no shared state.
//   · its own --remote-debugging-port → a private CDP endpoint.
//   · CURIE_DESKTOP_BOOT_FAKE=1 → deterministic boot overlay.
...
// Seed an isolated CURIE_HOME with just enough config (NOT sessions) so the
// spawned instance reaches an empty chat view instead of the onboarding wizard.
// A separate CURIE_HOME dir means a separate gateway lock — no collision with
...
export async function startIsolatedInstance({
  ...
  const home = curieHome ?? mkTemp('curie-perf-home-')
  const userData = userDataDir ?? mkTemp('curie-perf-ud-')
  const devUrl = prod ? null : `http://127.0.0.1:${devPort}`

  if (seedConfig && !curieHome) {
    seedConfigFrom(join(homedir(), '.curie'), home)
  }
  ...
  try {
    ...
    // CURIE_HOME (backend + sessions). No DEV_SERVER env in prod → dist load.
    const electronBin = require('electron')
    // NB: do NOT set CURIE_DESKTOP_BOOT_FAKE here — it injects artificial
    ...
    const env = {
      ...
      CURIE_DESKTOP_CDP_PORT: String(port),
      XCURSOR_SIZE: '24'
    }

    if (devUrl) {
      env.CURIE_DESKTOP_DEV_SERVER = devUrl
    }
...
export async function coldStartSamples({ runs = 3, port = 9222, devPort = 5174, prod = false, warm = true } = {}) {
  ...
  if (warm) {
    ...
    const home = mkdtempSync(join(tmpdir(), 'curie-perf-cold-home-'))
    const userDataDir = mkdtempSync(join(tmpdir(), 'curie-perf-cold-ud-'))
    seedConfigFrom(join(homedir(), '.curie'), home)
