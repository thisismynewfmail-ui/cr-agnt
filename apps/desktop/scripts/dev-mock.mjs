...
function createSandbox() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `curie-dev-mock-${Date.now()}`))
  ...
  return { root, curieHome, userDataDir, cleanup: () => fs.rmSync(root, { recursive: true, force: true }) }
}

function writeMockConfig(curieHome, mockUrl) {
  fs.writeFileSync(
    path.join(curieHome, 'config.yaml'),
...
`,
  ...
  fs.writeFileSync(path.join(curieHome, '.env'), 'MOCK_API_KEY=e2e-mock-key\n', 'utf8')
}
...
async function main() {
  ...
  writeMockConfig(sandbox.curieHome, mock.url)
  console.log(`  CURIE_HOME: ${sandbox.curieHome}`)
  ...
  const env = {
    ...
    CURIE_DESKTOP_APP_NAME: `CurieDevMock-${Date.now()}`,
  }
