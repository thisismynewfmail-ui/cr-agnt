...
const UPSTREAM_URL = 'https://github.com/thisismynewfmail-ui/Cur-Agnt.git'
...
const LEGACY_STORAGE_KEY = 'curie.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'curie.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.curieDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `curie-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const curieHome = join(sandbox, 'curie-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(curieHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(curieHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        CURIE_DESKTOP_APP_NAME: `CurieShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        CURIE_DESKTOP_CURIE_ROOT: target.targetRoot,
        CURIE_DESKTOP_IGNORE_EXISTING: '1',
        CURIE_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'curie-short-session-ab-'))
  const prepared = []
