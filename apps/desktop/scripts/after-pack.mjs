 ...
 *
 * Stamps the Curie icon + identity onto the packed Windows Curie.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `curie desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Curie')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Curie'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Curie.exe keeps the stock Electron icon`)
  }
}
