 ...
 * product name (`Curie`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Curie'
 *
 * This is a hard failure with no obvious cause for the user — `curie desktop`
 ...
 * (the mac-unpacked Curie.app bundle) and Windows (win-unpacked), so we
 ...
 * Curie.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `curie desktop --build-only` (curie_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Curie.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Curie'}.exe`
