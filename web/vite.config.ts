...
const BACKEND = process.env.CURIE_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `curie dashboard` server injects a one-shot
 * session token into `index.html` (see `curie_cli/web_server.py`). The
 * Vite dev server serves its own `index.html`, so unless we forward that
 * token, every protected `/api/*` call 401s.
 *
 * This plugin fetches the running dashboard's `index.html` on each dev page
 * load, scrapes the `window.__CURIE_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function curieDevToken(): Plugin {
  const TOKEN_RE = /window\.__CURIE_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
    /window\.__CURIE_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;

  return {
    name: "curie:dev-session-token",
    apply: "serve",
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[curie] Could not find session token in ${BACKEND} — ` +
              `is \`curie dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__CURIE_SESSION_TOKEN__="${match[1]}";` +
              `window.__CURIE_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[curie] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`curie dashboard\` or set CURIE_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      "@curie/shared": path.resolve(__dirname, "../apps/shared/src"),
    },
  ...
  server: {
    proxy: {
      ...
      // Same host as `curie dashboard` must serve these; Vite has no
