...
import { api, CURIE_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "curie:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${CURIE_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${CURIE_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?curie_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-curie-plugin", manifest.name);
