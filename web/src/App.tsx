...
const SIDEBAR_COLLAPSED_KEY = "curie-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="curie-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="curie-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateCurie = status?.can_update_curie === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'curie update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "curie update";
    return (
      t.status.updateCurieConfirmMessage ??
      `This will run 'curie update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateCurieConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateCurie) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateCurie,
      runningLabel: t.status.updatingCurie,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Curie gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateCurieConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateCurieConfirmTitle ?? `${t.status.updateCurie}?`}
    />
    </>
  );
}
