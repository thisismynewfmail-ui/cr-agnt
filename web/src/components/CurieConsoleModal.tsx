...
interface CurieConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function CurieConsoleModal({ open, onClose }: CurieConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("curie> ");
  const inputPromptRef = useRef("curie> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "curie> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Curie Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="curie-console-title"
    >
      <div
        className={cn(
          themedBody,
