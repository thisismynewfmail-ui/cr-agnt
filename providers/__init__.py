...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with curie-agent)
2. User plugins: ``$CURIE_HOME/plugins/model-providers/<name>/``
3. Pip-installed plugins: distributions exposing a ``curie_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$CURIE_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$CURIE_HOME/plugins/model-providers/`` if it exists."""
    try:
        from curie_constants import get_curie_home

        d = get_curie_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple CURIE_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_curie_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``curie_agent.plugins``
    ...
    * **Provider targets only.** The ``curie_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$CURIE_HOME``) keep their
    ...
    try:
        from curie_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$CURIE_HOME/plugins/model-providers/<name>/``
    ...
    # 0. Pip-installed plugins — entry points in the ``curie_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$CURIE_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with curie-agent.
    ...
    # 2. User plugins — under $CURIE_HOME/plugins/model-providers/<name>/.
