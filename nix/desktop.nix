# nix/desktop.nix — Curie Desktop (Electron) app build + wrapper
#
# `curieAgent` is the fully-built `.#default` package — it ships the
# `curie` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `CURIE_DESKTOP_CURIE` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Curie CLI").
# No reimplementation of the agent resolution in this wrapper.
{
  ...
  # CURIE_HOME and CURIE_MANAGED here, which gives the app the same state
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "curie-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "curie-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = curieNpmLib.buildNpmPackage {
    dirs = [
      "apps/desktop"
      "apps/shared"
    ];
    pname = "curie-desktop-renderer";
    ...
    buildPhase = ''
      ...
      pushd apps/desktop
        ...
        ${lib.getExe curieNpmLib.node-gyp} rebuild \
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/curie-desktop $out/bin
    cp -r ${renderer}/* $out/share/curie-desktop/
    ...
    substituteInPlace $out/share/curie-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/curie-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # CURIE_DESKTOP_CURIE to the absolute path of the nix-built `curie`
    # binary so the desktop's resolver step 4 ("existing Curie CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/curie-desktop \
      --add-flags "$out/share/curie-desktop" \
      --set CURIE_DESKTOP_CURIE "${lib.getExe curieAgent}" \
    ...
    install -m 0644 ${../apps/desktop/assets/icon.png} \
      $out/share/icons/hicolor/1024x1024/apps/curie.png
    export PYTHONPATH=$(mktemp -d)
    cp ${../curie_cli/linux_desktop_entry.py} "$PYTHONPATH/linux_desktop_entry.py"
    export DESKTOP_EXEC="$out/bin/curie-desktop"
    export DESKTOP_ICON="$out/share/icons/hicolor/1024x1024/apps/curie.png"
    python3 -c 'import os; from linux_desktop_entry import render_desktop_entry; print(render_desktop_entry(os.environ["DESKTOP_EXEC"], os.environ["DESKTOP_ICON"]))' > $out/share/applications/curie.desktop
    runHook postInstall
  '';
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Curie Agent";
    homepage = "https://github.com/thisismynewfmail-ui/Cur-Agnt";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "curie-desktop";
  };
}
