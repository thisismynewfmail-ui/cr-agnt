# nix/web.nix — Curie Web Dashboard (Vite/React) frontend build
{ curieNpmLib, ... }:
curieNpmLib.buildNpmPackage {
  dirs = [
    "web"

    # @curie/shared ships as a file: workspace dep of web, so its source
  ...
  buildPhase = ''
    ...
    # outDir in vite.config.ts points to ../curie_cli/web_dist for the
