# nix/homeManagerModules.nix — the Home Manager module for curie-agent
#
# This module is the user-level equivalent of nixosModules.default. Curie is
...
#
# `services.curie-agent` is the same option set on both modules. All of the
...
#   changed   addToSystemPackages      -> programs.curie-agent.enable and
#                                        home.sessionVariables
#   added     programs.curie-agent    the CLI and the desktop application,
#                                      because Home Manager separates an
#                                      installation from a daemon
#   changed   stateDir (+ "/.curie")  -> curieHome, set directly
#
# To use the module:
#   imports = [ curie-agent.homeManagerModules.default ];
#   programs.curie-agent = {
#     enable = true;          # the curie CLI on your PATH
...
{ inputs, ... }:
{
  flake.homeManagerModules.default =
    ...
    let
      ...
      cfgPrograms = config.programs.curie-agent;
      ...
      # not reach it, and the application would open ~/.curie while the
      # services use curieHome. Thus the launcher carries the value itself.
      #
      # CURIE_MANAGED rides along only when the services are enabled. That
      ...
      desktopEnvironment = {
        CURIE_HOME = cfg.curieHome;
      }
      // lib.optionalAttrs cfg.enable {
        inherit (processEnvironment) CURIE_MANAGED;
      }
      // lib.optionalAttrs desktopUsesService {
        CURIE_DESKTOP_REMOTE_URL = "http://${cfg.backend.host}:${toString cfg.backend.port}";
      };
      ...
      # token the desktop resolver throws ("CURIE_DESKTOP_REMOTE_URL is set
      # but CURIE_DESKTOP_REMOTE_TOKEN is not"), so the two variables travel
      ...
      desktopRun = lib.optional desktopUsesService ''
        if [ -r ${lib.escapeShellArg cfg.backend.sessionTokenFile} ]; then
          CURIE_DESKTOP_REMOTE_TOKEN="$(tr -d '\r\n' < ${lib.escapeShellArg cfg.backend.sessionTokenFile})"
          export CURIE_DESKTOP_REMOTE_TOKEN
        else
          echo "curie-desktop: cannot read the session token at ${cfg.backend.sessionTokenFile}." >&2
          echo "curie-desktop: the application starts its own backend instead of the one of the service." >&2
        fi
      '';
    ...
    in
    {
      # ── programs.curie-agent — the installation ───────────────────────
      # Home Manager separates "install this application for me" from "run
      # this daemon". Curie needs both, and a person can want one without
      # the other: an application with no gateway, or a headless gateway on
      # a machine with no display.
      #
      # `services.curie-agent` stays the authority for the state and the
      # configuration. This module reads curieHome and the backend address
      # from it, and never the reverse.
      options.programs.curie-agent = {
        enable = lib.mkEnableOption ''
          the Curie Agent command line application.

          This adds `curie` to home.packages, and exports CURIE_HOME with
          home.sessionVariables. An interactive shell then uses the same
          state as `services.curie-agent`
        '';

        package = lib.mkOption {
          type = lib.types.package;
          default = effectivePackage;
          defaultText = lib.literalExpression "config.services.curie-agent.package";
          description = ''
            The curie-agent package to install.

            The default follows `services.curie-agent.package`, and applies
        ...
        desktop = {
          enable = lib.mkEnableOption ''
            the Curie Desktop application (Electron).

            This adds `curie-desktop` to home.packages, with an XDG
            launcher entry on Linux. The launcher starts the same Curie
            runtime that `package` gives, and reads the CURIE_HOME of
            `services.curie-agent`. Thus the application, the interactive
            shell and the services share one state directory.

            The Electron application carries its own Curie runtime with
            the usual distribution. This module gives it the Nix package
            instead, with CURIE_DESKTOP_CURIE. It installs no second copy
            of Curie, and it downloads nothing on the first start
          '';

          package = lib.mkOption {
            type = lib.types.package;
            default = cfgPrograms.package.curieDesktop;
            defaultText = lib.literalExpression "config.programs.curie-agent.package.curieDesktop";
            description = ''
              The curie-desktop package to use.

              The default follows `package`, and thus also
              `services.curie-agent.extraPythonPackages` and
              `extraDependencyGroups`, because the desktop application is a
              passthru of the agent package. A package that you set here
              carries its own Curie runtime, and this module cannot make
      ...
      options.services.curie-agent =
        ...
        // {
          curieHome = lib.mkOption {
            type = lib.types.str;
            default = "${config.home.homeDirectory}/.curie";
            defaultText = lib.literalExpression ''"''${config.home.homeDirectory}/.curie"'';
            description = ''
              The value of CURIE_HOME. This state directory holds
              config.yaml, .env, auth.json, the sessions, the skills, the
              memory and the cron jobs.

              The NixOS module takes a `stateDir` and adds `/.curie` to it.
              This module sets CURIE_HOME directly. Thus an existing
              ~/.curie continues to work, and you can give the directory any
              name.
            '';
            example = "/home/alice/.curie-work";
          };

          # `installPackage` moved to `programs.curie-agent.enable`. The
          ...
          # quiet removal gives them a machine with no `curie` and no
          ...
          installPackage = lib.mkOption {
            ...
            visible = false;
            description = ''
              Removed. Use `programs.curie-agent.enable` instead.
            '';
          };
      ...
      config = lib.mkMerge [
        ...
        (lib.mkIf cfgPrograms.enable {
          ...
          home.sessionVariables.CURIE_HOME = cfg.curieHome;
        })

        # A launcher from the desktop menu reads no shell profile, so the
        # CURIE_HOME that `programs.enable` exports does not reach it. Home
        # Manager writes only systemd.user.sessionVariables into
        # environment.d, and this module does not put CURIE_HOME there,
          ...
          assertions = [
            {
              ...
              # with no `curie` on the PATH and no message.
        ...
        (lib.mkIf cfg.enable (
          lib.mkMerge [
                ...
                ++ [
                  {
                    ...
                    message = "services.curie-agent.backend.waitFor = \"interface\" works on Linux only. Use \"hostname\" on Darwin.";
                  }
                ];
            }
            ...
            # ── Activation: directories, config, secrets, documents ────────
            {
              ...
              home.activation.curieAgentSetup =
                lib.hm.dag.entryAfter
                  ...
                  (
                    common.mkStateScript {
                      inherit pkgs cfg;
                      inherit (cfg) curieHome workingDirectory;
                      run = "$DRY_RUN_CMD ";
            ...
            (lib.mkIf (isLinux && cfg.gateway.enable) {
              systemd.user.services.curie-agent = mkUnit {
            ...
            (lib.mkIf (isLinux && cfg.backend.mode != "none") {
              systemd.user.services.curie-backend = mkUnit {
            ...
            (lib.mkIf (isDarwin && cfg.gateway.enable) {
              launchd.agents.curie-agent = mkAgent {
                argv = common.gatewayArgv cfg;
                logName = "curie-agent";
              };
            })
            ...
            (lib.mkIf (isDarwin && cfg.backend.mode != "none") {
              launchd.agents.curie-backend = mkAgent {
                argv = common.backendArgv { inherit pkgs cfg; };
                logName = "curie-backend";
              };
            })
          ]
        ))
