...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Curie derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            curie-agent = prev.curie-agent.overrideAttrs (_old: {
              CURIE_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $CURIE_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$CURIE_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        curie-agent = prev.curie-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "curie-agent-env" {
    curie-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "curie-agent-editable-env" {
    curie-agent = dependency-groups;
  };
}
