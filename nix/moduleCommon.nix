...
#
# `services.curie-agent` is the same option set on both modules. Both modules
...
#   homeManagerModules.nix  curieHome, programs.curie-agent (the CLI and
...
let
  ...
  deepConfigType = types.mkOptionType {
    name = "curie-config-attrs";
    description = "Curie YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    options = {
      ...
      auth = mkOption {
        ...
        description = ''
          ...
          (remote MCP servers). Tokens are stored in $CURIE_HOME/mcp-tokens/.
        '';
      };
  ...
  sharedOptions =
    ...
    {
      enable = lib.mkEnableOption "Curie Agent";
      ...
      package = mkOption {
        ...
        description = "The curie-agent package to use.";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          The Curie configuration, as an attribute set. The module joins the
          ...
          which includes the keys that `curie config set` and the settings
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          $CURIE_HOME/.env. Curie reads that file at each start, with
          load_curie_dotenv().

          Each activation writes .env again from the start. Thus a secret
          file cannot go into .env two times.
        '';
        example = literalExpression ''[ config.sops.secrets."curie/env".path ]'';
      };
      ...
      authFile = mkOption {
        ...
        description = ''
          ...
          does not exist. Thus a token that Curie refreshes at runtime stays
          after an activation.
        '';
      };
      ...
      documents = mkOption {
        ...
        description = ''
          ...
          Curie reads SOUL.md and memories/ from CURIE_HOME, so put those
          files in `curieHomeFiles`.
      ...
      curieHomeFiles = mkOption {
        type = documentsType;
        default = { };
        description = ''
          Files that the module installs into CURIE_HOME. Each key is a path
          relative to that directory, and the module makes the necessary
          subdirectories. Each value is a string or a path.

          Curie reads SOUL.md and the memory files from CURIE_HOME and not
          from the working directory. Declare those files here, or Curie
          does not load them.
        '';
        example = literalExpression ''
          {
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the curie plugins
          directory. Each package must contain a plugin.yaml and __init__.py
          at its root. Curie discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          curie_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as curie (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in curie-agent's
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `curie gateway`.";
      };
      ...
      # ── The backend: `curie serve` or `curie dashboard` ──────────────
      # `curie serve` and `curie dashboard` are the same entry point,
      # curie_cli.main:cmd_dashboard, with one flag of difference. serve runs
      # without a user interface. dashboard also serves the web application.
      # Both give the /api/ws and /api/pty sockets that Curie Desktop
      ...
      # controls an external gateway, with `curie gateway restart`. It does
      ...
      backend = {
        mode = mkOption {
          ...
          description = ''
            ...
            - "serve"     — the backend without a user interface. It gives
                            the /api/ws and /api/pty sockets that Curie
                            Desktop connects to.
        ...
        sessionTokenFile = mkOption {
          ...
          description = ''
            ...
            CURIE_DASHBOARD_SESSION_TOKEN. That token authorizes the /api
            routes and the /api/ws socket. Curie Desktop presents the same
          ...
          '';
          example = literalExpression ''config.sops.secrets."curie/desktop-token".path'';
        };
      };
    };
  ...
  # removal leaves them with no `curie` on the PATH and no message. The
  ...
  installPackageRemovedMessage =
    value:
    ''
      services.curie-agent.installPackage was removed. Curie now
      separates the installation from the services, which is the
      Home Manager convention:

        programs.curie-agent.enable = ${lib.boolToString (value != false)};  # the curie CLI, and CURIE_HOME for your shells
        programs.curie-agent.desktop.enable = true;  # the desktop application

      `services.curie-agent` keeps the state, the configuration and
  ...
  mkConfigFiles =
    ...
    let
      generated = pkgs.writeText "curie-config.yaml" (
  ...
  mkDocumentTree =
    { pkgs, documents }:
    pkgs.runCommand "curie-documents" { } (
      ''
        mkdir -p $out
      ''
  ...
  mkEnvScript =
    { pkgs, environment }:
    let
      base = pkgs.writeText "curie-env-base" (
        lib.concatStringsSep "\n" (lib.mapAttrsToList (k: v: "${k}=${v}") environment)
        + lib.optionalString (environment != { }) "\n"
      );
    in
    pkgs.writeShellScript "curie-env-merge" ''
      set -eu
      ...
      for file in "$@"; do
        if [ -r "$file" ]; then
          ...
          echo "curie-agent: WARNING cannot read environmentFile $file" >&2
        fi
      done
    '';
  ...
  mkStateScript =
    {
      ...
      # CURIE_MANAGED variable of the service. The value tells the shell
    ...
    in
    ''
      # Directories. The service units and Curie make most of these
      ...
      ${run}mkdir -p ${
        lib.escapeShellArgs (
          ...
          ++ map (d: "${curieHome}/${d}") stateDirs
        )
      }

      # config.yaml: merge the Nix settings into the file on disk. Curie
      ...
      ${
        if cfg.configFile != null then
          "${inst} -m ${modes.config} -D ${configFiles.effective} ${curieHome}/config.yaml"
        else
          ''
            ${run}${configFiles.mergeScript} ${configFiles.generated} ${curieHome}/config.yaml
            ${run}chmod ${modes.config} ${curieHome}/config.yaml
          ''
      }

      # The managed-mode marker. It makes an interactive shell also refuse to
      # change the configuration that Nix owns.
      ${inst} -m ${modes.managed} ${pkgs.writeText "curie-managed" managedSystem} ${curieHome}/.managed

      ${lib.optionalString (cfg.environment != { } || cfg.environmentFiles != [ ]) ''
        ${run}${envScript} ${curieHome}/.env ${modes.env} ${lib.escapeShellArgs cfg.environmentFiles}
        ${lib.optionalString (owner != null) "${run}chown ${owner} ${curieHome}/.env"}
      ''}

      ${lib.optionalString (cfg.authFile != null) (
        if cfg.authFileForceOverwrite then
          "${inst} -m ${modes.auth} ${cfg.authFile} ${curieHome}/auth.json"
        else
          ''
            if [ ! -e ${curieHome}/auth.json ]; then
              ${inst} -m ${modes.auth} ${cfg.authFile} ${curieHome}/auth.json
            fi
          ''
      )}
      ...
      ${installDocuments homeDocumentTree curieHome cfg.curieHomeFiles}
      ...
      ${run}find ${curieHome}/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
      ${lib.concatMapStringsSep "\n" (plugin: ''
        if [ ! -f ${plugin}/plugin.yaml ]; then
          echo "curie-agent: ERROR extraPlugins entry '${plugin}' has no plugin.yaml" >&2
          exit 1
        fi
        ${run}ln -sfn ${plugin} ${curieHome}/plugins/nix-managed-${lib.getName plugin}
      '') cfg.extraPlugins}
    '';
  ...
  gatewayArgv =
    cfg:
    [
      "${effectivePackage cfg}/bin/curie"
      "gateway"
    ]
    ++ cfg.extraArgs;
  ...
  #
  # `exec` on the last line keeps curie as the MainPID of the unit. No shell
  ...
  backendLauncher =
    ...
    pkgs.writeShellScript "curie-backend-launch" (
      builtins.replaceStrings [ "@HOST@" ] [ ''"$_target"'' ] ''
        ...
        ${lib.optionalString (cfg.backend.sessionTokenFile != null) ''
          ...
          if [ ! -r "$_token_file" ]; then
            echo "curie-backend: cannot read the session token file '$_token_file'. The unit stops." >&2
            echo "curie-backend: backend.sessionTokenFile must name a runtime path that this user can read." >&2
            exit 1
          fi

          CURIE_DASHBOARD_SESSION_TOKEN="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$_token_file")"
          export CURIE_DASHBOARD_SESSION_TOKEN

          if [ -z "$CURIE_DASHBOARD_SESSION_TOKEN" ]; then
            echo "curie-backend: the session token file '$_token_file' is empty. The unit stops." >&2
            exit 1
          fi
        ''}
        ${
          if cfg.backend.waitFor == null then
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "curie-backend: '$_target' did not resolve after ''${_timeout}s. The unit stops." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "curie-backend: waits for '$_target' to resolve..." >&2
                fi
            ...
            ''
              ...
              while :; do
                ...
                if [ "$_waited" -ge "$_timeout" ]; then
                  echo "curie-backend: interface '$_iface' had no IPv4 address after ''${_timeout}s. The unit stops." >&2
                  echo "curie-backend: a fallback address can expose the backend more widely than you intend." >&2
                  exit 1
                fi

                if [ "$_waited" = 0 ]; then
                  echo "curie-backend: waits for an IPv4 address on '$_iface'..." >&2
                fi
                ${pkgs.coreutils}/bin/sleep 2
                _waited=$(( _waited + 2 ))
              done
        ...
        echo "curie-backend: binds to $_target:${toString cfg.backend.port} (from $_how)" >&2
  ...
  backendDescription =
    ...
    if cfg.backend.mode == "dashboard" then
      "Curie Agent web dashboard and desktop backend"
    else
      "Curie Agent backend for Curie Desktop";

  # The environment that each Curie process needs, from either module.
  #
  # managedSystem gives the value of CURIE_MANAGED. The CLI reads that
    ...
    [
      {
        ...
        message = ''
          ...
          To give Curie an identity and a memory, use
          ${optionPath}.curieHomeFiles instead. Those files go to
          CURIE_HOME. Curie reads SOUL.md and memories/ only from there.
        '';
      }
    ];
  ...
  # The subdirectories of CURIE_HOME that both modules make.
  stateSubdirs = [
    "cron"
    "sessions"
    "logs"
