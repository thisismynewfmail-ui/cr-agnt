# nix/packages.nix — Curie Agent package built with uv2nix
{ inputs, ... }:
{
  perSystem =
    {
    ...
    let
      ...
      minimal = pkgs.callPackage ./curie-agent.nix {
    ...
    in
    {
      packages = {
        ...
        tui = full.curieTui;
        web = full.curieWeb;
        desktop = full.curieDesktop;

        update-npm-lockfile = full.curieNpmLib.updateNpmLockfile;
      };
    };
}
