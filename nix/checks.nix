...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      curie-agent = self'.packages.default;
      curieVenv = curie-agent.curieVenv;
      ...
      evalHomeModule =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              home = {
                username = "curie-check";
                homeDirectory = "/home/curie-check";
                stateVersion = "24.11";
              };
            }
            { services.curie-agent = settings; }
      ...
      evalHomeSplit =
        ...
        inputs.home-manager.lib.homeManagerConfiguration {
          ...
          modules = [
            ...
            {
              programs.curie-agent = programs;
              services.curie-agent = services;
            }
          ];
        };

      # The option names that each module defines under
      # services.curie-agent. The internal names that the module system adds
      # are not in the list.
      moduleOptionNames =
        eval: lib.attrNames (lib.filterAttrs (n: _: !lib.hasPrefix "_" n) eval.options.services.curie-agent);
      ...
      configKeys = pkgs.runCommand "curie-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${curieVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "curie-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "curie-build-package" { } ''
          echo "PASS: package built at ${curie-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "curie-build-devshell" { } ''
          ...
          let
            enabled = evalHomeSplit {
              ...
              services = {
                ...
                environment.CURIE_TEST = "1";
                environmentFiles = [ "/run/secrets/curie-env" ];
                curieHomeFiles."SOUL.md" = "test soul";
            ...
            processes =
              if pkgs.stdenv.hostPlatform.isDarwin then
                ...
                }) (lib.filterAttrs (n: _: lib.hasPrefix "curie" n) cfg.launchd.agents)
              else
                lib.mapAttrs (_: unit: {
                  argv = [ unit.Service.ExecStart ];
                  env = unit.Service.Environment;
                }) (lib.filterAttrs (n: _: lib.hasPrefix "curie" n) cfg.systemd.user.services);
            ...
            activation = cfg.home.activation.curieAgentSetup.data;

            failures =
              lib.optional (names != [
                "curie-agent"
                "curie-backend"
              ]) "expected curie-agent + curie-backend processes, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/curie gateway" (argvOf "curie-agent")
              ) "gateway process does not run `curie gateway`: ${argvOf "curie-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/curie serve" (argvOf "curie-backend")
              ) "backend process does not run `curie serve`: ${argvOf "curie-backend"}"
              ++ lib.optional (
                !lib.hasInfix "--no-open" (argvOf "curie-backend")
              ) "backend must pass --no-open so a service never opens a browser"
              ++ lib.optional (
                lib.any (n: !lib.hasInfix "/home/curie-check/.curie" (envOf n)) names
              ) "gateway and backend must share one CURIE_HOME"
              ++ lib.optional (
                cfg.home.sessionVariables.CURIE_HOME or null != "/home/curie-check/.curie"
              ) "programs.curie-agent.enable must export CURIE_HOME for interactive shells"
              ++ lib.optional (
                !lib.hasInfix "curie-config-merge" activation
              ) "activation must deep-merge config.yaml, not overwrite it"
              ++ lib.optional (
                !lib.hasInfix "/home/curie-check/.curie/SOUL.md" activation
              ) "curieHomeFiles must install into CURIE_HOME"
              ++ lib.optional (
                !lib.hasInfix "/home/test-user/workspace/AGENTS.md" activation
              ) "documents must install into workingDirectory"
              # The CLI reads CURIE_MANAGED to name the rebuild command when
              # it refuses to write the configuration. A Home Manager install
              # has no nixos-rebuild command. Thus it must not report NixOS.
              ++ lib.optional (
                !lib.any (n: lib.hasInfix "CURIE_MANAGED=home-manager" (envOf n)) names
              ) "processes must report CURIE_MANAGED=home-manager"
              ++ lib.optional (
                !lib.hasInfix "curie-managed" activation
              ) "activation must write a .managed marker naming the managing system";
          in
          pkgs.runCommand "curie-home-manager-module" { } (
            if failures != [ ] then
        ...
        workspace-files-need-a-directory =
          let
            ...
            # the priority.
            sameAsDefault = "/home/curie-check";
            ...
            cases = [
              ...
              {
                name = "curieHomeFiles needs no directory";
                ok = accepts { curieHomeFiles."SOUL.md" = "x"; };
              }
              {
          ...
          in
          pkgs.runCommand "curie-workspace-files-need-a-directory" { } (
            if failed != [ ] then
        ...
        # ── The desktop application shares one CURIE_HOME ───────────────
        # `programs.enable` exports CURIE_HOME with home.sessionVariables,
        ...
        # no shell profile. Thus the desktop application would open ~/.curie
        # while the services use the CURIE_HOME of the module, and the user
        ...
        home-manager-desktop =
          let
            tokenFile = "/run/secrets/curie-desktop-token";
            ...
            enabled = evalHomeSplit {
              ...
              services = {
                enable = true;
                curieHome = "/home/curie-check/.curie-work";
            ...
            desktopPackages = builtins.filter (p: (p.pname or "") == "curie-desktop") cfg.home.packages;
            ...
            # second Curie runtime beside the services is the fault that
            # `programs.enable` plus a plain desktop package would give.
            agentPackages = builtins.filter (p: (p.pname or "") == "curie-agent") cfg.home.packages;
            ...
            backendScript =
              let
                argv =
                  if pkgs.stdenv.hostPlatform.isDarwin then
                    cfg.launchd.agents.curie-backend.config.ProgramArguments
                  else
                    [ cfg.systemd.user.services.curie-backend.Service.ExecStart ];
            ...
            failures =
              ...
              ) "programs.desktop.enable must install exactly one curie-desktop package, got ${toString (lib.length desktopPackages)}"
              ++ lib.optional (
                setValue "CURIE_HOME" != "/home/curie-check/.curie-work"
              ) "the launcher must carry CURIE_HOME: a GUI launcher reads no shell profile, so home.sessionVariables never reaches it (got: ${toString (setValue "CURIE_HOME")})"
              ++ lib.optional (
                setValue "CURIE_MANAGED" != "home-manager"
              ) "the launcher must report CURIE_MANAGED=home-manager while the services own the configuration (got: ${toString (setValue "CURIE_MANAGED")})"
              ++ lib.optional (
                lib.length agentPackages == 1
                && setValue "CURIE_DESKTOP_CURIE" != "${lib.head agentPackages}/bin/curie"
              ) "the launcher must pin the agent package that programs.enable installs, and not a second runtime: ${toString (setValue "CURIE_DESKTOP_CURIE")}"

              # ── The application reaches the backend of the service ──────
              ++ lib.optional (
                setValue "CURIE_DESKTOP_REMOTE_URL" != "http://127.0.0.1:9231"
              ) "the launcher must name the backend of the service, or the application starts a second one (got: ${toString (setValue "CURIE_DESKTOP_REMOTE_URL")})"
              ++ lib.optional (
                !lib.hasInfix "CURIE_DESKTOP_REMOTE_TOKEN" wrapper
              ) "the launcher must give a token with the URL: the desktop resolver throws when the URL is set alone"
              ++ lib.optional (
                !lib.hasInfix "CURIE_DASHBOARD_SESSION_TOKEN" backendScript
              ...
              ++ lib.optional (
                setValue "CURIE_DESKTOP_REMOTE_TOKEN" != null
              ) "the token must never be a --set value: makeWrapper writes it into the world-readable Nix store";
          in
          pkgs.runCommand "curie-home-manager-desktop" { } (
            if failures != [ ] then
              throw "Home Manager desktop check failed:\n${lib.concatMapStringsSep "\n" (f: "  - ${f}") failures}"
            else
              ''
                echo "PASS: the desktop launcher shares CURIE_HOME, the runtime and the backend of the service"
                mkdir -p $out
                echo "ok" > $out/result
              ''
          );
        ...
        home-manager-desktop-standalone =
          let
            ...
            failures =
              ...
              ++ lib.optional (
                !lib.hasInfix "--set CURIE_HOME" wrapper
              ) "the launcher must carry CURIE_HOME even with no services"
              ++ lib.optional (
                lib.hasInfix "CURIE_MANAGED" wrapper
              ) "the launcher must not claim a managed install when no activation writes one"
              ++ lib.optional (
                lib.hasInfix "CURIE_DESKTOP_REMOTE_URL" wrapper
              ) "the launcher must not name a backend when the services run none"
              ++ lib.optional (
                cfg.systemd.user.services ? curie-backend || cfg.launchd.agents ? curie-backend
              ) "programs.enable alone must start no service";
          in
          pkgs.runCommand "curie-home-manager-desktop-standalone" { } (
            if failures != [ ] then
        ...
        # command line. A silent removal thus leaves them with no `curie`
        ...
        home-manager-install-package-removed =
          let
            ...
            cases = [
              {
                value = true;
                expect = "programs.curie-agent.enable = true;";
              }
              {
                value = false;
                expect = "programs.curie-agent.enable = false;";
              }
            ];
          ...
          in
          pkgs.runCommand "curie-home-manager-install-package-removed" { } (
            if failures != [ ] then
        ...
        module-option-parity =
          ...
          in
          pkgs.runCommand "curie-module-option-parity" { } (
            if failures != [ ] then
        ...
        nixos-module =
          let
            ...
            units = lib.filterAttrs (n: _: lib.hasPrefix "curie" n) cfg.systemd.services;
            names = lib.attrNames units;
            execOf = name: units.${name}.serviceConfig.ExecStart;
            activation = cfg.system.activationScripts."curie-agent-setup".text;

            failures =
              lib.optional (names != [
                "curie-agent"
                "curie-backend"
              ]) "expected curie-agent + curie-backend units, got: ${toString names}"
              ++ lib.optional (
                !lib.hasInfix "bin/curie gateway" (execOf "curie-agent")
              ) "gateway unit does not run `curie gateway`: ${execOf "curie-agent"}"
              ++ lib.optional (
                !lib.hasInfix "bin/curie dashboard" (execOf "curie-backend")
              ) "backend unit does not run `curie dashboard`: ${execOf "curie-backend"}"
              ++ lib.optional (
                units.curie-agent.environment.CURIE_HOME != units.curie-backend.environment.CURIE_HOME
              ) "gateway and backend must share one CURIE_HOME"
              ++ lib.optional (
                !lib.hasInfix "/var/lib/curie/.curie/SOUL.md" activation
              ) "curieHomeFiles must install into CURIE_HOME";
          ...
          in
          pkgs.runCommand "curie-nixos-module" { } (
            if failures != [ ] then
        ...
        # polls and then execs curie, and the assertions reject a
        # configuration that cannot work.
        backend-bind-wait =
          let
            execOf =
              settings:
              (evalNixosModule ({ enable = true; } // settings)).config.systemd.services.curie-backend.serviceConfig.ExecStart;
            ...
            failures =
              ...
              lib.optional (!lib.hasInfix "bin/curie serve --host 127.0.0.1" direct)
                "without waitFor the backend must exec curie directly, got: ${direct}"
              ++ lib.optional (lib.hasInfix "curie-backend-launch" direct)
                "without waitFor the backend must not use the launcher"

              # The hostname mode polls the resolver, then binds the name.
              ++ lib.optional (!lib.hasInfix "curie-backend-launch" hostnameWait)
              ...
              ++ lib.optional (!lib.hasInfix "exec " hostnameScript)
                "the launcher must exec curie, so that it keeps the MainPID"
              ...
              ++ lib.optional (!lib.hasInfix "bin/curie dashboard" interfaceScript)
          ...
          in
          pkgs.runCommand "curie-backend-bind-wait" { } (
            if failures != [ ] then
        ...
        # $CURIE_HOME/.env. The important property is that a second run
        ...
        env-file-assembly =
          let
            envScript = (import ./moduleCommon.nix { inherit lib; }).mkEnvScript {
              inherit pkgs;
              environment = {
                CURIE_PUBLIC = "visible";
              };
            };
          in
          pkgs.runCommand "curie-env-file-assembly" { } ''
            set -e
            workdir=$(mktemp -d)
            ...
            grep -qx 'CURIE_PUBLIC=visible' "$workdir/.env" || \
        ...
        service-argv =
          let
            ...
            sentinel = "--curie-nix-argv-probe";
            probe = argv: lib.escapeShellArgs (argv ++ [ sentinel ]);
          in
          pkgs.runCommand "curie-service-argv" { } ''
            set -e
        ...
        package-contents = pkgs.runCommand "curie-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${curie-agent}/bin/curie || (echo "FAIL: curie binary missing"; exit 1)
          test -x ${curie-agent}/bin/curie-agent || (echo "FAIL: curie-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${curie-agent}/bin/curie --version 2>&1 | grep -qi "curie" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "curie-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in curie curie-agent curie-acp; do
            test -x ${curie-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "curie-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking curie --help ==="
          ${curie-agent}/bin/curie --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${curie-agent}/bin/curie --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "curie-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${curie-agent}/share/curie-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${curie-agent}/share/curie-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "CURIE_BUNDLED_SKILLS" ${curie-agent}/bin/curie || \
            (echo "FAIL: CURIE_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: CURIE_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${curie-agent}/share/curie-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${curie-agent}/share/curie-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "CURIE_OPTIONAL_SKILLS" ${curie-agent}/bin/curie || \
            (echo "FAIL: CURIE_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, CURIE_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "curie-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${curie-agent}/share/curie-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${curie-agent}/share/curie-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "CURIE_BUNDLED_PLUGINS" ${curie-agent}/bin/curie || \
            (echo "FAIL: CURIE_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: CURIE_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "curie-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${curie-agent}/share/curie-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${curie-agent}/share/curie-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${curie-agent}/share/curie-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "CURIE_BUNDLED_LOCALES" ${curie-agent}/bin/curie || \
            (echo "FAIL: CURIE_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: CURIE_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + CURIE_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && CURIE_BUNDLED_LOCALES=${curie-agent}/share/curie-agent/locales \
            ${curieVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with CURIE_BUNDLED_LOCALES set"; exit 1)
        ...
        # CURIE_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "curie-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${curie-agent}/share/curie-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${curie-agent}/share/curie-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "CURIE_OPTIONAL_MCPS" ${curie-agent}/bin/curie || \
            (echo "FAIL: CURIE_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: CURIE_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${curie-agent}/bin/curie mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: curie mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "curie-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${curie-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${curie-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "CURIE_TUI_DIR" ${curie-agent}/bin/curie || \
            (echo "FAIL: CURIE_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: CURIE_TUI_DIR set in wrapper"
        ...
        # Verify CURIE_NODE is set in wrapper and points to Node 26+
        # (Curie pins its toolchain to Node 26 everywhere)
        curie-node = pkgs.runCommand "curie-node-version" { } ''
          set -e
          echo "=== Checking CURIE_NODE in wrapper ==="
          grep -q "CURIE_NODE" ${curie-agent}/bin/curie || \
            (echo "FAIL: CURIE_NODE not set in wrapper"; exit 1)
          echo "PASS: CURIE_NODE present in wrapper"

          CURIE_NODE=$(sed -n "s/^export CURIE_NODE='\(.*\)'/\1/p" ${curie-agent}/bin/curie)
          test -x "$CURIE_NODE" || (echo "FAIL: CURIE_NODE=$CURIE_NODE not executable"; exit 1)
          echo "PASS: CURIE_NODE executable at $CURIE_NODE"

          NODE_MAJOR=$("$CURIE_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Curie requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All CURIE_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify CURIE_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "curie-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(CURIE_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking CURIE_MANAGED guards ==="
          check_blocked "config set" ${curie-agent}/bin/curie config set model foo
          check_blocked "config edit" ${curie-agent}/bin/curie config edit
        ...
        extra-python-packages = let
          ...
          curieWithExtra = curie-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "curie-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${curieWithExtra}/bin/curie || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${curieWithExtra}/bin/curie || \
          ...
          if grep -q "PYTHONPATH" ${curie-agent}/bin/curie; then
        ...
        extra-dependency-groups = let
          curieWithGroups = curie-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "curie-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${curieWithGroups}"
          echo "venv: ${curieWithGroups.curieVenv}"
        ...
        messaging-variant = pkgs.runCommand "curie-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.curieVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "curie-config-roundtrip" {
          ...
          merge_and_load() {
            local curie_home="$1"
            export CURIE_HOME="$curie_home"
            ${configMergeScript} ${nixSettings} "$curie_home/config.yaml"
            ${curieVenv}/bin/python3 -c '
import json, sys
