# nix/nixosModules.nix — the NixOS module for curie-agent
#
...
#
# Container mode: curie runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $CURIE_HOME/.env
# and read by curie at startup — no container recreation needed for env changes.
#
# Tool resolution: the curie wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }:
{
  flake.nixosModules.default =
    ...
    let
      cfg = config.services.curie-agent;
      common = import ./moduleCommon.nix { inherit lib; };

      effectivePackage = common.effectivePackage cfg;
      curie-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

      curieHome = "${cfg.stateDir}/.curie";
      ...
      # CURIE_HOME via addToSystemPackages, so they can save settings through the
      ...
      # curie user + sudo on first boot (writable layer persists), then
      # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
      containerEntrypoint = pkgs.writeShellScript "curie-container-entrypoint" ''
        set -eu

        CURIE_UID="''${CURIE_UID:?CURIE_UID must be set}"
        CURIE_GID="''${CURIE_GID:?CURIE_GID must be set}"

        # ── Group: ensure a group with GID=$CURIE_GID exists ──
        # Check by GID (not name) to avoid collisions with pre-existing groups
        # (e.g. GID 100 = "users" on Ubuntu)
        EXISTING_GROUP=$(getent group "$CURIE_GID" 2>/dev/null | cut -d: -f1 || true)
        if [ -n "$EXISTING_GROUP" ]; then
          GROUP_NAME="$EXISTING_GROUP"
        else
          GROUP_NAME="curie"
          if command -v groupadd >/dev/null 2>&1; then
            groupadd -g "$CURIE_GID" "$GROUP_NAME"
          elif command -v addgroup >/dev/null 2>&1; then
            addgroup -g "$CURIE_GID" "$GROUP_NAME" 2>/dev/null || true
          fi
        fi

        # ── User: ensure a user with UID=$CURIE_UID exists ──
        PASSWD_ENTRY=$(getent passwd "$CURIE_UID" 2>/dev/null || true)
        if [ -n "$PASSWD_ENTRY" ]; then
          TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
          TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
        else
          TARGET_USER="curie"
          TARGET_HOME="/home/curie"
          if command -v useradd >/dev/null 2>&1; then
            useradd -u "$CURIE_UID" -g "$CURIE_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
          elif command -v adduser >/dev/null 2>&1; then
            adduser -u "$CURIE_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
          fi
        fi
        mkdir -p "$TARGET_HOME"
        chown "$CURIE_UID:$CURIE_GID" "$TARGET_HOME"
        chmod 0750 "$TARGET_HOME"

        # Ensure CURIE_HOME is owned by the target user.
        ...
        if [ -n "''${CURIE_HOME:-}" ] && [ -d "$CURIE_HOME" ]; then
          find "$CURIE_HOME" \! -user "$CURIE_UID" -exec chown "$CURIE_UID:$CURIE_GID" {} +
        fi
        ...
        if [ ! -f /var/lib/curie-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
          ...
          touch /var/lib/curie-tools-provisioned
        fi

        if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/curie ]; then
          mkdir -p /etc/sudoers.d
          echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/curie
          chmod 0440 /etc/sudoers.d/curie
        fi
        ...
        if command -v setpriv >/dev/null 2>&1; then
          exec setpriv --reuid="$CURIE_UID" --regid="$CURIE_GID" --init-groups "$@"
      ...
      # so they can update without recreation. Env vars go through $CURIE_HOME/.env.
      ...
      containerModeFile = pkgs.writeText "curie-container-mode" ''
        # Written by the NixOS activation script. Do not edit manually.
        backend=${cfg.container.backend}
        container_name=${containerName}
        exec_user=${cfg.user}
        curie_bin=${containerDataDir}/current-package/bin/curie
      '';

      # Default: /var/lib/curie/workspace → /data/workspace.
      ...
      commonServiceConfig = {
        ...
        # so interactive users in the curie group can read/write them.
        UMask = "0007";
      ...
      // common.processEnvironment { inherit curieHome; };

      unitPath = common.processPath { inherit pkgs cfg; };

    in
    {
      options.services.curie-agent =
        common.sharedOptions {
          defaultPackage = curie-agent;
          defaultPackageText = lib.literalExpression "curie-agent.packages.\${system}.default";
        ...
        // (
          with lib;
          {
            ...
            stateDir = mkOption {
              type = types.str;
              default = "/var/lib/curie";
              description = "State directory. Contains .curie/ subdir (CURIE_HOME).";
            };
            ...
            addToSystemPackages = mkOption {
              ...
              default = false;
              description = ''
                Add the curie CLI to environment.systemPackages and export
                CURIE_HOME system-wide (via environment.variables) so interactive
            ...
            container = {
              ...
              hostUsers = mkOption {
                ...
                default = [ ];
                description = ''
                  Interactive users who get a ~/.curie symlink to the service
                  stateDir. These users are automatically added to the curie group.
                '';
                example = [ "sidbin" ];
              };
            };
      ...
      config = lib.mkIf cfg.enable (
        lib.mkMerge [
          ...
          (lib.mkIf (cfg.mcpServers != { }) {
            services.curie-agent.settings.mcp_servers = common.mcpServersToConfig cfg.mcpServers;
          })
          ...
          # ── Host CLI ──────────────────────────────────────────────────────
          # Add the curie CLI to system PATH and export CURIE_HOME system-wide
          # so interactive shells share state (sessions, skills, cron) with the
          # gateway service instead of creating a separate ~/.curie/.
          (lib.mkIf cfg.addToSystemPackages {
            environment.systemPackages = [ effectivePackage ];
            environment.variables.CURIE_HOME = curieHome;
          })
          ...
          # ── Assertions ─────────────────────────────────────────────────────
          {
            assertions =
              common.pluginNameAssertions {
                inherit cfg;
                optionPath = "services.curie-agent";
              }
              ++ common.workspaceFilesAssertions {
                inherit cfg;
                opt = options.services.curie-agent.workingDirectory;
                optionPath = "services.curie-agent";
              }
              ...
              ++ [
                {
                  ...
                  message = "services.curie-agent: backend.mode is not supported together with container.enable — the container runs the gateway only.";
                }
              ];
          }

          # ── Per-user profile for extraPackages ───────────────────────────
          # Wire extraPackages into the curie user's per-user profile so the
          ...
          (lib.mkIf
            (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != [ ])
            {
              warnings = [
                ''
                  services.curie-agent: container.enable is true and container.hostUsers
                  is set, but addToSystemPackages is false. Without a host-installed curie
                  binary, container routing will not work for interactive users.
                  Set addToSystemPackages = true or ensure curie is on PATH.
                ''
              ];
            }
          )
          ...
          # ── Directories ───────────────────────────────────────────────────
          {
            systemd.tmpfiles.rules = [
              ...
              "d ${curieHome}                  2770 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.stateDir}/home           0750 ${cfg.user} ${cfg.group} - -"
              "d ${cfg.workingDirectory}        2770 ${cfg.user} ${cfg.group} - -"
            ]
            ++ map (d: "d ${curieHome}/${d} 2770 ${cfg.user} ${cfg.group} - -") common.stateSubdirs;
          }

          # ── Activation: link config + auth + documents ────────────────────
          {
            system.activationScripts."curie-agent-setup" =
              lib.stringAfter
                (
                ...
                ''
                  ...
                  chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${curieHome} ${cfg.stateDir}/home ${cfg.workingDirectory}
                  chmod 2770 ${cfg.stateDir} ${curieHome} ${cfg.workingDirectory}
                  ...
                  find ${curieHome} -maxdepth 1 \
                    \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
                    -exec chmod g+rw {} + 2>/dev/null || true
                  for _subdir in ${lib.concatStringsSep " " common.stateSubdirs}; do
                    mkdir -p "${curieHome}/$_subdir"
                    chown ${cfg.user}:${cfg.group} "${curieHome}/$_subdir"
                    chmod 2770 "${curieHome}/$_subdir"
                    find "${curieHome}/$_subdir" -type f \
                      -exec chmod g+rw {} + 2>/dev/null || true
                  done

                  ${common.mkStateScript {
                    inherit pkgs cfg curieHome;
                    workingDirectory = cfg.workingDirectory;
                  ...
                  chown -h ${cfg.user}:${cfg.group} ${curieHome}/plugins/nix-managed-* 2>/dev/null || true
                  ...
                  ${
                    if cfg.container.enable then
                      ''
                        install -o ${cfg.user} -g ${cfg.group} -m 0644 ${containerModeFile} ${curieHome}/.container-mode
                      ''
                    else
                      ''
                        rm -f ${curieHome}/.container-mode
                        ...
                        ${lib.concatStringsSep "\n" (
                          map (
                            ...
                            let
                              ...
                              symlinkPath = "${userHome}/.curie";
                            in
                            ''
                              if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${curieHome}" ]; then
                                rm -f "${symlinkPath}"
                                echo "curie-agent: removed symlink ${symlinkPath}"
                              fi
                            ''
                  ...
                  # Create ~/.curie -> stateDir/.curie for each hostUser so the
          ...
          (lib.mkIf (!cfg.container.enable) {
            systemd.services.curie-agent = {
              description = "Curie Agent Gateway";
              wantedBy = [ "multi-user.target" ];
              after = [ "network-online.target" ];
              wants = [ "network-online.target" ];

              # cfg.environment and cfg.environmentFiles are written to
              # $CURIE_HOME/.env by the activation script. load_curie_dotenv()
          ...
          (lib.mkIf (!cfg.container.enable && cfg.backend.mode != "none") {
            systemd.services.curie-backend = {
          ...
          (lib.mkIf cfg.container.enable {
            ...
            systemd.services.curie-agent = {
              description = "Curie Agent Gateway (container)";
              wantedBy = [ "multi-user.target" ];
              after = [
              ...
              preStart = ''
                ...
                if [ "$NEED_CREATE" = "true" ]; then
                  ...
                  CURIE_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
                  CURIE_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
                  ...
                  ${containerBin} create \
                    ...
                    --env CURIE_UID="$CURIE_UID" \
                    --env CURIE_GID="$CURIE_GID" \
                    --env CURIE_HOME=${containerDataDir}/.curie \
                    --env CURIE_MANAGED=true \
                    --env HOME=${containerHomeDir} \
                    ${lib.concatStringsSep " " cfg.container.extraOptions} \
                    ${cfg.container.image} \
                    ${containerDataDir}/current-package/bin/curie gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
