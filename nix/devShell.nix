...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      curieNpmLib = self'.packages.default.passthru.curieNpmLib;
      ...
      curieAgentDevShellHook = self'.packages.default.passthru.devShellHook;
    in
    {
      devShells.default = pkgs.mkShell {
        packages = with pkgs; [
          (pkgs.runCommand "curie" { } ''
            mkdir -p $out/bin
            install -Dm755 ${../curie} $out/bin/curie
          '')
        ...
        shellHook = ''
          ${curieAgentDevShellHook}
          ${curieNpmLib.mkNpmDevShellHook npmPackageJsonPaths}
          ...
          export CURIE_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          ...
          echo "Curie Agent dev shell in $CURIE_PYTHON_SRC_ROOT"
          echo "Ready. Run 'curie' or 'sandbox curie' to start."
        '';
      };
    };
}
