...
class PlatformEntry:
    ...
    # status displays (``curie setup``, ``curie status``, the dashboard
    ...
    # Env vars this platform needs (for ``curie setup`` display).
    ...
    # built-ins).  Used by ``curie gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved CURIE_HOME and overlay the
        ...
        # plugin-discovery time added several seconds to *every* `curie`
        # invocation -- including plain `curie chat`, which never touches any
        ...
        # `curie setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``curie setup``/``gateway status``, channel
