...
# ``CURIE_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``CURIE_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
    ...
    """
    raw = os.environ.get("CURIE_AUTO_CONTINUE_FRESHNESS")
...
# ``sessions_dir / f"{session_id}.json"`` in curie_state, request-dump
...
def _slack_tools_loaded() -> bool:
    ...
    Two independent paths grant Slack capability:
      1. Native `slack` toolset enabled via `curie tools` (opt-in, default
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `curie tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `curie tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_curie_home()}/cron/output/)"
    )
...
def build_channel_continuity_note(
    ...
    return (
        f"[System note: This {where} had an earlier Curie session "
...
class SessionStore:
    ...
    def __init__(self, sessions_dir: Path, config: GatewayConfig,
        ...
        # ``get_curie_home()`` for the turn (its docstring lists "sessions"
        ...
        # profile name -> its CURIE_HOME (or None to use the ambient scope).
        ...
        try:
            ...
            self._routing_home: Optional[Path] = Path(get_curie_home())
        except Exception:
    ...
    def _open_session_db_for_active_scope(self, db_path: Optional[Path] = None):
        ...
        time, and that helper follows the context-local CURIE_HOME override
        ...
        """
        from curie_state import SessionDB, _default_db_path, get_shared_session_db
    ...
    def _profile_home_for_key(self, session_key: Optional[str]) -> Optional[Path]:
        """CURIE_HOME of the profile that owns *session_key*, or None.
        ...
        try:
            from curie_cli.profiles import get_profile_dir, profile_exists
    ...
    def _db_for_key(self, session_key: Optional[str]):
        ...
        ``_db`` follows the ambient CURIE_HOME, and only the inbound message
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.curie/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.curie/state.db "
                "and are shown by `curie sessions list` and `/sessions`. "
        ...
        while True:
            try:
                ...
                if isinstance(exc, (StateDbReplacedError, StateDbCorruptError)):
                    ...
                    try:
                        from curie_state import divert_session_transcript_jsonl
