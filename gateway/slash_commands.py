...
Module-level run.py helpers a handler needs (``_curie_home``,
...
from curie_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: curie gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Curie commands.
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        try:
            from curie_cli.lifecycle import invoke_hook as _invoke_hook
        ...
        # The card reports the session: model, provider, context. It used to
        # append a random "Tip:" line as well, which is a message of the day
        # bolted onto a status readout — unrelated to the thing being reported,
        # different on every /new, and after the first few it is only ever
        # noise between the user and the numbers they asked for.
        if session_info:
    ...
    async def _handle_profile_command(self, event: MessageEvent) -> str:
        ...
        from curie_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_kanban_command(self, event: MessageEvent) -> str:
        ...
        import re
        import shlex
        from curie_cli.kanban import run_slash
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`curie gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _curie_home
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _curie_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            await asyncio.to_thread(
                atomic_json_write,
                _curie_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Curie Agent version."""
        from curie_cli.slash_exec import CommandContext, execute_command
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _curie_home, _load_gateway_config
        ...
        from curie_cli.providers import get_label
        ...
        # via the shared single-owner parser (curie_cli.model_switch).
        ...
        custom_provs = None
        excluded_provs = []
        config_path = (_command_profile_home or _curie_home) / "config.yaml"
        try:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected_scoped(
                        ...
                        try:
                            from curie_cli.context_switch_guard import (
                        ...
                        from curie_cli.model_switch import format_model_for_display
                        ...
                        if persist_global:
                            try:
                                ...
                                if isinstance(_raw_model, dict):
                                    ...
                                    _persist_cfg["model"] = _persist_model_cfg
                                try:
                                    from curie_cli.route_identity import should_clear_context_pin_async
                        ...
                        from curie_cli.model_switch import resolve_display_context_length_async
                        _sw_config_ctx = None
                        _sw_model_cfg = {}
                        try:
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Curie default runtime
        ...
        from curie_cli import codex_runtime_switch as crs
        ...
        # Load + persist via the same helpers used for /model and /unlock
        try:
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        if lower.startswith("draft"):
            ...
            try:
                import asyncio
                from curie_cli.goals import draft_contract
            ...
            from curie_cli.goals import parse_contract
    ...
    async def _handle_heartbeat_command(self, event: "MessageEvent") -> str:
        ...
        """
        from curie_cli.heartbeat import parse_interval, format_interval, MIN_INTERVAL_SECONDS
        ...
        return (
            ...
            "elapsed. Lives while the gateway runs — use `curie cron` for durable schedules."
        )
    ...
    async def _handle_loop_command(self, event: "MessageEvent") -> str:
        ...
        """
        try:
            from curie_cli.loops import dispatch_loop_command, goal_blocks_loop_tick
        except Exception as exc:
    ...
    async def _handle_diff_command(self, event: MessageEvent) -> str:
        ...
        Curie itself changed. ``--stat`` limits output to the summary.
    ...
    def _save_gateway_config_key(self, key_path: str, value) -> bool:
        ...
        from gateway.run import _curie_home
        from curie_cli.config import read_user_config_raw
        config_path = _curie_home / "config.yaml"
        try:
    ...
    async def _handle_memory_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _curie_home
        from curie_cli.write_approval_commands import handle_pending_subcommand
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.curie/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``curie skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _curie_home
        from curie_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `curie skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.curie/pending/skills/{pending_id}.json)")
        return out
    ...
    async def _handle_fast_command(self, event: MessageEvent) -> Optional[str]:
        ...
        from curie_cli.models import model_supports_fast_mode
    ...
    async def _handle_approvals_command(self, event: MessageEvent) -> str:
        ...
        from gateway.slash_access import policy_for_source
        from curie_cli.approval_mode import run_approval_mode_command
    ...
    async def _handle_unlock_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /unlock — toggle dangerous command approval bypass for this session only."""
        from tools.approval import (
        ...
        session_key = self._session_key_for_source(event.source)
        current = is_session_unlock_enabled(session_key)
        if current:
            disable_session_unlock(session_key)
            return EphemeralReply(t("gateway.unlock.disabled"))
        else:
            enable_session_unlock(session_key)
            return EphemeralReply(t("gateway.unlock.enabled"))
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _curie_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_busy_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        """Handle /busy — control what happens when messaging while Curie is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            ...
            if arg == "queue":
                behavior = "Messages will be queued for the next turn while Curie is busy."
            elif arg == "steer":
                behavior = "Messages will be steered into the current run (after the next tool call)."
            else:
                behavior = "Messages will interrupt the current run while Curie is busy."
            return EphemeralReply(
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _curie_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_save_command(self, event: MessageEvent) -> str:
        ...
        """
        from curie_cli.session_export import (
            SAVE_USAGE,
        ...
        if redact:
            from curie_cli.session_export_md import redact_session_data

            export_data = redact_session_data(export_data)

        import tempfile

        temp_dir = tempfile.mkdtemp(prefix="curie_save_")
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from curie_cli.session_listing import (
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        try:
            await self._session_db.create_session(
                ...
                # path (curie_state.py:1994-2009) that searches by the complete
    ...
    async def _handle_insights_command(self, event: MessageEvent) -> str:
        ...
        try:
            from curie_state import get_shared_session_db, release_shared_session_db
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `curie bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``curie debug share`` from the CLI.
        """
        import asyncio
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Curie Agent to the latest version.

        Spawns ``curie update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``curie update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _curie_home, _resolve_curie_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        from curie_cli.config import is_managed, format_managed_message
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Curie Agent')}"
        ...
        curie_cmd = _resolve_curie_bin()
        if not curie_cmd:
            return t("gateway.update.curie_cmd_not_found")

        pending_path = _curie_home / ".update_pending.json"
        output_path = _curie_home / ".update_output.txt"
        exit_code_path = _curie_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `curie update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `curie update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # than through curie_cmd (venv\Scripts\curie.exe): the shim
                ...
                curie_cmd_str = " ".join(shlex.quote(part) for part in curie_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {curie_cmd_str} update --gateway"
