...
``<CURIE_HOME>/state/gateway.lifecycle.json``:
...
def _process_curie_home() -> Path:
    """CURIE_HOME for process-level identity files (ignore task overrides)."""
...
def get_lifecycle_sentinel_path(home: Optional[Path] = None) -> Path:
    """Return ``<CURIE_HOME>/state/gateway.lifecycle.json``."""
...
def check_state_db_integrity(home: Optional[Path] = None) -> str:
    ...
    ``_enforce_macos_synchronous_full`` in :mod:`curie_state` — macOS
...
def read_prior_exit_label(profile_home: Path) -> str:
    ...
    ``curie_cli.container_boot`` to annotate ``container-boot.log``.
    """
    try:
