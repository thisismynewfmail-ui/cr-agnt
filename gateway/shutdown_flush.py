...
1. ``flush_pending_to_file()`` — called BEFORE ``_pending_messages.clear()``
   ...
   under ``<curie_home>/pending_messages/``.
...
def _get_flush_dir():
    """Return the pending-messages flush directory under the active CURIE_HOME."""
    from curie_constants import get_curie_home

    flush_dir = get_curie_home() / "pending_messages"
...
def spool_dropped_transcript_message(
    ...
    ``<curie_home>/pending_messages/``), so a runtime cap rotation no
