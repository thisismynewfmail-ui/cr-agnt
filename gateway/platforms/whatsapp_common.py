...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "▮ *Curie Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to CURIE_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/curie), this function
    mirrors the bridge source to a writable CURIE_HOME location and returns that
    ...
    # Try CURIE_HOME location first
    curie_home = get_curie_home()
    curie_home_bridge = curie_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to CURIE_HOME if needed
    if curie_home_bridge.exists():
        return curie_home_bridge

    # Mirror the bridge source to CURIE_HOME
    try:
        curie_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            curie_home_bridge,
            dirs_exist_ok=False,
