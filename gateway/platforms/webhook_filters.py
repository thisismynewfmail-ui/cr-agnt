...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.curie to the active profile home."""
    ...
    if raw == "~/.curie":
        return curie_home
    if raw.startswith("~/.curie/"):
        return curie_home / raw.removeprefix("~/.curie/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return curie_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under CURIE_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from curie_constants import get_curie_home

    scripts_root = (get_curie_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.curie" or raw_text.startswith("~/.curie/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__curie_ignore__") is True
        ):
            return False, None
