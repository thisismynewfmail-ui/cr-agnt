...
There are two ways to add a platform to the Curie gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.curie/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Curie code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `curie gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `curie_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Curie core.
...
And add it to the `curie-gateway` composite:

```python
"curie-gateway": {
    "includes": [..., "curie-your-platform"]
}
```
...
## 12. Status Display (`curie_cli/status.py`)
...
## 13. Gateway Setup Wizard (`curie_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ curie_cli/ toolsets.py \
  --include="*.py" -l | sort -u
