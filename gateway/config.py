...
def _normalize_multiplex_profile_allowlist(value: Any) -> Optional[List[str]]:
    ...
    from curie_cli.profiles import normalize_profile_name, validate_profile_name
...
# dashboard's pre-write mutation validation (curie_cli/web_server.py) so
...
class PlatformConfig:
    ...
    # "Curie is thinking…"). Platforms with textless indicators (Discord,
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_curie_home() / "sessions")
    ...
    # gateway behaves exactly as before — single CURIE_HOME, no profile stamping.
    ...
    # advertise a route. CURIE_ROOM_LINK_URL remains the operator override.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.curie/config.yaml (primary user-facing config)
    3. ~/.curie/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_curie_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through curie_cli.config.load_config, so without this a managed
            ...
            # (e.g. via `curie config set gateway.<key> ...`, which naturally
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `curie config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict) and isinstance(gateway_section, dict):
                ...
                # ``curie config set gateway.streaming.*``
