...
def _gateway_room_grant_secret_for_home(home_value: str) -> bytes:
    ...
    return hmac.new(
        material,
        b"curie-hosted-room-installation-grant-v1",
        hashlib.sha256,
    ).digest()
...
def gateway_room_grant_secret(root: Path | str | None = None) -> bytes:
    ...
    if root is None:
        ...
        # Profile routing uses a context-local CURIE_HOME override. The process
        # environment retains the installation root and is the authority here.
        root = os.environ.get("CURIE_HOME") or get_curie_home()
...
def catalog_mapping(
    *,
    ...
    persistent_process = bool(persistent_process and os.getenv("CURIE_DESKTOP") != "1")
    checked_policy = RoomExecutionPolicy.from_mapping(
        execution_policy
        or execution_policy_mapping(
            target_profile=(
                str(target_profile or "").strip()
                or (os.getenv("CURIE_PROFILE") or "default").strip()
                or "default"
            )
        )
    )
    # A RoomLink run is initiated by another installation. Process-wide UNLOCK
...
def _configured_room_link_url() -> str | None:
    ...
    override = os.getenv("CURIE_ROOM_LINK_URL")
