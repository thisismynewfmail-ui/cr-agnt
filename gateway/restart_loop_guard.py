...
Defenses 1 and 2 (the ``_CURIE_GATEWAY`` guard on ``curie gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.curie.gateway")``,
...
State lives in ``<CURIE_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_curie_home() / "gateway" / "restart_loop.json"
