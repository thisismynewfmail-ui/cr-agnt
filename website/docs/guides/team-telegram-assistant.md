...
This tutorial walks you through setting up a Telegram bot powered by Curie Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Curie Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.curie/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Curie itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Curie Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_curie_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Curie Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Curie Gateway...
...
# View live logs
journalctl --user -u curie-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `curie gateway install` to update the plist.
:::
...
```bash
curie gateway stop && curie gateway start
```
...
curie pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.curie/.env`:
...
Control how much detail the bot shows when using tools. In `~/.curie/config.yaml`:
...
Customize how the bot communicates by editing `~/.curie/SOUL.md`:

For a full guide, see [Use SOUL.md with Curie](/guides/use-soul-with-curie).
...
```bash
# From the CLI
curie cron list          # View all scheduled jobs
curie cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u curie-gateway` (Linux) or `~/.curie/logs/gateway.log` (macOS) |
| Cron job output | `~/.curie/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.curie/cron/jobs.json` |
| Pairing data | `~/.curie/pairing/` |
| Session history | `~/.curie/sessions/` |
...
- **[Personality](/user-guide/features/soul)** — built-in personality presets and custom persona definitions
