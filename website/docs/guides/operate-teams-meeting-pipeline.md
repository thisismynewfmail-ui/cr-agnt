...
```bash
curie teams-pipeline token-health
curie teams-pipeline token-health --force-refresh
```
...
```bash
curie teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
curie teams-pipeline maintain-subscriptions
curie teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Curie cron (recommended if you already run the Curie gateway)

Curie ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.curie/scripts/`. First create the script:

```bash
mkdir -p ~/.curie/scripts
cat > ~/.curie/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec curie teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.curie/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
curie cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/curie-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Curie Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=curie
EnvironmentFile=/etc/curie/env
ExecStart=/usr/local/bin/curie teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/curie-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Curie Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now curie-teams-pipeline-maintain.timer
systemctl list-timers curie-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/curie teams-pipeline maintain-subscriptions >> /var/log/curie/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.curie/.env` at the top of a wrapper script that crontab calls.
...
```bash
curie teams-pipeline subscriptions   # should show expirationDateTime advanced
curie teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
curie teams-pipeline list
curie teams-pipeline list --status failed
curie teams-pipeline show <job-id>
```

### Replay a stored job

```bash
curie teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
curie teams-pipeline fetch --meeting-id <meeting-id>
curie teams-pipeline fetch --join-web-url "<join-url>"
curie teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `curie teams-pipeline maintain-subscriptions --dry-run`
- inspect `curie teams-pipeline list --status failed`
...
- run `curie teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `curie teams-pipeline run`
...
- [ ] `curie teams-pipeline validate` returns an OK snapshot
- [ ] `curie teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Curie cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
