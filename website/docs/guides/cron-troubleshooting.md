...
description: "Diagnose and fix common Curie cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`curie gateway` for foreground, or `curie gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `curie cron tick`.
...
```bash
date
curie cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.curie/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.curie/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.curie/.env` |
...
| `local` | Write access to `~/.curie/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `curie cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (Curie Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `curie skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `curie skills list`.
...
2. `~/.curie/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `curie cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Curie config directory). Verify:
```bash
ls ~/.curie/scripts/your-script.py   # Must exist
curie cron edit <job_id> --script ~/.curie/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `curie skills install <skill-name>`.
...
Jobs are stored in `~/.curie/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.curie/cron/jobs.json
chmod 600 ~/.curie/cron/jobs.json   # Your user should own it
```
...
```bash
curie cron list                    # Show all jobs, states, next_run times
curie cron run <job_id>            # Schedule for next tick (for testing)
curie cron edit <job_id>           # Fix configuration issues
curie logs                         # View recent Curie logs
curie skills list                  # Verify installed skills
```
...
1. Run the job with `curie cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.curie/logs/agent.log` for scheduler messages and `~/.curie/logs/errors.log` for warnings
3. Open an issue at [github.com/thisismynewfmail-ui/Cur-Agnt](https://github.com/thisismynewfmail-ui/Cur-Agnt) with:
