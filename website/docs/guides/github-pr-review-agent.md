...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Curie Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Curie instantly when PRs are opened or updated.
:::
...
- **Curie Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  curie gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.curie/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Curie can access GitHub. Start a chat:
...
```
Run: gh pr list --repo thisismynewfmail-ui/Cur-Agnt --state open --limit 3
```
...
Still in the chat, ask Curie to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo thisismynewfmail-ui/Cur-Agnt
```
...
A skill gives Curie consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.curie/skills/code-review
```

Create `~/.curie/skills/code-review/SKILL.md`:
...
Verify it loaded — start `curie` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Curie your team's standards:
...
2. Teach Curie your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
curie gateway status    # Is the gateway running?
curie cron list         # Is the job enabled?
```
