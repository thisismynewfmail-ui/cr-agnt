...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `curie send`."
---

# Pipe Script Output to Messaging Platforms

`curie send` is a small, scriptable CLI that pushes a message to any
messaging platform Curie is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | curie send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `curie
...
curie send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
curie send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
curie send --list

# Filter by platform
curie send --list telegram
```
...
Any platform Curie ships adapters for works as a target:
...
`curie send` resolves the message body in this order:

1. **Positional argument** — `curie send --to telegram "hi"`
2. **`--file PATH`** — `curie send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | curie send --to telegram`

When stdin is a TTY (no pipe), Curie does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  curie send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `curie send` reuses your Curie config, the same script works on
any host where Curie is installed — no need to export bot tokens into
...
`curie send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  curie send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | curie send \
...
```bash
./train.py --epochs 200 && \
  curie send --to telegram "training done" || \
  curie send --to telegram "training failed (exit $?)"
```
...
curie send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(curie send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `curie send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `curie send` calls
the platform's REST endpoint directly using credentials from
`~/.curie/.env` and `~/.curie/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `curie gateway start` and retry.
...
# Machine-readable
curie send --list --json
```

The listing is built from `~/.curie/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`curie gateway
...
| Approach | Multi-platform | Reuses Curie creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `curie send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`curie send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `curie send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `curie send` shares with cron delivery.
