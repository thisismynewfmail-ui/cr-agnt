---
sidebar_position: 8
title: "Use Voice Mode with Curie"
description: "A practical guide to setting up and using Curie voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Curie
...
- you want Curie sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Curie.
...
## Step 1: make sure normal Curie works first
...
Curie supports both local and cloud speech stacks.
...
### If you use `curie setup`

If you choose NeuTTS in the setup wizard, Curie checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Curie transcribes and responds
...
- asking Curie to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Curie loop.

## Tuning CLI behavior

### Silence threshold

If Curie starts/stops too aggressively, tune:
...
Curie stays a normal chat bot, but can speak replies.
...
- you want Curie to function like a portable research or ops assistant
...
Curie joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Curie detects speech boundaries
- transcripts are posted in the associated text channel
- Curie responds in text and audio
...
1. get text Curie working
2. run `curie setup tts` to enable voice support
