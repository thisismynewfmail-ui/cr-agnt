...
Copy-paste blueprints for common automation patterns. Each blueprint uses Curie's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`curie webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `curie webhook subscribe`) |
...
```bash
curie cron create "0 2 * * *" \
  "You are a project manager triaging the thisismynewfmail-ui/Cur-Agnt GitHub repo.

1. Run: gh issue list --repo thisismynewfmail-ui/Cur-Agnt --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
curie webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
curie cron create "0 9 * * 1" \
  "Scan the thisismynewfmail-ui/Cur-Agnt repo for documentation drift.

1. Run: gh pr list --repo thisismynewfmail-ui/Cur-Agnt --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (curie_cli/commands.py, curie_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (curie_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
curie cron create "0 6 * * *" \
  "Run a dependency security audit on the curie-agent project.

1. cd ~/.curie/curie-agent && source .venv/bin/activate
...
```bash
curie webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
curie webhook subscribe alert-triage \
...
```python title="~/.curie/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Curie-Monitor/1.0"})
...
curie cron create "every 30m" \
  ...
  --script ~/.curie/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
curie cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Curie Agent development." \
...
```bash
curie webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
curie webhook subscribe stripe-payments \
...
```bash
curie cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the curie-agent codebase.
