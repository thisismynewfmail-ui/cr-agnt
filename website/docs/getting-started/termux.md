...
description: "Run Curie Agent directly on an Android phone with Termux"
---

# Curie on Android with Termux
...
Curie Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Curie from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed Curie installs are marked with install method `apt`. Curie therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade curie-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Curie runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.
...
- links `curie` into `$PREFIX/bin` so it stays on your Termux PATH
...
Curie requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `curie` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/curie" "$PREFIX/bin/curie"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `curie` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.curie/.env`.
...
### `curie doctor` says ripgrep or Node is missing
