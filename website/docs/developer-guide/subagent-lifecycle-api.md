...
Plugins can launch and supervise fresh Curie child sessions without importing
...
active agent turn fails closed with `No active Curie parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Curie can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Curie's existing unsafe-tool block remains enforced.
