...
| Variable | Meaning |
|----------|---------|
| `CURIE_MAIN_CHECKOUT` | The deps checkout — where `node_modules` really lives, and whose `.venv/bin/python` runs the backend. |
| `CURIE_GUI_DEPS_CHECKOUT` | Where the desktop deps (`apps/desktop/node_modules`) live. Defaults to `CURIE_MAIN_CHECKOUT`; override only if you keep desktop deps elsewhere. |

Neither is read by Curie itself — they're private to these helpers. The variables Curie *does* read are covered in [Environment Variables](../reference/environment-variables.md).

## `htui` — TUI from the worktree

The Ink TUI has a dev path already: `curie --tui --dev` runs the TypeScript sources via `tsx` instead of the prebuilt bundle. `htui` is a one-liner over it that also points the run at the current worktree's `ui-tui/`:

```bash
htui() {
  local root
  root="$(_curie_root)" || { echo "htui: not in a Curie checkout" >&2; return 1; }
  ( cd "$root" && PYTHONPATH="$root" \
      "$CURIE_MAIN_CHECKOUT/.venv/bin/python" -m curie_cli.main --tui --dev "$@" )
}
```

`--dev` compiles from source, so it links `ui-tui/node_modules` from `CURIE_MAIN_CHECKOUT` when the root lockfile matches and installs locally otherwise (see [`_curie_root` / linking helpers](#shared-helpers)).

:::warning `--dev` and `CURIE_TUI_DIR` are mutually exclusive
`CURIE_TUI_DIR` points Curie at a *prebuilt* bundle (Nix, system packages), which has no source to hot-reload. If it's set in your shell, `curie --tui --dev` exits with an error. Run `unset CURIE_TUI_DIR` before `htui`.
:::
...
hgui() {
  ...
  root="$(_curie_root)" || { echo "hgui: not in a Curie checkout" >&2; return 1; }
  deps="${CURIE_GUI_DEPS_CHECKOUT:-$CURIE_MAIN_CHECKOUT}"
  ...
  if cmp -s "$root/package-lock.json" "$deps/package-lock.json"; then
    _curie_link_deps "$desktop" "$deps/apps/desktop"
    _curie_link_deps "$root" "$deps"
  else
  ...
  trap '_curie_gui_cleanup "$root"' INT TERM EXIT

  ( cd "$desktop"
    export PATH="$root/node_modules/.bin:$PATH"
    CURIE_DESKTOP_CURIE_ROOT="$root" \
    CURIE_DESKTOP_PYTHON="$CURIE_MAIN_CHECKOUT/.venv/bin/python" \
    CURIE_DESKTOP_IGNORE_EXISTING=1 \
    CURIE_DESKTOP_CWD="$root" \
    npm run dev )
}
```
...
|----------|----------------|
| `CURIE_DESKTOP_CURIE_ROOT` | Runs the backend from **this worktree**, not the packaged/PATH `curie`. |
| `CURIE_DESKTOP_PYTHON` | Reuses the deps checkout's venv instead of re-resolving a Python. |
| `CURIE_DESKTOP_IGNORE_EXISTING` | Ignores any `curie` on `PATH` so it can't shadow the worktree. |
| `CURIE_DESKTOP_CWD` | Opens the desktop chat rooted at the worktree. |
...
```bash
# The enclosing worktree, verified as a real Curie checkout.
_curie_root() {
  local root
  root="$(git rev-parse --show-toplevel 2>/dev/null)" || return 1
  [[ -f "$root/curie_cli/main.py" && -d "$root/ui-tui" ]] && print -r "$root"
}
...
_curie_gui_cleanup() {
  ...
  lsof -t -i:5174 >/dev/null 2>&1 && killport 5174
  pgrep -f 'curie_cli\.main.*dashboard.*--port 0' 2>/dev/null | xargs -r kill -TERM 2>/dev/null
}
```
...
- [TUI](../user-guide/tui.md) — `curie --tui --dev` and the `CURIE_TUI_DIR` prebuild path
- [Desktop App](../user-guide/desktop.md) — building from source and the backend resolution ladder
- [`apps/desktop/README.md`](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/apps/desktop/README.md) — dev server, sandbox script, and packaging
- [Environment Variables](../reference/environment-variables.md) — every `CURIE_*` variable Curie reads
