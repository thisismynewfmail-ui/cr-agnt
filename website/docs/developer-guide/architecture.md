...
sidebar_position: 1
title: "Architecture"
description: "Curie Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Curie Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ curie_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # CurieCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── curie_state.py           # SQLite session/state database with FTS5
├── curie_constants.py       # CURIE_HOME, profile-aware paths
...
│
├── curie_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `curie` subcommands (large file)
...
│   ├── skills_config.py      # curie skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # curie tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # curie gateway start/stop
│
...
```text
User input → CurieCLI.process_input()
...
Three discovery sources: `~/.curie/plugins/` (user), `.curie/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `curie plugins` or `config.yaml`.
...
Exposes Curie as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`curie -p <name>`) gets its own CURIE_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
