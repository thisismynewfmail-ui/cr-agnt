...
title: "Desktop Plugin SDK (@curie/plugin-sdk)"
description: "Extend the native Curie Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Curie Desktop](/user-guide/desktop) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `CuriePlugin`.
It imports one module — `@curie/plugin-sdk` — and gets everything: the app's
...
`$CURIE_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Curie. This page is the **native
desktop app** (`curie desktop`) SDK — the `@curie/plugin-sdk` module and
`$CURIE_HOME/desktop-plugins/`. The **web dashboard** (`curie dashboard`) has
its own, unrelated plugin system on `window.__CURIE_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](/user-guide/features/extending-the-dashboard). Python
CLI/gateway plugins are documented at [Build a Curie Plugin](/developer-guide/plugins).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$CURIE_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$CURIE_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `CuriePlugin` contract, appear in **Settings → Plugins**,
...
Create `$CURIE_HOME/desktop-plugins/hello/plugin.js` (that's `~/.curie/...`
by default, or `~/.curie/profiles/<name>/...` under a named profile). The folder
name must equal the plugin `id`.

```javascript
// ~/.curie/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@curie/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Curie' }),
      jsx('div', {
...
The only importable specifiers are `@curie/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `CuriePlugin`:

```ts
interface CuriePlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `curie.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@curie/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@curie/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@curie/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@curie/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@curie/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@curie/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@curie/plugin-sdk'
...
`window.curie.send('get-price eth')` (or a declarative
`<button data-curie-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, TITLEBAR_AREAS } from '@curie/plugin-sdk'
...
for requests; `targetProfile` is the backend Curie profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Curie
...
ctx.os.notify({
  ...
  // Body click → focus Curie + navigate. Same vocabulary as OS deep links:
  activate: 'curie://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'curie://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `curie://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `curie://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@curie/plugin-sdk'
...
```javascript
import { queryClient } from '@curie/plugin-sdk'
...
a package into `~/.curie/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.curie/plugins` is not reachable as a filesystem —
...
the `curie://` scheme — a plain anchor on your website or README:
...
`dashboard/` subfolder of a regular Curie plugin and declare it in a
`manifest.json`:
...
curie-agent codebase directly (`curie_state`, `curie_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.curie/`)
...
(`curie.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `CuriePlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@curie/plugin-sdk` alias.
2. It's still lint-fenced to `@curie/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@curie/plugin-sdk`, `react`,
...
| Plugin contract | `CuriePlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `CurieOpenTarget`, `Contribution` |
...
### Agents: the `curie-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`curie-desktop-plugins`** skill — it carries the same contract as this page in
...
`$CURIE_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `curie logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@curie/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.curie/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.curie/logs/errors.log` for
