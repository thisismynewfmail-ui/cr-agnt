...
The ACP adapter wraps Curie' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
curie acp / curie-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.curie/.env
  -> configure stderr logging
  -> construct CurieACPAgent
...
- `allow_once` -> Curie `once`
- `allow_always` -> Curie `always`
- reject options -> Curie `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Curie tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["curie-acp"])
...
Instead it reuses Curie' runtime resolver:

- `acp_adapter/auth.py`
- `curie_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Curie provider/credentials. It also always advertises a terminal setup auth method (`curie-setup`, args `--setup`) so first-run ACP clients can open Curie' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.curie/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `curie-acp` toolset definition
- `curie_cli/main.py` — `curie acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `curie-acp` script
