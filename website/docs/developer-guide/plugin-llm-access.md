...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Curie already has applies. The plugin never sees a
...
curie-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Curie provider id (e.g. "openrouter")
...
using Curie built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Curie provider
        # (must be one Curie already knows about — same names as
        # `curie model` and config.yaml model.provider).
        ...
        # plugin sends — Curie does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.curie/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Curie' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Curie subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/tests/agent/test_plugin_llm.py)
