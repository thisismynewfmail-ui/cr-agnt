...
Curie runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.curie/plugins/`, registering a backend the user selects
...
| `curie setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `curie status` / `curie doctor` | `doctor_checks()` |
...
```python title="~/.curie/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.curie"    # where synced cache files land, or None
...
```yaml title="~/.curie/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
curie plugins enable acmebox
curie config set terminal.backend acmebox
```
