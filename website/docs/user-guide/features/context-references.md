...
Type `@` followed by a reference to inject content directly into your message. Curie expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Curie env: `$CURIE_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$CURIE_HOME/skills/.hub/`
