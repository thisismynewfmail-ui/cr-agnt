...
When Curie Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.curie/SOUL.md`, or as a named preset under
`SOUL.md` in `$CURIE_HOME` — it is re-read on every message, so an edit
lands on the next reply.
...
If you use Curie' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.curie/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Curie Agent's deliverable
