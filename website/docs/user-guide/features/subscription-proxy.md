...
chat completions — use your Curie-managed provider subscription as their
...
| Use case | "Use Curie as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Curie stores
the refresh token in `~/.curie/auth.json` — the same place all Curie
...
interface in `curie_cli/proxy/adapters/`.
...
If you see `not logged in`, run `curie portal`. If you see
...
`curie_cli/proxy/adapters/<provider>.py` and registering it in
