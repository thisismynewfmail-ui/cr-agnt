...
Curie Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Curie integration. It
...
**Fresh installs already have the driver.** The Curie installer
...
- **`curie tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
curie computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `curie computer-use
status` to verify the install.

Already have cua-driver? Curie reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `curie update`, and the first
`computer_use` call of a session, Curie checks the local version and
...
selected with `CURIE_CUA_DRIVER_CMD` stays
under your control, so Curie reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Curie autodetection is a
planned cua-driver follow-up, so currently point Curie at that directory or
...
interface. The built-in toolset provides Curie actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `curie computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Curie host identity. |
...
```
curie -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.curie/config.yaml`.

## Permission modes and logged-in browser profiles

Curie maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Curie session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Curie approvals; Cua stops at its protected boundary |
...
| `--unlock`, `/unlock`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Curie risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.curie/cua-manifest.yaml
```
...
for the format). Curie launches a private runtime with
...
fails loudly at session start rather than silently downgrading. Session UNLOCK
...
identity instead of resetting with every Curie build), and Curie verifies
...
state. It does not select, share, or keep a runtime alive. Turning `/unlock` off,
resetting or closing the Curie session, cancellation cleanup, or process exit
closes that transport session. Curie also stops private runtimes that it
launched for bounded or unrestricted access. One Curie
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Curie host identity.
...
Curie run declares a public cua-driver **session name** (something like
...
without it. Curie disables it automatically where it is a known failure
...
Curie keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Curie-side `computer_use` workflow and action vocabulary. For
...
The command installs the pack under `~/.cua-driver/skills/cua-driver`. Curie
autodetection is a planned cua-driver follow-up, so currently point Curie at
...
These are **platform deep dives, not duplicates of the Curie skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Curie
...
image capture also saves a bounded, shareable copy under Curie' image cache
...
Curie applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.curie/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Curie applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Curie agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Curie agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Curie warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Curie from inside the RDP / console
...
```
CURIE_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
CURIE_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Curie disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Curie sets
...
When it's on, `curie computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Curie at a binary you built from source instead
of the published release. Curie resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`CURIE_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Curie testing (see notes).
...
### Option B — point Curie straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`CURIE_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
CURIE_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
CURIE_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Curie is using your build

- `curie computer-use status` prints the resolved binary path and
  version.
- `curie computer-use doctor` confirms the binary is reachable and
...
- **Curie spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Curie UNLOCK, Curie instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Curie
  change needed either way.
- **Local builds skip the version check.** Curie warns when the
...
**First action when anything's off: run `curie computer-use doctor`.**
...
Run `curie computer-use install` to fetch the cua-driver binary, or
run `curie tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `curie computer-use doctor` will
...
- **Curie-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Curie `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Curie autodetection is a
  planned follow-up; currently point Curie at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Curie among them)
