...
Curie can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<CURIE_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Curie renders the sprite at full fidelity when your
...
curie pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `curie pets list [query] [--limit N]` |
| List installed pets | `curie pets list --installed` |
| Install a pet | `curie pets install <slug> [--select] [--force]` |
...
| Preview/animate | `curie pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `curie pets off` |
| Remove an installed pet | `curie pets remove <slug>` |
| Diagnose setup | `curie pets doctor` |

`curie pets show` flags:
...
Beyond installing pre-made pets from the gallery, Curie can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `curie pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `curie tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `CURIE_PET_IMAGE_PROVIDER` env var (e.g. `CURIE_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Curie is
...
Run `curie pets doctor` — it reports:
...
- The [`curie-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-curie-agent.md)
