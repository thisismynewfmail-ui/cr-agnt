...
Curie runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Curie
...
1. Curie captures a baseline of current diagnostics for the file.
...
…). Curie auto-detects the binary on PATH or in
`<CURIE_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Curie at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<CURIE_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`curie lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Curie installs both packages together when you
run `curie lsp install typescript` or auto-install fires on first
use.

## CLI

```
curie lsp status          # service state + per-server install status
curie lsp list            # registry, optionally --installed-only
curie lsp install <id>    # eagerly install one server
curie lsp install-all     # try every server with a known recipe
curie lsp restart         # tear down running clients
curie lsp which <id>      # print resolved binary path
```

`curie lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <CURIE_HOME>/lsp/bin
...
When `install_strategy: auto`, Curie installs binaries into
`<CURIE_HOME>/lsp/bin/`. NPM packages land in
`<CURIE_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Curie-owned and is
...
**`curie lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<CURIE_HOME>/lsp/bin/`. Run
`curie lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `curie lsp status`**
...
When `curie lsp status` shows a `Backend warnings` section, install
...
`~/.curie/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.curie/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `curie lsp restart` to clear the set;
