...
sidebar_position: 8
title: "Context Files"
description: "Project context files — .curie.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
---

# Context Files

Curie Agent automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Curie instance and is loaded from `CURIE_HOME` only.

## Supported Context Files

| File | Purpose | Discovery |
|------|---------|-----------| 
| **.curie.md** / **CURIE.md** | Project instructions (highest priority) | Walks to git root |
...
| **SOUL.md** | Global personality and tone customization for this Curie instance | `CURIE_HOME/SOUL.md` only |
| **.cursorrules** | Cursor IDE coding conventions | CWD only |
| **.cursor/rules/*.mdc** | Cursor IDE rule modules | CWD only |

:::info Priority system
Only **one** project context type is loaded per session (first match wins): `.curie.md` → `AGENTS.override.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
...
When your working directory sits inside a git repository, Curie loads a **merged chain** of `AGENTS.md` files at session start: the git-root `AGENTS.md` first, then the `AGENTS.md` in every intermediate directory down to your working directory. Deeper files appear later in the prompt, so more specific guidance takes precedence. Each file gets its own provenance header (e.g. `## ../../AGENTS.md`), and identical copies along the chain are deduplicated.
...
`SOUL.md` controls the agent's personality, tone, and communication style. See the [Personality](/user-guide/features/soul) page for full details.

**Location:**

- `~/.curie/SOUL.md`
- or `$CURIE_HOME/SOUL.md` if you run Curie with a custom home directory

Important details:

- Curie seeds a default `SOUL.md` automatically if one does not exist yet
- Curie loads `SOUL.md` only from `CURIE_HOME`
- Curie does not probe the working directory for `SOUL.md`
...
Curie is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.curie.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.

This means your existing Cursor conventions automatically apply when using Curie.
...
1. **Scan working directory** — checks for `.curie.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
