...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Curie's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Curie instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
curie memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `curie` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Curie syncs them back at session init.
...
The `curie honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `curie memory setup honcho` (or run `curie memory setup` and pick it from the list); the `curie honcho` subcommand then appears on the next invocation.

```bash
curie memory setup honcho    # Configure Honcho directly (works before activation)
curie honcho status          # Connection status, config, and key settings
curie honcho setup           # Redirects to `curie memory setup` (post-activation alias)
curie honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
curie honcho peer            # Show or update peer names + dialectic reasoning level
curie honcho mode            # Show or set recall mode (hybrid/context/tools)
curie honcho tokens          # Show or set token budget for context and dialectic
curie honcho identity        # Seed or show the AI peer's Honcho identity
curie honcho sync            # Sync Honcho config to all existing profiles
curie honcho peers           # Show peer identities across all profiles
curie honcho sessions        # List known Honcho session mappings
curie honcho map             # Map current directory to a Honcho session name
curie honcho enable          # Enable Honcho for the active profile
curie honcho disable         # Disable Honcho for the active profile
curie honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `curie honcho`

If you previously used the standalone `curie honcho setup`:
...
No re-login or re-setup needed. Run `curie memory setup` and select "honcho" — the wizard detects your existing config.
