...
`~/.curie/config.yaml`, `~/.curie/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/curie`:

```text
/etc/curie/
├── config.yaml     # managed config layer (wins over ~/.curie/config.yaml)
└── .env            # managed env layer (wins over ~/.curie/.env + shell)
```
...
The location can be relocated with the `CURIE_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `CURIE_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Curie.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export CURIE_MANAGED_DIR=/opt/org/curie-policy
```

:::warning
A user who can set `CURIE_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `curie doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/curie/config.yaml` (managed) | `/etc/curie/.env` (managed) |
| 2 | `~/.curie/config.yaml` (user) | `~/.curie/.env` (user) |
...
```bash
curie config        # shows a header naming the managed source + the pinned keys
curie doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Curie refuses and names the source:

```bash
$ curie config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/curie/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `curie config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/curie

# Pin some config values for every user on this machine
sudo tee /etc/curie/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/curie/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/curie
sudo chmod 0644 /etc/curie/config.yaml /etc/curie/.env
```

Changes take effect on the next Curie start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`curie doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Curie as `root`), managed scope is advisory.
