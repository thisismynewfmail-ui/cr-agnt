...
If you only run one Curie agent, you don't need this page — see
...
[Connecting Desktop to Many Curie Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Curie agents that should all
...
(`ai.curie.gateway-<name>.plist`) or systemd user service
(`curie-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
curie profile create coder
curie profile create personal-bot
curie profile create research
...
```bash
curie config set gateway.multiplex_profiles true
curie gateway restart
```

Equivalently, in the default profile's `~/.curie/config.yaml`:
...
You do **not** run `curie gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `curie gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.curie/profiles/coder/.env`; the default listener key is rejected.
...
home). `curie status` reports the multiplexer and the profiles it serves;
`curie status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/curie-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: curie-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    curie gateway "$action"
  else
    curie -p "$profile" gateway "$action"
  fi
}
...
```bash
curie-gateways start      # start every configured profile
curie-gateways stop       # stop every configured profile
curie-gateways restart    # restart all
curie-gateways status     # status across all
curie-gateways list       # delegates to `curie gateway list`
```

:::tip
The `default` profile is targeted with `curie gateway <action>` (no `-p`),
not `curie -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `curie -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.curie.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/curie-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.curie.gateway.plist` /
`curie-gateway.service`.
...
```bash
# Default profile
tail -f ~/.curie/logs/gateway.log
tail -f ~/.curie/logs/gateway.error.log

# Named profile
tail -f ~/.curie/profiles/<name>/logs/gateway.log
tail -f ~/.curie/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.curie/logs/gateway.log ~/.curie/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
curie logs -f                  # follow default profile
curie -p coder logs -f         # follow one profile
curie logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
curie profile list             # profiles + model + gateway state
curie-gateways status          # full status across every profile
launchctl list | grep curie    # macOS — PIDs and labels
systemctl --user list-units 'curie-gateway-*'   # Linux — units
```
...
The default profile uses `~/.curie/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
curie config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.curie/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=curie --why="gateways running" \
  sleep infinity &
...
`curie-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.curie/.env ~/.curie/profiles/*/.env
```

## Updating the code

`curie update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `curie gateway start` after a previous `curie gateway stop`. The
...
```bash
ps -ef | grep "curie_cli.*-p <profile>"
cat ~/.curie/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.curie.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.curie.gateway-<profile>.plist

# Linux
systemctl --user restart curie-gateway-<profile>.service
```

### Health check

```bash
curie doctor                  # default profile
curie -p <profile> doctor     # one profile
```
