...
Curie Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Curie checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.curie/config.yaml`:
...
| `timeout` | `300` | Seconds Curie waits for an approval reply before timing out. |
...
| `destructive_slash_confirm` | `true` | When true, destructive session slash commands (`/clear`, `/new`, `/reset`, `/undo`) prompt before discarding conversation state. Three-option dialog (Approve Once / Always Approve / Cancel) routed through native yes/no buttons on Telegram, Discord, and Slack; text fallback elsewhere. Users who click **Always Approve** flip this key to `false`. The TUI also honors this setting for its `/clear`, `/new`, and `/reset` modal; `CURIE_TUI_NO_CONFIRM=1` force-skips that modal regardless of the configured value. |
...
| **off** | Disable all approval checks — equivalent to running with `--unlock`. All commands execute without prompts. |
...
UNLOCK mode bypasses **all** dangerous command approval prompts for the current session. It can be activated three ways:

1. **CLI flag**: Start a session with `curie --unlock` or `curie chat --unlock`
2. **Slash command**: Type `/unlock` during a session to toggle it on/off
3. **Environment variable**: Set `CURIE_UNLOCK_MODE=1`

The `/unlock` command is a **toggle** — each use flips the mode on or off:

```
> /unlock
  ⚡ UNLOCK mode ON — all commands auto-approved. Use with caution.

> /unlock
  ⚠ UNLOCK mode OFF — dangerous commands will require approval.
```

UNLOCK mode is available in both CLI and gateway sessions. Internally, it sets the `CURIE_UNLOCK_MODE` environment variable which is checked before every command execution.

When UNLOCK is active, Curie shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:

- A red banner line at session start when UNLOCK is already active: `⚠ UNLOCK mode — all approval prompts bypassed`. Hidden when UNLOCK is off so the default banner stays uncluttered.
- A `⚠ UNLOCK` fragment in the status bar across all width tiers, updated live as you toggle UNLOCK on or off (rich-text renderer and plain-text fallback).

:::danger
UNLOCK mode disables **all** dangerous command safety checks for the session — **except** the hardline blocklist (see below). Use only when you fully trust the commands being generated (e.g., well-tested automation scripts in disposable environments).
:::
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Curie refuses to run them **regardless** of:

- `--unlock` / `/unlock` toggled on
- `approvals.mode: off`
- Cron jobs running in headless `approve` mode
- User explicitly clicking "allow always"

The blocklist is the floor below `--unlock`. It trips **before** the approval layer even sees the command, and there's no override flag. Patterns currently covered (not exhaustive; kept in sync with `tools/approval.py::UNRECOVERABLE_BLOCKLIST`):
...
The hardline blocklist is fixed and code-shipped. `approvals.deny` is its user-editable counterpart: a list of glob patterns that block matching terminal commands unconditionally — **before** `--unlock`, `/unlock`, and `approvals.mode: off` are consulted. Use it to run unlock-with-exceptions: "let the agent do everything, except these specific things, ever."
...
Configure the timeout in `~/.curie/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.curie/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.curie/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` curie/gateway | Self-termination prevention |
...
The `CURIE_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.curie/config.yaml`:
...
:::tip
Use `curie config edit` to review or remove patterns from your permanent allowlist.
:::

### Mining Approval History (`curie approvals suggest`)
...
```bash
curie approvals suggest            # dry run — prints a numbered proposal
curie approvals suggest --apply 1,3  # merge picks into command_allowlist
curie approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.curie/state.db`) for
...
Before `write_file` or `patch` touches disk, Curie checks the target path against a denylist and an optional sandbox. Blocked writes return an error to the agent immediately — **there is no approval prompt** and no way to override from the chat UI. The model may still claim the edit succeeded; when `display.file_mutation_verifier` is on (default), trust the [file-mutation verifier footer](./configuration.md#file-mutation-verifier) over the assistant's closing summary.

### Protected paths (always blocked)

These categories are always denied, even when `CURIE_WRITE_SAFE_ROOT` is unset:

| Category | Examples |
|----------|----------|
| OS credential stores | `~/.ssh/` (keys, `authorized_keys`), `~/.aws/`, `~/.kube/`, `/etc/sudoers`, `~/.netrc` |
| Curie credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under CURIE_HOME (active profile and global root) |
| Project secret files | `.env`, `.env.local`, `.env.production`, `.envrc` anywhere on disk |

Sensitive paths inside the safe root are still blocked — pointing `CURIE_WRITE_SAFE_ROOT` at `$HOME` does not allow writing `~/.ssh/id_rsa`.

Safe-root violations return `Write denied: '…' is outside CURIE_WRITE_SAFE_ROOT (…)`. Credential-path blocks use `Write denied: '…' is a protected system/credential file.`
...
### CURIE_WRITE_SAFE_ROOT (optional sandbox)
...
- Set automatically in the [official Docker image](https://github.com/thisismynewfmail-ui/Cur-Agnt) (`CURIE_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.curie/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.curie/cron/jobs.json`, profile skills, or other Curie state outside that prefix

To allow both a workspace and Curie home:

```bash
export CURIE_WRITE_SAFE_ROOT=/path/to/project:/home/you/.curie
```

Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [CURIE_WRITE_SAFE_ROOT](../reference/environment-variables.md#curie_write_safe_root).

### Cron and other Curie state

Do not ask the agent to `patch` `~/.curie/cron/jobs.json` directly. Use the `cronjob` tool, [`curie cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Curie control files when write safety blocks direct edits.
...
When running the messaging gateway, Curie controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.curie/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.curie/.env to allow open access,
...
For more flexible authorization, Curie includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `curie pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.curie/config.yaml`:
...
curie pairing approve telegram ABC12DEF

# Revoke a user's access
curie pairing revoke telegram 123456789

# Clear all pending codes
curie pairing clear-pending
```

:::tip Docker users: run pairing commands as the `curie` user
The official Docker image runs the gateway as the unprivileged `curie` user
...
```bash
docker exec -u curie curie-agent curie pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/thisismynewfmail-ui/Cur-Agnt/issues/10270
:::

**Storage:** Pairing data is stored in `~/.curie/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Curie applies strict security hardening to every container.
...
Container resources are configurable in `~/.curie/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.curie/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `CURIE_HOME`. Skills declare these in frontmatter:
...
When loaded, Curie checks if these files exist in the active profile's `CURIE_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Curie infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Curie infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Curie integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Curie under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.curie/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.curie/logs/` for unauthorized access attempts
10. **Keep updated** — run `curie update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.curie/.env`:
...
TERMINAL_SSH_USER=curie
TERMINAL_SSH_KEY=~/.ssh/curie_agent_key
```
...
Curie ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `curie_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `curie doctor` for the full remediation.
- **`curie doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Curie installs these **lazily** on first use rather than eagerly under `curie-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `curie tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `curie tools`.
