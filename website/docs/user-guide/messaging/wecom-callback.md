...
Connect Curie to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Curie supports two WeCom integration modes:
...
> Run `curie gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Curie decrypts the message, queues it for the agent
...
(Use `curie gateway start` only after `curie gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Curie and the token the
...
in `~/.curie/.env` values around `=` will also break signature checks. After
fixing, restart `curie gateway run`.
...
Check `curie gateway run` logs for the bound host/port. If the adapter bound to
