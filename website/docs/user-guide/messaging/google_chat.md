...
sidebar_position: 12
title: "Google Chat"
description: "Set up Curie Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Curie Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Curie
...
> Run `curie gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Curie can read (e.g.,
`~/.curie/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a curie restart)
...
- Principal: `curie-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Curie calls
...
- **App name**: whatever you want users to see ("Curie" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/curie-chat-events`.
...
`ADDED_TO_SPACE` event that Curie uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Curie

Add the Google Chat section to `~/.curie/.env`:
...
Send "hola" in the test DM. The bot posts a "Curie is thinking…" marker, then
...
`~/.curie/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Curie is thinking…").
...
Thread support: when a user replies inside a thread, Curie detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Curie session.
...
3. Download the JSON. Move it onto the host that runs Curie.
4. Register the client with Curie (run under the profile you want it scoped to):
...
curie -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Curie home (e.g.
`~/.curie/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.curie/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.curie/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Curie isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `curie gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Curie will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.curie/google_chat_user_tokens/<sanitized_email>.json` (filesystem
