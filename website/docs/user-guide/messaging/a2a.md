...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The Curie A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your Curie over HTTP.

It interoperates with any A2A-compliant peer — another Curie, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.

## When to use A2A

- **Curie ↔ Curie across machines** — let your desktop agent hand tasks to a Curie on a server, or vice versa, each with its own memory, tools, and credentials.
...
- **Being a callable service** — expose your Curie so other frameworks' agents can send it tasks.
...
```bash
curie tools enable a2a --platform cli        # CLI/TUI sessions
curie tools enable a2a --platform telegram   # or any messaging platform
curie tools enable a2a --platform a2a        # let inbound A2A tasks call peers (agent chaining)
```
...
With the platform enabled, Curie serves:
...
- **Audit log** — every exchange appends to `~/.curie/a2a_audit.jsonl`.
