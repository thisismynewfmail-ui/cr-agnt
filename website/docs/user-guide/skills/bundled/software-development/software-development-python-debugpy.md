...
- A long-running process (curie gateway, tui_gateway) misbehaves and you can't restart it
...
The curie test runner and pytest both support this:
...
For long-lived processes: Curie gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.

### Setup

```bash
source <curie-agent-repo>/.venv/bin/activate
pip install debugpy
```
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Curie you have two practical options:
...
```json
{
  "name": "Attach to Curie",
  "type": "debugpy",
  "request": "attach",
  "connect": { "host": "127.0.0.1", "port": 5678 },
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "<curie-agent-repo>" }
  ]
}
```
...
## Debugging Curie-specific Processes

### Tests
See Recipe 3. The wrapper captures subprocess output, so run pytest directly for interactive pdb.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `curie` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `curie --tui`)
...
debugpy.wait_for_client()
```
Start `curie --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Curie subagents, debug one process at a time.
