...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _curie_env="${CURIE_HOME:-$HOME/.curie}/.env"; [ -f "$_curie_env" ] && grep -q "^GITHUB_TOKEN=" "$_curie_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_curie_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${CURIE_HOME:-$HOME/.curie}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Curie Agent\",
    \"comments\": [
...
```bash
source "${CURIE_HOME:-$HOME/.curie}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Curie Agent. Code looks clean — good test coverage, no security concerns."
