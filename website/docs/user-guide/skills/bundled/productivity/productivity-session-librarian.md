...
| List/filter by metadata (age, source, cost, tokens, workspace) | `curie sessions list` / `stats` via terminal |
| Rename | `curie sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `curie sessions archive <filters>` |
| Delete (destructive) | `curie sessions delete` / `curie sessions prune <filters>` |
| Export before deleting anything valuable | `curie sessions export --session-id <id> --format md` |
...
`curie sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `curie sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `curie sessions list`
