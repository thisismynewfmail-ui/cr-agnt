...
| Version | `2.0.0` |
| Author | Francesco Bonacci (f-trycua), Curie Agent |
| License | MIT |
...
Curie drives [cua-driver](https://github.com/trycua/cua) under the hood.
This wrapper skill teaches the Curie `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. Curie autodetection is a
planned cua-driver follow-up, so currently point Curie at the resulting
...
|---|---|
| `cua-driver not installed` | Run `curie computer-use install`, or `curie tools` and enable Computer Use |
| Captures consistently return empty / "no on-screen window" | On Linux: DISPLAY may not be set (X11) or you're on pure Wayland — ask the user to run `curie computer-use doctor`. On Windows: you may be in Session 0 (SSH session) instead of the interactive desktop — see the cua-driver `WINDOWS.md` deep-dive |
...
| Type text disappears into a terminal emulator | cua-driver detects terminals (Ghostty, iTerm2, Terminal.app, Windows Terminal, mintty, etc.) and routes through key-event synthesis — should "just work" on a recent cua-driver. If it doesn't, ask the user to run `curie computer-use doctor` |
...
| Anything else weird | **First action: ask the user to run `curie computer-use doctor`.** It runs the cua-driver `health_report` MCP tool and prints a structured per-check matrix. Their output tells you (and them) exactly what's wrong |
...
Curie intentionally keeps THIS skill focused on the Curie-side
...
Curie autodetection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point Curie at that
