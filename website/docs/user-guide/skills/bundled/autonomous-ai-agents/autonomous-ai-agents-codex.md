...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`curie-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-curie-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Curie terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Curie itself, `model.provider: openai-codex` uses Curie-managed Codex
OAuth from `~/.curie/auth.json` after `curie auth add openai-codex`. For the
...
## Curie Gateway Caveat

When invoking the Codex CLI from a Curie gateway/service context (for example,
