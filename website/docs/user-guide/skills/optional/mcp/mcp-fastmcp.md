...
| | |
|---|---|
| Source | Optional — install with `curie skills install official/mcp/fastmcp` |
...
| Related skills | [`curie-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-curie-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Curie or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Curie. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.curie/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.curie/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Curie integration, either:

- configure the server in `~/.curie/config.yaml` using the `native-mcp` skill, or
...
### Curie cannot see the deployed server

The server-building part may be correct while the Curie config is not. Load the `native-mcp` skill and configure the server in `~/.curie/config.yaml`, then restart Curie.
