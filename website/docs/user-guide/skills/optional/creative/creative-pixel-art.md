...
| | |
|---|---|
| Source | Optional — install with `curie skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.curie/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.curie/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — Curie installs package this)
