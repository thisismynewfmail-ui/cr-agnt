...
| | |
|---|---|
| Source | Optional — install with `curie skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Curie MCP catalog. The
...
Two halves, in this order: the editor side must be up before Curie connects.
...
### One-time, Curie side
    ...
    curie mcp install unreal-engine
...
`~/.curie/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Curie — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Curie connects
...
2. Start the Curie session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Curie
they appear as:
...
then happens at `curie mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Curie runs batched calls
...
- **Start order matters.** Editor + server up first, then the Curie
...
- **Timeouts on long operations.** Curie' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.curie/config.yaml` for
