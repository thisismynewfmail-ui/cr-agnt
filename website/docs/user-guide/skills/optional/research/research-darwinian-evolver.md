...
| | |
|---|---|
| Source | Optional — install with `curie skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research\darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Curie Agent |
| License | MIT |
...
import upstream classes into Curie itself.
...
```bash
mkdir -p ~/.curie/cache/darwinian-evolver && cd ~/.curie/cache/darwinian-evolver
...
```bash
cd ~/.curie/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.curie/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.curie/skills/research/darwinian-evolver
DE_DIR=~/.curie/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Curie core.
   Custom driver scripts under `~/.curie/skills/...` are user-side and fine.
