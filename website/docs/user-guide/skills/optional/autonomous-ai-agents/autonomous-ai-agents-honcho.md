---
title: "Honcho — Configure and troubleshoot Honcho memory for Curie"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Curie"
---
...
Configure and troubleshoot Honcho memory for Curie.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `curie skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Curie

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Curie profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/curie#running-honcho-locally-with-curie

### Verify

```bash
curie honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Curie profile's AI peer
...
Honcho models conversations as interactions between **peers**. Curie creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Curie instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Curie run |
| `global` | Single session across all directories |

Manual override: `curie honcho map my-project-name`
...
Each Curie profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block curie.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `curie.coder` host block in `honcho.json`
...
```bash
curie honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Curie when Honcho memory is active.
...
Config file: `$CURIE_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `curie honcho setup`. Ensure `memory.provider: honcho` is in `~/.curie/config.yaml`.

### Memory not persisting across sessions
Check `curie honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `curie profile create <name> --clone`. For existing profiles: `curie honcho sync`.
...
| Command | Description |
|---------|-------------|
| `curie honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `curie honcho status` | Show resolved config, connection test, peer info for active profile |
| `curie honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `curie honcho disable` | Disable Honcho for the active profile |
| `curie honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `curie honcho peers` | Show peer identities across all profiles |
| `curie honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `curie honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `curie honcho sessions` | List known directory-to-session-name mappings |
| `curie honcho map <name>` | Map current working directory to a Honcho session name |
| `curie honcho identity` | Seed AI peer identity or show both peer representations |
| `curie honcho sync` | Create host blocks for all Curie profiles that don't have one yet |
| `curie honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Curie + Honcho |
| `curie memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `curie memory status` | Show active memory provider and config |
| `curie memory off` | Disable external memory provider |
