---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Curie"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Curie"
---
...
Import an OpenClaw setup (memories, skills) into Curie.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `curie skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | Curie Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Curie`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Curie Migration

Use this skill when a user wants to move their OpenClaw setup into Curie Agent with minimal manual cleanup.
...
```bash
curie claw migrate              # Full interactive migration
...
curie claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `curie setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_curie.py` to:

- import `SOUL.md` into the Curie home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Curie memory entries
- merge OpenClaw command approval patterns into Curie `command_allowlist`
- migrate Curie-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Curie working-directory configuration
- copy OpenClaw skills into `~/.curie/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Curie workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.curie/tts/`
- archive non-secret docs that do not have a direct Curie destination
...
- `scripts/openclaw_to_curie.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.curie/skills/migration/openclaw-migration/scripts/openclaw_to_curie.py`

Do not guess a shorter path like `~/.curie/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.curie/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Curie-compatible secrets, currently:
...
Curie CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Curie. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Curie workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Curie skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.curie/skills/migration/openclaw-migration/scripts/openclaw_to_curie.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.curie/skills/migration/openclaw-migration/scripts/openclaw_to_curie.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.curie/skills/migration/openclaw-migration/scripts/openclaw_to_curie.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.curie/skills/migration/openclaw-migration/scripts/openclaw_to_curie.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.curie/skills/migration/openclaw-migration/scripts/openclaw_to_curie.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.curie/skills/migration/openclaw-migration/scripts/openclaw_to_curie.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Curie unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Curie targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Curie destination. Unsupported auth blobs must still be reported as skipped.
...
- Curie persona state imported
- Curie memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.curie/skills/openclaw-imports/`
