...
| | |
|---|---|
| Source | Optional — install with `curie skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Curie Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Curie terminal calls (desktop app flow only)
...
### Service Account (recommended for Curie)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${CURIE_HOME:-~/.curie}/.env` (the skill will prompt for this on first load).
...
## Curie Execution Pattern (desktop app flow)

Curie terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/curie-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/curie-op.sock"
