...
| | |
|---|---|
| Source | Optional — install with `curie skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain\hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Curie Agent |
| License | MIT |
...
The script reads `${CURIE_HOME:-~/.curie}/.env` for two optional defaults:
...
Helper script: `~/.curie/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
```bash
python ~/.curie/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py <command> [args]
```
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${CURIE_HOME:-~/.curie}/.env`.
...
```bash
python ~/.curie/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py dexs

python ~/.curie/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py \
