...
| | |
|---|---|
| Source | Optional — install with `curie skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${CURIE_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${CURIE_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${CURIE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client curie
```
...
```bash
PUBLISH="${CURIE_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client curie
```
...
```bash
DRIVE="${CURIE_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `curie`)    |
