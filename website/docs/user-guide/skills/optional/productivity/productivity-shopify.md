...
| | |
|---|---|
| Source | Optional — install with `curie skills install official/productivity/shopify` |
...
4. Save to `${CURIE_HOME:-~/.curie}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Curie","productType":"Apparel","tags":["test"]}}'
```
