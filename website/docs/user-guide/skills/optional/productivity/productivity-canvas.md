...
| | |
|---|---|
| Source | Optional — install with `curie skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Curie Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${CURIE_HOME:-~/.curie}/.env`:
...
```bash
CANVAS="python $CURIE_HOME/skills/productivity/canvas/scripts/canvas_api.py"
