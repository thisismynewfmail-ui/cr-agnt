...
| | |
|---|---|
| Source | Optional — install with `curie skills install official/productivity/siyuan` |
...
3. Store it in `${CURIE_HOME:-~/.curie}/.env`:
   ```
...
```yaml
# In ~/.curie/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
