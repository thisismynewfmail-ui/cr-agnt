...
Curie Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.curie/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Curie stores session history so it can resume conversations, but it does not
...
Curie explicitly injects for that turn.
...
For example, if a user sends an image and asks Curie to make a meme from it,
Curie may inspect that image once with vision and run an image-processing
...
`curie sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `curie sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`curie` or `curie chat`) |
...
curie -c "my project"   # → resumes "my project #3"
```
...
curie --resume 20250305_091523_a1b2c3d4
curie -r 20250305_091523_a1b2c3d4

# Resume by title
curie --resume "refactoring auth"

# Resume the most recent session — same lookup as -c
curie --resume latest

# Or with the chat subcommand
curie chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `curie sessions list`.
...
curie --resume latest --in ./my-project

# Works with the TUI too
curie --tui --resume latest --in ./my-project
```
...
```bash
curie --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`curie sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
curie sessions list --workspace my-project
curie sessions list --workspace ~/code/curie-agent
```

### Conversation Recap on Resume

When you resume a session, Curie displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Curie session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.curie/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `curie -r "<title>"` from the shell) and pick up where the platform left off.
...
Curie automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `curie sessions list` or `curie sessions browse`.
...
```bash
curie sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Curie creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`curie -c "my project"`), it automatically picks the most recent session in the lineage.
...
Curie provides a full set of session management commands via `curie sessions`:
...
# Filter by platform
curie sessions list --source telegram

# Show more sessions
curie sessions list --limit 50
```
...
`curie sessions export` is one surface for every export format, selected with `--format`:
...
curie sessions export backup.jsonl

# Export sessions from a specific platform
curie sessions export telegram-history.jsonl --source telegram

# Export a single session
curie sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
curie sessions export backup.jsonl --redact
```
...
curie sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
curie sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
curie sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
curie sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `curie-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
curie sessions export --format trace

# One session to a local trace file
curie sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
curie sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.curie/session-exports`).

```bash
# Export one session to Markdown
curie sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
curie sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
curie sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
curie sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
curie sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
curie sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
curie sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
curie sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
curie sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
curie sessions pin 20250305_091523_a1b2c3d4
curie sessions pin 20250305 20250306

# Remove the pin
curie sessions unpin 20250305_091523_a1b2c3d4

# List pinned sessions
curie sessions pinned

# Machine-readable output, e.g. for a nightly backup of your pin set
curie sessions pinned --json > pinned-sessions.json
```
...
curie sessions prune

# Custom age threshold — bare numbers are days
curie sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
curie sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
curie sessions prune --newer-than 5h

# Explicit window with absolute timestamps
curie sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
curie sessions prune --source telegram
curie sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
curie sessions prune --newer-than 5h --title "smoke test"   # title substring
curie sessions prune --older-than 30 --max-messages 3        # tiny sessions
curie sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
curie sessions prune --model gpt-5 --older-than 1w           # by model (substring)
curie sessions prune --provider openrouter --older-than 60   # by billing provider
curie sessions prune --branch feature/old-experiment         # by git branch
curie sessions prune --user 12345678 --chat-type group       # by messaging origin
curie sessions prune --max-tokens 500 --older-than 7         # by token usage
curie sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
curie sessions prune --newer-than 5h --dry-run

# Skip confirmation
curie sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `curie sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `curie sessions prune` keeps the 90-day cutoff. Every
...
`curie sessions archive` takes the same filters as `prune` but soft-hides
...
curie sessions archive --newer-than 5h

# Archive by title substring, preview first
curie sessions archive --title "dry run" --dry-run
curie sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `curie sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`curie sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
curie sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`curie insights`](/reference/cli-commands#curie-insights).
...
`curie sessions repair-routing` finds message-bearing session rows with no
...
curie sessions repair-routing --apply

# Widen/narrow the contiguity window (default 900 seconds)
curie sessions repair-routing --max-gap-seconds 300
```
...
(`cp ~/.curie/state.db ~/.curie/state.db.bak`).


## Importing Sessions from Claude Code and Codex CLI

Started a conversation in another agent CLI? You can pull it into Curie and
continue it here. Curie reads Claude Code's session logs
...
curie sessions import

# Limit to one tool, or point at a specific file
curie sessions import --from claude
curie sessions import --from codex ~/.codex/sessions/2026/08/15/rollout-....jsonl
...
`curie sessions import` creates a new Curie session titled
`Imported from Claude Code: <first user message>` (or Codex CLI) and prints
the id plus a ready-to-paste `curie --resume <id>` command.
...
When Curie cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Curie uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Curie in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.curie/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.curie/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.curie/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.curie/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.curie/sessions/sessions.json` is a **legacy mirror** of it,
...
`curie sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.curie/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `curie sessions list`, the cause is
`state.db` not receiving them — run `curie sessions repair` and watch for a
...
`*.jsonl` files in `~/.curie/sessions/`. They are no longer written or
read by Curie. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Curie process in the same `CURIE_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.curie/config.yaml`:
...
`in its tip segment`). `curie sessions export` still works for such sessions.
...
curie sessions delete <session_id>
