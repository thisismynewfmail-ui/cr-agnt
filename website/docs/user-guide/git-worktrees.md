...
title: "Git Worktrees"
description: "Run multiple Curie agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Curie Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Curie so each session has a clean, isolated working directory.

## Why Use Worktrees with Curie?

Curie treats the **current working directory** as the project root:

- CLI: the directory where you run `curie` or `curie chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.curie/config.yaml`
...
Curie creates `.worktrees/my-experiment/` inside the repo (branch
`curie/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `curie-<id>`
...
git worktree add ../repo-feature feature/curie-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/curie-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Curie there:

```bash
cd ../repo-feature

# Start Curie in the worktree
curie
```
...
git worktree add ../repo-experiment-a feature/curie-a
git worktree add ../repo-experiment-b feature/curie-b
```
...
- Works on its own branch (`feature/curie-a` vs `feature/curie-b`).
...
- Curie checkpoint data under `~/.curie/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Curie experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/curie-checkpoints-docs`, `feature/curie-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Curie from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `curie -w` (Automatic Worktree Mode)

Curie has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `curie/curie-<hash>`).
...
```bash
curie -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `curie -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Curie session its own clean checkout.
