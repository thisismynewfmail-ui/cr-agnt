...
sidebar_position: 3
title: "Curie Desktop"
description: "The native Curie desktop app — a polished experience for chatting with Curie, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Curie Desktop

The Curie desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Curie Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `curie` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Curie has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`curie`) and **[TUI](./tui.md)** (`curie --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`curie dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Download the app from the [Curie Desktop product page](https://github.com/thisismynewfmail-ui/Cur-Agnt/desktop), or follow the [installation instructions for Curie Desktop](../getting-started/installation.md).

If you already have Curie installed, simply run
...
- **The same conversation history** as every other Curie surface — sessions started here resume in the CLI/TUI and vice versa.
...
- **Per-session UNLOCK toggle** — flip UNLOCK on or off for just this session (matching the TUI). UNLOCK bypasses the dangerous-command approval prompts, so know what you're turning off — see [Security → UNLOCK Mode](./security.md#unlock-mode).
...
Chatting against a Curie instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting Curie Desktop to a remote backend](./features/web-dashboard.md#connecting-curie-desktop-to-a-remote-backend).

#### Repository discovery

Curie Desktop discovers local Git repositories for the Projects sidebar by scanning your home directory to a bounded depth. You can change this per profile in **Settings → Workspace**, or in `config.yaml`:
...
- Set `repo_scan_enabled: false` to stop the filesystem scan completely. Existing disk-discovery cache rows for that profile are cleared; explicit projects and repositories inferred from intentional Curie sessions remain available.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `curie desktop --cwd <path>` (or the `CURIE_DESKTOP_CWD` environment variable).
...
- **Review pane** — **Cmd/Ctrl+G** toggles the working-tree review pane: branch and ahead/behind status, changed files (list or tree view), and diffs scoped to **Uncommitted**, **Branch**, or **Last turn** (just what the agent changed in its most recent turn). Stage/unstage files, revert changes, write a commit message (or **Generate commit message**), then **Commit** or **Commit & Push** — and **Create PR** via the GitHub CLI (`gh`), or hand the whole thing to the agent with **Ask Curie to open PR**. You can also create and switch branches from here.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `curie model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Curie management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md). The Skills tab lists your installed skills with enable/disable toggles, and below them the full built-in optional-skills catalog that ships with Curie — each entry has a one-click **Install** button that flips the row into the installed list once it finishes.
...
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#curie-cron).
- **Profiles** — switch between [Curie profiles](./profiles.md) (isolated config/skills/sessions).
...
roster where every [Curie profile](./profiles.md) appears as a bot with its
...
(recurring tasks backed by Curie cron). The roster lives in the left
...
each other's Bot Chats directly (`curie -p <bot> chat`). The backend teaches
...
- **Command palette** — press **Cmd+K** or **Cmd+P** (Ctrl+K / Ctrl+P on Windows/Linux) to jump to actions and navigate the app from the keyboard: open any page or settings section, jump to a session by title or id, switch model/theme/color mode, spawn a terminal, restart the gateway, update Curie, and more.
...
The [manual update process](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/getting-started/updating.md) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Curie agent, your config, and your chats stay. (Same as `curie uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `curie uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `curie uninstall --full`.)
...
You can do the same from the terminal — `curie uninstall --gui` for the GUI alone, or `curie uninstall` / `curie uninstall --full` for the agent too.

:::note
Running `curie uninstall --gui` from a **source checkout** (a `curie desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `curie desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `curie desktop`

To launch via the CLI, simply run `curie desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `curie update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `CURIE_DESKTOP_CWD`)           |
| `--curie-root PATH` | Override the Curie source root the app uses (sets `CURIE_DESKTOP_CURIE_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `curie` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Curie Agent runtime into `CURIE_HOME` (`~/.curie`, or `%LOCALAPPDATA%\curie` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `CURIE_DESKTOP_CURIE_ROOT`, then a completed managed install, then a probed `curie` on `PATH` (unless `--ignore-existing` / `CURIE_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `CURIE_DESKTOP_CURIE` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `curie serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `curie --tui`. The desktop app is **self-contained**: it runs its own `curie serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Curie backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.
...
- **Remote gateway** — enter the URL of a `curie serve` backend you run yourself and sign in. This is the mode the rest of this section walks through.
- **Curie Cloud** — sign in once to Curie Cloud and pick from the agents on your account; no URL to paste. The app discovers your agents (with an organization picker if your account spans several orgs), and connecting to one switches the session over automatically. The status bar shows the cloud connection while it's active.
...
Further down the same **Settings → Gateways** page, **Registered gateways** manages a named list of every Curie gateway the app knows about — the local runtime, any number of remote gateways (LAN, Tailscale, internet), Curie Cloud instances, and SSH hosts — all persisted together in one place. You can jump there from the plug button at the right end of the sidebar profile rail (**Connect another Curie gateway…**) or via **⌘K → Gateways**. The full guide, including the union agent roster, `@name-device` handles, fleet-wide updates, and the plugin SDK surface, is at [Connecting Desktop to Many Curie Instances](./multi-connection-desktop.md).
...
- Cloud entries come from the Curie Cloud sign-in/discovery flow above, not from a hand-typed URL.
- Tokens are stored encrypted with the OS keyring (with an explicit plain-text opt-in on keyring-less Linux).

Side-by-side routing is live: each registered gateway dials its own backends and sockets on demand (keyed per connection + profile), the plugin SDK exposes the union agent roster (`host.agents()` / `host.ensureAgent()`), and **Update all instances** on the Gateways page dispatches `curie update` to every eligible gateway at once — Curie Cloud entries are skipped (the platform updates them), and each instance reports its own result.


:::info The remote backend is a running `curie serve` process
"Remote backend" means a **`curie serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `curie serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.curie/.env` (the secrets file, mode 0600):
...
curie serve --host 0.0.0.0 --port 9119
```

Keep that `curie serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `curie serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `CURIE_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.curie/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/curie` work if you front it with a reverse proxy)
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `CURIE_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `CURIE_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateways settings panel.
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.curie/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `CURIE_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Curie Desktop to a remote backend](./features/web-dashboard.md#connecting-curie-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Curie Desktop](../reference/environment-variables.md#web-dashboard--curie-desktop).
...
`$CURIE_HOME/desktop-plugins/<id>/plugin.js`; the app loads it within seconds and
...
- **Open logs** — opens `CURIE_HOME/logs` in your file manager. On a remote
...
- **Send diagnostics** — uploads a redacted debug bundle to Nous-internal
  ...
  `curie debug share --nous`; secrets are always redacted, the bundle is
...
Boot logs land in `CURIE_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
rm "$HOME/.curie/curie-agent/.curie-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.curie/curie-agent/venv"

# Reset a stuck macOS microphone prompt
tccutil reset Microphone com.nousresearch.curie
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.curie/curie-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
```bash
CURIE_DESKTOP_CURIE_ROOT=/path/to/clone npm run dev
CURIE_HOME=/tmp/throwaway npm run dev
...
(Desktop, then Downloads, then Documents, ...) as Curie touches each folder.
A single **Full Disk Access** grant covers all of them, permanently — and
with Curie' stable signing identities it survives every update:

1. System Settings → **Privacy & Security → Full Disk Access** (or run
   `open "x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles"`)
2. Enable your terminal app — and **Curie.app** if you use Desktop.
3. Fully quit and relaunch them once.

`curie doctor` reports whether the current terminal context already has the
grant, and `curie setup` shows this tip on macOS when it doesn't.
...
```bash
tccutil reset ScreenCapture com.nousresearch.curie   # repeat per service
```

then toggle the fresh entry ON in System Settings and fully quit & relaunch
Curie. Grants are stable from then on.
...
certificate once and tell Curie to use it. The one-shot command does
...
```bash
curie desktop --setup-tcc-identity
```

Or do it manually:

1. Keychain Access → Certificate Assistant → **Create a Certificate…**
2. Name: `Curie Local Signing`, Identity Type: *Self-Signed Root*,
...
4. `curie config set desktop.macos_signing_identity "Curie Local Signing"`

Use `--identity <name>` with the command to create/use a differently named
certificate (default: `Curie Local Signing`). The command is idempotent —
...
`tccutil reset All com.nousresearch.curie` and re-grant.

## See also

- [CLI Guide](./cli.md) — the terminal interface
- [TUI](./tui.md) — the modern terminal UI used by `curie --tui` and the dashboard chat tab
