...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Curie Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Curie Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `curie setup --portal` — and you're ready to `curie chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Curie also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `curie --tui` — see the [TUI](tui.md) guide.
:::
...
curie chat --query-file prompt.txt
curie chat --query-file - < prompt.txt

# With a specific model
curie chat --model "anthropic/claude-sonnet-4"

# With a specific provider
curie chat --provider nous        # Use Nous Portal
curie chat --provider openrouter  # Force OpenRouter

# With specific toolsets
curie chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
curie -s curie-agent-dev,github-auth
curie chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
curie --continue             # Resume the most recent CLI session (-c)
curie --resume <session_id>  # Resume a specific session by ID (-r)
curie --resume latest        # Resume the most recent session (same as -c)
curie --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
curie chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
curie -w                         # Interactive mode in worktree
curie -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`curie -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
curie worktree list              # audit: age, size, verdict, reason per tree
curie worktree prune             # remove safe trees + delete merged branches
curie worktree prune --dry-run   # show the plan without changing anything
curie worktree prune --trees-only     # leave local branches alone
curie worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running curie session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.curie/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `curie -w` for days — no longer accumulate merged scratch trees
...
The `curie plugins` commands manage native Curie plugins and portable Agent
...
curie plugins disable <plugin-name>
curie plugins update <plugin-name>
curie plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Curie currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Curie CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Curie CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ UNLOCK | **UNLOCK mode warning** — shown whenever `CURIE_UNLOCK_MODE` is on (either `curie --unlock` at launch or `/unlock` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |

The bar adapts to terminal width — full layout at ≥ 76 columns, compact at 52–75, minimal (model + duration, plus the UNLOCK badge when active) below 52.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Curie to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.curie/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Curie supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Curie replies |
...
```bash
curie -s curie-agent-dev,github-auth
curie chat -s github-pr-workflow -s github-auth
```

Curie loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.curie/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
The agent's voice comes from `SOUL.md`; see [SOUL.md — the agent's voice](features/soul.md).

You can also define custom personalities in `~/.curie/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Curie..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Curie also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Curie recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Curie — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Curie is working, Curie prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Curie to the background — just like any terminal process. The shell prints a confirmation:

```
Curie Agent has been suspended. Run `fg` to bring Curie Agent back.
```
...
Resume this session with:
  curie --resume 20260225_143052_a1b2c3
...
```bash
curie --continue                          # Resume the most recent CLI session
curie -c                                  # Short form
curie -c "my project"                     # Resume a named session (latest in lineage)
curie --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
curie --resume "refactoring auth"         # Resume by title
curie --resume latest                     # Resume the most recent session (same as -c)
curie --resume latest --in ./my-project   # Latest session for ./my-project's workspace
curie -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `curie sessions rename <id> <title>` from the command line. Use `curie sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Curie's SQLite state database under `~/.curie/state.db`. The database keeps:
...
Curie immediately confirms the task and gives you back the prompt:
