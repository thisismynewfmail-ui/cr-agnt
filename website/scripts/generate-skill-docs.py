...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    curie_meta = (fm.get("metadata") or {}).get("curie") or {}
    tags = curie_meta.get("tags") or []
    related = curie_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `curie skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Curie Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Curie ships with a large built-in skill library copied into `~/.curie/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Curie also syncs bundled skills on `curie update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.curie/skills/` tree, it is still shipped with Curie; restore it with `curie skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with curie-agent — install via curie skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with curie-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "curie skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "curie skills install official/blockchain/solana",
        "curie skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "curie skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
