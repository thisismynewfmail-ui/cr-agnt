#!/usr/bin/env python3
"""Google Workspace API CLI for Curie Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Curie-facing JSON contract and falls back to the Python client
...
from _curie_home import get_curie_home
...
def _gws_binary() -> str | None:
    override = os.getenv("CURIE_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Curie Agent")
