...
Ledger path resolution (first wins):
  --ledger PATH
  $CURIE_CITATION_LEDGER
  $CURIE_HOME/cache/citations/ledger.json
"""
...
from _curie_home import get_curie_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("CURIE_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_curie_home() / "cache" / "citations" / "ledger.json"
