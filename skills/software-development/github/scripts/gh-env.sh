#!/usr/bin/env bash
# GitHub environment detection helper for Curie Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _curie_env="${CURIE_HOME:-$HOME/.curie}/.env"; [ -f "$_curie_env" ] && grep -q "^GITHUB_TOKEN=" "$_curie_env" 2>/dev/null; then
