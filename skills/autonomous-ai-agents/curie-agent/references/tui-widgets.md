...
Author widget apps for the Curie TUI (`curie --tui`): glanceable ambient
...
- The TUI must be in use (`curie --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.curie/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.curie/logs/tui_gateway_crash.log` if a widget never appears.
