...
Set via `curie model` (picker) or `curie setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/integrations/providers.md
...
| nous | OAuth device code | `curie auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `curie auth add openai-codex` |
| qwen-oauth | OAuth | `curie auth add qwen-oauth` |
| minimax-oauth | OAuth | `curie auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`curie auth`).
Fallback chain when the primary fails: `curie fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`curie_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   curie config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
