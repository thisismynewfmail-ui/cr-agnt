...
**three** code paths in Curie that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Curie — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Curie
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Curie is
   refusing my value."
...
curie config get delegation.max_concurrent_children

# 2. Did Curie' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.curie/logs/agent.log | tail
# If neither line appears, neither cap path executed.

# 3. Confirm the resolver returns what config says (in venv with curie on path)
...
appears, **the cap is the model**, not Curie.
...
- **`subagent_auto_approve: false` does not cap concurrency.** It only
  controls whether children inherit unlock / approval bypass. Don't mistake
  it for a throttle.
