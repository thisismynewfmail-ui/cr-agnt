...
Edit with `curie config edit` or `curie config set section.key value`.
Full reference: https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/configuration.md
...
`curie config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `curie tools` (interactive) or `curie tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_CURIE_CORE_TOOLS` is the default bundle most platforms inherit).
