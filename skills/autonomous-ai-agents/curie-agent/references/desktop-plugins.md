...
Write plugins for the Curie desktop app: statusbar items, layout panes,
...
system (`~/.curie/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$CURIE_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$CURIE_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
...
- The Curie desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$CURIE_HOME/desktop-plugins/` (usually
  `~/.curie/desktop-plugins/`).

## How to Run

1. Create `$CURIE_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.curie/...` by default, or `~/.curie/profiles/<profile>/...` under a
...
The ONLY import surface is `@curie/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Curie
  ...
  (`curie://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.curie/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@curie/plugin-sdk`, `react`, and
