...
Curie runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`curie config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Curie's env scrubber dropping
...
Curie-installed `venv/Scripts/` has no pip/pytest (stripped for size).
...
every Curie tool and most Windows APIs. Prefer forward slashes in code
