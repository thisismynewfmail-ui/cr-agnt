...
## Layer 1 — Is this thing a Curie plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Curie process**. Its LLM calls go through Curie's already-configured provider. | Already uses Portal if user's Curie is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Curie. That LLM call is already authenticated
via Curie's provider config. The plugin's own server auth (e.g.
...
What Curie Agent has that *feels* like OAuth — `curie login --provider nous`
opening a browser, user signs in, token lands in `~/.curie/auth.json` — is a
**Curie-specific browser flow**. Under the hood it produces a credential
Curie uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Curie's existing Portal credential out of `~/.curie/auth.json`.
...
rides on the credential Curie already holds.

Where this could live in Curie:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Curie itself.
...
1. First decide: Curie plugin (runs inside Curie) or separate app? If plugin,
   it already uses Portal via Curie's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Curie agent itself") is fully supported. Curie treats agent
...
pitch in the Tool Gateway announcement is specifically about Curie Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Curie is configured to use Portal as its provider. It is
