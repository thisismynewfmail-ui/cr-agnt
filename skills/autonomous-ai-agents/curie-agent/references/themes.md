# Themes / Skins — Author a Curie Color Theme

Author a Curie **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`curie_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.curie/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Curie itself.
...
- Write access to the Curie home dir — `~/.curie` by default, or `$CURIE_HOME`
  / the active profile's dir. Skins live in `<curie-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `curie config set`).
...
3. `write_file` it to `<curie-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `curie config set display.skin default` (or they can `/skin default`).
...
```
curie skin set <key> <hex>      # e.g. curie skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `curie skin set background "#08201f"` changes
only the background; `curie skin use <name>` / `curie skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.curie`** when a profile is active — resolve the real home
  from `$CURIE_HOME` first, falling back to `~/.curie`.
...
- **Never hand-edit `config.yaml` to activate.** Use `curie config set
...
- **You apply it, not the user.** `curie config set display.skin <name>` is
...
- `read_file` the written `<curie-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `curie config get display.skin` and confirm it reports `<name>`.
