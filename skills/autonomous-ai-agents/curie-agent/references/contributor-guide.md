...
For occasional contributors and PR authors. Full developer docs: https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs/developer-guide
...
Config: `~/.curie/config.yaml` (settings), `~/.curie/.env` (API keys) — both under `$CURIE_HOME` when it is set.
...
`_CURIE_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_curie_home()` for paths,
never hardcode `~/.curie`. For custom/local-only tools, write a plugin in
`~/.curie/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `curie_cli/commands.py`
...
- Tests auto-redirect `CURIE_HOME` to temp dirs — never touch real `~/.curie/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/curie_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_curie_home()` from `curie_constants` for all paths (profile-safe)
