...
Extracted verbatim from curie_state.py so the SessionDB mixin modules
(curie_state_search / curie_state_schema / curie_state_portability) can
reference them without importing curie_state (which would be a cycle).
curie_state re-imports every name here for backward compatibility.
"""
...
# optimized via ``curie sessions optimize-storage``. A legacy DB sits at
...
--
-- A host-declared conversation key (X-Curie-Session-Key / build_session_key)
...
# `curie sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `curie db optimize`. This is
...
# service, the Desktop app's `curie serve` backend, interactive CLI sessions,
...
#
# Semantics mirror `curie_state._cross_process_repair_lock` (the schema-
...
# curie_state) because the search/schema mixins cannot import curie_state
# (cycle).
#
...
def _describe_lock_holder(record) -> str:
    ...
    if not isinstance(record, dict) or "pid" not in record:
        return "unknown (no holder record; pre-fix writer or non-Curie)"
    pid = record.get("pid")
