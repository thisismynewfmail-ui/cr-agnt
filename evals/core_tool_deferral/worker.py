...
# --- hermetic env BEFORE any curie import -------------------------------
for var in list(os.environ):
    if var.endswith(("_API_KEY", "_TOKEN")) and var != "OPENROUTER_API_KEY":
        os.environ.pop(var, None)
os.environ.pop("FAL_KEY", None)
os.environ.pop("CURIE_PROFILE", None)

tmp_root = tempfile.mkdtemp(prefix=f"ab-{ARM}-{TASK_ID}-")
curie_home = os.path.join(tmp_root, ".curie")
workspace = os.path.join(tmp_root, "ws")
os.makedirs(curie_home)
os.makedirs(workspace)
with open(os.path.join(curie_home, "config.yaml"), "w", encoding="utf-8") as f:
    f.write("model:\n  provider: openrouter\n  model: %s\n" % MODEL)
...
PREAMBLE = ("You are running inside the Curie desktop app on the user's machine. "
...
ctx = {
    "workspace": workspace, "curie_home": curie_home,
