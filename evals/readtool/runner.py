"""Run the read-tool eval through the REAL Curie AIAgent.

For each task: fresh temp CURIE_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    curie_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".curie"
    ...
    try:
        ...
        shutil.rmtree(curie_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.curie/.env; set +a  — then relaunch."
        )
