...
# 5. Symlinks the 'curie' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}▮ Curie Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/curie-uv-install.$$.log")"
...
if is_termux; then
    # Curie currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Curie requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink curie into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up curie command..."

CURIE_BIN="$SCRIPT_DIR/venv/bin/curie"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$CURIE_BIN" "$COMMAND_LINK_DIR/curie"
echo -e "${GREEN}✓${NC} Symlinked curie → $COMMAND_LINK_DISPLAY_DIR/curie"
...
# ============================================================================
# Seed bundled skills into ~/.curie/skills/
# ============================================================================

CURIE_SKILLS_DIR="${CURIE_HOME:-$HOME/.curie}/skills"
mkdir -p "$CURIE_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.curie/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$CURIE_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  curie status        # Check configuration"
if is_termux; then
    echo "  curie gateway       # Run gateway in foreground"
else
    echo "  curie gateway install # Install gateway service (messaging + cron)"
fi
echo "  curie cron list     # View scheduled jobs"
echo "  curie doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m curie_cli.main setup
fi
