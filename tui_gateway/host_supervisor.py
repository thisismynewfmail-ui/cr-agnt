...
def _default_registry_path() -> Path:
    return get_curie_home() / "state" / _REGISTRY_NAME
...
class HostSupervisor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_curie_home: str | None = None,
        autostart: bool = True,
    ) -> None:
        ...
        self.expected_curie_home = expected_curie_home if expected_curie_home is not None else str(get_curie_home())
    ...
    def _spawn_locked(self, *, reason: str) -> None:
        ...
        if self.env:
            env.update(self.env)
        env["CURIE_COMPUTE_HOST_HEARTBEAT_SECS"] = str(self.heartbeat_secs)
    ...
    def _validate_hello(self) -> None:
        ...
        got_home = str(hello.get("curie_home") or "")
        if got_home and got_home != self.expected_curie_home:
            raise RuntimeError(f"compute host CURIE_HOME mismatch: {got_home} != {self.expected_curie_home}")
