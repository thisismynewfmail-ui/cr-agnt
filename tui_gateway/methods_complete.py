...
def _(rid, params: dict) -> dict:
    ...
    def _profile_mention_items(prefix: str) -> list[dict]:
        ...
        under the 'curie' alias when no real profile claims that name.
        """
        out: list[dict] = []
        try:
            ...
            if "curie".startswith(prefix.lower()) and "curie" not in seen:
                out.append(
                    {
                        "text": "@curie",
                        "display": "@curie",
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from curie_cli.commands import SlashCommandCompleter
        ...
        # An argument stage (`/model `, `/details c`) keeps the order
...
def _(rid, params: dict) -> dict:
    try:
        from curie_cli.inventory import build_model_options_payload
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from curie_cli.inventory import build_models_payload
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `curie model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.curie/.env via the unified credential lifecycle
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from curie_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
