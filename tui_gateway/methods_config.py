    ...
    try:
        with _profile_db(params) as db:
            ...
            with pdb.connect_closing() as conn:
                ...
                # zero Curie sessions still surface. The desktop's native scan
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from curie_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_curie_home), "display": display_curie_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _curie_home / "config.yaml"
        try:
        ...
        if (
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Curie provider is configured.",
                },
            )
...
def _(rid, params: dict) -> dict:
    ...
    collection + force-redaction pipeline as ``curie debug share --nous``
