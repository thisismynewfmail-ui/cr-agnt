...
# changing PATH, installing software, or touching the user's Curie home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("curie-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
$CurieHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
. $installScript -CurieHome $CurieHome -InstallDir $InstallDir
