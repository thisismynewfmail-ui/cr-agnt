...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[curie] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[curie`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[curie`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        CurieHome = $(if ($paths) { $paths.curie_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\curie\curie-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-curie'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-curie') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
