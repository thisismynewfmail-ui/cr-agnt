...
# slug; every URL below is derived from it.
#   CURIE_REPO=me/my-fork curl -fsSL .../install.sh | bash
#
# ============================================================================
...
# Python-driven tool session (e.g. Curie terminal tool). A pre-set PYTHONPATH
...
#
# Git does not read a credential prompt from stdin -- it opens /dev/tty
# directly -- so `curl ... | bash` does NOT protect the installer from one.
# Any remote git cannot reach anonymously (a repository that was renamed,
# deleted, or made private) produces "Username for 'https://github.com':" and
# the install hangs there until the user types something, with no indication
# of which remote is being asked about or why.
#
# Refusing the prompt turns that hang into an immediate, readable failure.
# Stored credentials are unaffected: credential helpers still run, and this
# only removes the interactive fallback -- which under a piped installer was
# never usable to begin with.
export GIT_TERMINAL_PROMPT=0
# The GUI equivalents, for the same reason: a graphical password box behind
# the terminal is a hang the user cannot even see.
export GIT_ASKPASS=""
export SSH_ASKPASS=""
...
# clone URLs, the usage banner and the Windows hint is how they drift apart:
...
# Names this repository has been published under before, space separated. An
# existing install still has one of these in its `origin`, and the installer
...
if [ -n "${CURIE_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$CURIE_INSTALL_DIR"
...
#   code at /usr/local/lib/curie-agent, command at /usr/local/bin/curie,
#   data still at /root/.curie (CURIE_HOME).  Matches Claude Code / Codex CLI
...
# Local neural text-to-speech (piper-tts). Opt-in rather than default: the
# wheel is small but it pulls onnxruntime, which is ~60MB on Linux and has no
# wheel at all on some platforms, so an unconditional install would make every
# fresh install slower and some of them fail.
WITH_TTS=false
NO_SKILLS=false
BRANCH="main"
INSTALL_COMMIT=""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --with-tts     Install piper-tts for local, offline speech"
            echo "                   (no API key, no network at speech time; adds"
            echo "                   ~60MB for onnxruntime). Also available later"
            echo "                   via: curie tools -> Text to Speech -> Piper"
            echo "  --no-skills    Start with a blank slate — seed no bundled skills, and"
            echo "                   write \$CURIE_HOME/.no-bundled-skills so future"
            echo "                   'curie update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Curie.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.curie/curie-agent"
            echo "                   default (root, Linux): /usr/local/lib/curie-agent"
            echo "  --curie-home PATH  Data directory (default: ~/.curie, or \$CURIE_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Curie installs the code under"
            echo "  /usr/local/lib/curie-agent and links the command into"
            echo "  /usr/local/bin/curie (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$CURIE_HOME"
            echo "  (default /root/.curie).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$CURIE_HOME/curie-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg,"
            echo "                              piper (local TTS), ui (bench console)"
...
print_banner() {
    ...
    echo "│  ▮ Curie Agent Installer                                │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  A bench terminal for an autonomous agent.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `curie update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Curie-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Curie Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install curie command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `curie` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $CURIE_HOME/curie-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $CURIE_HOME/curie-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/curie-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $CURIE_HOME/curie-agent — then preserve it)
#
# Always no-op when the user set --dir or $CURIE_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in CURIE_HOME.
    if is_termux; then
        INSTALL_DIR="$CURIE_HOME/curie-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$CURIE_HOME/curie-agent/.git" ]; then
            INSTALL_DIR="$CURIE_HOME/curie-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/curie-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/curie-agent"
        ...
        # /usr/local/bin/curie wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/curie"
        log_info "  Data:    $CURIE_HOME (unchanged)"
...
# Point a Curie-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $CURIE_HOME/node/bin — which is NOT on
...
# Curie's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Curie-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$CURIE_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$CURIE_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$CURIE_HOME/node/etc/npmrc"
}

get_curie_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/curie" ]; then
        echo "$link_dir/curie"
    else
        echo "curie"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm ${REPO_RAW_BASE}/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Curie owns its own uv at $CURIE_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (curie_cli/managed_uv.py) looks in the same
    # place, so install.sh and `curie update` stay in sync.
    local _managed_uv="$CURIE_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $CURIE_HOME/bin ..."
    mkdir -p "$CURIE_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/curie-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/curie-uv-installer.$$.sh")"
    ...
    # directly into $CURIE_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$CURIE_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_python() {
    if [ "$DISTRO" = "termux" ]; then
        ...
        # Curie currently declares requires-python >=3.11,<3.14.  Termux can
        ...
        log_error "Termux Python $PYTHON_FOUND_VERSION is not supported; Curie requires Python >=3.11,<3.14"
...
# replaced with the Curie-managed Node $NODE_VERSION.
...
check_node() {
    ...
    # Repair pre-existing Curie-managed installs where `npm install -g` lands
    ...
    if command -v node &> /dev/null && command -v npm &> /dev/null \
        ...
        log_warn "min-release-age-exclude) — installing Curie-managed Node $NODE_VERSION instead..."
        install_node
        return
    fi

    # Prefer a Curie-managed Node from a previous run over a too-old system one.
    if [ -x "$CURIE_HOME/node/bin/node" ] && [ -x "$CURIE_HOME/node/bin/npm" ] \
        && node_satisfies_build "$("$CURIE_HOME/node/bin/node" --version)"; then
        export PATH="$CURIE_HOME/node/bin:$PATH"
        log_success "Node.js $("$CURIE_HOME/node/bin/node" --version) found (Curie-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null && ! command -v npm &> /dev/null; then
        log_warn "node found but npm is not on PATH (stray node symlink?) — installing Curie-managed Node $NODE_VERSION LTS..."
    elif command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is unsupported (Curie requires Node 22.22+, 24.11+, or 26+) — installing Curie-managed Node $NODE_VERSION..."
...
# Download and adopt one Node release line (e.g. 26) into ~/.curie/node/.
#
...
install_node_line() {
    ...
    log_info "Extracting to ~/.curie/node/..."
    ...
    # Place into ~/.curie/node/ and symlink binaries into the same bin dir
    # the curie command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$CURIE_HOME/node"
    mkdir -p "$CURIE_HOME"
    mv "$extracted_dir" "$CURIE_HOME/node"
    rm -rf "$tmp_dir"
    ...
    ln -sf "$CURIE_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$CURIE_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$CURIE_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    if ! installed_ver=$("$CURIE_HOME/node/bin/node" --version 2>&1); then
    ...
    log_success "Node.js $installed_ver installed to ~/.curie/node/"
    HAS_NODE=true
    return 0
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Curie install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Curie Agent itself does not require or retain root access."
...
# Reduce a GitHub remote URL to its "owner/name", lowercased. Prints nothing
# for a non-GitHub remote, which is the signal to leave it alone.
remote_slug() {
    local url="${1:-}"
    url="${url%/}"
    url="${url%.git}"
...
# repository the caller has not authenticated to. The install stops dead at a
# username prompt naming a repository the user has never heard of.
#
# A remote that is neither the current name nor a former one is somebody's
# fork or mirror and is deliberate: left untouched.
retarget_stale_origin() {
    local dir="$1"
    local origin_url current former
    origin_url="$(git -C "$dir" remote get-url origin 2>/dev/null)" || return 0
    [ -n "$origin_url" ] || return 0

    current="$(remote_slug "$origin_url")"
    [ -n "$current" ] || return 0
    # Already pointing at the repository this installer installs.
    [ "$current" = "$(printf '%s' "$REPO_SLUG" | tr '[:upper:]' '[:lower:]')" ] && return 0

    for former in $REPO_SLUG_FORMER; do
        former="$(printf '%s' "$former" | tr '[:upper:]' '[:lower:]')"
        if [ "$current" = "$former" ]; then
            log_warn "This install points at $current, which has been renamed."
            log_info "Updating its git remote to $REPO_SLUG..."
            if git -C "$dir" remote set-url origin "$REPO_URL_HTTPS"; then
                log_success "Remote updated — future updates will reach the right repository"
            else
                log_error "Could not update the git remote in $dir"
                log_info "  Fix it by hand with:"
                log_info "    git -C \"$dir\" remote set-url origin $REPO_URL_HTTPS"
                exit 1
            fi
            return 0
        fi
...
clone_repo() {
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            # old address in origin, and every command below goes through it.
            retarget_stale_origin "$INSTALL_DIR"
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `curie update` path
                # (#4735).
                ...
                local stash_name
                stash_name="curie-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if ! git fetch origin "$BRANCH"; then
                # `set -e` would exit here with git's own message and nothing
                # else. The one fact that makes this diagnosable is which
                # remote was being fetched, and git does not print it on an
                # auth failure -- it prints the repository it could not read.
                log_error "Could not fetch $BRANCH from the remote."
                log_info "  Remote: $(git remote get-url origin 2>/dev/null || echo 'unset')"
                log_info "  Expected: $REPO_URL_HTTPS"
                log_info ""
                log_info "  If the remote above is wrong, point it at the right one:"
                log_info "    git -C \"$INSTALL_DIR\" remote set-url origin $REPO_URL_HTTPS"
            ...
            # cannot succeed — mirror ``curie update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if [ "$restore_ok" = "yes" ] && [ -z "$conflicted_files" ]; then
                        ...
                        log_warn "Review git diff / git status if Curie behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up curie command..."

    if [ "$USE_VENV" = true ]; then
        CURIE_BIN="$INSTALL_DIR/venv/bin/python"
        CURIE_ENTRYPOINT="$INSTALL_DIR/curie"
    else
        CURIE_BIN="$(which curie 2>/dev/null || echo "")"
        if [ -z "$CURIE_BIN" ]; then
            log_warn "curie not found on PATH after install"
            return 0
        fi
    fi

    # Verify the interpreter and the checked-in entrypoint needed by the launcher.
    if [ ! -x "$CURIE_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$CURIE_ENTRYPOINT" ]; }; then
        log_warn "Curie launcher prerequisites not found"
    ...
    # Create a user-facing shim for the curie command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $CURIE_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$CURIE_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/curie"
    if [ "$USE_VENV" = true ]; then
        ...
        cat > "$command_link_dir/curie" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CURIE_BIN" "$CURIE_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/curie" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/curie"
    log_success "Installed curie launcher → $command_link_display_dir/curie"

    # Also expose `curie-agent`. The `curie-agent` console script declared in
    ...
    rm -f "$command_link_dir/curie-agent"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/curie-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CURIE_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/curie-agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CURIE_BIN" run_agent.py "\$@"
EOF
    fi
    chmod +x "$command_link_dir/curie-agent"
    log_success "Installed curie-agent launcher → $command_link_display_dir/curie-agent"

    # Also expose `curie-acp`. ACP hosts (Zed, JetBrains, Buzz) resolve the
    # agent by command name on the login-shell PATH, and the `curie-acp`
    # console script lives inside the venv, which is not on that PATH. Without
    # this launcher those hosts report Curie as not installed. (#21454 applies
    # here too: clear the path first so `cat >` cannot follow an old symlink
    # into the venv and overwrite the console script.)
    rm -f "$command_link_dir/curie-acp"
    if [ "$USE_VENV" = true ]; then
        cat > "$command_link_dir/curie-acp" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CURIE_BIN" "$CURIE_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/curie-acp" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    chmod +x "$command_link_dir/curie-acp"
    log_success "Installed curie-acp launcher → $command_link_display_dir/curie-acp"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "curie command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v curie' \
                >/dev/null 2>&1; then
        ...
        log_info "curie not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Curie Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Curie Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Curie Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so curie works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.curie directory structure (config at top level, code in subdir)
    mkdir -p "$CURIE_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.curie/.env (top level, easy to find)
    if [ ! -f "$CURIE_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$CURIE_HOME/.env"
            log_success "Created ~/.curie/.env from template"
        else
            touch "$CURIE_HOME/.env"
            log_success "Created ~/.curie/.env"
        fi
    else
        log_info "~/.curie/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$CURIE_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.curie/config.yaml (top level, easy to find)
    if [ ! -f "$CURIE_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$CURIE_HOME/config.yaml"
            log_success "Created ~/.curie/config.yaml from template"
        fi
    else
        log_info "~/.curie/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in curie_cli/default_soul.py — the
    ...
    if [ ! -f "$CURIE_HOME/SOUL.md" ]; then
        cat > "$CURIE_HOME/SOUL.md" << 'SOUL_EOF'
...
SOUL_EOF
        log_success "Created ~/.curie/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.curie/"

    # Seed bundled skills into ~/.curie/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `curie update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'curie update'." \
            > "$CURIE_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $CURIE_HOME/.no-bundled-skills"
        log_info "  Future 'curie update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.curie/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.curie/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$CURIE_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$CURIE_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.curie/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Curie Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Curie will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Curie Agent browser tools — explicit browser override."
...
# install — the same closure `curie update` installs
# (curie_cli/main.py::_update_node_dependencies). Prebuilt/partial checkouts
...
install_node_deps() {
    ...
    # Keep the checkout clean so `curie update` doesn't autostash every run.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
install_browser_use_cli() {
    ...
    # `curie tools` can install it later.
    if [ "$SKIP_BROWSER" = true ]; then
    ...
    if [ -x "$CURIE_HOME/bin/browser-use" ]; then
        log_success "Browser Use CLI already installed"
        return 0
    fi

    log_info "Installing Browser Use CLI (default browser backend)..."
    # UV_TOOL_BIN_DIR keeps the binary inside Curie' managed bin dir, where
    # the browser tool resolves it — no reliance on the user's PATH.
    if run_with_timeout 600 env UV_NO_CONFIG=1 UV_TOOL_BIN_DIR="$CURIE_HOME/bin" \
        ...
        log_info "Install later with: $UV_CMD tool install browser-use  (or via 'curie tools')"
    fi
}
...
install_computer_use_driver() {
    ...
    # `curie tools`, the dashboard, or the desktop app — is a config flip,
    # not a surprise multi-minute binary fetch (the confusion this fixes:
    # users had to discover `curie computer-use install` on their own).
    ...
    # Same upstream installer `curie computer-use install` runs; time-boxed
    # so a stalled GitHub download can't hang the Curie install. The
    # upstream installer serializes with its own lock (600s stale window),
    # so give it a ceiling above that — matching Curie'
    ...
    if run_with_timeout 660 /bin/bash -c \
        ...
        >"$cua_log" 2>&1; then
        log_success "Computer Use driver installed (enable via 'curie tools' → Computer Use)"
    else
        log_warn "Computer Use driver install failed — it will install on demand when you enable the tool."
        log_info "Install later with: curie computer-use install"
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'curie setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m curie_cli.main setup < /dev/tty
    else
        python -m curie_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$CURIE_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Curie to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$CURIE_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'curie whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                CURIE_CMD="$(get_curie_command_path)"
                $CURIE_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'curie whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'curie gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $CURIE_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $CURIE_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: curie gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: curie gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $CURIE_CMD gateway > "$CURIE_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.curie/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: curie gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: curie gateway"
    fi
}

write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.curie-bootstrap-complete, which tells the Curie
    ...
    local marker_path="$INSTALL_DIR/.curie-bootstrap-complete"
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $CURIE_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $CURIE_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $CURIE_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}curie${NC}              Start chatting"
    echo -e "   ${GREEN}curie setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}curie config${NC}       View/edit configuration"
    echo -e "   ${GREEN}curie config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}curie gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}curie update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'curie' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'curie' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'curie' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
install_bench_console_deps() {
    # Install the project's own `ui` extra rather than a bare `textual`.
    #
    # This matters: Textual depends on rich, and `pip install textual` with no
    # constraints resolves rich to its newest release — which walks straight
    # past the `rich==14.3.3` pin in [dependencies] and leaves the install
    # with a rich the rest of the code was never tested against. Installing
    # `.[ui]` means pyproject's pins govern the whole resolution.
    log_info "Installing the bench console's dependencies..."

    local target_dir="$INSTALL_DIR"
    [ -f "$target_dir/pyproject.toml" ] || target_dir="$(pwd)"
    if [ ! -f "$target_dir/pyproject.toml" ]; then
        log_warn "Could not find pyproject.toml; run this from the install directory"
        return 0
    fi

    local pip_target=()
    if [ "$USE_VENV" = true ] && [ -x "$target_dir/venv/bin/python" ]; then
        pip_target=(--python "$target_dir/venv/bin/python")
    fi

    if [ -z "${UV_CMD:-}" ]; then
        log_warn "uv is unavailable; install manually: pip install '.[ui]'"
        return 0
    fi
    if (cd "$target_dir" && "$UV_CMD" pip install "${pip_target[@]}" -e ".[ui]"); then
        log_success "bench console ready — run: curie ui"
    else
        log_warn "Could not install the ui extra; \`curie ui\` will not start"
        log_info "  Retry with: uv pip install -e '.[ui]'"
    fi
    return 0
}

install_local_tts() {
    # piper-tts: fully local neural speech (Open Home Foundation's piper1-gpl).
    # No API key and no network at speech time — only the first call downloads
    # a voice. Opt-in because it pulls onnxruntime, which is ~60MB on Linux
    # and unavailable on some platforms, so installing it unconditionally
    # would slow every fresh install and break a few outright.
    #
    # Not fatal on failure: TTS is one optional tool among many, and the same
    # install is reachable afterwards through `curie tools`, so a wheel that
    # will not resolve today must not take the whole install down.
    if [ "$WITH_TTS" != true ]; then
        return 0
    fi

    log_info "Installing piper-tts (local text-to-speech)..."

    local pip_target=()
    if [ "$USE_VENV" = true ] && [ -x "$INSTALL_DIR/venv/bin/python" ]; then
        pip_target=(--python "$INSTALL_DIR/venv/bin/python")
    fi

    # No `-U`: piper-tts pulls numpy and onnxruntime, both of which the
    # project constrains. Upgrading them to satisfy piper would silently
    # replace pinned versions the rest of the code depends on; let the
    # resolver work inside the existing constraints instead, and report the
    # conflict if there is one rather than papering over it.
    if [ -n "${UV_CMD:-}" ]; then
        if "$UV_CMD" pip install "${pip_target[@]}" piper-tts 2>&1 | tail -3; then
            log_success "piper-tts installed"
        else
            log_warn "piper-tts install failed — speech tools will stay unavailable"
            log_info "  Retry later with: curie tools  ->  Text to Speech  ->  Piper"
            return 0
        fi
    else
        local py="$PYTHON_PATH"
        [ -x "$INSTALL_DIR/venv/bin/python" ] && py="$INSTALL_DIR/venv/bin/python"
        if "$py" -m pip install piper-tts 2>&1 | tail -3; then
            log_success "piper-tts installed"
        else
            log_warn "piper-tts install failed — speech tools will stay unavailable"
            log_info "  Retry later with: curie tools  ->  Text to Speech  ->  Piper"
    ...
    # Playback needs a way out to the sound card. sounddevice is the low-latency
    # path; without it the tool writes a temp file and shells out to a system
    # player, so this is a nice-to-have and never a hard failure.
    if [ -n "$UV_CMD" ]; then
        "$UV_CMD" pip install "${pip_target[@]}" sounddevice soundfile >/dev/null 2>&1 \
            && log_success "audio playback ready (sounddevice)" \
            || log_info "sounddevice unavailable — playback will use a system player"
    fi

    log_info "  Default voice en_US-lessac-medium downloads on the first speech call"
    log_info "  Other voices: set tts.piper.voice in \$CURIE_HOME/config.yaml"
    return 0
}

verify_bench_console() {
    # `curie ui` needs Textual, which ships in the `ui` extra and therefore in
    # [all]. Confirm it actually landed rather than assuming: an extra can be
    # skipped by a fallback tier when something else in [all] fails to resolve,
    # and the first the user would hear of it is `curie ui` refusing to start.
    if [ "$USE_VENV" != true ]; then
        return 0
    fi
    ...
    # [web] extra already uses. If that fails too, say what to run.
    log_info "Textual not present — installing the ui extra directly..."
    install_bench_console_deps

    if "$py" -c "import textual" >/dev/null 2>&1; then
        return 0
    fi
    log_warn "Textual is still missing, so \`curie ui\` will not start"
    log_info "  Add it with: curie update --ensure ui"
    return 0
}

ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$CURIE_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$CURIE_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$CURIE_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$CURIE_HOME/node" --silent --ignore-scripts \
    ...
    for dep in "${DEPS[@]}"; do
        ...
        case "$dep" in
            ...
            ui|console)
                ...
                ;;
            *)
                log_warn "Unknown dependency: $dep (supported: node, browser, ripgrep, ffmpeg, piper, ui)"
                ;;
        esac
    done
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Curie` rename
...
# _purge_electron_build_cache() used by `curie desktop`; install.sh was the only
...
install_desktop() {
    ...
    # supported Node lines and prepends the Curie-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `curie desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Curie" ]; then
            app="$desktop_dir/release/linux-unpacked/Curie"
        elif [ -x "$desktop_dir/release/linux-unpacked/curie" ]; then
            app="$desktop_dir/release/linux-unpacked/curie"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Curie.app" \
            "$desktop_dir/release/mac/Curie.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # `curie desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading CURIE_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if CURIE_HOME="$CURIE_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
from pathlib import Path
from curie_cli.main import _desktop_macos_relaunchable_fixup
...
PYEOF
    ...
    # checkout stays clean for the next `curie update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Curie-managed Node
            # provisioned during prerequisites/node-deps (at $CURIE_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $CURIE_HOME.
    # $CURIE_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'curie update' on this host install.
