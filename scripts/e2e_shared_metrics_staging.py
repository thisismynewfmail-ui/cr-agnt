...
CURIE_HOME so the operator's own telemetry state is untouched.
...
def main() -> int:
    scratch = Path(tempfile.mkdtemp(prefix="curie-telemetry-e2e-"))
    os.environ["CURIE_HOME"] = str(scratch)
    ...
    from curie_cli.observability.shared_metrics_sender import SharedMetricsSender
    ...
    for index, metric_count in ((0, 3), (1, 140)):
        ...
        payload = {
            "schema_version": "curie.shared_metrics.v2",
    ...
    print(f"scratch CURIE_HOME : {scratch}")
    ...
    print("  aws s3 ls --recursive "
          "s3://curie-agent-telemetry-staging-767397871023-us-west-2-an/raw/ "
          "| tail -20")
    return 0
