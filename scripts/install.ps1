# ============================================================================
# Curie Agent Installer for Windows
# ============================================================================
...
#
# To install from a fork or a mirror, set CURIE_REPO to its "owner/name"
# slug before running; every URL below is derived from it.
#   $env:CURIE_REPO = 'me/my-fork'
#
# ============================================================================
...
param(
    ...
    # Install piper-tts for local, offline speech. Opt-in rather than default:
    # the wheel is small but it pulls onnxruntime, which is a ~60MB download,
    # so an unconditional install would slow every fresh install for a feature
    # most users never turn on. Also reachable afterwards through
    # `curie tools` -> Text to Speech -> Piper.
    [switch]$WithTts,
    [string]$Branch = "main",
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$CurieHome = $(if ($env:CURIE_HOME) { $env:CURIE_HOME } else { "$env:LOCALAPPDATA\curie" }),
    [string]$InstallDir = $(if ($env:CURIE_HOME) { "$env:CURIE_HOME\curie-agent" } else { "$env:LOCALAPPDATA\curie\curie-agent" }),
    ...
    # builds apps/desktop into a launchable Curie.exe.
    #
    # Why opt-in:
    #   * Curie-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Curie.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Curie.exe
    ...
    #     `curie desktop` already builds on demand.
    [switch]$IncludeDesktop
)

$ErrorActionPreference = "Stop"

# Never let git stop and ask for a password.
#
# Git does not read a credential prompt from stdin -- it opens the console
# directly -- so `irm | iex` does NOT protect the installer from one. Any
# remote git cannot reach anonymously (a repository that was renamed, deleted
# or made private) answers 404, which git cannot tell apart from a private
# repository, so it asks for credentials and the install stops there naming a
# repository the user has never heard of.
#
# Refusing the prompt turns that hang into an immediate, readable failure.
# Stored credentials still work: this removes only the interactive fallback,
# which behind a progress window was never usable anyway. Set after param()
# because PowerShell requires param() to be the first statement in a script.
...
# CURIE_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[curie] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'CurieInstall.LongPath').Type) {
            Add-Type -Namespace 'CurieInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [CurieInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -CurieHome / -InstallDir is normalized in place
...
if ($PSBoundParameters.ContainsKey('CurieHome')) {
    $CurieHome = ConvertTo-LongPath $CurieHome
} else {
    $CurieHome = ConvertTo-LongPath $(
        if ($env:CURIE_HOME) { $env:CURIE_HOME } else { "$env:LOCALAPPDATA\curie" }
    )
}
if ($PSBoundParameters.ContainsKey('InstallDir')) {
    ...
    $InstallDir = ConvertTo-LongPath $(
        if ($env:CURIE_HOME) { "$env:CURIE_HOME\curie-agent" } else { "$env:LOCALAPPDATA\curie\curie-agent" }
    )
}
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: CurieHome=$CurieHome InstallDir=$InstallDir"
}
...
# One slug, and every URL derived from it. Repeating "owner/name" across the
# clone URLs, the archive fallback and the recovery hint is how they drift
...
$RepoWebBase = "https://github.com/${RepoSlug}"
$RepoRawBase = "https://raw.githubusercontent.com/${RepoSlug}"
# Names this repository has been published under before. An existing install
# still has one of these in its `origin`, and the update path below goes
...
function Write-Banner {
    ...
    Write-Host "|             * Curie Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $CurieHome)) {
        New-Item -ItemType Directory -Force -Path $CurieHome | Out-Null
    }
    $envFile = Join-Path $CurieHome ".env"
...
function Install-AgentBrowser {
    ...
    # which every consumer (tools/browser_tool.py, `curie update`'s npx
    ...
    $prefixDir = Join-Path $CurieHome "node"
...
function Install-Uv {
    # Curie owns its own uv at $CurieHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (curie_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `curie update` stay in sync.
    $managedUv = Join-Path $CurieHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $CurieHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $CurieHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $CurieHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $CurieHome "bin"
        ...
        # (curie_cli/managed_uv.py looks only at $CurieHome\bin\uv.exe).
...
# Put the Curie-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone curie-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Curie-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Curie-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("curie-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and CURIE_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Curie belongs to the checkout, never to another
    # application or a user-level uv configuration. Keep this aligned with
    # curie_cli.managed_uv.managed_python_env(), which owns the update path.
    foreach ($name in @(
    ...
    $managedRoot = Join-Path $InstallDir ".curie-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Curie-managed interpreter
    ...
    #
    # Under Curie-Setup.exe each stage runs in a fresh powershell.exe. The
    ...
    foreach ($ver in $candidates) {
        ...
        try {
            ...
            throw "Failed to resolve Curie-managed Python $ver`: $_"
        } finally {
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Curie setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Curie uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\curie\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Curie needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Curie copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\curie\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``CURIE_GIT_BASH_PATH`` (User scope) so Curie can find it in a fresh
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        ...
        Write-Info "Trying a Curie-managed PortableGit install instead..."
    }

    # Download PortableGit into $CurieHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $CurieHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Curie features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Curie needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``CURIE_GIT_BASH_PATH`` (User env scope) so Curie can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $CurieHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $CurieHome\git\usr\bin\bash.exe
    $candidates += "$CurieHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$CurieHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("CURIE_GIT_BASH_PATH", $candidate, "User")
            $env:CURIE_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set CURIE_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Curie may not find Git Bash."
    Write-Info "If needed, set CURIE_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Curie-managed tree. Keeping this probe separate
...
function Test-SystemNodeReady {
    ...
    if (Test-NodeVersionOk $version) {
        ...
        Write-Warn "Node.js $version is unsupported (Curie requires Node 22.22+, 24.11+, or 26+)"
        return $false
    }
    ...
    if ($npmVersion) {
        Write-Warn "Node.js $version uses npm $npmVersion, which does not satisfy Curie requirement $npmRange"
    } else {
...
function Test-Node {
    ...
    Write-Info "Using a Curie-managed Node.js installation instead..."

    # Prefer a Curie-managed Node from a previous run over a too-old system one.
    $managedNode = "$CurieHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$CurieHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$CurieHome\node"
        Write-Success "Node.js $version found (Curie-managed)"
        ...
        Update-ManagedNpm "$CurieHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Curie-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $CurieHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $CurieHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\curie-node-extract"
            ...
            if ($extractedDir) {
                ...
                Get-ChildItem "$CurieHome" -Directory -Filter "node.old-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                Get-ChildItem "$CurieHome" -Directory -Filter "node.new-*" -ErrorAction SilentlyContinue |
                    Where-Object { $_.LastWriteTime -lt (Get-Date).AddMinutes(-10) } |
                    Remove-Item -Recurse -Force -ErrorAction SilentlyContinue
                ...
                if (Test-Path "$CurieHome\node") {
                    try {
                        Rename-Item "$CurieHome\node" $backup -ErrorAction Stop
                    } catch {
                        Write-Warn "Curie-managed Node.js is in use by a running app; deferring its upgrade. Close the app and re-run the update."
                ...
                $version = & "$CurieHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $CurieHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\curie-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
# Reduce a GitHub remote URL to its "owner/name", lowercased. Returns an
# empty string for a non-GitHub remote, which is the signal to leave it alone.
function Get-RemoteSlug {
    ...
    $scp = $value.IndexOf("github.com:")
    if ($scp -ge 0) {
        return $value.Substring($scp + "github.com:".Length).ToLowerInvariant()
    }
    $path = $value.IndexOf("github.com/")
    if ($path -ge 0) {
        return $value.Substring($path + "github.com/".Length).ToLowerInvariant()
    }
    return ""
}

# Point an existing checkout at the repository that exists.
#
# This is the half of a rename that clone URLs cannot fix. An install made
# before the move has the old address in its `origin`, and every update goes
# through `origin` -- so it fetches a repository GitHub no longer serves,
# which answers 404, which git reports as "Repository not found" only *after*
# asking for a password, because a 404 is indistinguishable from a private
# repository the caller has not authenticated to.
#
# A remote that is neither the current name nor a former one is somebody's
# fork or mirror and is deliberate: left untouched.
function Repair-StaleOrigin {
    param([string]$Dir)

    $prevEAP = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    try {
        $originUrl = (git -C $Dir remote get-url origin 2>$null | Select-Object -First 1)
        if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($originUrl)) { return }

        $current = Get-RemoteSlug $originUrl
        if ([string]::IsNullOrWhiteSpace($current)) { return }
        if ($current -eq $RepoSlug.ToLowerInvariant()) { return }

        foreach ($former in $RepoSlugFormer) {
            if ($current -ne $former.ToLowerInvariant()) { continue }

            Write-Warn "This install points at $current, which has been renamed."
            Write-Info "Updating its git remote to $RepoSlug..."
            git -C $Dir remote set-url origin $RepoUrlHttps 2>$null
            if ($LASTEXITCODE -eq 0) {
                Write-Success "Remote updated -- future updates will reach the right repository"
            } else {
                Write-Warn "Could not update the git remote in $Dir"
                Write-Info "  Fix it by hand with:"
                Write-Info "    git -C `"$Dir`" remote set-url origin $RepoUrlHttps"
            }
            return
        }
    } finally {
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            # Before any fetch: an install predating a rename still has the
            # old address in origin, and every git command below goes through
            # it. See Repair-StaleOrigin.
            Repair-StaleOrigin $InstallDir
            Push-Location $InstallDir
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `curie
                    # update` path (#4735).
                    ...
                    $stashName = "curie-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. curie-setup.exe bakes its build-time commit
                    ...
                    # ff-only pull cannot succeed -- mirror ``curie update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if (($restoreExit -eq 0) -and ($conflictedFiles.Count -eq 0)) {
                            ...
                            Write-Warn "Review git diff / git status if Curie behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running curie processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "$RepoWebBase/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "$RepoWebBase/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "$RepoWebBase/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\curie-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\curie-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `curie update` (see the notes at the shared clone-path
    ...
    # next `curie update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Curie-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Curie-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    if (Test-Path -LiteralPath "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running curie process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running curie processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Curie_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (curie.exe) plus its child tree.
            & taskkill /F /T /IM curie.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM curie.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m curie_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not curie.exe.
            ...
            # Curie children re-exec through .curie-runtime, so killing only
            ...
            # user (see curie_cli/gateway_windows.py), so the installer always
        ...
        try {
            ...
            throw (
                ...
                "A process still has the install directory open (often a non-Curie " +
    ...
    $env:UV_PYTHON = $venvPythonExe
        ...
        # the task fires normally on next logon and `curie update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Curie (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `curie.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install curie-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and curie.exe broken with
    ...
    # here, not 30 seconds later when the user runs `curie`.
    if (-not $NoVenv) {
        $venvPython = "$InstallDir\venv\Scripts\python.exe"
        if (-not (Test-Path $venvPython)) {
            throw "Install reported success but $venvPython does not exist. The dependency sync likely landed in a sibling .venv\ directory. Re-run the installer; if it persists, close Curie processes and preserve existing venv directories before retrying. Do not delete venv in place."
        }
        ...
        if ($importExitCode -ne 0) {
            ...
            $hint = if (Test-Path $sibling) {
                "Detected sibling .venv\ at $sibling -- uv synced there instead of venv\. Close Curie processes, preserve the existing venv, and rerun the installer so the transactional recovery path can move directories safely."
            } else {
    ...
    Complete-VenvTransaction
        ...
        # a failed update leaves Curie and its blocker probe usable.
        Restore-VenvBackup
        Pop-Location
        throw
    }
    ...
    if (-not $NoVenv) {
        # uv on Windows can register curie.exe in dist-info/RECORD but fail to
        ...
        # `curie` command while curie-agent.exe / curie-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m curie_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `curie dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\curie_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `curie dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `curie dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: curie_cli/web_server.py"
        }
    }
...
function Install-CurieCommandLaunchers {
    param(
        [Parameter(Mandatory=$true)] [string]$Root,
        [Parameter(Mandatory=$true)] [string]$Destination
    )

    # Expose ONLY the curie launchers on PATH -- never the whole
    # venv\Scripts directory, which contains python.exe / pip.exe and
    # silently hijacks the `python` command in every terminal (#83797).
    # Requiring curie.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "curie.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the curie command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # curie_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("curie", "curie-acp")) {
    ...
    $requiredExe = Join-Path $Destination "curie.exe"
    $requiredCmd = Join-Path $Destination "curie.cmd"
    if (-not ((Test-Path -LiteralPath $requiredExe -PathType Leaf) -or
              (Test-Path -LiteralPath $requiredCmd -PathType Leaf))) {
        throw "Cannot set up the curie command: launcher was not installed: $requiredExe"
    }
    return $Destination
}

function Set-PathVariable {
    Write-Info "Setting up curie command..."
    
    if ($NoVenv) {
        $curieBin = "$InstallDir"
    } else {
        # $CurieHome\bin is the managed binary dir (shared with the managed
        # uv), OUTSIDE the git checkout: `curie update`'s autostash
        ...
        # installer staged under curie-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-CurieCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $curieBin = "$CurieHome\bin"
        Install-CurieCommandLaunchers -Root $InstallDir -Destination $curieBin | Out-Null
    }
    ...
    #   curie-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The curie-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept curie via $curieBin)"
        }
    }
    
    if ($currentPath -notlike "*$curieBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$curieBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $curieBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set CURIE_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\curie instead
    # of the Unix default ~/.curie
    $currentCurieHome = [Environment]::GetEnvironmentVariable("CURIE_HOME", "User")
    if (-not $currentCurieHome -or $currentCurieHome -ne $CurieHome) {
        [Environment]::SetEnvironmentVariable("CURIE_HOME", $CurieHome, "User")
        Write-Success "Set CURIE_HOME=$CurieHome"
    }
    $env:CURIE_HOME = $CurieHome
    
    # Update current session
    $env:Path = "$curieBin;$env:Path"
    
    Write-Success "curie command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.curie-bootstrap-complete which tells the Curie
    ...
    $markerPath = Join-Path $InstallDir ".curie-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the CURIE_HOME directory structure ($CurieHome, default %LOCALAPPDATA%\curie)
    New-Item -ItemType Directory -Force -Path "$CurieHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CurieHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CurieHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CurieHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CurieHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CurieHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CurieHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CurieHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CurieHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$CurieHome\.env"
    ...
    # Create config.yaml
    $configPath = "$CurieHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Curie's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$CurieHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in curie_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $CurieHome"
    
    # Seed bundled skills into $CurieHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $CurieHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            Write-Success "Skills synced to $CurieHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$CurieHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $CurieHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Curie-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'curie setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\curie-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\curie-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\curie-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `curie tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $CurieHome "bin"
    $managedBu = Join-Path $managedBin "browser-use.exe"
    # MANAGED-FIRST: only Curie' managed copy short-circuits. A browser-use
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Curie' managed bin dir,
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Info "Install later with: uv tool install browser-use  (or via 'curie tools')"
        }
    } catch {
...
# Provision it at install time so enabling the tool later -- via `curie
...
# paths still lazy-install via install_cua_driver() (curie_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `curie computer-use install` runs. Bounded
        ...
        # Curie' _CUA_INSTALLER_TIMEOUT (660s).
        ...
        if (Wait-Job $job -Timeout 660) {
            ...
            if ($installedCuaDriver -and (Test-CuaDriverRuntimeContract -DriverPath $installedCuaDriver.Source)) {
                Write-Success "Computer Use driver installed (enable via 'curie tools' -> Computer Use)"
            } else {
                Write-Warn "Computer Use driver install did not produce a compatible runtime -- repair it before enabling the tool."
                Write-Info "Install later with: curie computer-use install"
            }
        } else {
...
# the final `electron` -> `Curie` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Curie.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_curie_desktop command
    # resolves apps/desktop/release/win-unpacked/Curie.exe directly,
    ...
    # enforces the supported Node lines and prepends the Curie-managed Node to
    ...
    # The --dir mode produces an unpacked Curie.exe in
    ...
    # are NOT trying to work around). The Curie icon + product name are
    # stamped onto Curie.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\curie-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Curie.exe",
        "$desktopDir\release\win-arm64-unpacked\Curie.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Curie.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Curie icon + identity are stamped onto Curie.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, curie-agent#38216).
    ...
    #    Curie.exe. We deliberately do NOT point them at `curie desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Curie.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Curie.lnk')
        )
        ...
        # with the Curie icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.curie/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'curie setup'."
        return
    }
    ...
    # Run curie setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m curie_cli.main setup
    } else {
        python -m curie_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $curieCmd = "$InstallDir\venv\Scripts\curie.exe"
    if (-not (Test-Path $curieCmd)) {
        $curieCmd = "curie"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$CurieHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'curie whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: curie gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$CurieHome\logs\gateway.log"
            Start-Process -FilePath $curieCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$CurieHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: curie gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: curie gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$CurieHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$CurieHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$CurieHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$CurieHome\curie-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   curie              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   curie setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   curie config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   curie config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   curie gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   curie update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Curie repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Curie-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Curie to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
    ...
    @{ Name = "local-tts";        Title = "Installing local text-to-speech";      Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-LocalTts" }
    @{ Name = "bench-console";    Title = "Verifying the bench console";          Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-BenchConsole" }
...
# Opt-in, so the no-op case is the normal one. Reported as skipped rather than
# ok so a GUI driver reading the manifest can tell "speech ready" from "speech
# not requested" -- the same distinction Stage-Node draws for browser tools.
function Stage-LocalTts         {
    if (-not $WithTts) {
        $script:_StageSkippedReason = "piper-tts not requested; pass -WithTts, or add it later from ``curie tools`` -> Text to Speech -> Piper"
        return
    }
    Resolve-UvCmd
    Install-LocalTts
}
function Stage-BenchConsole     { Test-BenchConsole }
...
function Install-LocalTts {
    <#
    .SYNOPSIS
        Install piper-tts (local neural speech) into the install's venv.

    .DESCRIPTION
        Fully local: no API key, and no network at speech time -- only the
        first call downloads a voice. Installed through the project directory
        so pyproject's pins govern the resolution: piper-tts pulls numpy and
        onnxruntime, both of which the project constrains, and installing it
        bare would let the resolver replace pinned versions the rest of the
        code depends on.

        Never fatal. Text-to-speech is one optional tool among many and the
        same install is reachable later from `curie tools`, so a wheel that
        will not resolve today must not take the whole install down.
    #>
    if (-not $WithTts) { return }

    Write-Info "Installing piper-tts (local text-to-speech)..."
    if (-not $script:UvCmd) { Resolve-UvCmd }
    if (-not $script:UvCmd) {
        Write-Warn "uv is unavailable; install manually: uv pip install piper-tts"
        return
    }
    if (-not (Test-Path (Join-Path $InstallDir "pyproject.toml"))) {
        Write-Warn "Install directory has no pyproject.toml; skipping piper-tts"
        return
    }

    Push-Location $InstallDir
    try {
        & $script:UvCmd pip install piper-tts
        if ($LASTEXITCODE -eq 0) {
            Write-Success "piper-tts installed"
        } else {
            Write-Warn "piper-tts install failed -- speech tools will stay unavailable"
            Write-Info "  Retry later with: curie tools  ->  Text to Speech  ->  Piper"
            return
        }

        # Playback needs a route to the sound card. sounddevice is the
        # low-latency path; without it the tool writes a temp file and hands it
        # to a system player, so this is a nice-to-have, never a failure.
        & $script:UvCmd pip install sounddevice soundfile 2>&1 | Out-Null
        if ($LASTEXITCODE -eq 0) {
            Write-Success "audio playback ready (sounddevice)"
        } else {
            Write-Info "sounddevice unavailable -- playback will use a system player"
        }
    } finally {
        Pop-Location
    }

    Write-Info "  Default voice en_US-lessac-medium downloads on the first speech call"
    Write-Info "  Other voices: set tts.piper.voice in $CurieHome\config.yaml"
}

function Install-BenchConsoleDeps {
    <#
    .SYNOPSIS
        Install the `ui` extra, which supplies Textual for `curie ui`.

    .DESCRIPTION
        Installs the project extra rather than a bare `textual`: Textual
        depends on rich, and an unconstrained install resolves rich to its
        newest release, walking past the pin in [dependencies] and leaving the
        install with a rich the rest of the code was never tested against.
    #>
    Write-Info "Installing the bench console's dependencies..."
    if (-not $script:UvCmd) { Resolve-UvCmd }
    if (-not $script:UvCmd) {
        Write-Warn "uv is unavailable; install manually: uv pip install -e '.[ui]'"
        return
    }
    if (-not (Test-Path (Join-Path $InstallDir "pyproject.toml"))) {
        Write-Warn "Install directory has no pyproject.toml; skipping the ui extra"
        return
    }
    Push-Location $InstallDir
    try {
        & $script:UvCmd pip install -e ".[ui]"
        if ($LASTEXITCODE -eq 0) {
            Write-Success "bench console ready -- run: curie ui"
        } else {
            Write-Warn "Could not install the ui extra; `curie ui` will not start"
            Write-Info "  Retry with: uv pip install -e '.[ui]'"
        }
    } finally {
        Pop-Location
    }
...
function Test-BenchConsole {
    <#
    .SYNOPSIS
        Confirm Textual actually landed, rather than assuming it did.

    .DESCRIPTION
        `curie ui` needs Textual, which ships in the `ui` extra and therefore
        in [all]. But the tiered install cascade can fall back to a narrower
        extra set when something else in [all] fails to resolve, and the first
        the user would hear about it is `curie ui` refusing to start.
    #>
    ...
    $ErrorActionPreference = "Continue"
    try {
        & $pythonExe -c "import textual" 2>&1 | Out-Null
    ...
    # Missing rather than broken: the tiered dependency cascade can fall
    # through to a narrower extra set when something else in [all] fails to
    # resolve, or the lockfile can predate the extra. Install it directly
    # rather than just reporting the gap -- the same last-resort pattern the
    # [web] extra already uses.
    Write-Info "Textual not present -- installing the ui extra directly..."
    Install-BenchConsoleDeps
    ...
    $ErrorActionPreference = $prevEAP
    if (-not $ok) {
        Write-Warn "Textual is still missing, so `curie ui` will not start"
        Write-Info "  Add it with: curie update --ensure ui"
    }
}
...
function Invoke-EnsureMode {
    ...
    foreach ($dep in $depList) {
        ...
        switch ($dep) {
            ...
            default {
                Write-Err "Unknown dependency: $dep (supported: node, browser, ripgrep, ffmpeg, piper, ui)"
                exit 1
            }
        }
    }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri '$RepoRawBase/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
