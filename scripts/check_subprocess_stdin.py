...
When Curie runs in TUI mode, the gateway child process communicates with
...
# ``get_curie_home() / "plugins"`` (user) and ``./.curie/plugins/`` (project,
# gated behind ``CURIE_ENABLE_PROJECT_PLUGINS``) — see
# ``curie_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``curie_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import curie_constants (this script
    ...
    # plugins in get_curie_home()/plugins/ and project plugins in
    # ./.curie/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_curie_home() / "plugins"]
    if os.environ.get("CURIE_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".curie" / "plugins")
