...
# tree generates llms.txt — the index every LLM (Curie included, via the
# curie-agent skill) reads to learn what Curie can do — and
...
_MCP_CATALOG_FILES = {"curie_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Curie-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``curie serve`` backend, the Docker image)
