# Behavioral test for install.ps1's curie launcher staging (PR #92092,
...
#
# The test lifts the real Install-CurieCommandLaunchers function from the
...
# the caller (Set-PathVariable passes $CurieHome\bin -- the managed binary
...
$fn = $ast.Find({
    ...
    $n.Name -eq 'Install-CurieCommandLaunchers'
}, $true)

if (-not $fn) {
    throw "Install-CurieCommandLaunchers not found in $installPs1"
}
...
$caseRoot = [System.IO.Path]::GetFullPath((Join-Path $tempBase (
    'curie-cli-launcher-test-' + [guid]::NewGuid().ToString('N')
)))
...
try {
    $installRoot = Join-Path $caseRoot 'curie-agent'
    ...
    $missingThrew = $false
    try {
        Install-CurieCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    } catch {
        $missingThrew = $_.Exception.Message -like '*required launcher not found*'
    }
    Assert-True $missingThrew 'missing curie.exe fails the launcher stage'
    ...
    $curieV1 = [byte[]](77, 90, 1)
    $curieV2 = [byte[]](77, 90, 2)
    $acp = [byte[]](77, 90, 3)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'curie.exe'), $curieV1)
    Set-Content -Path (Join-Path $installRoot 'venv\pyvenv.cfg') `
        -Value "home = X" -Encoding Ascii

    $staged = Install-CurieCommandLaunchers -Root $installRoot -Destination $binDir
    Assert-True ($staged -eq $binDir) 'returns the destination it staged into'
    Assert-BytesEqual $curieV1 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'curie.exe'))) `
        'normal venv: exe copy lands in the destination'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'curie-acp.exe'))) `
        'optional ACP launcher may be absent'

    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'curie.exe'), $curieV2)
    [System.IO.File]::WriteAllBytes((Join-Path $scriptsDir 'curie-acp.exe'), $acp)
    Install-CurieCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-BytesEqual $curieV2 `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'curie.exe'))) `
        'installer refreshes an existing Curie launcher'
    Assert-BytesEqual $acp `
        ([System.IO.File]::ReadAllBytes((Join-Path $binDir 'curie-acp.exe'))) `
    ...
    Install-CurieCommandLaunchers -Root $installRoot -Destination $binDir | Out-Null
    Assert-True (Test-Path -LiteralPath (Join-Path $binDir 'curie.cmd')) `
        'relocatable venv: .cmd delegator staged'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $binDir 'curie.exe'))) `
        'relocatable venv: stale exe copy removed'
    $cmdBody = [System.IO.File]::ReadAllText((Join-Path $binDir 'curie.cmd'))
    Assert-True ($cmdBody.Contains((Join-Path $scriptsDir 'curie.exe')) -and $cmdBody.Contains('%*')) `
