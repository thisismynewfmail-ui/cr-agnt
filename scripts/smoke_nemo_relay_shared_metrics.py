"""Run a real Curie CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_curie_executable(curie_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "curie",
        Path(".venv") / "Scripts" / "curie.exe",
    ):
        candidate = curie_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("curie")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Curie executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Curie source checkout containing .venv/bin/curie",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated CURIE_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["curie.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['curie.client.active']}"
        )
    [model] = by_name["curie.model_route.count"]
    expected_model = {
        "name": "curie.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['curie.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["curie.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['curie.task_run.started']}"
        )
    [terminal] = by_name["curie.task_run.finished"]
    ...
    [tool] = by_name["curie.tool_call.count"]
    ...
    lifecycle = by_name["curie.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["curie.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Curie development environment requires jsonschema"
        ) from exc
    ...
    if metrics["curie.client.active"] != [
        {
            "name": "curie.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['curie.client.active']}"
        )
    [model] = metrics["curie.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['curie.model_route.count']}"
        )
    [terminal] = metrics["curie.task_run.finished"]
    ...
    [tool] = metrics["curie.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["curie.skill.lifecycle.count"]
    ...
    loads = metrics["curie.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    curie_repo = args.curie_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    curie = _resolve_curie_executable(curie_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="curie-relay-shared-metrics-"))
    home = root / "curie-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["CURIE_HOME"] = str(home)
        python_paths = [str(curie_repo)]
    ...
    (root / "curie.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "curie.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Curie exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Curie model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Curie did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Curie did not print the mock model response")
    ...
    skill_result = subprocess.run(
        [
            ...
            "-c",
            "\n".join([
                "from curie_cli.observability import relay_shared_metrics",
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "curie.shared_metrics.v2.schema.json",
    )

    print("Curie -> NeMo Relay shared-metrics smoke test passed")
