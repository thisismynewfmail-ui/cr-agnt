...
unprivileged `curie` user, giving the layout most people have —
$CURIE_HOME/curie-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  CURIE_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .curie-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `curie setup` & the browser tools for speed.
...
INSTALL_REF=""
# The official repository, as an "owner/name" slug. The sandbox serves the
...
GIT_ROOT="${CURIE_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${CURIE_DEV_SANDBOX_DIR:-.curie-sandbox}"
...
if [ "$PERSISTENT" = true ]; then
  ...
  SANDBOX_ROOT="$(mktemp -d -t curie-sandbox.XXXXXX)"
...
if [ -n "$INSTALL_REF" ]; then
  ...
  UPSTREAM_REPO="$(mktemp -d -t curie-sandbox-upstream.XXXXXX)"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  SANDBOX_HTTP_REPO_DIR="$SANDBOX_ROOT/root/http/github.com/$REPO_SLUG"
  mkdir -p "$SANDBOX_HTTP_REPO_DIR"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_HTTP_REPO_DIR/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_HTTP_REPO_DIR/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL "https://raw.githubusercontent.com/'"$REPO_SLUG"'/main/scripts/install.sh" | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/curie-agent.git update-ref refs/heads/main "$next_main"; then
...
# A user-level install writes the `curie` launcher to ~/.local/bin and the
# checkout to $CURIE_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/curie-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q curie-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  SNAPSHOT_REPO="$(mktemp -d -t curie-sandbox-snapshot.XXXXXX)"
  git -C "$SNAPSHOT_REPO" init -q
  git -C "$SNAPSHOT_REPO" fetch -q "$GIT_ROOT" "$COMMIT"
  git -C "$SNAPSHOT_REPO" config user.name 'Curie sandbox'
...
if [ -n "$INSTALL_REF" ]; then
  git --git-dir="$FAKE_REPO" fetch -q --force "$SOURCE_REPO" \
    "$SOURCE_REF:refs/curie-sandbox/next"
