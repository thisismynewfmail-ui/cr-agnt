...
IGNORED_PATTERNS = [
    ...
    re.compile(r"^Curie\s+(Agent|Audit)$", re.IGNORECASE),
    ...
    re.compile(r".*\[bot\]$", re.IGNORECASE),             # any GitHub [bot] suffix (curie-seaeye[bot] etc.)
...
IGNORED_EMAILS = {
    ...
    "curie@nousresearch.com",
    "curie-audit@example.com",
    "nousbot@nousresearch.com",
    "curie@habibilabs.dev",
    "omx@oh-my-codex.dev",
    "codex@openai.com",
...
def gh_pr_list():
    ...
    try:
        result = subprocess.run(
            [
                "gh", "pr", "list",
                "--repo", "thisismynewfmail-ui/Cur-Agnt",
                "--state", "merged",
