...
measured in the production session DB) through `curie chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    CURIE_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Curie home with credentials for the
...
(tracker: thisismynewfmail-ui/Cur-Agnt#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "CURIE_HOME": str(HOME),
                "CURIE_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "curie_cli.main", "chat", "--query", q,
