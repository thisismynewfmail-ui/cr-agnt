...
batch (tracker: [#77056](https://github.com/thisismynewfmail-ui/Cur-Agnt/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Curie home, same model, same tasks, same reps.
...
1. Create a dedicated Curie home with credentials for the models under test:
   ...
   integration at it; no Curie observability plugin needs to be enabled.
