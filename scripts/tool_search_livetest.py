#!/usr/bin/env python3
"""Live test harness for Curie Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any curie imports.
ORIGINAL_HOME = os.environ.get("CURIE_HOME")
ORIGINAL_AUTH = Path.home() / ".curie" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.curie/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.curie/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="curie_ts_live_"))
    curie_home = home_dir / ".curie"
    curie_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, curie_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".curie" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, curie_home / ".env")
        ...
        load_curie_dotenv(curie_home=str(Path.home() / ".curie"))
    ...
    (curie_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return curie_home
...
def reset_module_state():
    """Drop cached modules so the new CURIE_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "curie_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["CURIE_HOME"] = str(home)
...
def main():
    ...
    # Restore original CURIE_HOME
    if ORIGINAL_HOME is not None:
        os.environ["CURIE_HOME"] = ORIGINAL_HOME
    else:
