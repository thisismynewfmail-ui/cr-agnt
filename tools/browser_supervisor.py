...
One ``CDPSupervisor`` runs per Curie ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Curie logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "curie-dialog-bridge.invalid"
...
(() => {
  if (window.__curieDialogBridgeInstalled) return;
  window.__curieDialogBridgeInstalled = true;
  const ENDPOINT = "http://curie-dialog-bridge.invalid/";
