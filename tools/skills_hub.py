#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Curie Skills Hub.
...
Used by curie_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _curie_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            curie_meta = metadata.get("curie", {})
            if isinstance(curie_meta, dict):
                tags = curie_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    OFFICIAL_REPO = "thisismynewfmail-ui/Cur-Agnt"
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        # upstream project maintains (e.g. impeccable's Curie-native bundle
        # under pbakaus/impeccable:.curie/skills/impeccable). Install pulls
    ...
    def _fetch_from_live_repo(self, rel: str) -> Optional[SkillBundle]:
        ...
        ``curie update``. Rather than telling them to update first, resolve
    ...
    def _upstream_pointer(self, skill_dir: Path) -> Optional[Dict[str, str]]:
        ...
        A stub declares ``metadata.curie.upstream`` in its SKILL.md
        frontmatter:
    ...
    def _upstream_pointer_from_content(self, content: Union[str, bytes]) -> Optional[Dict[str, str]]:
        """Parse ``metadata.curie.upstream`` out of SKILL.md content."""
        ...
        if not isinstance(meta_block, dict):
            return None
        curie_meta = meta_block.get("curie")
        if not isinstance(curie_meta, dict):
            return None
        upstream = curie_meta.get("upstream")
...
def _category_skill_dirs(directory: Path) -> List[str]:
    ...
    ``curie_cli.skills_hub._existing_categories``.
    """
    skill_dirs: List[str] = []
...
# ---------------------------------------------------------------------------
# Curie centralized index source
# ---------------------------------------------------------------------------

CURIE_INDEX_URL = "https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs"
CURIE_INDEX_TTL = 6 * 3600  # 6 hours


def _curie_index_cache_file() -> Path:
    return _index_cache_dir() / "curie-index.json"


def _load_curie_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for CURIE_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    curie_index_cache_file = _curie_index_cache_file()
    if curie_index_cache_file.exists():
        try:
            age = time.time() - curie_index_cache_file.stat().st_mtime
            if age < CURIE_INDEX_TTL:
                return json.loads(curie_index_cache_file.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Curie index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Curie index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Curie index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        curie_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        curie_index_cache_file.write_text(json.dumps(data), encoding="utf-8")
    except OSError:
        pass
...
class CurieIndexSource(SkillSource):
    """Skill source backed by the centralized Curie Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "curie-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "curie-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        CurieIndexSource(auth=auth), # Centralized index (search + resolved install paths)
