...
Curie Cloud, docker) is a persistent line. This module is the gateway-side
...
file plumbing on the gateway's own CURIE root — no network. The public
...
# ``bot_mode.envelope_ttl_seconds`` default in curie_cli/config_defaults.py.
...
def _normalize_roster_row(row: Any) -> Optional[dict]:
    ...
    if not profile or not connection_id:
        return None
    if not handle:
        handle = "curie" if profile == "default" else profile
    if (
...
def _curie_cli() -> str:
    """Resolve the curie CLI beside this gateway's own interpreter.

    The deliver RPC runs on the target gateway, whose process is the venv
    python — its bin/Scripts directory holds the matching ``curie``
    entrypoint. A bare ``"curie"`` relies on PATH, which is exactly what
    ...
    sibling = exe.parent / ("curie.exe" if sys.platform == "win32" else "curie")
    if sibling.is_file():
        return str(sibling)
    found = shutil.which("curie")
    if found:
        return found
    return "curie"
...
# turns concurrently: deliveries spawn separate ``curie`` subprocesses, so
