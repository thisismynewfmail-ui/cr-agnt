"""Central registry for all curie-agent tools.
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        # import time (curie_constants itself is stdlib-only, so no cycle).
        from curie_constants import get_curie_home

        return Path(get_curie_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales. Cache results for ~30 s so env-var flips via ``curie tools``
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    cache. A multiplex gateway installs a Curie-home override for every
    ...
    try:
        ...
        browser_identity = (
            get_session_env("CURIE_SESSION_ID", ""),
            get_session_env("CURIE_BROWSER_CONTROL_PRINCIPAL", ""),
            get_session_env("CURIE_BROWSER_CONTROL_TRANSPORT_FAMILY", ""),
        )
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``curie tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin registrations are overlays keyed by resolved CURIE_HOME. A
    ...
    def _plugin_namespace_of_module(
        ...
        if module_namespace.startswith("curie_plugins."):
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``curie_plugins.{name}``), not the exact module string.
                # A handler defined in ``curie_plugins.pkg.handlers`` is
                # still owned by the ``curie_plugins.pkg`` package — exact
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``curie tools enable foo``)
