...
def _expand_tilde(path: str) -> str:
    ...
    mirrors ``curie_constants.get_subprocess_home()`` so that ``~`` resolves
...
def _truncate_to_char_budget(content: str, max_chars: int) -> tuple[str, int, bool]:
    ...
    ``read_file``). Where curie previously hard-rejected an oversized read
...
def _terminal_env_type_for_task(task_id: str = "default") -> str:
    ...
    try:
        ...
        if env is not None:
            ...
            if "daytona" in name:
                return "daytona"
            stamped = getattr(env, "_curie_backend_name", None)
...
def _file_ops_uses_host_paths(file_ops) -> bool:
    """Return True when *file_ops* targets the same host filesystem as Curie.
...
_curie_config_resolved: str | None = None
_curie_config_resolved_loaded = False


def _get_curie_config_resolved() -> str | None:
    """Return the resolved absolute path of the Curie config file (cached)."""
    global _curie_config_resolved, _curie_config_resolved_loaded
    if _curie_config_resolved_loaded:
        return _curie_config_resolved
    _curie_config_resolved_loaded = True
    try:
        from curie_cli.config import get_config_path
        _curie_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _curie_config_resolved = str(Path(_expand_tilde("~/.curie/config.yaml")).resolve())
        except Exception:
            _curie_config_resolved = None
    return _curie_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Curie config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    curie_config = _get_curie_config_resolved()
    if curie_config and (resolved == curie_config or normalized == curie_config):
        return (
            f"Refusing to write to Curie config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.curie/config.yaml directly or use 'curie config' instead."
        )
    return None
...
# .cursorrules (or a project-local .curie config tree) outlives the current
# turn and poisons every later session that loads it. Writes to these files
# therefore ALWAYS require human approval — even under --unlock / auto-approve —
...
_real_curie_home_cached: str | None = None
_real_curie_home_loaded = False


def _get_real_curie_home() -> str | None:
    """Return the realpath of the authoritative Curie home (cached)."""
    global _real_curie_home_cached, _real_curie_home_loaded
    if _real_curie_home_loaded:
        return _real_curie_home_cached
    _real_curie_home_loaded = True
    try:
        from curie_constants import get_curie_home
        _real_curie_home_cached = os.path.realpath(str(get_curie_home()))
    except Exception:
        try:
            _real_curie_home_cached = os.path.realpath(_expand_tilde("~/.curie"))
        except Exception:
            _real_curie_home_cached = None
    return _real_curie_home_cached
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # The authoritative ~/.curie home is governed by its own guards
    ...
    # ``.curie`` component rule below, which would otherwise match the
    # home directory itself.
    real_home = _get_real_curie_home()
    ...
    for candidate in (normalized, resolved):
        ...
        for pattern in extra_patterns:
            if fnmatch.fnmatch(base_lower, pattern.lower()):
                return base
        # Project-local .curie config dirs (e.g. <repo>/.curie/config.yaml)
        # are loaded as project context and steer behavior the same way.
        # Scope: the file's IMMEDIATE parent must be ``.curie`` — matching
        # any ancestor named .curie would gate every write inside a
        # checkout that happens to live under ~/.curie (e.g. the
        # curie-agent repo itself at ~/.curie/curie-agent).
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".curie":
            return candidate
    return None
...
def _request_protected_instruction_approval(
    ...
    honors --unlock and session/permanent allowlists, and the entire point
    here is one-operation approval EVERY time, with no persistent scope
    and no unlock bypass. Fail-closed when no human channel exists.
    """
...
def _check_approval_required_write(paths: list[str],
    ...
    offers once/session/always scopes and honors --unlock (the historical
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Curie mirror prefix for Docker file tools."""
    try:
        from tools.terminal_tool import (
            _active_environments,
            _env_lock,
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    container's sandbox mirror of Curie state.

    Two detectors (both #32049): these catch writes that would be
    SILENTLY LOST — the host Curie process never reads the mirror, so
    ...
    Returns ``None`` when the write is in-scope or outside Curie scope.
    """
    try:
...
def read_file_tool(path: str, offset: int = 1, limit: int = 2000, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Curie internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under CURIE_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == CURIE_HOME (e.g. "auth.json") still hits the
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with curie_tools.write_file() for very "
            "large files."
        )
