...
Design contract (matches the Curie "no lazy-reading escape hatches / don't
...
def _check_metadata_block(frontmatter: Dict[str, Any]) -> List[LintFinding]:
    ...
    curie_meta = meta.get("curie") if isinstance(meta, dict) else None
    if not isinstance(curie_meta, dict):
        findings.append(
            LintFinding(
                WARNING,
                "missing-metadata",
                "frontmatter is missing metadata.curie.{tags, related_skills}.",
            )
        )
    else:
        if "tags" not in curie_meta:
            findings.append(
                LintFinding(
                    WARNING, "missing-metadata", "metadata.curie.tags is missing."
                )
            )
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("curie", "agent", "curie agent") and (
        author != "Curie Agent"
    ):
        findings.append(
            LintFinding(
                WARNING,
                "author-caps",
                f"author '{author}' should be 'Curie Agent' (proper caps) "
