...
`curie skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.curie.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.curie.blueprint`` is a mapping containing
    ...
    curie = meta.get("curie") if isinstance(meta, dict) else None
    blueprint = curie.get("blueprint") if isinstance(curie, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.curie.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.curie.blueprint`` block) they can hand
    to ``curie skills publish`` to share. ``body`` is the plain-language
