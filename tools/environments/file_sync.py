...
_FORCE_SYNC_ENV = "CURIE_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.curie") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.curie to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``CURIE_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, curie_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.curie/`` directory as a tar archive,
        ...
        """
        with self._transaction_lock:
            self._sync_back_transaction(curie_home=curie_home)

    def _sync_back_transaction(self, curie_home: Path | None = None) -> None:
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="curie-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.curie/skills/a.md`` →
        ``~/.curie/skills/a.md``, a new remote file at
        ``/root/.curie/skills/b.md`` maps to ``~/.curie/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
