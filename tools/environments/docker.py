...
_EGRESS_LABEL_KEY = "curie-egress"
...
def _load_curie_env_vars() -> dict[str, str]:
    """Load ~/.curie/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Curie profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale curie-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=curie-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Curie process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=curie-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a curie process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Curie
    process leaves its container behind permanently because there's no
    subsequent Curie process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=curie-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=curie-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``CURIE_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("CURIE_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using CURIE_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'curie'
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``CURIE_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `curie egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `curie egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `curie egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `curie egress setup` to mint provider "
    ...
    container_ca = "/etc/ssl/certs/curie-egress-ca.crt"
    ...
    # through the proxy.  Keep the CURIE_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"CURIE_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``curie-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.curie/sandboxes/). Non-persistent
        ...
        if egress_env_overrides:
            try:
                from curie_cli.config import load_config as _load_cfg_for_collision
        ...
        try:
            from curie_cli.config import load_config as _load_cfg_for_precedence
        ...
        # ``_CURIE_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_CURIE_EGRESS_NODE_OPTIONS_APPEND", None,
        )
        ...
        # s6-overlay images (e.g. curie-agent:latest) already use /init as PID 1
        ...
        container_name = f"curie-{uuid.uuid4().hex[:8]}"
        # Labels make curie-created containers identifiable to:
        #   * the orphan reaper (`curie-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`curie-task-id`, `curie-profile`)
        #   * operators running `docker ps --filter label=curie-agent=1`
        ...
        label_args = [
            "--label", "curie-agent=1",
            "--label", f"curie-task-id={task_label}",
            "--label", f"curie-profile={profile_name}",
        ...
        self._labels = {
            "curie-agent": "1",
            "curie-task-id": task_label,
            "curie-profile": profile_name,
            _EGRESS_LABEL_KEY: egress_label,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Curie process
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # win over the generic Curie secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip Curie-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_curie_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_curie_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _CURIE_PROVIDER_ENV_BLOCKLIST)
        curie_env = _load_curie_env_vars() if forward_keys else {}
        unset_names: set[str] = set()
        for key in sorted(forward_keys):
            value = os.getenv(key) or curie_env.get(key)
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("curie-task-id", "")
        profile_label = self._labels.get("curie-profile", "")
        existing = self._find_reusable_container(
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"curie-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(
        ...
        matches containers that happened to be named ``curie-*`` but were
        started by some other tool.
        """
        try:
            filters = [
                "--filter", "label=curie-agent=1",
                "--filter", f"label=curie-task-id={task_label}",
                "--filter", f"label=curie-profile={profile_label}",
            ]
            if egress_label != "off":
                ...
                # curie-egress label is present and not "off".  Without
                # this, a container created with egress=on can be silently
                # reused after the operator runs "curie egress disable",
        ...
        # container) but can happen if a previous Curie process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Curie exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Curie process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Curie startup. That covers the
        ...
        # outlives Curie processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"curie-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``curie /quit``
