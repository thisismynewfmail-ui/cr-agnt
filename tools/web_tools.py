...
Backend is selected during ``curie tools`` setup (web.backend in config.yaml).
When available, Curie can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Curie config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Curie' config/.env layer (``curie config set``, ``curie tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.curie/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``curie tools``). A
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `curie tools` repaint.
        try:
            ...
            if provider is None and backend and selection_exists("web"):
                ...
                if disabled_key:
                    ...
                    error_text = (
                        ...
                        f"Re-enable it with `curie plugins enable {disabled_key}` "
                ...
                if backend and selection_exists("web"):
                    ...
                    if disabled_key:
                        ...
                        error_text = (
                            ...
                            f"config. Re-enable it with `curie plugins "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`curie plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
def _provider_is_ready(provider) -> bool:
    ...
    require a true availability probe — otherwise ``curie doctor`` paints a
...
if __name__ == "__main__":
    ...
    from curie_cli.config import get_env_value as _gev
