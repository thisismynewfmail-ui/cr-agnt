#!/usr/bin/env python3
"""Run a guided tour (highlight + narrate UI elements) in the Curie desktop GUI.
...
- ``surface="app"`` — the Curie desktop app's own DOM (tours of Curie itself).
...
call fail would leave Curie offering walkthroughs it cannot give.
"""
...
def tour_tool(
    ...
    if callback is None:
        return tool_error("tour is only available in the Curie desktop app.")
...
TOUR_SCHEMA = {
    ...
    "description": (
        ...
        "element, attach a titled popover. Surfaces: 'app' (Curie itself) "
