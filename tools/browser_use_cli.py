...
_PRIVATE_BROWSER_SENTINEL = "_CURIE_BU_PRIVATE_BROWSER"
...
_OWN_TAB_PREAMBLE = """\
# curie: pin this named session to its own tab (once per daemon process)
def _curie_ensure_own_tab():
    ...
    _marker = _os.path.join(
        _tf.gettempdir(), "curie-bu-owntab-%s-%s-%s" % (_uid, _name, _dpid)
    )
    ...
    try:
        ...
        pass
_curie_ensure_own_tab()
del _curie_ensure_own_tab
"""
...
def _base_subprocess_env() -> dict:
    ...
    # may differ from Curie's venv Python. PYTHONPATH/PYTHONHOME inherited
    # from the agent process point at Curie's venv site-packages, and a
...
def _read_browser_cfg() -> dict:
    ...
    try:
        from curie_cli.config import cfg_get, read_raw_config
...
def default_downgrade_notice() -> Optional[str]:
    ...
    try:
        ...
        stamp = Path(get_curie_home()) / "cache" / _NOTICE_STAMP_NAME
        try:
        ...
        return (
            ...
            "Run `curie tools` (Browser Automation → Browser Use) to install it, "
...
def _managed_bin_dir() -> Optional[str]:
    """Curie' own bin dir ($CURIE_HOME/bin) — where install.sh puts uv/uvx
    and where install_cli() links the browser-use binary."""
    try:
        from curie_constants import get_curie_home

        return str(Path(get_curie_home()) / "bin")
...
def install_cli(timeout_s: int = 600) -> Tuple[bool, str]:
    ...
    Resolution order for uv: Curie' managed uv (bootstrapped on demand via
    ``curie_cli.managed_uv.ensure_uv``) → uv on PATH. The binary is linked
    into ``$CURIE_HOME/bin`` (``UV_TOOL_BIN_DIR``) so ``_find_cli()``
    ...
    # prevent provisioning the canonical Curie-managed copy, or resolution
...
def _workspace_dir(task_id: Optional[str]) -> Optional[str]:
    ...
    try:
        ...
        path = Path(get_curie_home()) / "cache" / "browser-use" / "workspace" / safe
...
def _resolve_lightpanda_cdp(
    ...
    """Point the harness at a Curie-spawned ``lightpanda serve``.
    ...
    try:
        ...
        return (
            ...
            "to use local Chrome, or switch backends via `curie tools` → "
            "Browser Automation."
        )
...
def _resolve_backend_cdp(
    ...
    4. ``browser.engine: lightpanda``: a Curie-spawned ``lightpanda serve``
    ...
    try:
        ...
        return (
            ...
            "switch backends via `curie tools` → Browser Automation."
        )
    cdp = str((session_info or {}).get("cdp_url") or "")
    if not cdp:
        return (
...
def _resolve_real_profile_cdp(env: dict, force_local: bool) -> Optional[str]:
    ...
    default Chromium with their logins — a browser Curie launches on a
    SNAPSHOT of their real profile (see curie_cli.browser_connect). Two ways
    in:
...
def _dynamic_schema_overrides() -> dict:
    ...
    if _real_profile_consented():
        ...
        props["local"] = {
            "type": "boolean",
            "description": (
                "Drive the user's own local browser (a Curie-managed copy of "
