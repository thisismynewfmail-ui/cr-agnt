...
def _block_message(operation: str, root: Path) -> str:
    ...
    return (
        f"Blocked: `{operation}` would rewrite Curie's live source checkout "
        ...
        "checkout, stop Curie, run the command externally, then restart "
        "Curie."
    )
...
def _scratch_dir_hint() -> str:
    ...
    base = Path(curie_home).expanduser() if curie_home else Path.home() / ".curie"
