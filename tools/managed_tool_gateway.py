...
def auth_json_path():
    """Return the Curie auth store path, respecting CURIE_HOME overrides."""
    return get_curie_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`curie tools`, banner/status paint, provider
...
# managed vendor's gateway URL is pinned: adding one is a Curie release, and
