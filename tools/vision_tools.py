...
# ``agent.auxiliary_client`` pulls credential_pool → curie_cli.auth → httpx
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("CURIE_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
            ...
            pass
    try:
        from curie_cli.config import cfg_get, load_config
        cfg = load_config()
...
#: Resolved once at import for the module constant existing callers read.
#: New code calls :func:`download_timeout` instead, so a config change takes
#: effect on the next download rather than on the next process — this module
#: lives inside a long-running agent, and a setting that needs a restart to
#: apply is one the reader will conclude does not work.
_VISION_DOWNLOAD_TIMEOUT = _resolve_download_timeout()

#: Floor for a video download. Videos are an order of magnitude larger than
#: images over the same link, so the image default is not a sane bound for one.
MIN_VIDEO_DOWNLOAD_TIMEOUT_SECONDS = 300.0


def download_timeout(minimum: float = 0.0) -> float:
    """The HTTP download bound, read fresh, with an optional floor."""
    try:
        resolved = _resolve_download_timeout()
    except Exception:
        resolved = _VISION_DOWNLOAD_TIMEOUT
    return max(float(resolved), float(minimum))

# ── The analysis budget ──────────────────────────────────────────────────
#
# How long the vision model is allowed to take, in total, for one call to
# these tools. Two hours by default, and the number is deliberate rather than
# generous-by-accident: a vision payload is a whole image, and a reasoning
# model reading a dense scan, a page of small print or a screenshot full of
# code legitimately thinks for a long time before it writes anything. The old
# two-minute cap cut those off mid-answer and reported a timeout, which reads
# as "vision is broken" rather than as "the deadline was too short".
#
# It is a *wall-clock budget for the whole tool call*, not a per-attempt one.
# This path can call the model up to three times — a downscale-and-retry when
# the API rejects an oversized payload, and one retry on an empty
# (reasoning-only) response — and giving each attempt its own full timeout
# meant the tool's real worst case was three times whatever the setting said.
# Nothing downstream could budget for that, which is how the executor deadline
# and this one got out of step in the first place.
#
# A stalled call is not what this bounds: a dead stream is still caught in
# ~60s by the auxiliary client's no-progress watchdog, which re-arms on every
# substantive token. This only bounds a call that is genuinely still working.
DEFAULT_VISION_TIMEOUT_SECONDS = 7200.0

#: Floor for the video path. Video payloads are larger than images and every
#: provider that accepts them is slower about it, so a config value below this
#: is treated as unset rather than obeyed.
MIN_VIDEO_TIMEOUT_SECONDS = 180.0


def resolve_vision_timeout(minimum: float = 0.0) -> float:
    """The total budget for one vision tool call, in seconds.

    ``auxiliary.vision.timeout`` in config.yaml, falling back to
    :data:`DEFAULT_VISION_TIMEOUT_SECONDS`. One resolver for both the image
    and the video path: they read the same key, and when each carried its own
    hardcoded fallback the two disagreed about what "unset" meant (120s vs
    180s) — which is only visible when config cannot be read at all, i.e. in
    exactly the situation where a wrong answer is hardest to diagnose.

    Total, not per-attempt: see :data:`DEFAULT_VISION_TIMEOUT_SECONDS`.
    """
    timeout = DEFAULT_VISION_TIMEOUT_SECONDS
    try:
        from curie_cli.config import cfg_get, load_config

        value = cfg_get(load_config(), "auxiliary", "vision", "timeout")
        # ``bool`` is rejected rather than converted: YAML ``true`` would
        # otherwise become a one-second budget, which is a stranger failure
        # than the one this whole change exists to remove.
        if value is not None and not isinstance(value, bool):
            timeout = float(value)
    except Exception:
        # An unreadable config is not a reason to refuse to look at an image;
        # so is a value of a shape nothing here expects.
        timeout = DEFAULT_VISION_TIMEOUT_SECONDS
    if timeout <= 0:
        # The codebase's "0 disables the bound" convention. There is still a
        # bound in practice — the no-progress watchdog — so this is "no total
        # ceiling", not "hang forever".
        return 0.0
    return max(timeout, float(minimum))


def resolve_vision_temperature() -> float:
    """``auxiliary.vision.temperature``, or the deterministic default."""
    try:
        from curie_cli.config import cfg_get, load_config

        value = cfg_get(load_config(), "auxiliary", "vision", "temperature")
        if value is not None and not isinstance(value, bool):
            return float(value)
    except Exception:
        pass
...
class _CallBudget:
    """One wall-clock budget shared by every attempt in a tool call.

    Handed to :func:`async_call_llm` as ``timeout=`` on each attempt, with
    whatever is left rather than the whole thing again. Without it, "the
    vision timeout is two hours" quietly meant "up to six", and a retry that
    began one second before the budget expired started with a fresh full one.
    """
    ...
    def remaining(self) -> Optional[float]:
        """Seconds left, or None when this budget does not bound anything."""
        if self.unbounded:
            return None
        return self.total - (time.monotonic() - self._started)

    def spent(self) -> bool:
        """Whether a further attempt would start with no time to run in.

        A retry needs enough of the budget left to be worth making. Below the
        floor the honest answer is the one already in hand, not a second call
        that will be cancelled a moment after it starts.
        """
        left = self.remaining()
        return left is not None and left < _RETRY_FLOOR_SECONDS


#: The least a retry may start with. A call given two seconds produces a
#: timeout, not an answer, and a timeout raised over a usable-if-imperfect
#: result is strictly worse than the result.
_RETRY_FLOOR_SECONDS = 20.0
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: CURIE_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("CURIE_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
def _supported_media_types() -> frozenset:
    ...
    try:
        ...
        from curie_cli.local_runtime.capabilities import (
...
def _normalize_to_supported_image(
    ...
    out_dir = get_curie_dir("cache/vision", "temp_vision_images")
...
async def _vision_analyze_native(
    ...
    try:
        ...
        temp_dir = get_curie_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $CURIE_HOME/cache/vision/ and cleaned up
    ...
    try:
        ...
        # Call the vision API via the centralized router, under one wall-clock
        # budget for the whole call rather than one per attempt: this block can
        # reach the model up to three times, and a per-attempt timeout made the
        # tool's real worst case three times whatever the setting said.
        budget = _CallBudget(resolve_vision_timeout())
        call_kwargs = {
            "task": "vision",
            "messages": messages,
            "temperature": resolve_vision_temperature(),
        ...
        async def _ask() -> Any:
            """One attempt, with whatever is left of the budget."""
            call_kwargs["timeout"] = budget.remaining()
        ...
        # Retry once on empty content (reasoning-only response), but only with
        # enough budget left for the second call to produce something.
        if not analysis and not budget.spent():
            logger.warning("Vision LLM returned empty content, retrying once")
            response = await _ask()
            analysis = extract_content_or_reasoning(response)
        elif not analysis:
            logger.warning(
                "Vision LLM returned empty content and the %.0fs budget is "
                "spent; not retrying", budget.total,
            )
...
def check_image_analysis_requirements() -> bool:
    """Whether ``vision_analyze`` can serve a request by either route.

    Two independent routes serve this tool, and only one of them needs an
    auxiliary provider:

    * the native route (:func:`_should_use_native_vision_fast_path`) hands the
      image straight to the main model and needs no auxiliary credentials at
      all; and
    * the legacy route asks a separate auxiliary vision model to describe it.

    Gating the tool on the auxiliary chain alone therefore removed
    ``vision_analyze`` from the model's tool list for exactly the users the
    native route exists to serve: a vision-capable main model (Claude, GPT,
    Gemini 3) with no OpenRouter or Nous credit resolved no auxiliary vision
    client, so the tool disappeared and the model could not look at an image
    it was perfectly able to read. Check the cheaper, credential-free route
    first, and only fall back to probing the auxiliary chain.

    ``video_analyze`` deliberately keeps the narrower
    :func:`check_vision_requirements` gate — it has no native route, so an
    auxiliary client really is required there.
    """
...
registry.register(
    ...
    check_fn=check_image_analysis_requirements,
    is_async=True,
    emoji="👁️",
)
...
async def _materialize_video_from_terminal_backend(video_source: str, task_id: Optional[str]) -> Path:
    ...
    temp_dir = get_curie_dir("cache/video", "temp_video_files")
    temp_dir.mkdir(parents=True, exist_ok=True)
...
async def _download_video(video_url: str, destination: Path, max_retries: int = 3) -> Path:
    ...
    for attempt in range(max_retries):
        try:
            ...
            async with create_ssrf_safe_async_client(
                # Was a hardcoded 60s while the image path was configurable —
                # so a video that took longer than a minute to fetch failed
                # with no setting that could change it, and the failure read
                # as "video analysis is broken".
                timeout=download_timeout(MIN_VIDEO_DOWNLOAD_TIMEOUT_SECONDS),
                follow_redirects=True,
                event_hooks={"response": [_ssrf_redirect_guard]},
            ) as client:
                await _stream_download_to_file(
...
async def video_analyze_tool(
    ...
    try:
        ...
        # Same budget key as the image path, with a floor: a video payload is
        # larger than an image and every provider that accepts one is slower
        # about it, so a config value below the floor is treated as unset.
        budget = _CallBudget(
            resolve_vision_timeout(minimum=MIN_VIDEO_TIMEOUT_SECONDS)
        )
        call_kwargs = {
            "task": "vision",
            "messages": messages,
        ...
        if not analysis and not budget.spent():
            ...
            logger.warning(
                "Empty video response and the %.0fs budget is spent; "
                "not retrying", budget.total,
            )
