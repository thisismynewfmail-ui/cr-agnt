...
it is automatically downloaded from GitHub releases to $CURIE_HOME/bin/tirith.
...
def _get_curie_home() -> str:
    """Return the Curie home directory, respecting CURIE_HOME env var."""
    return str(get_curie_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_curie_home(), ".tirith-install-failed")
...
def _curie_bin_dir() -> str:
    """Return $CURIE_HOME/bin, creating it if needed."""
    d = os.path.join(_get_curie_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $CURIE_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_curie_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $CURIE_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $CURIE_HOME/bin/tirith
    ...
    curie_bin = os.path.join(_curie_bin_dir(), "tirith")
    if os.path.isfile(curie_bin) and os.access(curie_bin, os.X_OK):
        _resolved_path = curie_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return curie_bin
