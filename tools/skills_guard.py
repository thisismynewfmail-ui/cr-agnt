...
Trust levels:
  - builtin:   Ships with Curie. Never scanned, always trusted.
...
_CURIE_CONFIG_FILES = r'\.curie/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.curie/\.env|\~/\.curie/\.env',
     "curie_env_access", "critical", "exfiltration",
     "directly references Curie secrets file"),
    ...
    #     For Curie/other config files it is high (caution) — legitimate
    ...
    (_prose_modify_re(_CURIE_CONFIG_FILES),
     "curie_config_mod", "high", "persistence",
     "modification language aimed at Curie configuration files (verify intent)"),
    (_shell_write_re(_CURIE_CONFIG_FILES),
     "curie_config_mod_shell", "critical", "persistence",
     "shell write (redirect/sed -i/tee/cp/mv) targeting Curie configuration files"),
    (r'\.curie/config\.yaml|\.curie/SOUL\.md',
     "curie_config_ref", "low", "persistence",
     "references Curie configuration files (informational; only modification intent is scored)"),
...
# scan. `.skillignore` is the Curie-native name; `.clawhubignore` is honored
