...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `curie model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``curie auth add
    ...
    2. The environment / ``~/.curie/.env``. Under a multiplexed gateway turn
       ...
       ``curie_cli.config.get_env_value`` (os.environ, then ``.env``),
       matching the tools' historical behaviour exactly.
    3. The credential pool / auth store for ``provider_id`` (``curie auth
    ...
    when omitted, ``curie_cli.config.get_env_value`` is used directly.
    """
    ...
    try:
        ...
        # `curie auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``curie auth add openai-api``) via
...
def read_selection(section: str) -> str | None:
    """Return the stored `curie tools` provider string for a config section.
...
# config check (curie_cli.config.validate_config_structure) and
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    if note:
        failure = note
    return (
        f"{section} is configured to use {selection_name} (set via curie "
        f"tools), but {failure}. Run 'curie tools' to change it."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.curie/.env`` (via
    ``curie_cli.config.get_env_value`` when available) so tool-side
