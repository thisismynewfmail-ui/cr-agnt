...
from curie_cli.config import cfg_get
...
# Freeze UNLOCK mode at module import time. Reading os.environ on every call
# would allow any skill running inside the process to set this variable and
# instantly bypass all approval checks — a prompt-injection escalation path.
_UNLOCK_MODE_FROZEN: bool = is_truthy_value(os.getenv("CURIE_UNLOCK_MODE", ""))
...
# Curie session id (observability identity, distinct from the gateway
...
# os.environ["CURIE_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export CURIE_INTERACTIVE.
_curie_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "curie_interactive",
    default=None,
)


def set_curie_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["CURIE_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``CURIE_INTERACTIVE`` env var for legacy callers.
    """
    return _curie_interactive_ctx.set("1" if interactive else "")


def reset_curie_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_curie_interactive_context`."""
    _curie_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``CURIE_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _curie_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("CURIE_INTERACTIVE")
...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    try:
        ...
        # Forward the Curie session id so observer plugins parent approval
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("CURIE_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("CURIE_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("CURIE_SESSION_PLATFORM", "") or ""
...
def _is_cron_approval_context() -> bool:
    ...
    try:
        ...
        return is_truthy_value(get_session_env("CURIE_CRON_SESSION", ""))
    except Exception:
        return env_var_enabled("CURIE_CRON_SESSION")
...
def _is_unattended_platform_approval_context() -> bool:
    ...
    ``CURIE_SESSION_PLATFORM`` like chat gateways do, but there is no human
...
def _is_single_query_approval_context() -> bool:
    ...
    ``curie chat -q "..."`` runs one turn and exits with no user waiting to
    answer approval prompts, but it still exports ``CURIE_INTERACTIVE=1`` so
    ...
    try:
        ...
        return is_truthy_value(get_session_env("CURIE_SINGLE_QUERY_SESSION", ""))
    except Exception:
        return env_var_enabled("CURIE_SINGLE_QUERY_SESSION")


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set CURIE_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind CURIE_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds CURIE_SESSION_PLATFORM via
    ...
    if _is_unattended_platform_approval_context():
        return False
    if env_var_enabled("CURIE_GATEWAY_SESSION"):
        return True
...
def _should_fall_through_to_cli_approval(
    *,
    ...
    ``CURIE_EXEC_ASK`` (and sometimes a session platform marker) can leak into
...
# via shell expansions like $HOME or $CURIE_HOME, or by the resolved absolute
# active profile home path such as /home/curie/.curie/config.yaml. The
# resolved-absolute form is folded into the ~/.curie/ patterns at detection
...
# go stale when CURIE_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_CURIE_ENV_PATH = (
    r'(?:~\/\.curie/|'
    r'(?:\$home|\$\{home\})/\.curie/|'
    r'(?:\$curie_home|\$\{curie_home\})/)'
    r'\.env\b'
)
# ~/.curie/config.yaml IS the security policy: approvals.mode, unlock, and the
...
# theater. Mirrors _CURIE_ENV_PATH; matches the CURIE_HOME override form as
# well as ~/.curie/.
_CURIE_CONFIG_PATH = (
    r'(?:~\/\.curie/|'
    r'(?:\$home|\$\{home\})/\.curie/|'
...
# of --unlock, /unlock, approvals.mode=off, or cron approve mode.  This is a
# floor below unlock: opting into unlock is the user trusting the agent with
...
# chmod -R 777, curl|sh) stay in DANGEROUS_PATTERNS where unlock can pass
# them through — that's what unlock is for.
#
...
def detect_hardline_command(command: str) -> tuple:
    ...
    Hardline patterns are NEVER bypassable, even in UNLOCK mode.
...
def _match_user_deny_rule(command: str) -> str | None:
    ...
    a deny match fires BEFORE the unlock / mode=off bypass. It is the
    user-editable counterpart to the code-shipped hardline blocklist:
    "never let the agent run this, even under unlock".
...
def _user_deny_block_result(pattern: str) -> dict:
    ...
    return {
        ...
        "message": (
            ...
            "executed via the agent — not even with --unlock, /unlock, or "
...
def _save_blocked_payload(command: str) -> Optional[str]:
    ...
    try:
        ...
        script_dir = get_curie_home() / "cache" / "blocked-scripts"
        ...
        path.write_text(
            "#!/bin/bash\n"
            "# Auto-saved by Curie: this command exceeded the inline command\n"
...
def _hardline_block_result(description: str, command: str = "") -> dict:
    ...
    message = (
        ...
        "be executed via the agent — not even with --unlock, /unlock, "
...
DANGEROUS_PATTERNS = [
    ...
    # These are native Windows EXEs / cmdlets reachable from ANY Curie
    ...
    (r'\bappdata[\\/](?:local|roaming)[\\/]curie[^\n]*\.env\b', "access to Curie secrets (Windows path)"),
    # ─────────────────────────────────────────────────────────────────────
    ...
    # `curie` and `gateway` (e.g. `curie -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bcurie\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart curie gateway (kills running agents)"),
    (r'\bcurie\s+update\b', "curie update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `curie gateway restart`
    ...
    # a lifecycle command past the guard — same treatment as the `curie ...
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart curie-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart curie-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(curie|gateway|cli\.py)\b', "kill curie/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill curie` but not
    # `kill -9 $(pgrep -f curie)` because the substitution is opaque
    ...
    # the `curie gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.curie.gateway`).
    # Catch the operations that stop, restart, or unload it.
    #
    # Order-independent (2026-08-02 incident): the previous version required
    # "curie"/"ai.curie" to appear AFTER the launchctl verb in the same
    ...
    # 'ai.curie.gateway-apollo:...' ...; do label=${item%%:*}; launchctl
    # bootout "$label"; done` — never has the literal text "curie" appear
    ...
    # an unrelated "curie" mention now also matches) — for an approval gate
    # that's the correct direction to err: an extra approval prompt is
    # cheap, a missed one took down the whole gateway fleet.
    (r'(?=[\s\S]*\blaunchctl\s+(?:stop|kickstart|bootout|unload|kill|disable|remove)\b)(?=[\s\S]*\b(?:curie|ai\.curie)\b)', "stop/restart curie launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Curie file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.curie/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Curie-managed security file (~/.curie/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_CURIE_CONFIG_PATH}|{_CURIE_ENV_PATH})', "in-place edit of Curie config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_CURIE_CONFIG_PATH}|{_CURIE_ENV_PATH})', "in-place edit of Curie config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_CURIE_CONFIG_PATH}|{_CURIE_ENV_PATH})', "in-place edit of Curie config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # be bypassed even with unlock). Handles both \n and \r\n line endings. Line
    ...
    # ~/ and ~/.curie/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / CURIE_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Curie home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\curie), so folding the user home
    # first would eat the prefix the Curie-home fold needs.
    command = _rewrite_resolved_curie_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / CURIE_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.curie``); the matched
...
def _rewrite_resolved_curie_home(command: str) -> str:
    """Rewrite the resolved absolute Curie home prefix to ``~/.curie/``.

    Resolves the active ``CURIE_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.curie/`` so the static ``_CURIE_CONFIG_PATH`` / ``_CURIE_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/curie/.curie/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$CURIE_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from curie_constants import get_curie_home
        home = get_curie_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.curie")
...
def _mask_quoted_newlines(command: str) -> str:
    ...
    start. That made any multi-line quoted argument (``curie send`` message
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Curie ad-hoc temp script."""
    try:
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"curie-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None


_GATEWAY_LIFECYCLE_SPLICE_DESCRIPTION = (
    "stop/restart curie gateway via shell-spliced verb (kills running agents)"
)
...
def _is_shell_token_spliced_gateway_lifecycle(command: str) -> bool:
    ...
    ``launchctl kick"start" -k gui/501/ai.curie.gateway`` auto-approved
    ...
    anchored on a curie-gateway identifier — reusing its prose
...
def _release_permission_mode_dependents(session_key: str) -> None:
    """Drop resources whose immutable mode is derived from Curie UNLOCK.

    The import stays lazy so approval-only sessions do not load computer-use.
    Releasing on both edges makes enabling UNLOCK replace an existing standard
    backend and makes disabling UNLOCK revoke a private unrestricted daemon
...
def enable_session_unlock(session_key: str) -> None:
    """Enable UNLOCK bypass for a single session key."""
    if not session_key:
        return
    with _lock:
        _session_unlock.add(session_key)
    _release_permission_mode_dependents(session_key)


def disable_session_unlock(session_key: str) -> None:
    """Disable UNLOCK bypass for a single session key."""
    if not session_key:
        return
    with _lock:
        _session_unlock.discard(session_key)
    _release_permission_mode_dependents(session_key)


def clear_session(session_key: str) -> None:
    """Remove all approval and unlock state for a given session."""
    if not session_key:
        return
    with _lock:
    ...
    # at the same boundary that clears the session's approval and unlock
...
def is_session_unlock_enabled(session_key: str) -> bool:
    """Return True when UNLOCK bypass is enabled for a specific session."""
    if not session_key:
        return False
    with _lock:
        return session_key in _session_unlock


def is_current_session_unlock_enabled() -> bool:
    """Return True when the active approval session has UNLOCK bypass enabled."""
    return is_session_unlock_enabled(get_current_session_key(default=""))
...
def _prompt_dangerous_approval_inner(command: str, description: str,
    ...
    os.environ["CURIE_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "CURIE_SPINNER_PAUSE" in os.environ:
            del os.environ["CURIE_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active_for_session(session_key: str) -> bool:
    """Return whether one exact session bypasses Curie approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--unlock`` / ``CURIE_UNLOCK_MODE`` (frozen at import time
        so a mid-process skill can't flip it — a prompt-injection escalation
        path; see ``_UNLOCK_MODE_FROZEN`` above),
      - the session-scoped gateway ``/unlock`` toggle,
    ...
    return (
        _UNLOCK_MODE_FROZEN
        or is_session_unlock_enabled(session_key)
...
def _run_approval_gate(
    *,
    ...
    unlock bypass → session-cache short-circuit → interactive/gateway/cron
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            CURIE_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    """
    # --unlock bypasses all approval prompts (session- or process-scoped).
    # Hardline blocks are handled by the caller BEFORE this gate, so unlock
    # here only skips the recoverable approval layer.
    if _UNLOCK_MODE_FROZEN or is_current_session_unlock_enabled():
    ...
    # Single-query (-q) sessions export CURIE_INTERACTIVE=1 but have no user
    ...
    if not is_cli and not is_gateway:
        ...
        if _is_cron_approval_context():
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set CURIE_INTERACTIVE or "
                "CURIE_GATEWAY_SESSION to answer the prompt.",
                autoapprove_log_prefix, pattern_key, description,
            )
            return {
                "approved": False,
        ...
        logger.warning(
            "%s (pattern: %s): %s — set CURIE_INTERACTIVE or "
            "CURIE_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("CURIE_EXEC_ASK"):
        ...
        # can see (CURIE_EXEC_ASK / platform-marker leaks into CLI).
...
def check_dangerous_command(command: str, env_type: str,
    ...
    # unconditionally, BEFORE the unlock bypass.  Opting into unlock is
    ...
    # hardline floor, these fire BEFORE the unlock bypass — a deny rule is the
    # user saying "never, even under unlock".
    ...
    # --unlock: bypass all approval prompts. Gateway /unlock is session-scoped;
    # CLI --unlock remains process-scoped via the env var for local use.
    if _UNLOCK_MODE_FROZEN or is_current_session_unlock_enabled():
        return {"approved": True, "message": None}
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``CURIE_INTERACTIVE``) fails CLOSED — a plugin-
...
def get_plugin_manager():
    ...
    from curie_cli.plugins import discover_plugins, get_plugin_manager as _get_manager
...
def _present_with_selected_transport(
    *,
    ...
    try:
        from agent.redact import redact_sensitive_text
        from curie_cli.approval_transport import ApprovalRequest, invoke_approval_transport
...
def check_all_command_guards(command: str, env_type: str,
    ...
    # kill -1). Applies BEFORE unlock / mode=off / cron approve-mode so
    ...
    # SUDO_PASSWORD has been configured.  This must fire BEFORE the unlock
    # check so even unlock/smart approval/mode=off cannot bypass it.
    ...
    # User-defined deny rules (approvals.deny in config.yaml): like the
    # hardline floor, these fire BEFORE the unlock / mode=off bypass — a deny
    # rule is the user saying "never, even under unlock".
    deny_pattern = _match_user_deny_rule(command)
    if deny_pattern is not None:
        logger.warning("User deny rule %r blocked command: %s",
                       deny_pattern, command[:200])
    ...
    # --unlock or approvals.mode=off: bypass all approval prompts.
    # Gateway /unlock is session-scoped; CLI --unlock remains process-scoped.
    approval_mode = _get_approval_mode()
    if _UNLOCK_MODE_FROZEN or is_current_session_unlock_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("CURIE_EXEC_ASK")
    ...
    if _is_single_query_approval_context():
        is_cli = False
        is_gateway = False
        # CURIE_EXEC_ASK routes through the gateway decision loop (no human
        ...
        if _is_single_query_approval_context():
            if _get_single_query_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from curie_cli.config import load_config_readonly as _load_cfg
...
def check_execute_code_guard(code: str, env_type: str,
    ...
    # --unlock or approvals.mode=off: bypass (session- or process-scoped).
    approval_mode = _get_approval_mode()
    if _UNLOCK_MODE_FROZEN or is_current_session_unlock_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}
    ...
    # Ask-mode (CURIE_EXEC_ASK) still takes this path even when INTERACTIVE
    ...
    if notify_cb is None:
        # CURIE_EXEC_ASK (and sometimes a session platform marker) can leak
