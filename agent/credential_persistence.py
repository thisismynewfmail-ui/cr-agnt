...
``auth.json``.  They intentionally have no dependency on ``curie_cli.auth`` so
...
# Sources Curie owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Curie-owned OAuth/device-code state)
