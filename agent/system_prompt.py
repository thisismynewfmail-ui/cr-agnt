...
upstream prefix cache warm.  See ``curie-agent-dev``'s
...
_PLUGIN_SECTION_FRAME_RE = re.compile(
    ...
    r"<!-- curie-plugin-section-chars:(?P<chars>[0-9]{1,4}) -->\n\n",
    re.MULTILINE,
)
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``CURIE_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("CURIE_DESKTOP_TERMINAL")):
        return hint
...
def _plugin_session_info(agent: Any) -> Dict[str, str]:
    ...
    try:
        ...
        # the CURIE_HOME ContextVar (#86313 class; plugin half per @helix4u).
...
def _frozen_plugin_prompt_sections(agent: Any) -> tuple:
    ...
    if isinstance(stored_prompt, str) and stored_prompt:
        ...
        return rendered
    try:
        from curie_cli.plugins import render_system_prompt_sections
...
def _plugin_section_blocks(sections: tuple, position: str) -> List[str]:
    from curie_cli.plugins import format_system_prompt_sections
...
def _agent_home(agent: Any) -> Optional[Path]:
    ...
    1. A bound CURIE_HOME ContextVar override wins. Surfaces that multiplex
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Curie never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want CURIE_HOME persona while keeping
    # cwd project instructions disabled.
    _soul_loaded = False
    if agent.load_soul_identity or not agent.skip_context_files:
        # Scope the SOUL.md read to the agent's OWN home (see _agent_home) —
        # ambient resolution on a thread that lost the CURIE_HOME ContextVar
    ...
    # Pointer to the docs (and, when it exists, the curie-agent skill) for
    # user questions about Curie itself. The skill_view() pointer is a
    # dangling reference in two cases — no skill tools in the toolset
    # (Blank Slate) OR the curie-agent skill not installed — so the
    ...
    stable_parts.append(CURIE_AGENT_HELP_GUIDANCE_NO_SKILLS)
    ...
    # CURIE_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # the curie-agent skill actually present in the index (gating on the
    # rendered index line keeps this a pure string check — no second
    # filesystem scan, and it inherits the index cache's stability).
    if _has_skill_view and "- curie-agent:" in skills_prompt:
        stable_parts[_help_guidance_slot] = CURIE_AGENT_HELP_GUIDANCE
    ...
    # Coding posture (base Curie, any interactive coding surface in a code
    ...
    # `curie -p <bot> chat --in ~ -c "Bot Chat"` and the desktop pins), on
    # installs where Bot Mode manages profiles (ui_meta['curie-bots']).
    ...
    # Active-profile hint — names the Curie profile the agent is running
    # under so it doesn't conflate ~/.curie/skills/ (default profile) with
    # ~/.curie/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    # ambient CURIE_HOME: on a build thread that lost the ContextVar this
    ...
    # from get_default_curie_root(): get_curie_home() on a bound profile
    ...
    # to the legacy behavior (and patchable via this module's get_curie_home).
    if _agent_home_path is not None:
        _home_str = str(_agent_home_path)
        _root_str = str(get_default_curie_root())
    else:
        _home_str = _root_str = str(get_curie_home())
    if active_profile == "default":
        post_workspace_parts.append(
            "Active Curie profile: default. Other profiles (if any) live "
        ...
        # data sits at the ROOT (get_default_curie_root()), which in ambient
        # profile mode is NOT get_curie_home().
        profile_home = _home_str
        default_root = get_default_curie_root()
        post_workspace_parts.append(
            f"Active Curie profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Curie). Every other surface (desktop chat panel,
        ...
        if getattr(agent, "_context_cwd_is_launch_artifact", False):
            ...
            # then reject Curie's bundled contributor AGENTS.md (#97448).
            context_cwd = None
    ...
    from curie_time import get_timezone as _curie_tz, now as _curie_now
    now = _curie_now()
