...
def _repair_tool_call_arguments(raw_args: str, tool_name: str = "?") -> str:
    ...
    Callers that need to tell "repaired successfully" apart from "gave up and
    substituted an empty object" must use :func:`repair_tool_call_arguments_ex`
    — a bare ``"{}"`` return from this function is ambiguous, because an empty
    object is also a perfectly legitimate repair result (a no-argument tool
    call, or ``None``/whitespace normalisation).
    """
    return repair_tool_call_arguments_ex(raw_args, tool_name).arguments


class RepairedToolArguments(NamedTuple):
    """Outcome of a tool-call argument repair attempt.

    ``arguments``  wire-valid JSON string, always safe to send.
    ``repaired``   True when the input needed (and received) surgery.
    ``recovered``  True when ``arguments`` still carries what the model asked
                   for.  The repair ladder has two kinds of pass:

                   *Lossless* passes re-serialise a string that parsed in
                   full (control-char tolerance, escaping, ``None``
                   normalisation).  Nothing is dropped, so ``recovered`` is
                   always True — including when the faithful result happens
                   to be ``{}``, which is exactly right for a parameterless
                   tool call.

                   *Lossy* passes fabricate the missing tail of a truncated
                   string (closing unbalanced braces, trimming excess ones)
                   or give up entirely.  There ``recovered`` reports whether
                   anything survived: a salvaged ``{"path": "/tmp/x"}`` is
                   worth sending, whereas a lone ``"{"`` closed into ``"{}"``
                   salvaged nothing and means the arguments are genuinely
                   lost.
    """
...
def repair_tool_call_arguments_ex(
    raw_args: str, tool_name: str = "?"
) -> RepairedToolArguments:
    """Repair malformed tool_call argument JSON, reporting what happened.

    Same repair ladder as :func:`_repair_tool_call_arguments`, but the return
    value distinguishes a successful repair from the last-resort empty-object
    substitution.  Truncation detection in the streaming path depends on that
    distinction: treating "legitimately empty" as "truncated" is what produced
    spurious "stream stalled mid tool-call" reports.
    """
    raw_stripped = raw_args.strip() if isinstance(raw_args, str) else ""

    # Fast-path: empty / whitespace-only -> empty object
    if not raw_stripped:
        logger.debug("Empty tool_call arguments for %s -> {}", tool_name)
        return RepairedToolArguments("{}", repaired=False, recovered=True)

    # Python-literal None -> normalise to {}
    if raw_stripped == "None":
        logger.warning("Sanitized Python-None tool_call arguments for %s", tool_name)
        return RepairedToolArguments("{}", repaired=True, recovered=True)
    ...
    try:
        ...
        _changed = reserialised != raw_stripped
        if _changed:
            logger.warning(
                "Repaired unescaped control chars in tool_call arguments for %s",
                tool_name,
            )
        return RepairedToolArguments(
            reserialised, repaired=_changed, recovered=True
        )
    ...
    try:
        _salvaged = json.loads(fixed)
        logger.warning(
            "Repaired malformed tool_call arguments for %s: %s → %s",
            tool_name, raw_stripped[:80], fixed[:80],
        )
        # Lossy pass: the terminators above were fabricated, so the tail of
        # the model's arguments may be missing.  Anything non-empty is still
        # worth sending; an empty container means the surgery closed a bare
        # "{" and salvaged nothing at all.
        return RepairedToolArguments(
            fixed, repaired=True, recovered=bool(_salvaged),
        )
    except json.JSONDecodeError:
        pass
    ...
    try:
        ...
        if escaped != fixed:
            _salvaged = json.loads(escaped)
            logger.warning(
                "Repaired control-char-laced tool_call arguments for %s: %s → %s",
                tool_name, raw_stripped[:80], escaped[:80],
            )
            # ``fixed`` has already been through the lossy structural pass,
            # so the same salvage test applies here.
            return RepairedToolArguments(
                escaped, repaired=True, recovered=bool(_salvaged),
            )
    except (json.JSONDecodeError, TypeError, ValueError):
        pass
    ...
    return RepairedToolArguments("{}", repaired=True, recovered=False)
...
def serialized_messages_bytes(messages: list) -> int:
    ...
    error, but Curie' context estimator deliberately prices an image at a
