"""OpenAI-shape bridge shared by Curie' ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach Curie as
...
the agent's *own* tool notifications. Curie' agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from Curie' perspective).
...
of its own (Copilot) forwards everything Curie offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only Curie'
agent-level tools, because re-offering the overlapping ones makes Curie re-run
...
class StreamChunks(list):
    ...
    Curie reads provider-level extras off the object returned by
    ``chat.completions.create`` (e.g. ``curie_projected_messages``, consumed by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just Curie'
