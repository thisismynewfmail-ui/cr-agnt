"""Upload a Curie session transcript to Hugging Face as an agent trace.

Curie stores sessions in its own SQLite store (``curie_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``curie trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Curie' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "curie-traces"
_CURIE_VERSION = "curie-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Curie OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Curie' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Curie conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.curie/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `curie trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/curie-traces`` dataset. Returns a
