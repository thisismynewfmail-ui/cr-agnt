...
def _resolve_requests_verify(base_url: str = "") -> bool | str:
    ...
    3. Env vars ``CURIE_CA_BUNDLE`` / ``REQUESTS_CA_BUNDLE`` / ``SSL_CERT_FILE``
    ...
    for env_var in ("CURIE_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _local_probe_disk_cache_path() -> Path:
    ...
    return get_curie_home() / "cache" / "local_endpoint_probes.json"
...
def _get_model_metadata_cache_path() -> Path:
    ...
    from curie_constants import get_curie_home
    return get_curie_home() / "cache" / "openrouter_model_metadata.json"
...
def _get_endpoint_metadata_cache_path() -> Path:
    ...
    from curie_constants import get_curie_home
    return get_curie_home() / "cache" / "endpoint_model_metadata.json"
...
def _endpoint_disk_cache_get(normalized: str) -> Optional[Dict[str, Dict[str, Any]]]:
    ...
    (``curie -q``, cron, every Bot Mode DM hop) start cold and re-probed the
...
# Minimum context length required to run Curie Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Curie from probing-down to
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    # (often 8K) makes it unusable for Curie' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Curie minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from curie_constants import get_curie_home
    return get_curie_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Curie grow conversations past the runtime limit and Ollama
...
# who never asked for it. The variant suffix is a Curie-side alias: it is
...
# Curie-side picker suffix that opts a Codex slug into the live-verified
...
def strip_codex_context_variant_suffix(model: Optional[str]) -> str:
    ...
    The suffix is a Curie picker alias (``gpt-5.6-sol-900k``); the Codex
...
def _resolve_codex_oauth_context_length_with_source(
    ...
    # ``-900k`` variants are Curie picker aliases — the Codex catalog only
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from curie_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Curie sends to providers:
