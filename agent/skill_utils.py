...
from curie_constants import get_config_path, get_skills_dir, is_termux
...
# Org mirrors live under ~/.curie/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    CURIE_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``CURIE_KANBAN_TASK`` / ``CURIE_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("CURIE_KANBAN_TASK") or os.getenv("CURIE_KANBAN_BOARD"):
        ...
        # The Curie Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``curie_cli.config`` on the
...
# `curie-agent` skill is the agent's own operating manual — it drives
# configuring, extending, and troubleshooting Curie itself, and the system
# prompt unconditionally points at it. Disabling it leaves the agent unable
# to help with Curie, so disable requests for these names are ignored
# everywhere the disabled list is consulted.
ESSENTIAL_SKILLS: frozenset = frozenset({"curie-agent"})
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``CURIE_PLATFORM`` or
            ``CURIE_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("CURIE_PLATFORM")
        or get_session_env("CURIE_SESSION_PLATFORM")
    )
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``curie config set`` and JSON-mode editor saves store lists as quoted
...
# which becomes the dominant cost of ``curie`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.curie/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``curie`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against CURIE_HOME, not cwd
        if not p.is_absolute():
            p = (curie_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    this directory instead of the profile-local ``~/.curie/skills/``, and
    ...
    against CURIE_HOME.  A value that resolves to the local skills dir is
...
def display_skill_create_dir() -> str:
    ...
    if create_dir is None:
        return f"{display_curie_home()}/skills/"
    try:
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.curie/skills/`` first, then external.
    ...
    NOTE: trusted project-local dirs (``./.curie/skills`` at the git root) are
...
#   <root>/.curie/skills/   — Curie-native location
...
# a one-line "run `curie skills trust`" notice.
#
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    The exclusion matters when CURIE_HOME itself lives inside a git checkout:
    ``<root>/.curie/skills`` would otherwise double as both the profile-local
...
def get_untrusted_project_skills_root() -> Optional[Tuple[Path, int]]:
    ...
    ``curie skills trust``. Returns None when there is nothing to notify
...
#
# Trust (`curie skills trust`) is a REPO-level decision made once; the repo's
...
#
# The scan cache lives under CURIE_HOME, never inside the repo (we don't
...
def _project_scan_cache_dir() -> Path:
    ...
    return get_curie_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.curie/skills/`` (including via symlinks) or configured
    ...
    # ~/.curie/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Curie can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    curie = metadata.get("curie") or {}
    if not isinstance(curie, dict):
        curie = {}
    return {
        "fallback_for_toolsets": curie.get("fallback_for_toolsets", []),
        "requires_toolsets": curie.get("requires_toolsets", []),
        "fallback_for_tools": curie.get("fallback_for_tools", []),
        "requires_tools": curie.get("requires_tools", []),
        ...
        "session_platforms": curie.get("session_platforms", []),
    }
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Curie metadata, VCS, virtualenv/dependency, cache, and skill
