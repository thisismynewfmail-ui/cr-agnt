...
def _ensure_file_checkpoint(
    ...
    # which can differ from the Curie process cwd (notably in Docker).  Resolve
...
def _resolve_concurrent_tool_timeout() -> float | None:
    ...
    in config.yaml wins, the legacy ``CURIE_CONCURRENT_TOOL_TIMEOUT_S`` env var
    ...
    return resolve_timeout(
        ...
        env_var="CURIE_CONCURRENT_TOOL_TIMEOUT_S",
    )


# ── Tools that own their own deadline ────────────────────────────────────
#
# The generic deadline above is a backstop for a tool that has no opinion
# about how long it should take: a page fetch, a shell command, a file read.
# A handful of tools are not that. They wait on a second model, they publish
# their own budget for it, and a user who raises that budget is telling the
# system how long that work is allowed to run.
#
# The vision tools are the case this exists for. ``auxiliary.vision.timeout``
# shipped at an hour and was raised to two, while this deadline stayed at
# seven minutes — so the tool was cancelled with ``tool_timeout`` at 420s no
# matter what the vision setting said, and the setting looked like it did
# nothing. Worse, the same call reached through the gateway (a Telegram photo
# goes to ``vision_analyze_tool`` directly, not through this executor) had no
# executor deadline at all and worked, which made the failure look like a
# terminal-only bug in vision rather than a deadline two layers up.
#
# So: a tool named here gets its own budget, read from the config key that
# owns it, plus a margin for the work around the model call — downloading the
# image, decoding it, resizing an oversized one, base64-encoding the result.
# The two can no longer drift, because there is only one number and this
# reads it.
#
#   tool name -> (dotted config key of the owning budget, margin in seconds)
_TOOL_OWNED_BUDGETS: dict[str, tuple[tuple[str, ...], float]] = {
    "vision_analyze": (("auxiliary", "vision", "timeout"), 180.0),
    "video_analyze": (("auxiliary", "vision", "timeout"), 300.0),
}

#: Explicit per-tool override, for anything not in the table above:
#: ``timeouts: {tools: {per_tool: {shell: 900}}}``. Wins over both.
_PER_TOOL_TIMEOUT_PREFIX = "tools.per_tool"


#: What :func:`_resolve_owned_tool_timeout` returns for a tool whose owning
#: budget is explicitly unbounded. It cannot be ``None``: that already means
#: "this tool makes no claim, use the generic deadline", and conflating the
#: two put a tool configured for *no* ceiling straight back under the generic
#: one — the exact failure this table exists to remove, one layer down.
UNBOUNDED_TOOL_TIMEOUT = 0.0


def _resolve_owned_tool_timeout(function_name: str) -> float | None:
    """The deadline a named tool is entitled to.

    Three sources, most specific first: an explicit ``timeouts.tools.per_tool``
    entry, the budget the tool itself owns (:data:`_TOOL_OWNED_BUDGETS`), or
    nothing.

    Returns ``None`` when the tool makes no claim (the caller uses the generic
    deadline), :data:`UNBOUNDED_TOOL_TIMEOUT` when its owning budget is
    explicitly unbounded, and otherwise the seconds it is entitled to.
    """
    ...
    explicit = resolve_timeout(
        f"{_PER_TOOL_TIMEOUT_PREFIX}.{function_name}", default=None
    )
    if explicit is not None:
        return explicit
    if _has_explicit_per_tool_entry(function_name):
        # Written down and resolved to "unbounded": resolve_timeout normalizes
        # 0/negative to None, which here is indistinguishable from absent.
        return UNBOUNDED_TOOL_TIMEOUT

    owned = _TOOL_OWNED_BUDGETS.get(function_name)
    if owned is None:
        return None
    path, margin = owned
    try:
        ...
        raw = cfg_get(load_config(), *path)
    except Exception:
        # An unreadable config is not a reason to shorten a tool's leash to
        # something it was never designed for; fall through to the generic
        # deadline, which is what happened before this table existed.
        return None
    ...
    try:
        ...
        logger.warning(
            "%s: %s is not a number (%r); using the generic tool deadline",
            function_name, ".".join(path), raw,
        )
        return None
    if budget <= 0:
        # The owning budget is explicitly unbounded, so this deadline must be
        # too — otherwise the generic one silently becomes the real limit,
        # which is the whole failure this table exists to remove.
        return UNBOUNDED_TOOL_TIMEOUT
    return clamp_timeout(budget + max(0.0, margin))


def _has_explicit_per_tool_entry(function_name: str) -> bool:
    """Whether ``timeouts.tools.per_tool.<name>`` is written down at all."""
    try:
        from agent.deadline import _lookup_dotted, _timeouts_section

        return (
            _lookup_dotted(
                _timeouts_section(), f"{_PER_TOOL_TIMEOUT_PREFIX}.{function_name}"
            )
            is not None
        )
    except Exception:
...
def _resolve_batch_tool_timeout(function_names) -> float | None:
    """The deadline for a concurrent batch: the longest any member claims.

    A batch that contains one tool with its own budget must not be cut at the
    generic deadline, because that would cancel every tool in it — including
    the one whose budget said it was still allowed to be working.
    """
    generic = _resolve_concurrent_tool_timeout()
    if generic is None:
        return None
    longest = float(generic)
    for name in function_names or ():
        resolved = _resolve_tool_call_timeout(name, generic)
        if resolved is None:
            # A member is unbounded, so the batch cannot bound it either.
            return None
        longest = max(longest, float(resolved))
    return longest


def _resolve_tool_call_timeout(
    function_name: str, generic: float | None
) -> float | None:
    """The deadline for one call: the larger of the generic and the tool's own.

    ``max``, not "the tool's if it has one": the generic deadline can be
    raised by config or by an env var, and a user who has done that has asked
    for *more* time, not for a tool with its own budget to get less.

    ``None`` out means unbounded, which every caller here already understands
    — so a tool whose budget says "no ceiling" gets none, instead of quietly
    falling back under the generic deadline.
    """
    owned = _resolve_owned_tool_timeout(function_name)
    if owned is None:
        return generic
    if owned == UNBOUNDED_TOOL_TIMEOUT:
        return None
    if generic is None:
        # Generic is already unbounded; nothing to raise it to.
        return None
    return max(float(generic), float(owned))
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Curie process before the normal turn-end persistence path runs.
    ...
    try:
        ...
        agent._incremental_persistence_failed = True
        from curie_state import classify_persistence_error
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    """Run Relay rewrites before Curie policy and dispatch exactly once."""
    from agent import relay_tools
    from curie_cli.middleware import (
    ...
    def _authorized_dispatch(final_args: dict[str, Any]) -> Any:
        with dispatch_lock:
            if state["dispatched"]:
                raise RuntimeError(
                    "Curie tool execution callback invoked more than once"
                )
        ...
        if block_message is None:
            ...
            def _resolve_pre_tool_block():
                nonlocal final_args
                try:
                    from curie_cli.plugins import _dispatch_pre_tool_call_hooks
    ...
    def _curie_pipeline(relay_args: dict[str, Any]) -> Any:
...
def _resolve_sequential_tool_timeout() -> float | None:
    ...
    ``CURIE_CONCURRENT_TOOL_TIMEOUT_S`` legacy bridge) so the two executor
...
def _run_sequential_tool_execution_middleware(
    ...
    *,
    ...
    The same is true of a tool that waits on a second *model* rather than on a
    person — the vision tools and their ``auxiliary.vision.timeout`` — so the
    deadline is resolved per call rather than once for every tool alike. See
    :data:`_TOOL_OWNED_BUDGETS`.
    """
    timeout_s = _resolve_tool_call_timeout(
        function_name, _resolve_sequential_tool_timeout()
    )
    kwargs = {
        "function_name": function_name,
        "function_args": function_args,
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    #
    # One deadline covers the whole batch, so it has to be the longest any
    # member is entitled to: the workers run in parallel and the batch cannot
    # finish before its slowest one. A batch holding a vision call therefore
    # waits on the vision budget, and the cheap tools beside it get the same
    # leash — an accepted cost, and the alternative (per-worker deadlines
    # inside one batch wait) buys nothing here, since every still-running tool
    # is reported individually when the deadline does fire.
    timeout_s = _resolve_batch_tool_timeout(
        [name for _tc, name, *_rest in parsed_calls]
    )
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if function_name == "todo_list":
            ...
            def _execute(next_args: dict) -> Any:
                ...
                if not session_db:
                    from curie_state import format_session_db_unavailable
