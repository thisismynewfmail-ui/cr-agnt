...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Curie with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.curie/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Curie
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``curie tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``curie setup`` for
