...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the CURIE_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Curie runtime / deployment
    "CURIE_HOME", "CURIE_PROFILE", "CURIE_GATEWAY_LOCK_DIR",
    "CURIE_MAX_ITERATIONS", "CURIE_MAX_TOKENS", "CURIE_API_TIMEOUT",
    "CURIE_REDACT_SECRETS", "CURIE_NOUS_TIMEOUT_SECONDS",
    "_CURIE_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "CURIE_KANBAN_DB", "CURIE_KANBAN_WORKSPACES_ROOT", "CURIE_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "CURIE_KANBAN_",
    "CURIE_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Curie writes
    ...
    ``curie_cli.config._parse_env_value``) but never mutates the process
    ...
    # Parse values with the canonical Curie parser: save_env_value
    ...
    from curie_cli.config import _parse_env_value
...
def build_profile_secret_scope(curie_home: Path) -> Dict[str, str]:
    ...
    """
    home = Path(curie_home)
    secrets = load_env_file(home / ".env")

    try:
        from curie_cli.env_loader import get_secret_source_values
