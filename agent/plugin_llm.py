...
Plugins built on Curie Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Curie
...
:class:`~curie_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Curie' main config
...
handles every provider, fallback chain, and per-task override Curie
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`curie_cli.plugins.PluginContext`
