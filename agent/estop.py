...
``curie pause`` writes a sentinel file at ``$CURIE_HOME/ESTOP``;
``curie resume`` removes it. While the sentinel exists:
...
* new gateway turns get a brief "Curie is paused" reply instead of an
...
hold even if the file was created by ``touch ~/.curie/ESTOP``.
...
def _curie_home() -> Path:
    """Resolve the active CURIE_HOME (profile-aware) at call time."""
    try:
...
def _canonical_root() -> Path:
    """Fleet-wide Curie root, even when this process is a profile gateway.

    Profile gateways launch with CURIE_HOME=~/.curie/profiles/<name>.
    ``curie pause`` from an operator seat writes ~/.curie/ESTOP. If we
    ...
    try:
        ...
        return Path(get_default_curie_root())
    except Exception:
        return Path(os.path.expanduser("~/.curie"))


def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `curie pause`."""
    return _curie_home() / SENTINEL_NAME
...
def is_engaged() -> bool:
    ...
    Engaged if ANY candidate sentinel exists: the process CURIE_HOME
    (profile-local) or the fleet canonical root (~/.curie). Fail SAFE on
...
def disengage() -> bool:
    ...
    ``curie resume`` from a profile gateway still clears an operator pause
    written at ~/.curie/ESTOP.
    """
    lifted = False
    for path in _candidate_sentinel_paths():
        try:
...
def paused_reply() -> Optional[str]:
    ...
    if reason:
        return (
            f"⏸️ Curie is paused ({reason}). New work is on hold; "
            "run `curie resume` to pick things back up."
        )
    return (
        "⏸️ Curie is paused. New work is on hold; "
        "run `curie resume` to pick things back up."
    )
