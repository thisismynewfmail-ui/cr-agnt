"""Fold an agent-as-provider's own activity back into Curie' turn state.

Most providers are models: they ask Curie to run a tool and Curie runs it, so
...
time Curie sees the response that work is already done.

Those calls must never come back as pending ``tool_calls`` — Curie would re-run
...
* the **skill-review nudge**, whose counter (``_iters_since_skill``) only moves
  on Curie tool iterations, of which there are none.

So the provider client hands both back on the completion object and this helper
applies them: ``curie_projected_messages`` (already-completed
``assistant(tool_calls=[…])`` + ``tool(result)`` history rows) and
``curie_provider_tool_iterations`` (how many tool iterations happened inside
...
def splice_provider_projection(
    ...
    """
    projected = getattr(response, "curie_projected_messages", None)
    ...
    raw_iters = getattr(response, "curie_provider_tool_iterations", 0)
    try:
