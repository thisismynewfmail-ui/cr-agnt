...
cache problem must never block Curie startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Curie home used for cache paths.

    ``home_path`` is whatever ``load_curie_dotenv()`` already resolved;
    falling back to ``$CURIE_HOME`` / ``~/.curie`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        from curie_constants import get_curie_home

        home_path = get_curie_home()
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<curie_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if CURIE_HOME is ever made traversable.
            try:
