...
def enable_happy_eyeballs_on_client(client) -> None:
    ...
    refresh / device-login / usage-probe clients in ``curie_cli.auth``.
...
class _SafeWriter:
    ...
    When curie-agent runs as a systemd service, Docker container, or headless
...
def build_keepalive_http_client(
    ...
    *,
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` and ``CURIE_CA_BUNDLE`` settings the main
