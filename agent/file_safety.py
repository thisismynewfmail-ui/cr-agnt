...
def _curie_home_path() -> Path:
    """Resolve the active CURIE_HOME (profile-aware) without circular imports."""
    try:
        from curie_constants import get_curie_home  # local import to avoid cycles
        return get_curie_home()
    except Exception:
        return Path(os.path.expanduser("~/.curie"))


def _curie_root_path() -> Path:
    """Resolve the Curie root dir (always the parent of any profile, never per-profile)."""
    try:
        from curie_constants import get_default_curie_root  # local import to avoid cycles
...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    curie_home = _curie_home_path()
    curie_root = _curie_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(curie_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(curie_root / ".anthropic_oauth.json"),
            # Bitwarden Secrets Manager encrypted disk cache.
            str(curie_home / "cache" / "bws_cache.enc.json"),
            str(curie_root / "cache" / "bws_cache.enc.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved CURIE_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("CURIE_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def _classify_write_denial(path: str) -> Optional[str]:
    ...
    curie_dirs = []
    for base in (_curie_home_path(), _curie_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in curie_dirs:
                curie_dirs.append(real)
        except Exception:
            continue

    for base_real in curie_dirs:
...
def get_write_denied_error(path: str, *, verb: str = "Write") -> Optional[str]:
    ...
    if denial == "safe_root":
        ...
        return (
            f"{verb} denied: '{path}' is outside CURIE_WRITE_SAFE_ROOT "
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Curie path.

    Three categories are blocked:

      * Internal Curie cache files under ``CURIE_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under CURIE_HOME and the global Curie
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.curie/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active CURIE_HOME (profile-aware) AND the global
    # Curie root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (CURIE_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    curie_dirs: list[Path] = []
    for base in (_curie_home_path(), _curie_root_path()):
        try:
            real = base.resolve()
            if real not in curie_dirs:
    ...
    for hd in curie_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Curie cache file "
    ...
    for hd in curie_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Curie credential store "
    ...
    for hd in curie_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Curie MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Curie MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
    ...
    for hd in curie_dirs:
        ...
        if resolved == browser_profile:
            return (
                f"Access denied: {path} is the Curie real-profile browser "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is inside the Curie real-profile browser "
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Curie read (see
...
#
# Curie profiles are separate CURIE_HOME dirs under
...
#
# Reference: May 2026 incident where a curie-security profile session
# edited skills under both ``~/.curie/profiles/curie-security/skills/``
# AND ``~/.curie/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under CURIE_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from CURIE_HOME.
    ...
    """
    try:
        home_real = _curie_home_path().resolve()
        root_real = _curie_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Curie scope, or is inside
...
#
#   <CURIE_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.curie/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.curie/…`` segment and warns
# regardless of which Curie profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.curie/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.curie`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.curie/…`` and returns the
    index where the inner Curie-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .curie / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".curie":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Curie state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.curie``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.curie`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Curie resolver to
    succeed, so it works correctly even when called from contexts where
    CURIE_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Curie process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real CURIE_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.curie/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.curie/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host CURIE_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Curie state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Curie "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real CURIE_HOME. Use the "
