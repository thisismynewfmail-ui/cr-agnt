"""Anthropic Messages API adapter for Curie Agent.

Translates between Curie's internal OpenAI-style message format and
...
# Curie effort → Anthropic adaptive-thinking effort (output_config.effort).
...
# Anthropic's OAuth billing classifier fingerprints certain Curie tool
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to curie's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to curie's outer conversation
    ...
    if _is_kimi_coding_endpoint(base_url):
        ...
        # HTTP-Referer + X-Title + CurieAgent User-Agent.
    ...
    if _is_opencode_endpoint(base_url):
        ...
        headers.setdefault("HTTP-Referer", "https://github.com/thisismynewfmail-ui/Cur-Agnt")
        headers.setdefault("X-Title", "Curie Agent")
        headers.setdefault("User-Agent", f"CurieAgent/{_CURIE_VERSION}")
        kwargs["default_headers"] = headers

    client = _anthropic_sdk.Anthropic(**kwargs)
    # Bearer-only construction leaves ``api_key`` unset, so the SDK fills it
    # from ``ANTHROPIC_API_KEY`` (Curie loads that into the process env from
    # ``~/.curie/.env``). The result is dual auth —
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Curie Agent", "Claude Code")
                text = text.replace("Curie agent", "Claude Code")
                text = text.replace("curie-agent", "claude-code")
        ...
        #      a) bare Curie-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Curie surfaces in its CLI. We
