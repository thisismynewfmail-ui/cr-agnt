...
Every Curie request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Curie Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=curie-agent",
        "client=curie-client-v<__version__>",
    ]

The version is sourced live from ``curie_cli.__version__`` so it auto-aligns
...
``curie_cli.__version__`` is the canonical source of truth.
"""
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar(
    "curie_affinity_scope", default=None
)
...
def _curie_version() -> str:
    """Return the current Curie release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``curie_cli`` cannot be imported (should
...
def curie_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=curie-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=curie-client-v{_curie_version()}"


def conversation_tag(session_id: str) -> str:
    """Return the ``conversation=...`` tag for a Curie session/conversation.

    Format: ``conversation=<session_id>``. ``session_id`` is the canonical
    Curie conversation identifier (``AIAgent.session_id``) — the same value
    used for ``~/.curie/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Curie
    ...
    """
    tags = ["product=curie-agent", curie_client_tag()]
