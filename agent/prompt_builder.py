...
_CURIE_MD_NAMES = (".curie.md", "CURIE.md")


def _find_curie_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.curie.md`` or ``CURIE.md``.
    ...
    # could pick up a .curie.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _CURIE_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Curie Agent, built by Nous Research. Be direct: match the "
...
CURIE_AGENT_HELP_GUIDANCE = (
    ...
    # precedence. Injected only when skill_view exists AND the curie-agent
    # skill is actually installed (see system_prompt.py slot resolution).
    "You run on Curie Agent (by Nous Research). When the user needs help with "
    "Curie itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. The `curie-agent` skill has the actual commands and proven "
    "workflows — load it with skill_view(name='curie-agent') before configuring, "
    "modifying, or troubleshooting Curie so you don't guess or invent workarounds."
)
...
CURIE_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on Curie Agent (by Nous Research). When the user needs help with "
    "Curie itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs is the "
    "authoritative reference and always holds the latest, most up-to-date "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.curie/kanban.db`. Your task id is in "
    "`$CURIE_KANBAN_TASK`; your workspace is `$CURIE_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $CURIE_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $CURIE_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${CURIE_KANBAN_BRANCH:-wt/$CURIE_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$CURIE_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`curie profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `curie kanban <verb>` for board operations. Use "
...
#
# The curie-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to curie-agent's Python
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Curie appends their message to the "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Curie floating over another application, so the
    user is rarely asking about Curie — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Curie "
        ...
        "rather than anything inside Curie. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Curie terminal UI (TUI). Files: there is no "
    ...
    "desktop": (
        ...
        "You are chatting inside the Curie desktop app, a graphical chat "
        ...
        "data-curie-send=\"prompt\" on any clickable element (or "
        "window.curie.send(\"prompt\")) sends that prompt as a hidden user "
...
# container / remote host rather than on the machine where Curie itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Curie runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Curie itself is running. The host OS, home, and cwd "
                f"of the Curie process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Curie "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Curie
    ...
    extra = (os.getenv("CURIE_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_curie_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("CURIE_PLATFORM") or os.environ.get("CURIE_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.curie/skills/`` directory.  External dirs
    ...
    # CURIE_HOME would otherwise fall back to the launch (default) home and
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_curie_home_override(str(skills_dir.parent))
    else:
        skills_dir = get_skills_dir()
        _home_token = None
    try:
        external_dirs = get_all_skills_dirs()[1:]  # skip local (index 0)
        # Trusted project-local dirs (./.curie/skills, ./.agents/skills at
        ...
        if _home_token is not None:
            reset_curie_home_override(_home_token)
...
def load_soul_md(
    ...
    """Load SOUL.md from CURIE_HOME and return its content, or None.
    ...
    ambient — which on a thread that lost the CURIE_HOME ContextVar falls
    back to the launch home and reads the wrong profile's SOUL.md (#50233,
    same class as the skills-index leak fixed in #86313).
    """
    try:
        from curie_cli.config import ensure_curie_home
        ensure_curie_home()
    except Exception as e:
        logger.debug("Could not ensure CURIE_HOME before loading SOUL.md: %s", e)

    _home = Path(home_override) if home_override is not None else get_curie_home()
...
def _load_curie_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.curie.md / CURIE.md — walk to git root."""
    curie_md_path = _find_curie_md(cwd_path)
    if not curie_md_path:
        return ""
    try:
        content = curie_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = curie_md_path.name
        try:
            rel = str(curie_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".curie.md", context_length=context_length,
            read_path=str(curie_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", curie_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .curie.md / CURIE.md  (walk to git root)
      2. AGENTS.md / agents.md   (merged chain: git root → cwd)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from CURIE_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Curie install/source
    ...
    # explicitly configured cwd is honored verbatim — the Curie tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Curie in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Curie install tree (%s) — set terminal.cwd to "
        ...
        project_context = (
            _load_curie_md(cwd_path, context_length)
    ...
    # SOUL.md from CURIE_HOME only — skip when already loaded as identity
    if not skip_soul:
