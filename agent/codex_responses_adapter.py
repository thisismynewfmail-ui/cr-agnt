...
def _clamp_responses_call_id(call_id: str) -> str:
    ...
    component the built-in ``curie-tools`` server already overflows 64 chars,
...
# output items.  Curie injects xAI's native ``web_search`` for the xAI
...
# server-assigned base64 blobs that can run 400+ chars, while Curie-minted
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on Curie threading encrypted reasoning back across
    ...
    ``convert_messages``). Dropping the checkpoint costs nothing: Curie'
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a Curie-side metadata key and not part
...
def _normalize_codex_response(
    ...
    *,
    ...
    if tool_calls:
        ...
        # See https://github.com/thisismynewfmail-ui/Cur-Agnt/issues/64434
