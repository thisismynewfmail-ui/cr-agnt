...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Curie-Session-Id header), and an
        ...
        if env_var_enabled("CURIE_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from curie_cli.config import load_config as _load_moa_cfg
    ...
    # that are canonicalized only after Curie sends the request. Honor its
    ...
    if not _supports_anthropic_cache_markers:
        ...
        from curie_cli.providers import custom_provider_aliases
    ...
    if _supports_anthropic_cache_markers and (
        ...
        try:
            from curie_cli.config import get_custom_provider_model_capability
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to curie's outer conversation loop,
    ...
    if agent.provider == "opencode-free":
        from curie_cli.models import opencode_zen_free_headers
    ...
    # All primary construction and recovery paths must identify Curie to the
...
def _apply_switched_provider_request_overrides(agent, new_provider):
    ...
    if custom_providers is None:
        try:
            from curie_cli.config import load_config, get_compatible_custom_providers
...
def switch_model(
    ...
    try:
        ...
        from curie_cli.config import load_config as _sm_load_config
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    try:
        from curie_cli.middleware import apply_tool_request_middleware
    ...
    from curie_cli.middleware import run_tool_execution_middleware
...
def _session_id_for_heal_log() -> str:
    try:
        from curie_logging import _session_context
...
def get_sanitizer_heal_stats() -> Dict[str, Dict[str, Any]]:
    ...
    Surfaced by diagnostics (``curie doctor`` / debug share callers) so
...
def _log_empty_non_final_heal(healed: int) -> None:
    ...
    / ``curie doctor`` — once per session, never re-armed by a new window.
    """
    key = _session_id_for_heal_log() or "-"
        ...
        if threshold > 0 and count >= threshold and not state["escalated"]:
            ...
            if key not in _empty_heal_user_notified:
                ...
                _empty_heal_pending_notice[key] = (
                    ...
                    "session's history — run /debug share or `curie "
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: curie' own dispatch loop intentionally keeps an
    ...
    # provider-agnostic: Gemini reaches Curie under many model strings and
...
def _iter_httpx_pool_objects(http_client: Any):
    ...
    Curie' keepalive client (#10324 / ``_build_keepalive_http_client``) and
