...
Providers live in ``~/.curie/plugins/<name>/`` (user, opt-in via
...
* ``cache_path_base`` — where auto-synced ``~/.curie/cache`` files land
  inside the backend (e.g. ``"~/.curie"`` for home-synced backends,
  ``"/root/.curie"`` for root-homed containers), or ``None`` when host
...
The factory stamps ``_curie_backend_name`` on the returned object so
...
class TerminalEnvironmentProvider(abc.ABC):
    ...
    def cache_path_base(self) -> Optional[str]:
        """Base dir for synced Curie cache files inside the backend."""
        return None
    ...
    def setup_instructions(self) -> List[str]:
        """Lines printed by ``curie setup`` after this backend is selected.
