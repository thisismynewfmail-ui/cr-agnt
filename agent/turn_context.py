...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``curie logs``.
    ...
    if (
        ...
        # Curie only initiates compaction in "curie" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Curie preflight compression for codex app-server "
                "(mode=%s); Curie will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Curie (via _emit_status), so it always
