...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `curie backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - curie_home (str): The active CURIE_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.curie``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "curie").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'curie memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], curie_home: str) -> None:
        ...
        Called by 'curie memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``curie_home`` is the active CURIE_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE CURIE_HOME.

        ``curie backup`` only walks CURIE_HOME, so any provider state kept
        ...
        of the archive, and ``curie import`` restores them to their original
