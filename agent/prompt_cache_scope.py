...
A host that mints one physical ``session_id`` per RESPONSE (Curie Studio's
...
Curie sends — ``prompt_cache_key`` on both OpenAI-wire transports, plus the
...
Curie must not infer the logical conversation from the id's SYNTAX (that
...
``X-Curie-Session-Key`` header, which branching deliberately does NOT key
...
def _agent_source(
    ...
    :meth:`~curie_state.SessionDB.declared_scope_identity` still takes.
    ...
    diverge whenever ``CURIE_SESSION_SOURCE`` overrides the platform, and the
...
def declared_conversation_scope(agent: Any) -> Optional[str]:
    ...
    Resolved from ``agent._gateway_session_key`` (the ``X-Curie-Session-Key``
