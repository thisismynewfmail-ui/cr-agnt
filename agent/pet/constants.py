...
row taxonomy is inferred from the atlas shape so Curie can render both legacy
...
# rows (1536x1872), while older Curie/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Curie' activity state names. They are not always identical to the
...
# Legacy Curie/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Curie activity names -> accepted row-name aliases in descending
