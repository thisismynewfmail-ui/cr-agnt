...
as ``kind: backend``) or ``~/.curie/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        backend can't honor are not advertised), and by ``curie tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$CURIE_HOME/cache/videos/``, creating parents as needed."""
    from curie_constants import get_curie_home

    path = get_curie_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$CURIE_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$CURIE_HOME/cache/videos/``.
