"""Projects codex app-server events into Curie' messages list.

The translator that lets Curie' memory/skill review keep working under the
...
Curie' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple Codex tool calls within one Codex turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Curie
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Curie' existing tool_calls path does."""
...
class CodexEventProjector:
    ...
    items but Curie stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Curie already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Curie' messages store text only.
        text_parts: list[str] = []
