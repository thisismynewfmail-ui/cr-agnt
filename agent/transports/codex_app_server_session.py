...
Owns one Codex thread per Curie session. Drives `turn/start`, consumes
...
# Curie' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_CURIE_TO_CODEX_PERMISSION_PROFILE = {
    "auto": "workspace-write",
    "approval-required": "read-only-with-approval",
    "unrestricted": "full-access",
    # Backstop aliases used by some skills/tests. "yolo" is retained because
    # it is the spelling Codex itself uses for this profile.
    "unlock": "full-access",
    "yolo": "full-access",
}
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Curie/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Curie session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _CURIE_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("CURIE_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="curie",
            client_title="Curie Agent",
            client_version=_get_curie_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Curie' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Curie' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own curie-tools callback we
            # auto-accept — the user already approved Curie' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Curie
        approval-mode/timeout logic. The Curie-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Curie approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Curie approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Curie returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Curie approval mode/timeout resolution
    ...
    # "prompt expired"; the Curie-side messaging already distinguishes them.
    return "decline"
...
def _get_curie_version() -> str:
    """Best-effort Curie version string for codex's userAgent line."""
    try:
