...
projection into Curie' display, approval bridging, transcript projection into
...
"codex_app_server"`. Curie' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Curie secret — gateway
        ...
        spawn_env = curie_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("CURIE_KANBAN_TASK"):
            kanban_db = spawn_env.get("CURIE_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "CURIE_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("CURIE_HOME", os.path.expanduser("~/.curie")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "curie",
        client_title: str = "Curie Agent",
