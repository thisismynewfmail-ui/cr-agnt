...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Curie starts and finds ``~/.openclaw/``.

    Points users at ``curie claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``curie claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Curie, run "
        "`curie claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `curie claw cleanup` (renames it to ~/.openclaw.pre-migration — "
