...
appropriate.  All installs go to a Curie-owned bin staging dir,
``<CURIE_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``curie lsp
  status``.
...
# ``<CURIE_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `curie lsp status` reports the host's presence.
...
def curie_lsp_bin_dir() -> Path:
    """Return the Curie-owned bin staging dir for LSP servers."""
    from curie_constants import get_curie_home

    p = get_curie_home() / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(curie_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    """
    # Managed npm first: $CURIE_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Curie installed and
    ...
    if npm is None:
        ...
        return None
    staging = curie_lsp_bin_dir().parent  # <CURIE_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a curie-owned target dir.
    ...
    console script.
    """
    pip_target = curie_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = curie_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``curie lsp status`` CLI to give users a quick
