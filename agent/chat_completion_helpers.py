...
from curie_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from curie_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def _check_stale_giveup(agent) -> None:
    ...
    _giveup = env_int("CURIE_STREAM_STALE_GIVEUP", 5)
...
def _tool_call_requires_arguments(api_kwargs: dict, tool_name: str) -> bool:
    """True when ``tool_name``'s declared schema cannot be satisfied by ``{}``.

    Streaming tool calls arrive as ``name`` first and ``arguments`` in later
    fragments, so an empty argument string is ambiguous: either the stream
    died before argument generation started, or the tool genuinely takes no
    arguments and the model correctly sent none.

    Consulting the request's own ``tools`` payload resolves it.  A tool with a
    non-empty ``required`` list (or, for schemas that omit ``required``, any
    declared properties at all when the schema also forbids extras) really is
    incomplete without arguments; a parameterless tool is not.  Unknown tools
    are treated as requiring arguments so a genuine mid-stream drop on an
    unrecognised name still gets caught.
    """
    ...
    for entry in tools:
        ...
        if isinstance(entry, dict):
            ...
            if fn is None and entry.get("name"):
                fn = entry
        else:
        ...
        if not isinstance(params, dict):
            # Declared with no schema at all — nothing can be required.
            return False
        required = params.get("required")
        if isinstance(required, (list, tuple)):
            return bool(required)
        # No ``required`` key: JSON Schema says every property is optional,
        # so ``{}`` validates.
        return False
    return True
...
def _derive_stream_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    if _cfg_stale is not None:
        ...
        _base = env_float("CURIE_STREAM_STALE_TIMEOUT", 180.0)
...
def _managed_local_load_notice(agent, api_kwargs: dict) -> "Optional[str]":
    ...
    try:
        ...
        from curie_cli.local_runtime.load_progress import (
            get_loading_progress,
            get_prefill_progress,
        )
        from curie_cli.local_runtime.supervisor import state_path
...
def _resolve_direct_stale_timeout(agent, api_kwargs: dict) -> float:
    ...
    ``CURIE_API_CALL_STALE_TIMEOUT`` → reasoning-model floor → context-size
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # activity. Operators can tune via CURIE_CODEX_TTFB_TIMEOUT_SECONDS and
    # CURIE_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each).
    ...
    # limit. Tunable via CURIE_CODEX_HARD_TIMEOUT_SECONDS (set to 0 to
    # disable the ceiling entirely; that restores the pre-fix behavior).
    _codex_hard_timeout = _env_float("CURIE_CODEX_HARD_TIMEOUT_SECONDS", 1500.0)
    if (
    ...
    # CURIE_CODEX_TTFB_TIMEOUT_SECONDS=0 to disable this watchdog entirely.
    _ttfb_enabled = _codex_watchdog_enabled
    _ttfb_timeout = _env_float("CURIE_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if _ttfb_timeout <= 0:
        _ttfb_enabled = False
    elif _openai_codex_backend:
        _ttfb_disable_above = _env_float("CURIE_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0)
        _ttfb_strict = os.environ.get("CURIE_CODEX_TTFB_STRICT", "").strip().lower() in {
            "1", "true", "yes", "on"
        }
        if (
            not _ttfb_strict
            ...
            if _ttfb_timeout < _large_request_ttfb_timeout:
                logger.info(
                    ...
                    "Set CURIE_CODEX_TTFB_STRICT=1 to keep the smaller cutoff.",
                    _ttfb_timeout,
        ...
        _ttfb_cap = _env_float("CURIE_CODEX_TTFB_MAX_SECONDS", 120.0)
        if _ttfb_cap > 0 and _ttfb_timeout > _ttfb_cap:
            logger.info(
                "Capping openai-codex no-byte TTFB timeout from %.0fs to %.0fs "
                "(context=~%s tokens). Set CURIE_CODEX_TTFB_MAX_SECONDS to tune.",
                _ttfb_timeout,
                _ttfb_cap,
                f"{_est_tokens_for_codex_watchdog:,}",
            )
    ...
    _codex_idle_timeout = _env_float(
        "CURIE_CODEX_EVENT_STALE_TIMEOUT_SECONDS",
...
def build_api_kwargs(agent, api_messages: list, tools_for_api: list | None = None) -> dict:
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "curie",
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    # Respects CURIE_REDACT_SECRETS via redact_sensitive_text — no-op
    # when disabled. (#19798)
    if isinstance(_san_content, str) and _san_content:
        from agent.redact import redact_sensitive_text
    ...
    try:
        ...
        if not fb_api_mode_explicit and fb_api_mode == "chat_completions":
            if fb_provider == "openai-codex":
                ...
                from curie_cli.providers import nous_api_mode
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    if getattr(agent, "suppress_status_output", False):
        # Strict machine-readable mode (curie chat -Q, oneshot, background
    ...
    try:
        ...
        for msg in messages:
            ...
            # and every Curie-internal underscore-prefixed scaffolding key.
            ...
            # substitute it before dropping the key (Curie bookkeeping,
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
        ...
        try:
            ...
            while t.is_alive():
                ...
                if _stale_elapsed > _bedrock_stale_timeout:
                    ...
                    # crosses CURIE_STREAM_STALE_GIVEUP, so a persistently wedged
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        # wins over the CURIE_API_TIMEOUT env default if the user set it.
        _provider_timeout_cfg = get_provider_request_timeout(agent.provider, agent.model)
        _base_timeout = (
            _provider_timeout_cfg
            if _provider_timeout_cfg is not None
            else env_float("CURIE_API_TIMEOUT", 1800.0)
        )
        # Read timeout: config wins here too.  Otherwise use
        # CURIE_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = env_float("CURIE_STREAM_READ_TIMEOUT", 120.0)
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode CURIE_STREAM_READ_TIMEOUT.
        ...
        def _accept_stream_chunk(_chunk: Any) -> bool:
            ...
            # already-received tool-call chunk back to Curie. Preserve only
        ...
        if agent.provider == "moa":
            # Curie interrupts the managed stream; Relay retains sole
        ...
        # completed response. Relay records that attempt while Curie preserves
            ...
            for idx in sorted(tool_calls_acc):
                ...
                if arguments and arguments.strip():
                    try:
                        ...
                        #
                        # ``recovered`` is the ONLY truncation signal here.  A
                        # bare ``"{}"`` result is ambiguous — an empty object
                        # is also a legitimate repair outcome — and treating
                        # it as truncation is what made healthy turns report
                        # "stream stalled mid tool-call".  Only the
                        # every-pass-failed fallback (recovered=False) means
                        # the model's real arguments are actually lost.
                        _fix = repair_tool_call_arguments_ex(arguments, tool_name)
                        if _fix.recovered:
                            arguments = _fix.arguments
                        else:
                            # Keep the raw partial string.  On the genuine
                            # output-cap path (finish_reason="length") the
                            # tool call survives and these bytes are the only
                            # record of what the model was writing; replacing
                            # them with "{}" destroys it.
                            has_truncated_tool_args = True
                elif _tool_call_requires_arguments(api_kwargs, tool_name):
                    # This tool call's arguments never received a single byte,
                    # and the tool's own schema says it cannot run without
                    # them (name arrived, argument generation never started
                    # before the connection died).  Left unflagged, the empty
                    # string is silently coerced to "{}" at the dispatch
                    # boundary and executed with no arguments and no retry
                    # (#80498), so route it through the dropped-mid-tool-call
                    # stub path.
                    #
                    # A tool with NO required parameters is the opposite case:
                    # empty arguments are exactly right for it, and flagging
                    # them was a pure false positive on every provider that
                    # omits ``finish_reason``.
                    has_truncated_tool_args = True
        ...
        # A final usage object proves the provider finished generating and
        # closed the stream cleanly (#91373) — the same reasoning the
        # text-only guard below already applies.  Plenty of
        # OpenAI-compatible servers (llama.cpp, vLLM forks, LiteLLM/
        # OpenRouter passthroughs, bespoke gateways) never emit a
        # ``finish_reason`` at all, so its absence on its own is not
        # evidence of a drop; without this the watchdog reported a stall on
        # every large tool call those endpoints served.
    ...
    def _call():
        ...
        _max_stream_retries = env_int("CURIE_STREAM_RETRIES", 2)
            ...
            for _stream_attempt in range(_max_stream_retries + 1):
                ...
                try:
                    ...
                    if deltas_were_sent["yes"]:
                        ...
                        # Deliberately NOT a stream delta.  Splicing a
                        # warning into the assistant's visible text told the
                        # user about an internal transport retry they can do
                        # nothing about, and it landed mid-sentence in the
                        # reply they were reading.  The reconnect is silent to
                        # the transcript and reported on the status line only;
                        # the structured WARNING from ``_emit_stream_drop``
                        # below remains the diagnostic record.
                        try:
                            agent._buffer_status(
                                f"↻ reconnecting to "
                                f"{api_kwargs.get('model', 'the provider')} "
                                f"(attempt {_stream_attempt + 2}"
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = env_float("CURIE_STREAM_STALE_TIMEOUT", 180.0)
    ...
    # unless the user explicitly set CURIE_STREAM_STALE_TIMEOUT; override the
    # local ceiling with CURIE_LOCAL_STREAM_STALE_TIMEOUT (documented in
    # website/docs/reference/environment-variables.md).
    if _stream_stale_timeout_base == 180.0 and agent.base_url and is_local_endpoint(agent.base_url):
        # Read config.yaml ``agent.local_stream_stale_timeout`` (default 900),
        # env var ``CURIE_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch.
        ...
        try:
            ...
            pass
        _stream_stale_timeout = env_float("CURIE_LOCAL_STREAM_STALE_TIMEOUT", _local_default)
        logger.debug(
    ...
    # Mid-generation patience.  The budget above is sized for the *opening*
    # silence — how long to wait for a provider that may never answer.  Once
    # chunks have actually arrived the situation is different in kind: the
    # endpoint is demonstrably alive and the model is mid-turn, and a pause
    # there is a thinking pause, not a dead socket.  Killing the connection
    # then throws away every token already generated (including a tool call's
    # partially streamed arguments) and re-streams the whole turn, which is
    # what produced repeated "connection dropped mid tool-call" reports on
    # OpenAI-compatible endpoints whose models pause between a long text
    # preamble and the tool call that follows it.
    #
    # A wedged endpoint is still bounded — just on a budget proportional to
    # the work at risk rather than to first-token latency.  The budget is a
    # MULTIPLE of the opening one rather than a flat floor, so it scales with
    # whatever patience the user configured instead of overriding it: the
    # 180s default becomes 900s, and an operator who deliberately sets a
    # 5-second budget still gets a 5-second-scale stream.
    #
    # Overrides: CURIE_STREAM_ACTIVE_STALE_MULTIPLIER (<= 1 disables the
    # extension, restoring one budget for the whole stream) and
    # CURIE_STREAM_ACTIVE_STALE_TIMEOUT (an absolute value, winning over the
    # multiplier).
    if _stream_stale_timeout == float("inf"):
        _stream_active_stale_timeout = float("inf")
    else:
        _active_stale_absolute = env_float("CURIE_STREAM_ACTIVE_STALE_TIMEOUT", 0.0)
        if _active_stale_absolute > 0:
            _stream_active_stale_timeout = _active_stale_absolute
        else:
            _active_stale_multiplier = env_float(
                "CURIE_STREAM_ACTIVE_STALE_MULTIPLIER", 5.0
            )
            if _active_stale_multiplier <= 1.0:
                _stream_active_stale_timeout = _stream_stale_timeout
            else:
                _stream_active_stale_timeout = (
                    _stream_stale_timeout * _active_stale_multiplier
                )
                # Cap the multiplied result so an already-generous reasoning
                # floor cannot compound into an hours-long wait.
                _active_stale_cap = env_float(
                    "CURIE_STREAM_ACTIVE_STALE_CEILING", 3600.0
                )
                if _active_stale_cap > 0:
                    _stream_active_stale_timeout = min(
                        _stream_active_stale_timeout,
                        max(_stream_stale_timeout, _active_stale_cap),
                    )
        # Never below the opening budget: the endpoint has proven more, not
        # less, about itself by the time this applies.
        _stream_active_stale_timeout = max(
            _stream_active_stale_timeout, _stream_stale_timeout
        )
        ...
        while _call_alive():
            ...
            if _hb_now - _last_heartbeat >= _HEARTBEAT_INTERVAL:
                ...
                if _waiting_secs >= _HEARTBEAT_INTERVAL:
                    ...
                    _hb_diag = request_client_holder.get("diag") or {}
                    _hb_budget = (
                        _stream_active_stale_timeout
                        if _hb_diag.get("first_chunk_at") is not None
                        else _stream_stale_timeout
                    )
                    if _hb_budget is not None and _hb_budget != float("inf"):
                        _recovery = f"; auto-reconnect at {int(_hb_budget)}s"
                    else:
                        _recovery = ""
                    agent._emit_wait_notice(
                        f"⏳ waiting on {api_kwargs.get('model', 'the provider')} — "
            ...
            #
            # Which budget applies depends on whether anything has streamed
            # yet: the opening budget until the first chunk, the (larger)
            # mid-generation budget afterwards.  See the derivation above.
            _stale_elapsed = time.time() - last_chunk_time["t"]
            _attempt_diag = request_client_holder.get("diag") or {}
            _stream_had_output = _attempt_diag.get("first_chunk_at") is not None
            _stale_budget = (
                _stream_active_stale_timeout
                if _stream_had_output
                else _stream_stale_timeout
            )
            if _stale_elapsed > _stale_budget:
                _est_ctx = estimate_request_context_tokens(api_kwargs)
                logger.warning(
                    "Stream stale for %.0fs (threshold %.0fs, phase=%s) — no "
                    "chunks received. model=%s context=~%s tokens. "
                    "Killing connection.",
                    _stale_elapsed, _stale_budget,
                    "mid-generation" if _stream_had_output else "opening",
    ...
    if result["error"] is not None:
        if deltas_were_sent["yes"]:
            ...
            if _partial_names:
                ...
                # The conversation loop retries this stub automatically (up
                # to 4 times) the moment it sees PARTIAL_STREAM_STUB_ID, so
                # asking the user to "retry if you want to continue" was both
                # untrue and unactionable — and appending it to
                # ``_partial_text`` persisted the apology into the transcript,
                # where the model then read it back and apologised again.
                # Keep the incident in the log and on the transient status
                # line; leave the assistant's text alone.
                try:
                    agent._buffer_status(
                        f"↻ re-requesting {_name_str} (stream ended before "
                        f"the call was complete)"
                    )
                except Exception:
                    pass
                logger.warning(
                    "Partial stream dropped tool call(s) %s after %s chars "
                    "of text; retrying transparently: %s",
