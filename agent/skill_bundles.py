...
Storage
-------
Bundles live in ``~/.curie/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``CURIE_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``curie bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under CURIE_HOME.

    Honors ``CURIE_BUNDLES_DIR`` for tests; falls back to
    ``<CURIE_HOME>/skill-bundles``.
    """
    override = os.environ.get("CURIE_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_curie_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `curie bundles` CLI subcommand.
# ---------------------------------------------------------------------------
