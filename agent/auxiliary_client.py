...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.curie/auth.json active provider)
...
from curie_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``CURIE_CA_BUNDLE`` /
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `curie update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Curie owns auxiliary retry + provider/model fallback policy (the
    ...
    # Curie sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Curie control the budget; explicit callers can still
    # override via kwargs.
...
def _run_protected_sync_provider_call(
    ...
    threading.Thread(
        ...
        name="curie-protected-aux-provider",
        daemon=True,
    ).start()
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Curie triggers summarization.  Higher values delay
...
def _fast_model_from_catalog(provider_id: str) -> str:
    ...
    try:
        from curie_cli.auth import resolve_api_key_provider_credentials
        from curie_cli.models import fetch_models_with_pricing
        ...
        if not api_key and is_nous:
            ...
            try:
                from curie_cli.models import _resolve_nous_pricing_credentials
        ...
        _nous_kwargs = {}
        if is_nous:
            from curie_cli.models import _NOUS_CATALOG_TTL_SECONDS
...
def _nous_policy_blocks(model_id: str) -> bool:
    ...
    try:
        from curie_cli.models import nous_policy_allowed_ids, restrict_to_nous_policy
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://github.com/thisismynewfmail-ui/Cur-Agnt",
    "X-Title": "Curie Agent",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``CURIE_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``CURIE_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("CURIE_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("CURIE_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "CurieAgent",
}
...
_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://github.com/thisismynewfmail-ui/Cur-Agnt",
    "X-Title": "Curie Agent",
    "User-Agent": f"CurieAgent/{_CURIE_VERSION}",
}
...
# in lockstep with curie_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``curie_cli.__version__`` is
...
_AUTH_JSON_PATH = get_curie_home() / "auth.json"
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from curie_cli.auth import _nous_inference_env_override
...
def _nous_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("CURIE_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the Curie-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Curie plumbing (_reasoning_config
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``curie_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.curie/auth.json for an active Nous provider.
...
def _nous_api_key(provider: dict) -> str:
    ...
    from curie_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from curie_cli.auth import _agent_key_is_usable
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from curie_cli.auth import resolve_nous_runtime_credentials

        creds = resolve_nous_runtime_credentials(
            timeout_seconds=env_float("CURIE_NOUS_TIMEOUT_SECONDS", 15),
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``curie auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``curie_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from curie_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Curie auth store.
    ...
    try:
        from curie_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from curie_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from curie_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from curie_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if not _aux_probe_active():
        ...
        try:
            from curie_cli.models import get_nous_recommended_aux_model
...
def _resolve_moa_aggregator(preset_name: Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    ...
    try:
        from curie_cli.config import load_config
        from curie_cli.moa_config import resolve_moa_preset
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `curie setup` or `curie model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    from tools.xai_http import curie_xai_default_headers

    real_client = _create_openai_client(
        api_key=api_key,
        base_url=base_url,
        default_headers=curie_xai_default_headers(),
    )
    return CodexAuxiliaryClient(real_client, model), model
...
def _try_azure_foundry(
    *,
    ...
    :func:`curie_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from curie_cli.runtime_provider import _resolve_azure_foundry_runtime
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second curie
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``curie config aux reset``)."""
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from curie_cli.nous_account import get_nous_portal_account_info
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "openai-codex":
            from curie_cli.auth import resolve_codex_runtime_credentials
...
def _fallback_destination(
    ...
    attached = getattr(fb_client, "_curie_fallback_destination", None)
...
def _fallback_entry_api_key(entry: Dict[str, Any]) -> Optional[str]:
    ...
    gateway multiplexing (see ``curie_cli.fallback_config.resolve_entry_api_key``).
    """
...
def _resolve_fallback_entry(entry: Dict[str, Any]) -> Tuple[Optional[Any], Optional[str]]:
    ...
    if client is not None:
        try:
            client._curie_fallback_destination = _fallback_destination_from_entry(
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Curie' built-in discovery
...
def _resolve_auto_route(
    ...
    #    scenario where a user switches providers via `curie model` but the
    #    old OPENAI_BASE_URL lingers in ~/.curie/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: curie model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.curie/.env",
    ...
    if (main_provider and main_model
        ...
        if runtime_base_url and main_provider == "custom":
            ...
            try:
                from curie_cli.runtime_provider import _get_named_custom_provider
...
def _tag_effective_provider(client: Any, provider: str) -> None:
    ...
    if client is None or not provider:
        return
    try:
        setattr(client, "_curie_aux_effective_provider", provider)
...
def _effective_provider_for_client(client: Any, fallback: str) -> str:
    ...
    effective_provider = getattr(client, "_curie_aux_effective_provider", "")
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from curie_cli.copilot_auth import copilot_request_headers
        ...
        async_kwargs["default_headers"] = curie_xai_default_headers()
    else:
    ...
    # See _create_openai_client: disable SDK-internal retries so Curie owns
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: curie auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: curie model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: curie model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: curie doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("curie_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        try:
            from curie_cli.models import opencode_zen_free_runtime as _oc_free_rt
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(curie_xai_default_headers())
        else:
            # Fall back to profile.default_headers for providers that declare
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from curie_cli.models import _should_use_copilot_responses_api
...
# Wall-clock budget for one auxiliary generation call, in seconds — 5 hours.
#
# This is the fallback for an auxiliary task that does not pin its own
# ``auxiliary.<task>.timeout`` in config.yaml; tasks that do (vision,
# compression, approval, …) keep their own value, and an explicit per-call
# ``timeout=`` always wins over both.
#
# Long by design.  The old 30-second default was sized for a fast completion
# model answering a one-line prompt, and a reasoning model on the same task
# blows through it while still visibly working — the call was cancelled, the
# caller fell back to a degraded deterministic path, and the work was thrown
# away.  A generous ceiling costs nothing when the model is quick, because a
# *dead* stream is caught long before this by the progress watchdog: the
# no-progress window (``_AUX_STREAM_NO_PROGRESS_TIMEOUT_SECONDS``, 60s, capped
# by this budget) fails fast on a stream that stops producing, and re-arms on
# every substantive event.  This bound only stops a stream that keeps
# trickling forever.
_AUX_GENERATION_TIMEOUT_HOURS = 5.0
_DEFAULT_AUX_TIMEOUT = _AUX_GENERATION_TIMEOUT_HOURS * 3600.0  # 18000.0s
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`curie_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one Curie will call. A requested model covers a compressor
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate Curie reasoning into native
...
def _aux_stream_total_ceiling(effective_timeout: Optional[float]) -> float:
    ...
    # The 4x multiplier is headroom for a *short* configured timeout, where
    # 4x600s is still a reasonable backstop.  Applied to a long budget it
    # compounds absurdly (a 5-hour timeout would back-stop at 20 hours), so
    # once the timeout is already generous, grant a flat grace instead.
    if timeout >= _AUX_STREAM_CEILING_FLOOR_SECONDS:
        ceiling = min(ceiling, timeout + _AUX_STREAM_CEILING_FLOOR_SECONDS)
    return ceiling
...
def _is_managed_local_endpoint(base_url: Optional[str]) -> bool:
    """True when *base_url* targets the llama-server this Curie manages."""
    if not base_url:
        return False
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Curie reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `curie model`."
                    )
