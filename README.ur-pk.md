...
```bash
curie              # انٹرایکٹو CLI — بات چیت شروع کریں
curie model        # اپنا LLM پرووائیڈر اور ماڈل منتخب کریں
curie tools        # کنفیگر کریں کہ کون سے ٹولز ایکٹو ہیں
curie config set   # انفرادی کنفگ (config) ویلیوز سیٹ کریں
curie gateway      # میسجنگ گیٹ وے شروع کریں (ٹیلی گرام، ڈسکارڈ، وغیرہ)
curie setup        # مکمل سیٹ اپ وزرڈ چلائیں (یہ سب کچھ ایک ساتھ کنفیگر کر دے گا)
curie claw migrate # OpenClaw سے مائیگریٹ کریں (اگر آپ OpenClaw سے آ رہے ہیں)
curie update       # لیٹسٹ ورژن پر اپ ڈیٹ کریں
curie doctor       # کسی بھی مسئلے کی تشخیص کریں
```

</div>

📖 **[مکمل دستاویزات →](https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs)**
...
یہ آپ کو OAuth کے ذریعے لاگ ان کرواتا ہے، Nous کو آپ کا پرووائیڈر مقرر کرتا ہے، اور ٹول گیٹ وے کو آن کر دیتا ہے۔ `curie portal info` کمانڈ استعمال کر کے آپ کسی بھی وقت چیک کر سکتے ہیں کہ کون کون سی سروسز منسلک ہیں۔ مکمل تفصیلات [Tool Gateway دستاویزات کے صفحے](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/features/tool-gateway.md) پر موجود ہیں۔
...
ہرمیس کے دو بنیادی انٹر فیس ہیں: آپ ٹرمینل UI کو `curie` کے ساتھ شروع کریں، یا گیٹ وے چلا کر اس کے ساتھ ٹیلی گرام، ڈسکارڈ، سلیک، واٹس ایپ، سگنل، یا ای میل کے ذریعے بات کریں۔ جب آپ کسی بات چیت میں ہوتے ہیں، تو بہت سی سلیش (slash) کمانڈز دونوں انٹرفیسز میں ایک جیسی ہوتی ہیں۔

<div dir="ltr">

| کارروائی (Action)                         | سی ایل آئی (CLI)                              | میسجنگ پلیٹ فارمز (Messaging platforms)                                          |
| --------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------- |
| بات چیت شروع کریں                       | `curie`                                      | `curie gateway setup` اور `curie gateway start` چلائیں، پھر بوٹ کو میسج بھیجیں |
| نئی بات چیت شروع کریں                   | `/new` یا `/reset`                            | `/new` یا `/reset`                                                               |
| ماڈل تبدیل کریں                         | `/model [provider:model]`                     | `/model [provider:model]`                                                        |
| بینچ کنسول کھولیں                        | `curie ui`                                    | —                                                                                |
| ظاہری شکل تبدیل کریں                     | `/skin [name]`                                | —                                                                                |
...
مکمل کمانڈ لسٹ کے لیے، [CLI گائیڈ](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/cli.md) اور [میسجنگ گیٹ وے گائیڈ](https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs/user-guide/messaging) دیکھیں۔

---

## دستاویزات (Documentation)

تمام دستاویزات **[github.com/thisismynewfmail-ui/Cur-Agnt/docs](https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs)** پر موجود ہیں:

<div dir="ltr">

| سیکشن (Section)                                                                                     | تفصیل (What's Covered)                                     |
| --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| [فوری آغاز (Quickstart)](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/getting-started/quickstart.md)     | انسٹالیشن → سیٹ اپ → 2 منٹ میں پہلی بات چیت شروع کریں       |
| [CLI کا استعمال](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/cli.md)                         | کمانڈز، کی بائنڈنگز (keybindings)، پرسنلٹیز (personalities)، سیشنز |
| [کنفیگریشن (Configuration)](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/configuration.md)    | کنفگ فائل، پرووائیڈرز، ماڈلز، اور تمام آپشنز               |
| [میسجنگ گیٹ وے](https://github.com/thisismynewfmail-ui/Cur-Agnt/tree/main/website/docs/user-guide/messaging)                    | ٹیلی گرام، ڈسکارڈ، سلیک، واٹس ایپ، سگنل، ہوم اسسٹنٹ         |
| [سیکیورٹی (Security)](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/security.md)              | کمانڈ کی منظوری، DM پیئرنگ (pairing)، کنٹینر آئسولیشن       |
| [ٹولز اور ٹول سیٹس](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/features/tools.md)          | 40 سے زائد ٹولز، ٹول سیٹ سسٹم، ٹرمینل بیک اینڈز             |
| [مہارتوں کا سسٹم (Skills System)](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/features/skills.md)| پروسیجرل (Procedural) میموری، سکلز ہب، نئی مہارتیں بنانا    |
| [میموری (Memory)](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/features/memory.md)            | مستقل میموری، یوزر پروفائلز، بہترین طریقہ کار              |
| [MCP انضمام (Integration)](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/features/mcp.md)      | صلاحیتوں کو بڑھانے کے لیے کسی بھی MCP سرور کو جوڑیں        |
| [کرون (Cron) شیڈیولنگ](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/features/cron.md)         | پلیٹ فارم ڈیلیوری کے ساتھ شیڈول کیے گئے کام                 |
| [کانٹیکسٹ (Context) فائلز](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/user-guide/features/context-files.md)| پروجیکٹ کا سیاق و سباق (context) جو ہر بات چیت پر اثر انداز ہوتا ہے |
| [آرکیٹیکچر (Architecture)](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/developer-guide/architecture.md) | پروجیکٹ کا ڈھانچہ، ایجنٹ لوپ، اہم کلاسز                    |
| [تعاون (Contributing)](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/developer-guide/contributing.md)     | ڈیویلپمنٹ سیٹ اپ، PR کا طریقہ کار، کوڈنگ کا انداز          |
| [CLI حوالہ جات (Reference)](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/reference/cli-commands.md)      | تمام کمانڈز اور فلیگز (flags)                              |
| [انوائرمنٹ ویری ایبلز](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/reference/environment-variables.md)  | مکمل انوائرمنٹ ویری ایبل حوالہ جات                         |
...
**پہلی بار سیٹ اپ کے دوران:** سیٹ اپ وزرڈ (`curie setup`) خود بخود `~/.openclaw` کو پہچان لیتا ہے اور کنفیگریشن شروع ہونے سے پہلے مائیگریٹ (migrate) کرنے کا آپشن دیتا ہے۔
...
```bash
curie claw migrate              # انٹرایکٹو مائیگریشن (مکمل پری سیٹ)
curie claw migrate --dry-run    # جائزہ لیں کہ کیا کیا مائیگریٹ ہوگا
curie claw migrate --preset user-data   # حساس معلومات (secrets) کے بغیر مائیگریٹ کریں
curie claw migrate --overwrite  # موجودہ متصادم فائلوں کو اوور رائٹ کریں
```
...
- **مہارتیں (Skills)** — صارف کی بنائی گئی مہارتیں → `~/.curie/skills/openclaw-imports/`
...
تمام آپشنز دیکھنے کے لیے `curie claw migrate --help` استعمال کریں، یا انٹرایکٹو ایجنٹ کی مدد سے مائیگریٹ کرنے کے لیے `openclaw-migration` سکل کا استعمال کریں (جس میں ڈرائی رن (dry-run) پریویوز شامل ہیں)۔

---

## تعاون کریں (Contributing)

ہم آپ کے تعاون کا خیرمقدم کرتے ہیں! ڈیویلپمنٹ سیٹ اپ، کوڈ کے انداز اور PR کے طریقہ کار کے لیے براہ کرم ہماری [Contributing گائیڈ](https://github.com/thisismynewfmail-ui/Cur-Agnt/blob/main/website/docs/developer-guide/contributing.md) دیکھیں۔

معاونین (contributors) کے لیے فوری آغاز — کلون (clone) کریں اور `setup-curie.sh` چلائیں:
...
cd curie-agent
./setup-curie.sh     # uv کو انسٹال کرتا ہے، venv بناتا ہے، .[all] کو انسٹال کرتا ہے، اور ~/.local/bin/curie کا سیم لنک (symlink) بناتا ہے
./curie              # خود بخود venv کی شناخت کرتا ہے، پہلے `source` کرنے کی ضرورت نہیں
```
...
- 🐛 [مسائل (Issues)](https://github.com/thisismynewfmail-ui/Cur-Agnt/issues)
...
- 🔌 [CurieClaw](https://github.com/AaronWong1999/curieclaw) — کمیونٹی وی چیٹ (WeChat) برج: ہرمیس ایجنٹ اور OpenClaw کو ایک ہی وی چیٹ اکاؤنٹ پر چلائیں۔
