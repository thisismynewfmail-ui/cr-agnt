...
React + Ink terminal UI for Curie. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `CURIE_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `curie setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.curie/.curie_history` or under `CURIE_HOME`.
...
`/indicator`, `/unlock`, `/reasoning`, `/fast`, `/busy`, `/verbose`, `/usage`
...
### Setup (`setup.ts`)
`/setup` — launches external `curie setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/curie-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external curie setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.curie/tui-parent.log
    ...
    types/
      curie-ink.d.ts            type declarations for @curie/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent CurieCLI subprocess for slash commands
```
