...
import { terminalBackgroundHex } from '@curie/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $CURIE_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see CURIE_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['CURIE_TUI_BACKGROUND', process.env.CURIE_TUI_BACKGROUND ?? '(unset)'],
            ['CURIE_TUI_THEME', process.env.CURIE_TUI_THEME ?? '(unset)'],
