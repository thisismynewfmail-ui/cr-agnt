...
export const sessionCommands: SlashCommand[] = [
  ...
  {
    help: 'toggle unlock mode (per-session approvals)',
    name: 'unlock',
    run: (_arg, ctx) => {
      ctx.gateway
        .rpc<ConfigSetResponse>('config.set', { key: 'unlock', session_id: ctx.sid })
        .then(ctx.guarded<ConfigSetResponse>(r => ctx.transcript.sys(`unlock ${r.value === '1' ? 'on' : 'off'}`)))
    }
  },
