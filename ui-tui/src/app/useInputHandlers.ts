import { forceRedraw, useInput } from '@curie/ink'
...
export function useInputHandlers(ctx: InputHandlerContext): InputHandlerResult {
  ...
  useInput((ch, key) => {
    ...
    // shift-tab flips unlock without spending a turn (claude-code parity)
    if (key.shift && key.tab && !cState.completions.length) {
      if (!live.sid) {
        return void actions.sys('unlock needs an active session')
      }
      ...
      return void gateway.rpc<ConfigSetResponse>('config.set', { key: 'unlock', session_id: live.sid }).then(r => {
        if (r?.value === '1') {
          return actions.sys('unlock on')
        }

        if (r?.value === '0') {
          return actions.sys('unlock off')
        }

        if (r) {
          actions.sys('failed to toggle unlock')
        }
      })
    }
