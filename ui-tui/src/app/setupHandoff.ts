import type { RunExternalProcess } from '@curie/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`curie ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching curie: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`curie ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
