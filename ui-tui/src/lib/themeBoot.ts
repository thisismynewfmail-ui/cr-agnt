 ...
 * `curie-boot-background` / `curie-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports CURIE_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.curie matches get_curie_home()'s default.
const bootFilePath = () => join(process.env.CURIE_HOME ?? join(homedir(), '.curie'), 'tui-theme-boot.json')

// Never touch the user's real ~/.curie from test runs (the TS suite has no
// CURIE_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into CURIE_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.CURIE_TUI_THEME || env.CURIE_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.CURIE_TUI_BACKGROUND
  ) {
    env.CURIE_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.CURIE_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.CURIE_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into CURIE_TUI_THEME.
