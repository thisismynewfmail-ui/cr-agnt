...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CURIE_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Curie on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via CURIE_TUI_TRUECOLOR=1.
