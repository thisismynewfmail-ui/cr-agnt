...
import { Box, Text } from '@curie/ink'
...
/**
 * User widget apps — Curie authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$CURIE_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.curie/plugins/`: files under CURIE_HOME execute
...
const widgetsDir = () => join(process.env.CURIE_HOME?.trim() || join(homedir(), '.curie'), 'tui-widgets')
 ...
 *  every change, so a widget Curie writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Curie) ever writes must hot-load too — a 10s poll here read
