import type { InputEvent, Key } from '@curie/ink'
import * as Ink from '@curie/ink'
 ...
 * but that accepts `key.meta` on macOS — and curie-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.CURIE_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which curie-ink decodes as meta+'d'; without this branch it
